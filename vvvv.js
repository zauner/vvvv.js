...
(function () {
  ...
  const VVVVBrowserContext = { name: "browser" };
  ...
  VVVVBrowserContext.init = function (path_to_vvvv, mode, callback) {
    ...
    window.server_req = function () {
      console.log(
        "Calling remoty_only_require in browser context does not work."
      );
      return null;
    };
    ...
    function initialisationComplete(VVVV, $) {
      ...
      $("link[rel='VVVV'], script[language='VVVV']").each(function (i) {
        let href_attribute = "href";
        if ($(this).get(0).tagName === "SCRIPT") {
          if (process.env.VVVV_ENV === "development")
            console.warn(
              'DEPRECATED: loading patches via <script language="VVVV" src="..."> tag is deprecated. Use <link rel="VVVV" href="..."> instead.'
            );
          href_attribute = "src";
        }
        const patch = new VVVV.Patch($(this).attr(href_attribute), function () {
          const mainLoop = new VVVV.MainLoop(this);
          VVVVContext.MainLoops.push(mainLoop);
        });
        VVVVContext.Patches[i] = patch;
      });
  ...
  let VVVVNodeContext = { name: "nodejs" };
  ...
  VVVVNodeContext.init = function (path_to_vvvv, mode, callback) {
    ...
    VVVVContext.DocumentRoot = path.resolve(
      path.dirname(process.mainModule.filename)
    );
    VVVVContext.ServerRoot = path.resolve(
      path.dirname(process.mainModule.filename)
    );
    ...
    jsdom.env("<html></html>", function (err, w) {
      ...
      const vvvv = require(`${VVVVContext.Root}/core/vvvv.core`);
      if (typeof callback === "function") callback.call(window, vvvv);
    });
  };
  ...
  VVVVNodeContext.loadFile = function (filename, opts) {
    ...
    try {
      ...
      if (typeof opts.error === "function") opts.error.call();
      return;
    }
    if (typeof opts.success === "function") opts.success.call(window, data);
  };
  ...
  VVVVNodeContext.getRelativeRequire = function (system_require) {
    return function (b) {
      ...
      if (p[0] === ".") return system_require(p);
      else return system_require(VVVVContext.Root + "/" + p);
    };
  };
  ...
  else VVVVContext = VVVVNodeContext;
