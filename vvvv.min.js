...
c});for(var d=[],h="",f=!1,e=c.length,g=0;g<e;g++)","!=c[g]||f?"|"==c[g]?f=!f:h+=c[g]:(d.push(h),h="");d.push(h);return d}function c(){var c=window.location.hash.match("#([^/]+)/"+m.ressource+"$");c&&(console.log("launching editor ..."),(new VVVV.Editors[c[1]]).enable(m))}this.ressource=a;this.vvvv_version="45_26.1";this.boundingBox={width:0,height:0};this.windowHeight=this.windowWidth=500;this.pinMap={};this.nodeMap={};this.nodeList=[];this.linkList=[];this.success=b;this.error=g;this.XMLCode="";
...
m.nodeMap[$(this).attr("id")];"development"==VVVV_ENV&&console.log("node renamed, so deleting node "+e.id+" / "+e.nodename);_(e.inputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.fromPin.connectionChanged()})});_(e.outputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.toPin.connectionChanged();c.toPin.markPinAsChanged();c.toPin.reset_on_disconnect&&c.toPin.reset()})});m.nodeList.splice(m.nodeList.indexOf(e),1);delete m.nodeMap[e.id]}var q;q="InABox"==$(this).attr("componentmode")?
$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var a=void 0!=m.nodeMap[$(this).attr("id")];if(a)e=m.nodeMap[$(this).attr("id")];else{var k=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==k)return;if(void 0!=VVVV.NodeLibrary[k.toLowerCase()])e=new (VVVV.NodeLibrary[k.toLowerCase()])($(this).attr("id"),m),VVVV.NodeLibrary[k.toLowerCase()].requirements&&(m.resourcesPending++,_(VVVV.NodeLibrary[k.toLowerCase()].requirements).each(function(c){void 0===
VVVV.LoadedLibs[c]&&VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){m.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))e=new VVVV.Nodes.GenericShader($(this).attr("id"),m),e.isShader=!0,e.shaderFile=$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),e.nodename=k;else if(/.v4p$/.test($(this).attr("filename"))){m.resourcesPending++;var n=this,e=new VVVV.Core.Patch($(this).attr("filename"),function(){m.resourcesPending--;"development"==
VVVV_ENV&&console.log(this.nodename+"invoking update links");f(c);m.editor&&m.editor.addPatch(this);if(this.auto_evaluate){var d=m;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(m.mainloop);m.afterUpdate();0>=m.resourcesPending&&h&&(h(),h=void 0)},function(){m.resourcesPending--;this.not_implemented=!0;VVVV.onNotImplemented(k);f(c);m.afterUpdate();0>=m.resourcesPending&&h&&(h(),h=void 0)},m,$(n).attr("id"));e.isSubpatch=!0;m.editor&&!e.editor&&m.editor.addPatch(e);m.nodeMap[e.id]=e}else e=
new VVVV.Core.Node($(this).attr("id"),k,m),e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&m.editor.sendUndo(),VVVV.onNotImplemented(k);"development"==VVVV_ENV&&"complete"!=g&&console.log(m.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){n=m;do n.auto_evaluate=!0;while(n=n.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);n=e.getSubPatches();n.push(e);for(var b,
p=0;p<n.length;p++)b=VVVV.Helpers.prepareFilePath(n[p].nodename,n[p].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(e),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}m.nodeList.splice(m.nodeList.indexOf(e),1);e.destroy();delete m.nodeMap[e.id]}0<q.length&&(q.attr("left")&&(e.x=q.attr("left")/15,e.y=q.attr("top")/15,m.boundingBox.width=Math.max(m.boundingBox.width,e.x+100),m.boundingBox.height=Math.max(m.boundingBox.height,e.y+100)),q.attr("width")&&(e.width=q.attr("width"),e.height=
q.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&(e.isIOBox=!0);a||e.setup();n=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var g=0;g<f.length;g++)e.invisiblePins[h].values[g]!=f[g]&&e.invisiblePins[h].setValue(g,f[g]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==
...
c.beginPath();c.moveTo(this.x[0],this.y[0]);for(var h=1;h<this.x.length;h++)c.bezierCurveTo(this.c1x[h-1]+this.x[h-1],this.c1y[h-1]+this.y[h-1],this.c2x[h]+this.x[h],this.c2y[h]+this.y[h],this.x[h],this.y[h]);0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,w=0,l=0;l<a||0<m.getValue(0)&&(0<w||0<b%f.getSliceCount());l++)void 0==s[b]&&(s[b]=new u),0==w&&(s[b].renderState=
e.getValue(b),s[b].transform=f.getValue(b),d.isConnected()&&(s[b].clippingLayer=d.getValue(b))),s[b].x[w]=parseFloat(c.getValue(l)),s[b].y[w]=parseFloat(h.getValue(l)),s[b].c1x[w]=parseFloat(q.getValue(l)),s[b].c1y[w]=parseFloat(n.getValue(l)),s[b].c2x[w]=parseFloat(k.getValue(l)),s[b].c2y[w]=parseFloat(r.getValue(l)),w++,0<m.getValue(0)&&w>=m.getValue(b)&&(w<s[b].x.length&&g(s[b],w),b++,w=0);0>m.getValue(0)&&w<s[b].x.length?(g(s[b],w),s.splice(1)):s.splice(b);for(a=0;a<s.length;a++)p.setValue(a,
