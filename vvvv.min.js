var VVVV_ENV="production";$.ajaxPrefilter(function(b,a,h){if("script"==b.dataType||"script"==a.dataType)b.cache=!0});window.console||(window.console={log:function(b){}});VVVV={Config:{}};VVVV.Config.auto_undo=!1;VVVV.Nodes={};VVVV.PinTypes={};VVVV.NodeLibrary={};VVVV.NodeNames=[];VVVV.onNotImplemented=function(b){console.log("Warning: "+b+" is not implemented.")};VVVV.loadCounter=0;
...
VVVV.init=function(b,a,h){function g(){var d=new VVVV.Core.Patch("");_(VVVV.Nodes).each(function(c){var k=new c(0,d);"development"==VVVV_ENV&&console.log("Registering "+k.nodename);VVVV.NodeLibrary[k.nodename.toLowerCase()]=c;VVVV.NodeNames.push(k.nodename)});"development"==VVVV_ENV&&console.log("done ...");VVVV.Patches=[];VVVV.MainLoops=[];$("script[language='VVVV']").each(function(){var c=new VVVV.Core.Patch($(this).attr("src"),function(){var c=new VVVV.Core.MainLoop(this);VVVV.MainLoops.push(c)});
VVVV.Patches.push(c)});"function"===typeof h&&h.call()}VVVV.Root=b||"./";"development"==VVVV_ENV&&console.log("loading vvvv.js ...");if("development"==VVVV_ENV){document.getElementsByTagName("head");var e=function(d){d.target.removeEventListener("load",e);0>=--VVVV.loadCounter&&g()};0==$("script[src*=thirdparty]").length&&VVVV.loadScript("thirdparty.js",e);0==$("script[src*=underscore]").length&&VVVV.loadScript("lib/underscore/underscore-min.js",e);0!=$('script[src*="d3.js"]').length||"full"!=a&&
...
VVVV.Helpers={translateOperators:function(b){b=b.replace("Add","+");b=b.replace("Subtract","-");b=b.replace("Multiply","*");b=b.replace("Divide","/");return b=b.replace("EQ","=")},prepareFilePath:function(b,a){b=b.replace(/\\/g,"/");return b.match(/^%VVVV%/)?b.replace("%VVVV%",VVVV.Root):b.match(/^%PAGE%/)?b.replace("%PAGE%",location.pathname):b.match(/^(\/|.+:\/\/)/)?b:a.getPath()+b}};
VVVV.Core={Pin:function(b,a,h,g,e,d){this.direction=a;this.pinname=b;this.links=[];this.values=[];this.node=g;this.changed=!0;this.active=!1;this.reset_on_disconnect=e||!1;this.masterPin=this.slavePin=void 0;this.connectionChangedHandlers={};this.enumOptions=[];this.getValue=function(c,d){if(!d||1==d)return this.values[c%this.values.length];for(var e=[],g=0;g<d;g++)e.push(this.values[(c*d+g)%this.values.length]);return e};this.setValue=function(c,d){this.values[c]=d;this.changed=!0;this.node.dirty=
...
"");return g=VVVV.Helpers.translateOperators(g)};this.getWidth=function(){var g;g=100==this.width||0==this.width?this.isIOBox?60:Math.max(18,6*(this.label().length+2)):this.width/15;return g=Math.max(g,12*(_(this.inputPins).size()-1)+4)};this.getHeight=function(){return this.isIOBox&&100==this.height?18*this.IOBoxRows():100==this.height||this.isSubpatch?18:Math.max(18,this.height/15)};this.getUpstreamNodes=function(){var g=[];_(this.inputPins).each(function(e){0<e.links.length&&g.push(e.links[0].fromPin.node)});
...
c.attr("visible","1");!d.isConnected()&&0<=["Color","Generic","Enum"].indexOf(d.typeName)&&c.attr("values",_(d.values).map(function(c){return"|"+c+"|"}).join(","));g.append(c)});_(this.invisiblePins).each(function(d){var c=$("<PIN>");c.attr("pinname",d.pinname);c.attr("visible","0");0<=["Color","Generic","Enum"].indexOf(d.typeName)&&c.attr("values",_(d.values).map(function(c){return"|"+c+"|"}).join(","));g.append(c)});return g}},Link:function(b,a){this.fromPin=b;this.toPin=a;this.fromPin.links.push(this);
...
d.node.id&&l.linkList[g].toPin.pinname==d.pinname)&&(e=l.linkList[g]);if(!e){e=new VVVV.Core.Link(c,d);c.connectionChanged();d.connectionChanged();l.linkList.push(e);for(g=0;g<c.values.length;g++)d.setValue(g,c.getValue(g));d.setSliceCount(c.getSliceCount())}"complete"==a&&(k[c.node.id+"_"+c.pinname+"-"+d.node.id+"_"+d.pinname]=e)}});"complete"==a&&(_(c).each(function(c,d){if(void 0==k[d]){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var e=c.fromPin,g=
c.toPin;c.destroy();e.connectionChanged();g.connectionChanged();g.markPinAsChanged();g.reset_on_disconnect&&g.reset()}}),c={},_(k).each(function(d,e){c[e]=d}))}var b=this;do b.dirty=!0;while(b=b.parentPatch);if(b=/^<!DOCTYPE\s+PATCH\s+SYSTEM\s+"(.+)\\(.+)\.dtd/.exec(d))l.vvvv_version=b[2].replace(/[a-zA-Z]+/,"_");var a="diff";if(/\s<PATCH/.test(d)||"45_26">=l.vvvv_version)a="complete","development"==VVVV_ENV&&console.log("complete: "+this.nodename),l.XMLCode=d;$windowBounds=$(d).find('bounds[type="Window"]').first();
...
c.toPin.connectionChanged();c.toPin.markPinAsChanged();c.toPin.reset_on_disconnect&&c.toPin.reset()})});l.nodeList.splice(l.nodeList.indexOf(c),1);delete l.nodeMap[c.id]}$bounds="InABox"==$(this).attr("componentmode")?$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var k=void 0!=l.nodeMap[$(this).attr("id")];if(k)c=l.nodeMap[$(this).attr("id")];else{nodename=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==nodename)return;
if(void 0!=VVVV.NodeLibrary[nodename.toLowerCase()])c=new (VVVV.NodeLibrary[nodename.toLowerCase()])($(this).attr("id"),l),VVVV.NodeLibrary[nodename.toLowerCase()].requirements&&(l.resourcesPending++,_(VVVV.NodeLibrary[nodename.toLowerCase()].requirements).each(function(c){void 0===VVVV.LoadedLibs[c]&&VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){l.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))c=new VVVV.Nodes.GenericShader($(this).attr("id"),l),c.isShader=!0,c.shaderFile=
$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),c.nodename=nodename;else if(/.v4p$/.test($(this).attr("filename"))){l.resourcesPending++;r++;var p=this,c=new VVVV.Core.Patch(VVVV.Helpers.prepareFilePath($(this).attr("filename"),l),function(){c.nodename=$(p).attr("filename");c.id=$(p).attr("id");c.parentPatch=l;l.resourcesPending--;r--;"development"==VVVV_ENV&&console.log(c.nodename+"invoking update links");h(d);l.editor&&l.editor.addPatch(c);if(c.auto_evaluate){var k=
l;do k.auto_evaluate=!0;while(k=k.parentPatch)}l.afterUpdate();0>=l.resourcesPending&&e&&e()},function(){c.not_implemented=!0;VVVV.onNotImplemented(nodename)});c.setupObject();c.isSubpatch=!0;c.parentPatch=l;c.id=$(this).attr("id");l.nodeMap[c.id]=c}else c=new VVVV.Core.Node($(this).attr("id"),nodename,l),c.not_implemented=!0,"diff"==a&&!0==VVVV.Config.auto_undo&&l.editor.sendUndo(),VVVV.onNotImplemented(nodename);"development"==VVVV_ENV&&"complete"!=a&&console.log(l.nodename+": inserted new node "+
c.nodename)}if(c.auto_evaluate){var b=l;do b.auto_evaluate=!0;while(b=b.parentPatch)}"pronto"==$(this).attr("deleteme")&&("development"==VVVV_ENV&&console.log("removing node "+c.id),l.nodeList.splice(l.nodeList.indexOf(c),1),delete l.nodeMap[c.id]);0<$bounds.length&&($bounds.attr("left")&&(c.x=$bounds.attr("left")/15,c.y=$bounds.attr("top")/15,l.boundingBox.width=Math.max(l.boundingBox.width,c.x+100),l.boundingBox.height=Math.max(l.boundingBox.height,c.y+100)),$bounds.attr("width")&&(c.width=$bounds.attr("width"),
c.height=$bounds.attr("height")));/^iobox/.test(c.nodename.toLowerCase())&&(c.isIOBox=!0);k||c.setup();p=this;$(this).find("pin").each(function(){var e=$(this).attr("pinname"),k=g($(this).attr("values"));void 0!=k&&0<k.length&&c.addDefault(e,k);if(void 0==c.outputPins[e])if(void 0!=c.inputPins[e])c.inputPins[e].isConnected()||c.applyPinValuesFromXML(e);else if(void 0!=c.invisiblePins[e]){if(void 0!=k){for(var h=0;h<k.length;h++)c.invisiblePins[e].values[h]!=k[h]&&c.invisiblePins[e].setValue(h,k[h]);
c.invisiblePins[e].setSliceCount(k.length)}}else 1==$(this).attr("visible")&&"Configuration"!=$(this).attr("pintype")||c.isSubpatch?"Output"==$(this).attr("pintype")||0<$(d).find("link[srcnodeid="+c.id+"]").filter("link[srcpinname='"+e.replace(/[\[\]]/,"")+"']").length?void 0==c.outputPins[e]&&c.addOutputPin(e,k):void 0==c.inputPins[e]&&void 0==c.invisiblePins[e]&&c.addInputPin(e,k):void 0==c.inputPins[e]&&void 0==c.invisiblePins[e]&&c.addInvisiblePin(e,k)});k||(c.initialize(),l.nodeList.push(c));
"complete"==a&&(m[c.id]=c)});"complete"==a&&(_(p).each(function(c,d){void 0==m[d]&&("development"==VVVV_ENV&&console.log("removing node "+c.id),l.nodeList.splice(l.nodeList.indexOf(c),1),delete l.nodeMap[c.id])}),p={},_(m).each(function(c,d){p[d]=c}));0===r&&h(d);0>=this.resourcesPending&&e&&e()};this.getSubPatches=function(){for(var c=[],d=0;d<this.nodeList.length;d++)this.nodeList[d].isSubpatch&&(c.push(this.nodeList[d]),c=c.concat(this.nodeList[d].getSubPatches()));return c};this.afterEvaluate=
function(){};this.afterUpdate=function(){};this.toXML=function(){var c=$("<PATCH>");$bounds=$("<BOUNDS>");$bounds.attr("type","Window");$bounds.attr("width",parseInt(15*this.width));$bounds.attr("height",parseInt(15*this.height));c.append($bounds);for(var d=0;d<this.nodeList.length;d++)c.append(this.nodeList[d].serialize());for(d=0;d<this.linkList.length;d++)c.append(this.linkList[d].serialize());c='<!DOCTYPE PATCH  SYSTEM "http://vvvv.org/versions/vvvv45beta28.1.dtd" >\r\n  '+c.wrapAll("<d></d>").parent().html();
c=c.replace(/<patch/g,"<PATCH");c=c.replace(/<\/patch>/g,"\n  </PATCH>");c=c.replace(/<node/g,"\n  <NODE");c=c.replace(/<\/node>/g,"\n  </NODE>");c=c.replace(/<bounds/g,"\n  <BOUNDS");c=c.replace(/<\/bounds>/g,"\n  </BOUNDS>");c=c.replace(/<pin/g,"\n  <PIN");c=c.replace(/<\/pin>/g,"\n  </PIN>");c=c.replace(/<lonk/g,"\n  <LINK");return c=c.replace(/<\/lonk>/g,"\n  </LINK>")};this.evaluate=function(){function c(e){upstreamNodes=e.getUpstreamNodes();_(upstreamNodes).each(function(d){void 0==k[d.id]||
d.delays_output||c(d)});if(e.dirty||e.auto_evaluate||e.isSubpatch){if(d)var g=(new Date).getTime();if(e.auto_nil&&!e.isSubpatch&&e.hasNilInputs())_(e.outputPins).each(function(c){c.setSliceCount(0)});else{try{e.evaluate()}catch(h){console.log("VVVV.Js / Error evaluating "+e.nodename+": "+h.message)}d&&console.log(e.nodename+" / "+e.id+": "+((new Date).getTime()-g)+"ms");e.dirty=!1;_(e.inputPins).each(function(c){c.changed=!1})}}delete k[e.id];return!0}if(!(0<this.resourcesPending)){if(d)var e=(new Date).getTime();
for(var k={},g={},h=0;h<this.nodeList.length;h++){if(0==this.nodeList[h].getDownstreamNodes().length||this.nodeList[h].auto_evaluate||this.nodeList[h].delays_output)g[this.nodeList[h].id]=this.nodeList[h];k[this.nodeList[h].id]=this.nodeList[h]}d&&console.log("building node maps: "+((new Date).getTime()-e)+"ms");_(g).each(function(d,e,g){void 0!=k[d.id]&&c(d)});d&&(e=(new Date).getTime());this.afterEvaluate();d&&console.log("patch rendering: "+((new Date).getTime()-e)+"ms");d=!1}};$(window).keydown(function(c){84==
...
c.values[e])}})};this.processOutputConnectors=function(){var d=this;_(g).each(function(c){switch(c.property_class){default:d.setDOMByIOBox(c)}});_(h).each(function(c){if("event"==c.property_class)for(var d=0;d<c.values.length;d++)c.values[d]=0})};this.fetchValuesFromDOM=function(d){d.values.length=0;$(d.selector).each(function(c){var e;if(void 0==d.property_class)switch(this.nodeName){case "INPUT":"text"==$(this).attr("type")&&(e=$(this).val());"checkbox"==$(this).attr("type")&&(e=$(this).attr("checked")?
1:0);break;default:e=$(this).text()}"attribute"==d.property_class&&(e=$(this).attr(d.property));"style"==d.property_class&&(e=$(this).css(d.property));d.values[c]=e})};this.setDOMByIOBox=function(d){if(d.node.IOBoxOutputPin().pinIsChanged()){var c=d.node.IOBoxInputPin().values,e=$(d.selector).length;void 0==d.property_class&&($(d.selector).empty(),$(d.selector).val(""));for(var g=0;g<Math.max(c.length,e);g++){var h=g%e,l=g%c.length,b=$(d.selector).eq(h);if(0!=b.length){if(void 0==d.property_class)switch(b[0].nodeName){case "INPUT":b.val($(d.selector).eq(h).val()+
c[l]);break;default:b.html($(d.selector).eq(h).html()+c[l])}"attribute"==d.property_class&&b.attr(d.property,c[l]);"style"==d.property_class&&b.css(d.property,c[l])}}}}};VVVV.fps=60;
VVVV.Core.MainLoop=function(b,a){function h(){if(0==b.resourcesPending){g++;var p=(new Date).getTime();e.populateInputConnectors();b.evaluate();e.processOutputConnectors();var a=(new Date).getTime()-p;0==g%10&&(k&&(console.log(1/((p-c)/1E4)+"fps"),k=!1),c=p)}d&&window.setTimeout(h,Math.max(0,Math.round(1E3/VVVV.fps-a)))}VVVV.fps=a||60;var g=0,e=new VVVV.Core.DOMInterface(b),d=!0,c=0,k=!1;this.stop=function(){d=!1};this.start=function(){d||(d=!0,h())};this.isRunning=function(){return d};h();$(window).keydown(function(c){(102==
...
0,l=0;l<g.getSliceCount();l++)for(var a=0;a<h.getSliceCount();a++)0.5>e.getValue(0)&&l>a||(0.5>d.getValue(0)&&l<a||0.5>c.getValue(0)&&l==a)||(k.setValue(b,h.getValue(a)),p.setValue(b,g.getValue(l)),b++);k.setSliceCount(b);p.setSliceCount(b)}};VVVV.Nodes.Cross2d.prototype=new VVVV.Core.Node;
VVVV.Nodes.Polar2d=function(b,a){this.constructor(b,"Polar (2d)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var h=this.addInputPin("X",[1],this),g=this.addInputPin("Y",[0],this),e=this.addOutputPin("Angle",[0],this),d=this.addOutputPin("Length",[1],this);this.initialize=function(){};this.evaluate=function(){for(var c=this.getMaxInputSliceCount(),k=0;k<c;k++){var b=h.getValue(k),a=g.getValue(k);e.setValue(k,Math.atan2(a,
b)/(2*Math.PI));d.setValue(k,Math.sqrt(b*b+a*a))}e.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Polar2d.prototype=new VVVV.Core.Node;
VVVV.Nodes.Cartesian2d=function(b,a){this.constructor(b,"Cartesian (2d)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var h=this.addInputPin("Angle",[0],this),g=this.addInputPin("Length",[1],this),e=this.addOutputPin("X",[1],this),d=this.addOutputPin("Y",[0],this);this.evaluate=function(){for(var c=this.getMaxInputSliceCount(),k=0;k<c;k++){var b=h.getValue(k),a=g.getValue(k);e.setValue(k,a*Math.cos(2*b*Math.PI));
d.setValue(k,a*Math.sin(2*b*Math.PI))}e.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Cartesian2d.prototype=new VVVV.Core.Node;
...
function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var q=parseFloat(h.getValue(a)),n=parseFloat(g.getValue(a)),t=parseFloat(e.getValue(a)),v=parseFloat(d.getValue(a)),t=t-q,v=v-n;c.setValue(a,q+t/2);k.setValue(a,n+v/2);p.setValue(a,Math.sqrt(t*t+v*v));m.setValue(a,Math.atan2(v,t)/(2*Math.PI))}c.setSliceCount(b);k.setSliceCount(b);p.setSliceCount(b);m.setSliceCount(b)}};VVVV.Nodes.Points2Vector2d.prototype=new VVVV.Core.Node;
...
function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var q=parseFloat(h.getValue(a)),n=parseFloat(g.getValue(a)),t=parseFloat(e.getValue(a)),v=2*parseFloat(d.getValue(a)-0.25)*Math.PI,w=t/2*Math.sin(v),t=-t/2*Math.cos(v);c.setValue(a,q+w);k.setValue(a,n+t);p.setValue(a,q-w);m.setValue(a,n-t)}c.setSliceCount(b);k.setSliceCount(b);p.setSliceCount(b);m.setSliceCount(b)}};VVVV.Nodes.Vector2Points2d.prototype=new VVVV.Core.Node;VVVV.Nodes.Polar3d=function(b,a){this.constructor(b,"Polar (3d)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var h=this.addInputPin("X",[0],this),g=this.addInputPin("Y",[0],this),e=this.addInputPin("Z",[-1],this),d=this.addOutputPin("Pitch",[0],this),c=this.addOutputPin("Yaw",[0],this),k=this.addOutputPin("Length",[1],this);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var l=h.getValue(a),
r=g.getValue(a),q=e.getValue(a),n=Math.sqrt(l*l+r*r+q*q);c.setValue(a,Math.atan2(-l,-q)/(2*Math.PI));d.setValue(a,0==n?0:Math.acos(-r/n)/(2*Math.PI)-0.25);k.setValue(a,n)}c.setSliceCount(b);d.setSliceCount(b);k.setSliceCount(b)}};VVVV.Nodes.Polar3d.prototype=new VVVV.Core.Node;
...
2*(h.getValue(a)+0.25)*Math.PI,r=2*g.getValue(a)*Math.PI,q=e.getValue(a);k.setValue(a,-q*Math.cos(r)*Math.sin(l));d.setValue(a,-q*Math.sin(r)*Math.sin(l));c.setValue(a,-q*Math.cos(l))}d.setSliceCount(b);c.setSliceCount(b);k.setSliceCount(b)}};VVVV.Nodes.Cartesian3d.prototype=new VVVV.Core.Node;
...
c,3),b=h.getValue(c);mat4.multiplyVec3(b,k);e.setValue(3*c+0,k[0]);e.setValue(3*c+1,k[1]);e.setValue(3*c+2,k[2])}e.setSliceCount(Math.ceil(3*d))}};VVVV.Nodes.Multiply3dVector.prototype=new VVVV.Core.Node;VVVV.Nodes.LFO=function(b,a){this.constructor(b,"LFO (Animation)",a);this.meta={authors:["Matthias Zauner, sebl"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Not spreadable yet"]};this.auto_evaluate=!0;var h=this.addInputPin("Period",[1],this),g=this.addInputPin("Pause",[0],this),e=this.addInputPin("Reverse",[0],this),d=this.addInputPin("Reset",[0],this),c=this.addInputPin("Phase",[0],this),k=this.addOutputPin("Output",[0],this),p=this.addOutputPin("Cycles",[0],this),m=[],l=
[],r=(new Date).getTime(),q=(new Date).getTime();this.evaluate=function(){var b=this.getMaxInputSliceCount();r=(new Date).getTime()-q;for(var a=0;a<b;a++){var v=h.getValue(a%h.values.length),w=g.getValue(a%g.values.length),y=e.getValue(a%e.values.length),s=d.getValue(a%d.values.length),u=c.getValue(a%c.values.length);void 0==m[a]&&(m[a]=0);void 0==l[a]&&(l[a]=0);0.5>w&&(0!=v&&isFinite(v))&&(dv=1/(1E3*v)*r,0<y&&(dv*=-1),m[a]+=dv,0>m[a]&&(l[a]-=Math.ceil(-m[a]),m[a]=1+m[a]),1<m[a]&&(l[a]+=Math.floor(m[a])));
q=(new Date).getTime();0.5<=s&&(m[a]=0,l[a]=0);if(0.5>w||0.5<=s)k.setValue(a,(m[a]+u)%1),p.setValue(a,l[a]);m[a]%=1}k.setSliceCount(b);p.setSliceCount(b);m.splice(b);l.splice(b)}};VVVV.Nodes.LFO.prototype=new VVVV.Core.Node;
...
[0],this);this.addOutputPin("Acceleration Out",[0],this);var d=[],c=[],k=[],p=[],m=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=h.pinIsChanged()||g.pinIsChanged(),q=0;q<b;q++){void 0==d[q]&&(d[q]=(new Date).getTime());var n=(new Date).getTime()-d[q],t=parseFloat(h.getValue(q)),v=parseFloat(g.getValue(q));void 0==c[q]&&(c[q]=t);isFinite(t)&&isFinite(v)?(a&&(p[q]=void 0,m[q]=0<=t-c[q],k[q]=0<v?(t-c[q])/(1E3*v):0),m[q]==0<=t-c[q]&&(c[q]+=k[q]*n),void 0!=p[q]&&0<=t-c[q]!=0<=p[q]&&
(k[q]=0,c[q]=t),0!=p[q]&&e.setValue(q,c[q]),p[q]=t-c[q],d[q]=(new Date).getTime()):(c[q]=void 0,e.setValue(q,void 0))}a&&e.getSliceCount()!=b&&(d.splice(b),c.splice(b),k.splice(b),p.splice(b),e.setSliceCount(b))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
...
var a=h.pinIsChanged();if(void 0==h.getValue(0))a&&d.setValue(0,void 0);else{if(1==e.getValue(0)&&(a||g.pinIsChanged()))for(k.pop(),k.unshift(now),a=0;a<b;a++){if(void 0==c[a]){c[a]=Array(1024);for(var l=1024;l--;)c[a][l]=0}1024<=c[a].length&&c[a].pop();c[a].unshift(h.getValue(a))}for(a=0;a<b;a++){for(var r=now-1E3*g.getValue(a),q=!1,l=0;1024>l;l++)if(k[l]<=r){d.values[a]!=c[a][l]&&d.setValue(a,c[a][l]);q=!0;break}q||0==d.values[a]||d.setValue(a,0)}d.setSliceCount(b);c.splice(b)}}};
...
k;b++)e.setValue(b,0),d.setValue(b,1);for(b=0;b<k;b++){var a=void 0;1<=Math.round(g.getValue(b))&&(a=0);1<=Math.round(h.getValue(b))&&(a=1-parseFloat(e.getValue(b)));void 0!=a&&e.getValue(b)!=a?(e.setValue(b,a),d.setValue(b,1-a)):c||(e.setValue(b,0),d.setValue(b,1))}e.setSliceCount(k);d.setSliceCount(k);c=!0}};VVVV.Nodes.Toggle.prototype=new VVVV.Core.Node;
...
VVVV.Nodes.OrSpectral=function(b,a){this.constructor(b,"OR (Boolean Spectral)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var h=this.addInputPin("Input",[0],this),g=this.addInputPin("Bin Size",[-1],this),e=this.addOutputPin("Output",[0],this);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0,k=0,b=!1,a=0;a<d||0<g.getValue(0)&&(0<k||0!=c%g.values.length);a++)0==k&&(b=!1),b=b||0.5<=h.getValue(a),k++,0<g.getValue(0)?
k>=g.getValue(c)&&(e.setValue(c,b?1:0),c++,k=0):e.setValue(0,b?1:0);e.setSliceCount(c+(0<k))}};VVVV.Nodes.OrSpectral.prototype=new VVVV.Core.Node;
VVVV.Nodes.AndSpectral=function(b,a){this.constructor(b,"AND (Boolean Spectral)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var h=this.addInputPin("Input",[0],this),g=this.addInputPin("Bin Size",[-1],this),e=this.addOutputPin("Output",[0],this);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0,k=0,b=!1,a=0;a<d||0<g.getValue(0)&&(0<k||0!=c%g.values.length);a++)0==k&&(b=!0),b=b&&0.5<=h.getValue(a),k++,0<g.getValue(0)?
k>=g.getValue(c)&&(e.setValue(c,b?1:0),c++,k=0):e.setValue(0,b?1:0);e.setSliceCount(c+(0<k))}};VVVV.Nodes.AndSpectral.prototype=new VVVV.Core.Node;VVVV.Types.CanvasTexture={imageObject:void 0,loaded:!1};
...
[],this,VVVV.PinTypes.CanvasRenderState),p=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==p[a]&&(p[a]=new VVVV.Types.CanvasRenderState),p[a].copy_attributes(h.getValue(a)),p[a].strokeColor=g.getValue(a).split(","),p[a].lineWidth=e.getValue(a),p[a].capStyle=d.getValue(a),p[a].joinStyle=c.getValue(a),k.setValue(a,p[a]);k.setSliceCount(b)}};VVVV.Nodes.StrokeCanvas.prototype=new VVVV.Core.Node;
...
[],this,VVVV.PinTypes.CanvasRenderState),p=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==p[a]&&(p[a]=new VVVV.Types.CanvasRenderState),p[a].copy_attributes(h.getValue(a)),p[a].shadowOffsetX=e.getValue(a),p[a].shadowOffsetY=d.getValue(a),p[a].shadowBlur=c.getValue(a),p[a].shadowColor=g.getValue(a).split(","),k.setValue(a,p[a]);k.setSliceCount(b)}};VVVV.Nodes.ShadowCanvas.prototype=new VVVV.Core.Node;
...
this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==l[a]&&(l[a]=new r),l[a].transform=g.getValue(a),l[a].radius=parseFloat(d.getValue(a)),l[a].startAngle=2*parseFloat(c.getValue(a))*Math.PI,l[a].endAngle=2*parseFloat(k.getValue(a))*Math.PI,l[a].renderState=h.getValue(a),l[a].drawSegment=1==p.getValue(a),e.isConnected()&&(l[a].clippingLayer=e.getValue(a));for(a=0;a<l.length;a++)m.setValue(a,l[a]);
...
VVVV.Nodes.BezierCurveCanvas=function(b,a){function h(c,d){c.x.splice(d);c.y.splice(d);c.c1x.splice(d);c.c1y.splice(d);c.c2x.splice(d);c.c2y.splice(d)}this.constructor(b,"BezierCurve (Canvas VVVVjs)",a);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var g=this.addInputPin("Render State",[],this,!0,VVVV.PinTypes.CanvasRenderState),e=this.addInputPin("Transform",[],this,!0,VVVV.PinTypes.Transform),d=this.addInputPin("Clipping Layer",[],this,!0,VVVV.PinTypes.CanvasLayer),
...
1]+this.x[e-1],this.c1y[e-1]+this.y[e-1],this.c2x[e]+this.x[e],this.c2y[e]+this.y[e],this.x[e],this.y[e]);0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,s=0,u=0;u<a||0<q.getValue(0)&&(0<s||0<b%e.getSliceCount());u++)void 0==t[b]&&(t[b]=new v),0==s&&(t[b].renderState=g.getValue(b),t[b].transform=e.getValue(b),d.isConnected()&&(t[b].clippingLayer=d.getValue(b))),t[b].x[s]=
parseFloat(c.getValue(u)),t[b].y[s]=parseFloat(k.getValue(u)),t[b].c1x[s]=parseFloat(p.getValue(u)),t[b].c1y[s]=parseFloat(m.getValue(u)),t[b].c2x[s]=parseFloat(l.getValue(u)),t[b].c2y[s]=parseFloat(r.getValue(u)),s++,0<q.getValue(0)&&s>=q.getValue(b)&&(s<t[b].x.length&&h(t[b],s),b++,s=0);0>q.getValue(0)&&s<t[b].x.length?(h(t[b],s),t.splice(1)):t.splice(b);for(a=0;a<t.length;a++)n.setValue(a,t[a]);n.setSliceCount(t.length)}};VVVV.Nodes.BezierCurveCanvas.prototype=new VVVV.Core.Node;
...
VVVV.Nodes.GroupCanvas=function(b,a){this.constructor(b,"Group (Canvas VVVVjs)",a);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var h=[],g=this.addOutputPin("Layer",[],this,VVVV.PinTypes.CanvasLayer);this.initialize=function(){for(var e=0;2>e;e++)h[e]=this.addInputPin("Layer "+(e+1),[],this,!0,VVVV.PinTypes.CanvasLayer)};this.evaluate=function(){for(var e=0,d=0;2>d;d++)if(h[d].isConnected())for(var c=0;c<h[d].values.length;c++)g.setValue(e++,h[d].getValue(c));
...
0;VVVV.MousePositions[c.id].lb=0;break;case 2:VVVV.MousePositions._all.mb=0;VVVV.MousePositions[c.id].mb=0;break;case 3:VVVV.MousePositions._all.rb=0,VVVV.MousePositions[c.id].rb=0}})}this.constructor(b,"Renderer (Canvas VVVVjs)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Layers",[],this,!0,VVVV.PinTypes.CanvasLayer);this.addInputPin("Clear",[1],this);var e=this.addInputPin("Background Color",["0.0, 0.0, 0.0, 1.0"],
this,!0,VVVV.PinTypes.Color),d=this.addInputPin("Backbuffer Width",[0],this),c=this.addInputPin("Backbuffer Height",[0],this),k=this.addInputPin("View",[],this,!0,VVVV.PinTypes.Transform),p=this.addOutputPin("Canvas Out",[],this),m,l,r,q=[0,0,0,1],n;this.getContext=function(){var e=this.invisiblePins["Descriptive Name"].getValue(0);if((e=$(e).get(0))&&"CANVAS"==e.nodeName)n=e;else{var b=parseInt(d.getValue(0)),k=parseInt(c.getValue(0));n=$('<canvas width="'+(0<b?b:512)+'" height="'+(0<k?k:512)+'" id="vvvv-js-generated-renderer-'+
(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');e||(e="body");$(e).append(n);n=n.get(0)}l=n.width;r=n.height;n&&n.getContext&&(h(n),m=n.getContext("2d"),n.ctx=m)};this.evaluate=function(){this.invisiblePins["Descriptive Name"].pinIsChanged()&&($(n).hasClass("vvvv-js-generated-renderer")&&$(n).remove(),this.getContext());if(m){if(d.pinIsChanged()){var b=parseInt(d.getValue(0));0<b&&(l=b,$(n).attr("width",l))}c.pinIsChanged()&&(b=parseInt(c.getValue(0)),0<b&&(r=b,$(n).attr("height",
r)));e.pinIsChanged()&&(q=e.getValue(0).split(","),q[0]=parseInt(255*q[0]),q[1]=parseInt(255*q[1]),q[2]=parseInt(255*q[2]));defaultRenderState.apply(m);m.clearRect(0,0,l,r);m.save();m.translate(l/2,r/2);m.scale(l/2,-r/2);k.isConnected()&&(b=k.getValue(0),m.transform(b[0],b[1],b[4],b[5],b[12],b[13]));if(g.isConnected())for(b=0;b<g.values.length;b++)g.getValue(b).draw(m);m.restore();defaultRenderState.apply(m);m.globalCompositeOperation="destination-over";m.fillStyle="rgba("+q[0]+","+q[1]+","+q[2]+
","+q[3]+")";m.fillRect(0,0,l,r);p.setValue(0,n);n.loaded=!0}}};VVVV.Nodes.RendererCanvas.prototype=new VVVV.Core.Node;VVVV.PinTypes.Color={typeName:"Color",reset_on_disconnect:!1,defaultValue:function(){return"1.0, 1.0, 1.0, 1.0"},hsvToRgb:function(b,a,h){b*=360;var g=Math.floor(b/60),e=b/60-g;b=h*(1-a);var d=h*(1-a*e);a=h*(1-a*(1-e));switch(g){case 1:return[d,h,b];case 2:return[b,h,a];case 3:return[b,d,h];case 4:return[a,b,h];case 5:return[h,b,d];default:return[h,a,b]}},rgbToHsv:function(b,a,h){var g=Math.max(b,Math.max(a,h)),e=Math.min(b,Math.min(a,h)),d;g==e?d=0:g==b?d=60*(0+(a-h)/(g-e)):g==a?d=60*(2+(h-b)/(g-
e)):g==h&&(d=60*(4+(b-a)/(g-e)));0>d&&(d+=360);b=0;0!=g&&(b=(g-e)/g);return[d/360,b,g]}};
...
this.getMaxInputSliceCount(),a=0;a<b;a++){var m=h.getValue(a)||0,l=g.getValue(a)||0,r=e.getValue(a)||0,q=d.getValue(a)||0;c.setValue(a,m+","+l+","+r+","+q)}c.setSliceCount(b)}}};VVVV.Nodes.RGBJoin.prototype=new VVVV.Core.Node;
...
l=g.getValue(a),r=e.getValue(a),q=d.getValue(a),m=VVVV.PinTypes.Color.hsvToRgb(m,l,r);c.setValue(a,m[0]+","+m[1]+","+m[2]+","+q)}c.setSliceCount(b)}};VVVV.Nodes.HSVJoin.prototype=new VVVV.Core.Node;
...
if(h.pinIsChanged()){for(var a=0;a<b;a++){var r=VVVV.Helpers.prepareFilePath(h.getValue(a),this.parentPatch);if(void 0==k[a]||k[a].origSrc!=r)k[a]=new Image,k[a].loaded=!1,k[a].origSrc=r,k[a].onload=function(c){return function(){p=k[c].loaded=!0}}(a),k[a].src=r,c.setValue(a,0),g.setValue(a,k[a])}k.length=b;g.setSliceCount(b);e.setSliceCount(b);d.setSliceCount(b);c.setSliceCount(b)}if(p){for(a=0;a<b;a++)g.setValue(a,k[a]),e.setValue(a,k[a].width),d.setValue(a,k[a].height),c.setValue(a,k[a].loaded?
1:0);p=!1}}};VVVV.Nodes.FileTextureCanvas.prototype=new VVVV.Core.Node;
...
[-1],this),p=this.addInputPin("Do Seek",[0],this),m=this.addInputPin("Seek Position",[0],this),l=this.addInputPin("Filename",["http://html5doctor.com/demos/video-canvas-magic/video.ogg"],this),r=this.addOutputPin("Video",[],this),q=this.addOutputPin("Audio",[],this),n=this.addOutputPin("Duration",[0],this),t=this.addOutputPin("Position",[0],this),v=this.addOutputPin("Video Width",[0],this),w=this.addOutputPin("Video Height",[0],this),y=this.addOutputPin("Network Status",[""],this),s=this.addOutputPin("Ready Status",
[""],this),u=[];this.evaluate=function(){var b=this.getMaxInputSliceCount();if(l.pinIsChanged())for(var a=0;a<b;a++){filename=VVVV.Helpers.prepareFilePath(l.getValue(a),this.parentPatch);if(void 0==u[a]){var A=$('<video style="display:none"><source src="" type=video/ogg></video>');$("body").append(A);u[a]=A[0];u[a].volume=0;A=function(c){return function(){}}(a);u[a].onprogress=A;u[a].oncanplay=A;u[a].oncanplaythrough=A}filename!=u[a].currentSrc&&($(u[a]).find("source").first().attr("src",filename),
u[a].load(),0.5<e.getValue(a)?u[a].play():u[a].pause(),u[a].loaded=!0,r.setValue(a,u[a]),q.setValue(a,u[a]))}if(e.pinIsChanged())for(a=0;a<b;a++)0.5<e.getValue(a)?u[a].play():u[a].pause();if(p.pinIsChanged())for(a=0;a<b;a++)u[a%u.length].loaded&&0.5<=p.getValue(a)&&(u[a%u.length].currentTime=parseFloat(m.getValue(a)),0.5<e.getValue(a)&&u[a].play());for(a=0;a<b;a++){if(!u[a].paused){r.setValue(a,u[a]);q.setValue(a,u[a]);n.getValue(a)!=u[a].duration&&n.setValue(a,u[a].duration);t.setValue(a,u[a].currentTime);
var A=parseFloat(k.getValue(a)),G=parseFloat(c.getValue(a));u[a].currentTime<G&&(u[a].currentTime=G);if(u[a].currentTime>=u[a].duration||0<=A&&u[a].currentTime>=A)0.5<=d.getValue(a)?u[a].currentTime=G:u[a].pause()}if(u[a].videoWidth!=v.getValue(a)||u[a].videoHeight!=w.getValue(a))v.setValue(a,u[a].videoWidth),w.setValue(a,u[a].videoHeight);y.getValue(a)!=h[u[a].networkState]&&y.setValue(a,h[u[a].networkState]);s.getValue(a)!=g[u[a].readyState]&&s.setValue(a,g[u[a].readyState])}r.setSliceCount(b);
q.setSliceCount(b)}};VVVV.Nodes.FileStreamCanvas.prototype=new VVVV.Core.Node;VVVV.Nodes.AudioOutHTML5=function(b,a){this.constructor(b,"AudioOut (HTML5 VVVVjs)",a);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var h=this.addInputPin("Audio",[],this),g=this.addInputPin("Volume",[0.5],this);this.evaluate=function(){var e=this.getMaxInputSliceCount();if(g.pinIsChanged())for(var d=0;d<e;d++)h.getValue(d).volume=Math.max(0,Math.min(1,parseFloat(g.getValue(d))))}};
...
this.addOutputPin("Body",[""],this),p=this.addOutputPin("Fail",[0],this),m=this.addOutputPin("Success",[0],this),l,r,q,n=!1;this.evaluate=function(){this.getMaxInputSliceCount();var a=h.pinIsChanged()||g.pinIsChanged()||e.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==m.getValue(0)&&m.setValue(0,0);1==p.getValue(0)&&p.setValue(0,0);n&&(k.setValue(0,l),c.setValue(0,void 0),m.setValue(0,r),p.setValue(0,q),n=!1);a&&(void 0==h.getValue(0)?(k.setValue(0,""),c.setValue(0,"")):$.ajax({url:h.getValue(0),
type:"get",success:function(c,d,e){l=c;r=1;n=!0},error:function(c,d){l="";q=1;n=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
VVVV.Nodes.RandomSpread=function(b,a){function h(a){var e=[],d=0,c=0;(function(c){for(var d=0;256>d;d++)e[d]=d;for(var a=0,d=0;256>d;d++){var a=(a+e[d]+c.charCodeAt(d%c.length))%256,b=e[d];e[d]=e[a];e[a]=b}})(a);this.getRandomNumber=function(){for(var a=0,b=1,g=0;8>g;g++){d=(d+1)%256;c=(c+e[d])%256;var h=e[d];e[d]=e[c];e[c]=h;a+=e[(e[d]+e[c])%256]*b;b*=256}return a/1.8446744073709552E19}}this.constructor(b,"RandomSpread (Spreads)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],
...
[1],this),k=this.addOutputPin("Output",[0],this);this.evaluate=function(){var a=parseInt(c.getValue(0)),b=parseFloat(g.getValue(0)),l=parseFloat(d.getValue(0)),r=parseFloat(h.getValue(0)),q=e.getValue(0);""==q&&(q="Centered");var n=b/a;"Block"==q&&(n=b/(a-1));var t=n/2;if("Block"==q||"LeftJustified"==q)t=0;"RightJustified"==q&&(t=n);for(var v,w=0;w<a;w++)v=w*n+t,"Block"!=q&&0!=b&&(v=(v+l*b)%b),v=r-b/2+v,k.setValue(w,v.toFixed(4));k.setSliceCount(a)}};VVVV.Nodes.LinearSpread.prototype=new VVVV.Core.Node;
...
[0],this);this.sprcntin=this.addInputPin("Spread Count",[1],this);this.xout=this.addOutputPin("Output X",[0],this);this.yout=this.addOutputPin("Output Y",[0],this);this.evaluate=function(){for(var a=0.5*parseFloat(this.widhin.getValue(0)),b=0.5*parseFloat(this.heightin.getValue(0)),e=parseFloat(this.factorin.getValue(0)),d=parseFloat(this.phasein.getValue(0)),c=parseInt(this.sprcntin.getValue(0)),k=2*Math.PI,d=k*d,p=0,m=Math.max(this.inpyin.getSliceCount(),this.inpxin.getSliceCount()),l=0;l<m*c;l++)for(var r=
parseFloat(this.inpxin.getValue(l)),q=parseFloat(this.inpyin.getValue(l)),n=0;n<c;n++){var t=d+k*(n/c)*e,v=r+Math.cos(t)*a,t=q+Math.sin(t)*b;this.xout.setValue(p,v);this.yout.setValue(p,t);p++}this.xout.setSliceCount(c*m);this.yout.setSliceCount(c*m)}};VVVV.Nodes.CircularSpread.prototype=new VVVV.Core.Node;
...
[0],this),m=this.addOutputPin("Found At Position",[0],this);this.evaluate=function(){var a=this.getMaxInputSliceCount();c.setSliceCount(a);var b=e.getValue(0);d.getValue(0);for(var q=0,n=0;n<a;n++){var t=h.getValue(n),v=1;if("MatchesAny"==b||"ContainsAny"==b)v=g.getSliceCount();for(var w=0,y=0;y<v;y++){var s=g.getValue(y+n),u=[];if("Matches"==b||"MatchesAny"==b)s==t&&u.push(-1);else for(var s=RegExp(s,"gi"),z;z=s.exec(t);)u.push(z.index);for(s=0;s<u.length;s++)k.setValue(q,n%h.getSliceCount()),p.setValue(q,
(n+y)%g.getSliceCount()),m.setValue(q,u[s]+1),q++,w++}c.setValue(n,w)}k.setSliceCount(q);p.setSliceCount(q);m.setSliceCount(q)}};VVVV.Nodes.SiftString.prototype=new VVVV.Core.Node;
...
this),p=this.addOutputPin("Former Index",[0],this),m={None:"",Space:" ",Comma:",",CommaPlusSpace:", ",Semicolon:";",Colon:":",Pipe:"|",UnixPath:"/",DosPath:"\\",Dash:"-",NewLineAutoDetection:"\n",Dot:".",UnderScore:"_",Minus:"-"},l={Single:"'",Double:'"'};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,n=0;n<a;n++){var t=h.getValue(n),v=m[g.getValue(n)],w=e.getValue(n),y=l[d.getValue(n)],s=c.getValue(n);""==v&&(v=w);""==v&&(v=void 0);t=t.split(v);v=t.length;y=RegExp("[^"+y+"]*",
"g");for(w=0;w<v;w++)1==s?k.setValue(b,t[w]):k.setValue(b,t[w].match(y).join("")),p.setValue(b,n),b++}k.setSliceCount(b);p.setSliceCount(b)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
l=g.getValue(b),r=e.getValue(b),m=m==l||0==r&&m.toLowerCase()==l.toLowerCase()?1:0;d.setValue(b,m);c.setValue(b,1-m)}d.setSliceCount(a);c.setSliceCount(a)}};VVVV.Nodes.EQString.prototype=new VVVV.Core.Node;
...
[0],this),k=this.addOutputPin("Right Button",[0],this),p=0,m=0,l=0,r=0,q=0,n=0;$(document).mousemove(function(c){p=2*c.pageX/parseInt($("body").css("width"))-1;m=-(2*c.pageY/parseInt($("body").css("height"))-1)});$(document).bind("mousewheel",function(c){l+=c.originalEvent.wheelDelta/120});$(document).bind("DOMMouseScroll",function(c){l+=-c.originalEvent.detail/3});$(document).mousedown(function(c){switch(c.which){case 1:r=1;break;case 2:q=1;break;case 3:n=1}});$(document).mouseup(function(c){switch(c.which){case 1:r=
0;break;case 2:q=0;break;case 3:n=0}});this.evaluate=function(){h.getValue(0)!=p&&h.setValue(0,p);g.getValue(0)!=m&&g.setValue(0,m);e.getValue(0)!=l&&e.setValue(0,l);d.getValue(0)!=r&&d.setValue(0,r);c.getValue(0)!=q&&c.setValue(0,q);k.getValue(0)!=n&&k.setValue(0,n)}};VVVV.Nodes.MouseGlobal.prototype=new VVVV.Core.Node;
...
["Float"],this),p=this.addOutputPin("Output",[0.5],this);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=parseFloat(h.getValue(b)),q=parseFloat(g.getValue(b)),n=parseFloat(e.getValue(b)),t=parseFloat(d.getValue(b)),v=parseFloat(c.getValue(b)),w=k.getValue(b);if(n==q)p.setValue(b,t);else{r-=q;n-=q;switch(w){case "Clamp":r=Math.max(0,Math.min(n,r));break;case "Wrap":r=(n+r%n)%n}p.setValue(b,r*((v-t)/n)+t)}}p.setSliceCount(a)}};VVVV.Nodes.MapValue.prototype=new VVVV.Core.Node;
...
h.getValue(b),l=g.getValue(b),r=e.getValue(b),q=d.getValue(b),n=c.values[b];void 0==n&&c.setValue(b,0);if(0.5<=m){n=c.values[b];do m=Math.random()*r,0.5<=l&&(m=Math.round(m));while(0.5<=q&&m==n);c.setValue(b,m)}}c.setSliceCount(a)}};VVVV.Nodes.RandomValue.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.WebGlResource={typeName:"WebGlResource",reset_on_disconnect:!0,connectionChangedHandlers:{webglresource:function(){function b(a){return"WebGlResource"==a||"WebGlTexture"==a}if(this.direction!=PinDirection.Input){var a=this.node,h=a.findDownstreamNodes("Renderer (EX9)");a.renderContexts||(a.renderContexts=[]);void 0==a.contextChanged&&(a.contextChanged=!1);for(var g=0;g<h.length;g++)a.contextChanged|=!a.renderContexts[g]||a.renderContexts[g].canvas.id!=h[g].ctxt.id,a.renderContexts[g]=
h[g].ctxt,a.dirty=!0;a.renderContexts.length!=h.length&&(a.renderContexts.length=h.length,a.contextChanged=!0,a.dirty=!0);a.isSubpatch||_(a.inputPins).each(function(e){var d;e.markPinAsChanged();"Renderer (EX9)"!=a.nodename&&e.isConnected()&&(0<e.links.length?d=e.links[0].fromPin:0<e.masterPin.links[0].fromPin.links.length&&(d=e.masterPin.links[0].fromPin),d&&b(d.typeName)&&d.connectionChanged())});this.masterPin&&b(this.masterPin.typeName)&&this.masterPin.connectionChanged()}}},defaultValue:function(){return new VVVV.Types.Layer}};
...
VVVV.Types.ShaderProgram=function(){this.uniformSpecs={};this.attributeSpecs={};this.attribSemanticMap={};this.uniformSemanticMap={};var b="",a="",h,g;this.isSetup=!1;this.shaderProgram=void 0;this.log="";var e=this;this.extractSemantics=function(a){e.attributeSpecs={};e.attribSemanticMap={};e.uniformSpecs={};e.uniformSemanticMap={};for(var c=/(uniform|attribute) ([a-zA-Z]+)([0-9xD]*) ([a-zA-Z0-9_]+)( : ([A-Z0-9]+))?( = \{?([^;\}]+)\}?)?;/g,b;b=c.exec(a);)"attribute"!=b[1]||e.attributeSpecs[b[4]]?
...
function(){var a=this.renderContexts[0];if(a&&0.5<=d.getValue(0)){for(var b=[],l=[],r=[],q=0;q<this.getMaxInputSliceCount();q++)b[q]=parseFloat(h.getValue(q)),l[q]=parseFloat(e.getValue(q)),r[q]=parseFloat(g.getValue(q));k=new VVVV.Types.VertexBuffer(a,b);k.setSubBuffer("TEXCOORD0",2,l);k.setSubBuffer("NORMAL",3,r);k.create();c.setValue(0,k)}}};VVVV.Nodes.VertexBufferJoin.prototype=new VVVV.Core.Node;
...
m=[],l=[],r=[],q=0,n=0;n<b;n++)for(var t=0;t<a;t++)m.push(parseFloat(t)/(a-1)-0.5),m.push(0.5-parseFloat(n)/(b-1)),m.push(0),q++,l.push(0),l.push(0),l.push(1),r.push(parseFloat(t)/(a-1)),r.push(parseFloat(n)/(b-1));m=new VVVV.Types.VertexBuffer(c,m);m.setSubBuffer("TEXCOORD0",2,r);m.setSubBuffer("NORMAL",3,l);m.create();l=[];for(n=0;n<b-1;n++)for(t=0;t<a-1;t++)r=t+a*n,l.push(r),l.push(r+1),l.push(r+a+1),l.push(r+a+1),l.push(r+a),l.push(r);d=new VVVV.Types.Mesh(c,m,l);e.setValue(0,d)}}}};
...
parseInt(g.getValue(0)),m=parseInt(e.getValue(0)),l=parseFloat(h.getValue(0)),r=[],q=[],n=[],t=0;t<m+1;t++)for(var v=Math.cos(-parseFloat(t)/m*Math.PI),w=0;w<b;w++){var y=Math.cos(2*(parseFloat(w)/b)*Math.PI)*Math.cos(Math.asin(v)),s=Math.sin(2*(parseFloat(w)/b)*Math.PI)*Math.cos(Math.asin(v));r.push(y*l);r.push(v*l);r.push(s*l);q.push(y);q.push(v);q.push(s);n.push(parseFloat(w)/b);n.push(parseFloat(t)/m)}l=new VVVV.Types.VertexBuffer(a,r);l.setSubBuffer("TEXCOORD0",2,n);l.setSubBuffer("NORMAL",3,
q);l.create();q=[];for(t=0;t<m;t++)for(w=0;w<b;w++)n=b*t,r=w+n,q.push(r),q.push((r+1)%b+n),q.push((r+1)%b+b+n),q.push((r+1)%b+b+n),q.push(r+b),q.push(r);c=new VVVV.Types.Mesh(a,l,q);d.setValue(0,c)}}}};VVVV.Nodes.Sphere.prototype=new VVVV.Core.Node;
...
[],this,VVVV.PinTypes.WebGlResource),l=null;this.evaluate=function(){if(this.renderContexts){var a=this.renderContexts[0];if(a){var b=parseInt(k.getValue(0)),n=parseInt(p.getValue(0)),t=parseFloat(h.getValue(0)),v=parseFloat(g.getValue(0)),w=parseFloat(e.getValue(0)),y=parseFloat(d.getValue(0)),s=[],u=[],z=[];s.push(0);s.push(w/2);s.push(0);u.push(0);u.push(1);u.push(0);z.push(0);z.push(0);s.push(0);s.push(-w/2);s.push(0);u.push(0);u.push(-1);u.push(0);z.push(0);z.push(0);for(var D=0;D<n+1;D++)for(var A=
parseFloat(D)/n,G=(A-0.5)*-w,C=0;C<b+1;C++){var J=Math.cos(2*(parseFloat(C)/b)*Math.PI*y-Math.PI*y-Math.PI/2),E=Math.sin(2*(parseFloat(C)/b)*Math.PI*y-Math.PI*y-Math.PI/2),F=A*v+(1-A)*t;s.push(J*F);s.push(G);s.push(E*F);u.push(J);u.push(0);u.push(E);z.push(parseFloat(C)/b);z.push(parseFloat(D)/n)}t=new VVVV.Types.VertexBuffer(a,s);t.setSubBuffer("TEXCOORD0",2,z);t.setSubBuffer("NORMAL",3,u);t.create();u=[];if(0.5<c.getValue(0))for(A=0;2>A;A++)for(C=0;C<b;C++)u.push(A),u.push(2+C+A+A*n*(b+1)),u.push(2+
...
d.enumOptions=[""];this.addOutputPin("Layer",[],this,VVVV.PinTypes.WebGlResource);var c=[],k=null,p,m=!1,l=[],r=this;this.initialize=function(){var a=["Render State","Mesh","Transform","Technique"];_(r.inputPins).each(function(c){0>a.indexOf(c.pinname)&&(c.unvalidated=!0,l.push(c))});void 0==VVVV.ShaderCodeResources[r.shaderFile]?(VVVV.ShaderCodeResources[r.shaderFile]=new VVVV.Types.ShaderCodeResource,VVVV.ShaderCodeResources[r.shaderFile].addRelatedNode(r),$.ajax({url:VVVV.Helpers.prepareFilePath(r.shaderFile,
r.parentPatch),async:!1,dataType:"text",success:function(a){VVVV.ShaderCodeResources[r.shaderFile].setSourceCode(a)},error:function(){console.log("ERROR: Could not load shader file "+r.shaderFile.replace("%VVVV%",VVVV.Root));VVVV.onNotImplemented("Could not load shader file "+r.shaderFile.replace("%VVVV%",VVVV.Root))}})):VVVV.ShaderCodeResources[r.shaderFile].addRelatedNode(r)};this.shaderSourceUpdated=function(a){p=a;k||(k=new VVVV.Types.ShaderProgram);r.addUniformPins();r.setupShader();_(r.inputPins).each(function(a){a.markPinAsChanged()});
m=!0;this.parentPatch.afterUpdate()};this.addUniformPins=function(){k.extractSemantics(p);for(var a=[],c=0;c<l.length;c++)k.uniformSpecs[l[c].pinname.replace(/ /g,"_")]||(r.removeInputPin(l[c].pinname),a.push(c));for(c=0;c<a.length;c++)l.splice(a[c],1);_(k.uniformSpecs).each(function(a){if("VIEW"!=a.semantic&&"PROJECTION"!=a.semantic&&"WORLD"!=a.semantic){var c=!1,b=VVVV.PinTypes.Generic,e=[];switch(a.type){case "mat":b=VVVV.PinTypes.Transform;c=!0;break;case "sampler":b=VVVV.PinTypes.WebGlTexture;
...
console.log("ERROR: No vertex shader code for technique "+a+" found"):(e=b[4],void 0==(b=c.exec(p+"\nfragment_shader"))?console.log("ERROR: No fragment shader code for technique "+a+" found"):(k.setFragmentShader(g+b[4]),k.setVertexShader(g+e)))};this.evaluate=function(){if(this.renderContexts){var a=this.renderContexts[0];if(a&&k){if(!k.isSetup||this.contextChanged||d.pinIsChanged())this.setupShader(),k.setup(a)?VVVV.ShaderCodeResources[r.shaderFile].definingNode&&VVVV.ShaderCodeResources[r.shaderFile].definingNode.showStatus("success",
"Successfully compiled"):VVVV.ShaderCodeResources[r.shaderFile].definingNode&&VVVV.ShaderCodeResources[r.shaderFile].definingNode.showStatus("error",k.log);var b=0;_(this.inputPins).each(function(a){var c=a.getSliceCount();a=a.pinname.replace(/ /g,"_");k.uniformSpecs[a]&&("vec"==k.uniformSpecs[a].type&&"COLOR"!=k.uniformSpecs[a].semantic)&&(c=parseInt(c/k.uniformSpecs[a].dimension));c>b&&(b=c)});g.isConnected()&&void 0!=g.getValue(0)||(b=0);var p=c.length;if(this.contextChanged||m)p=0;b<p&&c.splice(b,
...
m.enumOptions=["NONE","DX16"];var l=this.addOutputPin("Actual Backbuffer Width",[0],this),r=this.addOutputPin("Actual Backbuffer Height",[0],this),q=this.addOutputPin("EX9 Out",[],this,VVVV.PinTypes.WebGlResource),n=0,t=0,v,w,y=this.ctxt=void 0,s,u,z;this.getContexts=function(){if(this.invisiblePins["Descriptive Name"]){var a=this.invisiblePins["Descriptive Name"].getValue(0),a=$(a).get(0),b;if(a&&"CANVAS"==a.nodeName)b=$(a);else{b=parseInt(d.getValue(0));var l=parseInt(c.getValue(0));b=$('<canvas width="'+
(0<b?b:512)+'" height="'+(0<l?l:512)+'" id="vvvv-js-generated-renderer-'+(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');a||(a="body");$(a).append(b)}if(b){h(b);try{y=b.get(0).getContext("experimental-webgl",{preserveDrawingBuffer:!0}),y.viewportWidth=parseInt(b.get(0).width),y.viewportHeight=parseInt(b.get(0).height)}catch(m){console.log(m)}this.ctxt=y;q.isConnected()&&this.renderContexts&&this.renderContexts[0]?(s=this.ctxt=this.renderContexts[0],u=s.createFramebuffer(),s.bindFramebuffer(s.FRAMEBUFFER,
u),u.width=b.get(0).width,u.height=b.get(0).height,z=s.createTexture(),s.bindTexture(s.TEXTURE_2D,z),s.pixelStorei(s.UNPACK_FLIP_Y_WEBGL,!0),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.LINEAR),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.LINEAR_MIPMAP_NEAREST),s.texImage2D(s.TEXTURE_2D,0,s.RGBA,u.width,u.height,0,s.RGBA,s.UNSIGNED_BYTE,null),s.generateMipmap(s.TEXTURE_2D),a=s.createRenderbuffer(),s.bindRenderbuffer(s.RENDERBUFFER,a),s.renderbufferStorage(s.RENDERBUFFER,s.DEPTH_COMPONENT16,
u.width,u.height),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,z,0),s.framebufferRenderbuffer(s.FRAMEBUFFER,s.DEPTH_ATTACHMENT,s.RENDERBUFFER,a),s.bindTexture(s.TEXTURE_2D,null),s.bindRenderbuffer(s.RENDERBUFFER,null),s.bindFramebuffer(s.FRAMEBUFFER,null)):this.renderContexts&&this.renderContexts[0]&&(this.renderContexts[0].deleteTexture(z),z=void 0);this.ctxt&&(n=parseInt(b.get(0).width),t=parseInt(b.get(0).height),s=this.ctxt,a=new Uint8Array([255,255,255]),s.DefaultTexture=
s.createTexture(),s.bindTexture(s.TEXTURE_2D,s.DefaultTexture),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.CLAMP_TO_EDGE),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.CLAMP_TO_EDGE),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.pixelStorei(s.UNPACK_ALIGNMENT,1),s.texImage2D(s.TEXTURE_2D,0,s.RGB,1,1,0,s.RGB,s.UNSIGNED_BYTE,a),s.bindTexture(s.TEXTURE_2D,null),this.inputPins.Layers.markPinAsChanged(),g.markPinAsChanged(),
e.markPinAsChanged(),k.markPinAsChanged(),p.markPinAsChanged())}}};var D=!1;this.evaluate=function(){s=this.ctxt;if(this.invisiblePins["Descriptive Name"].pinIsChanged()||this.contextChanged)y&&$(y.canvas).hasClass("vvvv-js-generated-renderer")&&$(y.canvas).remove(),this.getContexts(),this.inputPins.Layers.isConnected()&&this.inputPins.Layers.links[0].fromPin.connectionChanged();D||(l.setValue(0,n),r.setValue(0,t),D=!0);if(void 0!=s){if(d.pinIsChanged()&&(!this.renderContexts||!this.renderContexts[0])){var a=
parseInt(d.getValue(0));0<a&&(n=a,$(y.canvas).attr("width",n),l.setValue(0,n))}!c.pinIsChanged()||this.renderContexts&&this.renderContexts[0]||(a=parseInt(c.getValue(0)),0<a&&(t=a,$(y.canvas).attr("height",t),r.setValue(0,t)));this.renderContexts&&this.renderContexts[0]&&s==this.renderContexts[0]?s.bindFramebuffer(s.FRAMEBUFFER,u):s.bindFramebuffer(s.FRAMEBUFFER,null);a=_(e.getValue(0).split(",")).map(function(a){return parseFloat(a)});s.clearColor(a[0],a[1],a[2],a[3]);s.clear(s.COLOR_BUFFER_BIT|
s.DEPTH_BUFFER_BIT);"NONE"==m.getValue(0)?s.disable(s.DEPTH_TEST):s.enable(s.DEPTH_TEST);p.pinIsChanged()&&(p.isConnected()?(v=mat4.create(),mat4.set(p.getValue(0),v)):(v=mat4.create(),mat4.ortho(-1,1,-1,1,-100,100,v)),mat4.scale(v,[1,1,-1]),this.renderContexts&&this.renderContexts[0]&&mat4.scale(v,[1,-1,1]));k.pinIsChanged()&&(w=k.getValue(0));if(this.contextChanged)this.contextChanged=!1;else{s.viewport(0,0,n,t);var b=a=null,g=null;if(this.inputPins.Layers.isConnected())for(var h=this.inputPins.Layers.values,
E=0;E<h.length;E++)if(layer=h[E],void 0!=layer.shader){a!=layer.shader.shaderProgram&&(s.useProgram(layer.shader.shaderProgram),s.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.PROJECTION].position,!1,v),s.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.VIEW].position,!1,w));var F=layer.renderState;F||(F=defaultWebGlRenderState);F!=b&&F.apply(s);if(layer.mesh!=g||layer.shader.shaderProgram!=a)s.bindBuffer(s.ARRAY_BUFFER,layer.mesh.vertexBuffer.vbo),
_(layer.mesh.vertexBuffer.subBuffers).each(function(a){layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]]&&(s.enableVertexAttribArray(layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position),s.vertexAttribPointer(layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position,a.size,s.FLOAT,!1,0,a.offset))}),s.bindBuffer(s.ELEMENT_ARRAY_BUFFER,layer.mesh.indexBuffer);for(var b=layer.uniformNames.length,H=g=0;H<b;H++){var B=layer.uniforms[layer.uniformNames[H]];
if(void 0!=B.value&&!(0<E&&layer.shader.shaderProgram==a&&h[E-1].uniforms[layer.uniformNames[H]]&&B.value==h[E-1].uniforms[layer.uniformNames[H]].value)){start=(new Date).getTime();switch(B.uniformSpec.type){case "mat":s["uniformMatrix"+B.uniformSpec.dimension+"fv"](B.uniformSpec.position,!1,B.value);break;case "vec":s["uniform"+B.uniformSpec.dimension+"fv"](B.uniformSpec.position,B.value);break;case "int":s["uniform"+B.uniformSpec.dimension+"i"](B.uniformSpec.position,B.value);break;case "float":s["uniform"+
B.uniformSpec.dimension+"f"](B.uniformSpec.position,B.value);break;case "sampler":var I=B.value;I==VVVV.DefaultTexture&&(I=s.DefaultTexture);s.activeTexture(s["TEXTURE"+g]);s.bindTexture(s["TEXTURE_"+B.uniformSpec.dimension],I);s.uniform1i(B.uniformSpec.position,g);g++}loopstart=(new Date).getTime()}}s.drawElements(s[F.polygonDrawMode],layer.mesh.numIndices,s.UNSIGNED_SHORT,0);a=layer.shader.shaderProgram;b=F;g=layer.mesh}this.renderContexts&&this.renderContexts[0]&&(s.bindTexture(s.TEXTURE_2D,z),
s.generateMipmap(s.TEXTURE_2D),s.bindTexture(this.renderContexts[0].TEXTURE_2D,null));q.setValue(0,z)}}}};VVVV.Nodes.RendererWebGL.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Color.makeLabel=function(b,a){var h=a.IOBoxRows();a.IOBoxInputPin().getSliceCount();d3.select(b).selectAll(".vvvv-node-label").remove();for(var g=0;g<h;g++)d3.select(b).append("svg:rect").attr("class","vvvv-node-label").attr("height",a.getHeight()/h-8).attr("width",a.getWidth()).attr("y",12*g+4).attr("fill",function(b){b=a.IOBoxInputPin().getValue(g).split(",");for(var d=0;d<b.length;d++)b[d]=parseInt(256*b[d]);return"rgba("+b.join(",")+")"})};
VVVV.PinTypes.Color.openInputBox=function(b,a,h,g){function e(a){d=a.altKey&&a.shiftKey?3:a.altKey?1:a.shiftKey?2:0}var d=0;$(b.window.document).keydown(e).keyup(e);$inputbox=$('<div class="pininputbox color resettable"></div>');$inputbox.css("position",a.css("position"));$inputbox.css("width",a.css("width"));$inputbox.css("height",a.css("height"));$inputbox.css("left",a.css("left"));$inputbox.css("top",a.offset().top-8+"px");b=h.getValue(g).split(",");for(var c=0;c<b.length;c++)b[c]=parseInt(256*
...
VVVV.Editors.BrowserEditor.PatchWindow=function(b,a){var h;function g(){c.selectAll(".vvvv-node.selected").attr("class",function(a){return a.isIOBox?"vvvv-node vvvv-iobox":"vvvv-node"});q=[]}this.state=UIState.Idle;var e=0,d=0,c,k,p,m,l,r=void 0,q=[],n=b,t=0,v=VVVV.Root+"/patch.html";h=!1;var w=0,y=0,s=0,u=0;b.isSubpatch&&(v="patch.html");this.window=window.open(v,b.nodename,"location=no, width="+b.width+", height="+b.height+", toolbar=no");var z=this;window.setTimeout(function(){function l(a){h=
a.shiftKey}for(var m=0;m<n.nodeList.length;m++)t=Math.max(t,n.nodeList[m].id);z.window.document.title=b.nodename;root=d3.select(z.window.document.body);$("body",z.window.document).on("copy",function(a){for(var b=$("<PATCH>"),c=[],d=_(q).map(function(a){return a.id}),e=0;e<q.length;e++)b.append(q[e].serialize()),_(q[e].inputPins).each(function(a){for(var b=0;b<a.links.length;b++)0<=d.indexOf(a.links[b].fromPin.node.id)&&c.push(a.links[b])});for(e=0;e<c.length;e++)b.append(c[e].serialize());b='<!DOCTYPE PATCH  SYSTEM "http://vvvv.org/versions/vvvv45beta28.1.dtd" >\r\n'+
...
$("body",z.window.document).mousemove(function(a){p=a.pageX;v=a.pageY});$("body",z.window.document).on("paste",function(b){b=b.originalEvent.clipboardData.getData("text/plain");if(!b.match(/^<!DOCTYPE PATCH/))return!1;$patch=$(b);var e=void 0,d=void 0;$patch.find("node bounds").each(function(){var a=parseInt($(this).attr("left")),b=parseInt($(this).attr("top"));if(!e||a<e)e=a;if(!d||b<d)d=b});var h={};$patch.find("node").each(function(){t++;h[$(this).attr("id")]=t;$(this).attr("createme","pronto");
$(this).attr("id",t);var a=parseInt($(this).find("bounds").attr("left"))-e+15*p,b=parseInt($(this).find("bounds").attr("top"))-d+15*v;$(this).find("bounds").attr("left",a);$(this).find("bounds").attr("top",b)});$patch.find("link").each(function(){$(this).attr("createme","pronto");var a=$(this).attr("srcnodeid"),b=$(this).attr("dstnodeid");$(this).attr("srcnodeid",h[a]);$(this).attr("dstnodeid",h[b])});b=$patch.wrapAll("<d></d>").parent().html();a.update(n.nodename,b);g();var k=_(h).map(function(a){return a});
...
k.id}).attr("x2",function(b){return b.toPin.x+b.toPin.node.x+a}).attr("y2",function(a){return a.toPin.y+a.toPin.node.y+b})}}else z.state==UIState.AreaSelecting&&(s=d3.event.pageX,u=d3.event.pageY,c.select(".selection-area").attr("transform","translate("+Math.min(w,s)+", "+Math.min(y,u)+")").attr("width",Math.abs(s-w)).attr("height",Math.abs(u-y)),g(),c.selectAll(".vvvv-node").each(function(a){for(var b=[a.x,a.x+a.getWidth()],c=[a.y,a.y+a.getHeight()],e=!1,d=0;2>d;d++)for(var g=0;2>g;g++)b[d]>=Math.min(w,
s)&&(b[d]<=Math.max(w,s)&&c[g]>=Math.min(y,u)&&c[g]<=Math.max(y,u))&&(e=!0);e&&(d3.select(this).attr("class","vvvv-node selected"),q.push(a))}))}).on("contextmenu",function(){z.state==UIState.Connecting&&(c.select(".vvvv-link.current-link").remove(),z.state=UIState.Idle);d3.event.stopPropagation();d3.event.preventDefault();return!1}).on("mouseup",function(){if(z.state==UIState.Moving){z.state=UIState.Idle;for(var b=d3.event.pageX-e,g=d3.event.pageY-d,h="<PATCH>",k=0;k<q.length;k++)var l=q[k],h=h+
("<NODE componentmode='Node' id='"+l.id+"'><BOUNDS type='Node' left='"+15*(b+l.x)+"' top='"+15*(g+l.y)+"' width='"+l.width+"' height='"+l.height+"'/></NODE>");a.update(n.nodename,h+"</PATCH>")}z.state==UIState.AreaSelecting&&(c.select(".selection-area").remove(),z.state=UIState.Idle)}).on("dblclick",function(){function b(a){h.empty();var c=g.find("#node_filter").val().toLowerCase();a=VVVV.NodeNames.concat(_(VVVV.ShaderCodeResources).map(function(a,b){return b.replace("%VVVV%/effects/","")}));a=_(_(a).filter(function(a){return 0<=
VVVV.Helpers.translateOperators(a).toLowerCase().indexOf(c)})).sortBy(function(a){return a.toLowerCase().indexOf(c)});for(var e=0;e<a.length;e++)h.append($("<option>"+a[e]+"</option>"));h.find("option").first().attr("selected",!0)}function c(){var b=g.find("#node_filter").val(),h=g.find("#new_node option:selected").val(),k="";h||!b.match(/\.fx$/)&&!b.match(/\.v4p$/)||(h=b);h?((b=h.match(/^(.*\/([^\.]+))(\.vvvvjs)?\.fx$/))?(h=b[2]+" (EX9.Effect)",k=b[1]+".fx"):(b=h.match("([^.]+)(.vvvvjs)?.fx$"))?
(h=b[1]+" (EX9.Effect)",k="%VVVV%/effects/"+b[1]+".fx"):h.match("[^/].v4p$")&&(k=h=f),t++,h="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='"+h+"' systemname='"+h+"' "+(""!=k?"filename='"+k+"'":"")+">"),h+="<BOUNDS type='Node' left='"+15*e+"' top='"+15*d+"' width='100' height='100'/>"):(t++,h="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='IOBox (String)' systemname='IOBox (String)'>"),h+="<BOUNDS type='Node' left='"+15*e+"' top='"+15*d+"' width='100' height='100'/>",h+=
"<PIN pinname='Input String' values='|"+b+"|'/>");a.update(n.nodename,h+"</NODE>");g.remove()}$("#node_selection",z.window.document).remove();var e=d3.event.pageX,d=d3.event.pageY,g=$('<div id="node_selection"><input type="text" id="node_filter"/></div>'),h=$('<select id="new_node" size="8">');g.append(h);g.css("left",e);g.css("top",d);$("body",z.window.document).append(g);g.find("input").bind("paste",function(a){a.stopPropagation()});g.find("#node_filter").get(0).focus();b();g.find("#node_filter").keyup(function(a){if(13==
a.which)c();else{if(40==a.which)return h.find("option:selected").first().next().attr("selected",!0),!1;if(38==a.which)return h.find("option:selected").first().prev().attr("selected",!0),!1;b()}});g.find("#new_node").click(c)}).on("mousedown",function(){z.state==UIState.Idle&&(z.state=UIState.AreaSelecting,w=s=d3.event.pageX+1,y=u=d3.event.pageY+1,c.append("svg:rect").attr("class","selection-area").attr("stroke","#000").attr("stroke-dasharray","2,2").attr("stroke-width",1).attr("fill","rgba(0,0,0,0)").attr("transform",
"translate("+w+", "+y+")").attr("width",0).attr("height",0),g())});c.append("svg:rect").attr("class","background").attr("width",Math.max(n.width,n.boundingBox.width)).attr("height",Math.max(n.height,n.boundingBox.height)).on("click",function(){z.state=UIState.Idle;$(".resettable",z.window.document).remove();$("#node_selection",z.window.document).remove();g();r=void 0});$(z.window.document).keydown(l);$(z.window.document).keyup(l);$(z.window.document).keydown(function(b){if(46==b.which&&0<q.length){var e=
"<PATCH>";for(b=0;b<q.length;b++){var d=q[b],e=e+("<NODE id='"+q[b].id+"' deleteme='pronto'/>");_(d.inputPins).each(function(a){_(a.links).each(function(a){e+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(d.outputPins).each(function(a){_(a.links).each(function(a){e+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+
"' dstpinname='"+a.toPin.pinname+"'/>"})})}e+="</PATCH>";a.update(n.nodename,e);q=[]}else if(115!=b.which&&83!=b.which||!b.ctrlKey){if(73==b.which&&b.ctrlKey)return a.inspector?a.inspector.win.focus():a.openInspector("."),b.preventDefault(),!1;if(65==b.which&&b.ctrlKey)return g(),c.selectAll(".vvvv-node").each(function(a){q.push(a);d3.select(this).attr("class","vvvv-node selected")}),b.preventDefault(),!1}else return a.save(n.nodename,n.toXML()),b.preventDefault(),!1});$(z.window).resize(function(){n.width=
$(this).width();n.height=$(this).height();n.boundingBox.width=Math.max(n.width,n.boundingBox.width);n.boundingBox.height=Math.max(n.height,n.boundingBox.height);n.afterUpdate()});z.drawComplete();n.afterUpdate=function(){z.drawComplete();a.inspector&&window.setTimeout(a.inspector.update,100)};n.afterEvaluate=function(){k.filter(function(a){return a.isIOBox&&VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel}).each(function(a){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a)})}},1E3);this.close=
function(){this.window.close()};this.drawComplete=function(){k&&k.remove();l&&l.remove();n.width==$(z.window).width()&&n.height==$(z.window).height()||z.window.resizeTo(n.width,n.height);c.attr("width",Math.max(n.width,n.boundingBox.width)).attr("height",Math.max(n.height,n.boundingBox.height));c.selectAll(".background").attr("width",Math.max(n.width,n.boundingBox.width)).attr("height",Math.max(n.height,n.boundingBox.height));var b=c.append("svg:g").attr("class","link-group");k=c.selectAll("g.vvvv-node").data(n.nodeList).enter().append("svg:g").attr("class",
...
"rgba(0,0,0,0)":"rgba(0,0,0,1)"}).attr("y",function(a){return a.getHeight()}).attr("width",5).attr("height",14);k.each(function(a){if(a.isIOBox&&VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a);for(var b=a.IOBoxRows(),c=a.IOBoxInputPin().getSliceCount(),e=this,d=0;d<b;d++)(function(d){d3.select(e).append("svg:rect").attr("width",a.getWidth()+2).attr("height",a.getHeight()/b-4).attr("y",12*d+2).attr("fill","rgba(0,0,0,0)").on("contextmenu",
function(a){if(void 0!=a.IOBoxInputPin().getValue(0)){$(".resettable",z.window.document).remove();var e=$("<input type='text'/>");$("body",z.window.document).append(e);e.css("position","absolute");e.css("left",$(this).offset().left);e.css("top",$(this).offset().top+2);e.css("width",a.getWidth());e.css("height",a.getHeight()/b-4);VVVV.PinTypes[a.IOBoxInputPin().typeName].openInputBox(z.window,e,a.IOBoxInputPin(),d%c)}d3.event.stopPropagation();d3.event.preventDefault();return!1})})(d)}else d3.select(this).append("svg:text").text(function(a){return a.label()}).attr("class",
"vvvv-node-label").attr("shape-rendering","crispEdges").attr("dy",function(a,b){return 12*b+12}).attr("dx",4).attr("font-size",10).attr("font-family","Lucida Sans Unicode")});p=k.selectAll("g.vvvv-input-pin").data(function(a){return a.isSubpatch?_(a.inputPins).sortBy(function(a){return a.slavePin?a.slavePin.node.x:1}).map(function(a,b){return a}):_(a.inputPins).map(function(a,b){return a})}).enter().append("svg:g").attr("class","vvvv-input-pin").attr("transform",function(a,b){pinOffset=0;1<_(a.node.inputPins).size()&&
(pinOffset=(a.node.getWidth()-4)/(_(a.node.inputPins).size()-1));a.y=0;a.x=b*pinOffset;return"translate("+a.x+", 0)"});p.append("svg:rect").attr("width",4).attr("height",4).attr("fill",function(a){return a.node.isComment()?"rgba(0,0,0,0)":"#666666"}).on("mouseover",function(a,b){c.selectAll("#vvvv-node-"+a.node.id+" g.vvvv-input-pin").filter(function(a,c){return c==b}).each(function(){d3.select(this).append("svg:rect").attr("class","vvvv-input-pin-highlight").attr("width",4).attr("height",4).attr("fill",
"rgba(0,0,0,1)").attr("y",-4);d3.select(this).append("svg:text").text(function(a){return 1<a.getSliceCount()?a.pinname+"("+a.getSliceCount()+"): "+a.getValue(0):a.pinname+": "+a.getValue(0)}).attr("dy",30).attr("font-size",10).attr("font-family","Lucida Sans Unicode").attr("fill","rgba(0,0,0,1)")})}).on("mouseout",function(a,b){c.selectAll("#vvvv-node-"+a.node.id+" g.vvvv-input-pin text").remove();c.selectAll("#vvvv-node-"+a.node.id+" g.vvvv-input-pin rect.vvvv-input-pin-highlight").remove()});m=
k.selectAll("g.vvvv-output-pin").data(function(a){return _(a.outputPins).map(function(a,b){return a})}).enter().append("svg:g").attr("class","vvvv-output-pin").attr("transform",function(a,b){pinOffset=0;1<_(a.node.outputPins).size()&&(pinOffset=(a.node.getWidth()-4)/(_(a.node.outputPins).size()-1));a.y=a.node.getHeight()-4;a.x=b*pinOffset;return"translate("+a.x+", "+a.y+")"});m.append("svg:rect").attr("width",4).attr("height",4).attr("fill",function(a){return a.node.isComment()?"rgba(0,0,0,0)":"#666666"}).on("mouseover",
...
z.window.document).attr("width",$(this,z.window.document).parent().next().get(0).getBBox().width+4)});c.selectAll("g.vvvv-input-pin").on("contextmenu",function(a,b){if(VVVV.PinTypes[a.typeName].openInputBox&&void 0!=a.getValue(0)){$(".resettable",z.window.document).remove();var c=$("<input type='text'/>");$("body",z.window.document).append(c);c.css("position","absolute");c.css("left",$(this).offset().left);c.css("top",$(this).offset().top-3);c.css("width",50);c.css("height",14);VVVV.PinTypes[a.typeName].openInputBox(z,
c,a,0)}d3.event.stopPropagation();d3.event.preventDefault();return!1});var s=["Generic","Color","Enum"];c.selectAll("g.vvvv-input-pin, g.vvvv-output-pin").on("click",function(b,e){if(z.state!=UIState.Connecting){r=b;z.state=UIState.Connecting;c.append("svg:line").attr("class","vvvv-link current-link resettable").attr("stroke","#000").attr("stroke-width",1).attr("x1",b.x+b.node.x+2+0.5).attr("y1",b.y+b.node.y+2+0.5).attr("x2",b.x+b.node.x+2+0.5).attr("y2",b.y+b.node.y+2+0.5);var d=r.direction==PinDirection.Output?
"input":"output",g=r.node.getUpstreamNodes();c.selectAll("g.vvvv-"+d+"-pin").filter(function(a){return a.typeName!=r.typeName&&("Node"!=r.typeName||0<=s.indexOf(a.typeName))&&("Node"!=a.typeName||0<=s.indexOf(r.typeName))||0<=g.indexOf(a.node)||a.node==r.node?!1:!0}).append("svg:rect").attr("class","vvvv-connection-highlight").attr("width",4).attr("height",3).attr("fill","rgba(0,0,0,1)").attr("y",function(a){return a.direction==PinDirection.Input?-3:3})}else{if(r.direction==PinDirection.Input)var d=
b,h=r;else d=r,h=b;if(d.typeName==h.typeName||"Node"==d.typeName&&0>s.indexOf(h.typeName)||"Node"==h.typeName&&0>s.indexOf(d.typeName)){var k="<PATCH>";_(h.links).each(function(a){k+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"});k+="<LINK createme='pronto' srcnodeid='"+d.node.id+"' srcpinname='"+d.pinname+"' dstnodeid='"+h.node.id+"' dstpinname='"+h.pinname+"'/>";k+="</PATCH>";a.update(n.nodename,
k);c.select(".vvvv-link.current-link").remove();c.select(".vvvv-connection-highlight").remove();z.state=UIState.Idle}n.afterUpdate()}d3.event.stopPropagation()}).on("mousedown",function(){d3.event.stopPropagation()});c.selectAll("g.vvvv-link").on("contextmenu",function(b){a.update(n.nodename,"<PATCH><LINK deleteme='pronto' srcnodeid='"+b.fromPin.node.id+"' srcpinname='"+b.fromPin.pinname+"' dstnodeid='"+b.toPin.node.id+"' dstpinname='"+b.toPin.pinname+"'/></PATCH>");d3.event.preventDefault()});k.on("mousedown",
function(b){$(".resettable",z.window.document).remove();z.state=UIState.Moving;0>q.indexOf(b)&&(h||g(),d3.select(this).attr("class","vvvv-node selected"),q.push(b));e=d3.event.pageX;d=d3.event.pageY;a.inspector&&a.inspector.setNode(b);d3.event.preventDefault();d3.event.stopPropagation();return!1}).on("contextmenu",function(b){b.isSubpatch?a.openPatch(b):b.openUIWindow&&b.openUIWindow();d3.event.preventDefault();d3.event.stopPropagation();return!1})}};
