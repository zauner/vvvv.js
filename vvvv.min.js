...
c});for(var d=[],h="",f=!1,e=c.length,g=0;g<e;g++)","!=c[g]||f?"|"==c[g]?f=!f:h+=c[g]:(d.push(h),h="");d.push(h);return d}function c(){var c=window.location.hash.match("#([^/]+)/"+p.ressource+"$");c&&(console.log("launching editor ..."),(new VVVV.Editors[c[1]]).enable(p))}this.ressource=a;this.vvvv_version="45_26.1";this.boundingBox={width:0,height:0};this.windowHeight=this.windowWidth=500;this.pinMap={};this.nodeMap={};this.nodeList=[];this.linkList=[];this.success=b;this.error=g;this.XMLCode="";
this.editor=void 0;this.setupObject();e&&(this.parentPatch=e);f&&(this.id=f);var h=!1;this.getAbsolutePath=function(){var c=this.getRelativePath();this.parentPatch&&(c=this.parentPatch.getRelativePath()+c);return c};this.getRelativePath=function(){return this.nodename.match(/(.*\/)?[^/]+\.v4p/)[1]||""};this.destroy=function(){for(var c=0;c<this.nodeList.length;c++)this.nodeList[c].destroy(),delete this.nodeMap[this.nodeList[c].id],delete this.nodeList[c]};if("45_26">=this.vvvv_version)var q={},n=
{},k={},r={};var p=this;this.doLoad=function(c,h){function f(c){"complete"==g&&(n={});$(c).find('link[deleteme="pronto"]').each(function(){for(var c=!1,d=0;d<p.linkList.length;d++)p.linkList[d].fromPin.node.id==$(this).attr("srcnodeid")&&(p.linkList[d].fromPin.pinname==$(this).attr("srcpinname")&&p.linkList[d].toPin.node.id==$(this).attr("dstnodeid")&&p.linkList[d].toPin.pinname==$(this).attr("dstpinname"))&&(c=p.linkList[d]);if(c){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+
" -> "+c.toPin.pinname);var d=c.fromPin,h=c.toPin;c.destroy();d.connectionChanged();h.connectionChanged();h.markPinAsChanged();h.reset_on_disconnect&&h.reset()}});$(c).find('link[deleteme!="pronto"]').each(function(){var c=p.pinMap[$(this).attr("srcnodeid")+"_out_"+$(this).attr("srcpinname")],d=p.pinMap[$(this).attr("dstnodeid")+"_in_"+$(this).attr("dstpinname")];void 0==c&&p.nodeMap[$(this).attr("srcnodeid")]&&(c=p.nodeMap[$(this).attr("srcnodeid")].addOutputPin($(this).attr("srcpinname"),void 0));
void 0==d&&p.nodeMap[$(this).attr("dstnodeid")]&&(d=p.nodeMap[$(this).attr("dstnodeid")].addInputPin($(this).attr("dstpinname"),void 0));if(c&&d){for(var h=!1,f=0;f<p.linkList.length;f++)p.linkList[f].fromPin.node.id==c.node.id&&(p.linkList[f].fromPin.pinname==c.pinname&&p.linkList[f].toPin.node.id==d.node.id&&p.linkList[f].toPin.pinname==d.pinname)&&(h=p.linkList[f]);if(!h){h=new VVVV.Core.Link(c,d);c.connectionChanged();d.connectionChanged();p.linkList.push(h);for(f=0;f<c.values.length;f++)d.setValue(f,
...
if(e=/^<!DOCTYPE\s+PATCH\s+SYSTEM\s+"(.+)\\(.+)\.dtd/.exec(c))p.vvvv_version=e[2].replace(/[a-zA-Z]+/,"_");var g="diff";if(/\s<PATCH/.test(c)||"45_26">=p.vvvv_version)g="complete","development"==VVVV_ENV&&console.log("complete: "+this.nodename),p.XMLCode=c;e=$(c).find('bounds[type="Window"]').first();0<e.length&&(p.windowWidth=e.attr("width")/15,p.windowHeight=e.attr("height")/15);"complete"==g&&(r={});$(c).find("node").each(function(){if("pronto"==$(this).attr("createme")&&void 0!=p.nodeMap[$(this).attr("id")]){var e=
p.nodeMap[$(this).attr("id")];"development"==VVVV_ENV&&console.log("node renamed, so deleting node "+e.id+" / "+e.nodename);_(e.inputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.fromPin.connectionChanged()})});_(e.outputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.toPin.connectionChanged();c.toPin.markPinAsChanged();c.toPin.reset_on_disconnect&&c.toPin.reset()})});p.nodeList.splice(p.nodeList.indexOf(e),1);delete p.nodeMap[e.id]}var q;q="InABox"==$(this).attr("componentmode")?
$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var a=void 0!=p.nodeMap[$(this).attr("id")];if(a)e=p.nodeMap[$(this).attr("id")];else{var n=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==n)return;if(void 0!=VVVV.NodeLibrary[n.toLowerCase()])e=new (VVVV.NodeLibrary[n.toLowerCase()])($(this).attr("id"),p),VVVV.NodeLibrary[n.toLowerCase()].requirements&&(p.resourcesPending++,_(VVVV.NodeLibrary[n.toLowerCase()].requirements).each(function(c){void 0===
VVVV.LoadedLibs[c]&&VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){p.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))e=new VVVV.Nodes.GenericShader($(this).attr("id"),p),e.isShader=!0,e.shaderFile=$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),e.nodename=n;else if(/.v4p$/.test($(this).attr("filename"))){p.resourcesPending++;var k=this,e=new VVVV.Core.Patch($(this).attr("filename"),function(){p.resourcesPending--;"development"==
VVVV_ENV&&console.log(this.nodename+"invoking update links");f(c);p.editor&&p.editor.addPatch(this);if(this.auto_evaluate){var d=p;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(p.mainloop);p.afterUpdate();0>=p.resourcesPending&&h&&(h(),h=void 0)},function(){p.resourcesPending--;this.not_implemented=!0;VVVV.onNotImplemented(n);f(c);p.afterUpdate();0>=p.resourcesPending&&h&&(h(),h=void 0)},p,$(k).attr("id"));e.isSubpatch=!0;p.editor&&!e.editor&&p.editor.addPatch(e);p.nodeMap[e.id]=e}else e=
new VVVV.Core.Node($(this).attr("id"),n,p),e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&p.editor.sendUndo(),VVVV.onNotImplemented(n);"development"==VVVV_ENV&&"complete"!=g&&console.log(p.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){k=p;do k.auto_evaluate=!0;while(k=k.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);k=e.getSubPatches();k.push(e);for(var b,
m=0;m<k.length;m++)b=VVVV.Helpers.prepareFilePath(k[m].nodename,k[m].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(e),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}p.nodeList.splice(p.nodeList.indexOf(e),1);e.destroy();delete p.nodeMap[e.id]}0<q.length&&(q.attr("left")&&(e.x=q.attr("left")/15,e.y=q.attr("top")/15,p.boundingBox.width=Math.max(p.boundingBox.width,e.x+100),p.boundingBox.height=Math.max(p.boundingBox.height,e.y+100)),q.attr("width")&&(e.width=q.attr("width"),e.height=
q.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&(e.isIOBox=!0);a||e.setup();k=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var g=0;g<f.length;g++)e.invisiblePins[h].values[g]!=f[g]&&e.invisiblePins[h].setValue(g,f[g]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==
$(this).attr("visible")&&"Configuration"!=$(this).attr("pintype")||e.isSubpatch?"Output"==$(this).attr("pintype")||0<$(c).find("link[srcnodeid="+e.id+"]").filter("link[srcpinname='"+h.replace(/[\[\]]/,"")+"']").length?void 0==e.outputPins[h]&&e.addOutputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInvisiblePin(h,f)});a||(e.initialize(),p.nodeList.push(e));"complete"==g&&(r[e.id]=e)});"complete"==g&&(_(k).each(function(c,
d){void 0==r[d]&&("development"==VVVV_ENV&&console.log("removing node "+c.id),p.nodeList.splice(p.nodeList.indexOf(c),1),delete p.nodeMap[c.id])}),k={},_(r).each(function(c,d){k[d]=c}));0===this.resourcesPending&&f(c);0>=this.resourcesPending&&h&&(h(),h=void 0)};this.getSubPatches=function(){for(var c=[],d=0;d<this.nodeList.length;d++)this.nodeList[d].isSubpatch&&(c.push(this.nodeList[d]),c=c.concat(this.nodeList[d].getSubPatches()));return c};this.setMainloop=function(c){this.mainloop=c;for(var d=
0;d<this.nodeList.length;d++)this.nodeList[d].isSubpatch&&this.nodeList[d].setMainloop(c)};this.afterEvaluate=function(){};this.afterUpdate=function(){};this.toXML=function(){var c=$("<PATCH>"),d=$("<BOUNDS>");d.attr("type","Window");d.attr("width",parseInt(15*this.windowWidth));d.attr("height",parseInt(15*this.windowHeight));c.append(d);for(d=0;d<this.nodeList.length;d++)c.append(this.nodeList[d].serialize());for(d=0;d<this.linkList.length;d++)c.append(this.linkList[d].serialize());c='<!DOCTYPE PATCH  SYSTEM "http://vvvv.org/versions/vvvv45beta28.1.dtd" >\r\n  '+
...
!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(h=!0)});if(/\.v4p[^<>\s]*$/.test(a)){this.nodename=a;var m=this,t=a;this.parentPatch&&(t=VVVV.Helpers.prepareFilePath(a,this.parentPatch));VVVV.Patches[t]?m.doLoad(VVVV.Patches[t][0].toXML(),function(){VVVV.Patches[t].push(m);m.success&&m.success();m.afterUpdate()}):$.ajax({url:t,type:"get",dataType:"text",success:function(c){m.doLoad(c,function(){VVVV.Patches[t]=VVVV.Patches[t]||[];VVVV.Patches[t].push(m);m.success&&m.success();
m.afterUpdate()})},error:function(){m.error&&m.error()}})}else/^ws:\/\//.test(a)?(m=this,VVVV.Editors.Connector.host=a,VVVV.Editors.Connector.enable(this,{success:function(){m.success&&m.success()},error:function(){m.error&&m.error()}})):this.doLoad(a,function(){this.success&&this.success()});c();$(window).bind("hashchange",function(){c()})}};VVVV.Core.Patch.prototype=new VVVV.Core.Node;VVVV.Core.DOMInterface=function(a){function b(d){if("event"==d.property_class){var c=d.selector;"window"==c&&(c=window);"document"==c&&(c=document);$(c).each(function(c){d.values[c]=0;$(this).bind(d.property,function(){d.values[c]=1;return!1})})}}var g={},e={};a.domInterface=this;this.connect=function(d){_(g).each(function(c,h){c.node.id==d.id&&("event"==c.property_class&&$(c.selector).unbind(c.property),delete g[h])});_(e).each(function(c,h){c.node.id==d.id&&delete e[h]});var c=/([^\/]+)(\/(event|attribute|style)\/(.+))?/.exec(d.invisiblePins["Descriptive Name"].getValue(0));
...
w.push(-v/2);w.push(0);l.push(0);l.push(-1);l.push(0);A.push(0);A.push(0);for(var y=0;y<m+1;y++)for(var z=parseFloat(y)/m,F=(z-0.5)*-v,C=0;C<b+1;C++){var I=Math.cos(2*(parseFloat(C)/b)*Math.PI*u-Math.PI*u-Math.PI/2),J=Math.sin(2*(parseFloat(C)/b)*Math.PI*u-Math.PI*u-Math.PI/2),H=z*s+(1-z)*t;w.push(I*H);w.push(F);w.push(J*H);l.push(I);l.push(0);l.push(J);A.push(parseFloat(C)/b);A.push(parseFloat(y)/m)}t=new VVVV.Types.VertexBuffer(a,w);t.setSubBuffer("TEXCOORD0",2,A);t.setSubBuffer("NORMAL",3,l);t.create();
