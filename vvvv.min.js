...
b,g,e){for(var f=b.length;f<g;f++)b[f]=e.call(a,f);for(f=g;f<b.length;f++)a.removeInputPin(b[f].pinname);b.length=g;a.parentPatch.afterUpdate()}};
...
if(e=/^<!DOCTYPE\s+PATCH\s+SYSTEM\s+"(.+)\\(.+)\.dtd/.exec(c))n.vvvv_version=e[2].replace(/[a-zA-Z]+/,"_");var g="diff";if(/\s<PATCH/.test(c)||"45_26">=n.vvvv_version)g="complete","development"==VVVV_ENV&&console.log("complete: "+this.nodename),n.XMLCode=c;e=$(c).find('bounds[type="Window"]').first();0<e.length&&(n.windowWidth=e.attr("width")/15,n.windowHeight=e.attr("height")/15);"complete"==g&&(r={});$(c).find("node").each(function(){if("pronto"==$(this).attr("createme")&&void 0!=n.nodeMap[$(this).attr("id")]){var e=
...
$(this).find('bounds[type="Node"]').first();var a=void 0!=n.nodeMap[$(this).attr("id")];if(a)e=n.nodeMap[$(this).attr("id")];else{var k=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==k)return;if(void 0!=VVVV.NodeLibrary[k.toLowerCase()])e=new (VVVV.NodeLibrary[k.toLowerCase()])($(this).attr("id"),n),VVVV.NodeLibrary[k.toLowerCase()].requirements&&(n.resourcesPending++,_(VVVV.NodeLibrary[k.toLowerCase()].requirements).each(function(c){void 0===VVVV.LoadedLibs[c]&&
VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){n.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))e=new VVVV.Nodes.GenericShader($(this).attr("id"),n),e.isShader=!0,e.shaderFile=$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),e.nodename=k;else if(/.v4p$/.test($(this).attr("filename"))){n.resourcesPending++;var p=this,e=new VVVV.Core.Patch($(this).attr("filename"),function(){n.resourcesPending--;"development"==VVVV_ENV&&console.log(this.nodename+
"invoking update links");f(c);n.editor&&n.editor.addPatch(this);if(this.auto_evaluate){var d=n;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(n.mainloop);n.afterUpdate();0>=n.resourcesPending&&h&&(h(),h=void 0)},function(){n.resourcesPending--;this.not_implemented=!0;VVVV.onNotImplemented(k);f(c);n.afterUpdate();0>=n.resourcesPending&&h&&(h(),h=void 0)},n,$(p).attr("id"));e.isSubpatch=!0;n.editor&&!e.editor&&n.editor.addPatch(e);n.nodeMap[e.id]=e}else e=new VVVV.Core.Node($(this).attr("id"),
k,n),e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&n.editor.sendUndo(),VVVV.onNotImplemented(k);"development"==VVVV_ENV&&"complete"!=g&&console.log(n.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){p=n;do p.auto_evaluate=!0;while(p=p.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);p=e.getSubPatches();p.push(e);for(var b,q=0;q<p.length;q++)b=VVVV.Helpers.prepareFilePath(p[q].nodename,
p[q].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(e),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}n.nodeList.splice(n.nodeList.indexOf(e),1);e.destroy();delete n.nodeMap[e.id]}0<m.length&&(m.attr("left")&&(e.x=m.attr("left")/15,e.y=m.attr("top")/15,n.boundingBox.width=Math.max(n.boundingBox.width,e.x+100),n.boundingBox.height=Math.max(n.boundingBox.height,e.y+100)),m.attr("width")&&(e.width=m.attr("width"),e.height=m.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&
(e.isIOBox=!0);a||e.setup();p=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var m=0;m<f.length;m++)e.invisiblePins[h].values[m]!=f[m]&&e.invisiblePins[h].setValue(m,f[m]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==$(this).attr("visible")&&
"Configuration"!=$(this).attr("pintype")||e.isSubpatch?"Output"==$(this).attr("pintype")||0<$(c).find("link[srcnodeid="+e.id+"]").filter("link[srcpinname='"+h.replace(/[\[\]]/,"")+"']").length?void 0==e.outputPins[h]&&e.addOutputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInvisiblePin(h,f)});a||(e.initialize(),n.nodeList.push(e));"complete"==g&&(r[e.id]=e)});"complete"==g&&(_(k).each(function(c,d){void 0==
...
m[d.id]||d.delays_output||c(d)});if(f.dirty||f.auto_evaluate||f.isSubpatch){if(h)var a=(new Date).getTime();if(f.auto_nil&&!f.isSubpatch&&f.hasNilInputs())_(f.outputPins).each(function(c){c.setSliceCount(0)});else{try{f.evaluate()}catch(k){console.log("VVVV.Js / Error evaluating "+f.nodename+": "+k.message)}f.dirty=!1;_(f.inputPins).each(function(c){c.changed=!1})}h&&(d[f.nodename]||(d[f.nodename]={count:0,dt:0}),e=(new Date).getTime()-a,d[f.nodename].count++,d[f.nodename].dt+=e,console.log(f.nodename+
" / "+f.id+": "+e+"ms"))}delete m[f.id];return!0}if(!(0<this.resourcesPending)){if(h)var d={},f=(new Date).getTime(),e=0;for(var m={},g={},a=0;a<this.nodeList.length;a++){if(0==this.nodeList[a].getDownstreamNodes().length||this.nodeList[a].auto_evaluate||this.nodeList[a].delays_output)g[this.nodeList[a].id]=this.nodeList[a];m[this.nodeList[a].id]=this.nodeList[a]}h&&console.log("building node maps: "+((new Date).getTime()-f)+"ms");_(g).each(function(d,h,f){void 0!=m[d.id]&&c(d)});h&&(_(d).each(function(c,
d){console.log(c.count+"x "+d+": "+c.dt+"ms")}),f=(new Date).getTime());this.afterEvaluate();h&&console.log("patch rendering: "+((new Date).getTime()-f)+"ms");h=!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(h=!0)});if(/\.v4p[^<>\s]*$/.test(a)){this.nodename=a;var q=this,s=a;this.parentPatch&&(s=VVVV.Helpers.prepareFilePath(a,this.parentPatch));VVVV.Patches[s]?q.doLoad(VVVV.Patches[s][0].toXML(),function(){VVVV.Patches[s].push(q);q.success&&q.success();q.afterUpdate()}):$.ajax({url:s,
type:"get",dataType:"text",success:function(c){q.doLoad(c,function(){VVVV.Patches[s]=VVVV.Patches[s]||[];VVVV.Patches[s].push(q);q.success&&q.success();q.afterUpdate()})},error:function(){q.error&&q.error()}})}else/^ws:\/\//.test(a)?(q=this,VVVV.Editors.Connector.host=a,VVVV.Editors.Connector.enable(this,{success:function(){q.success&&q.success()},error:function(){q.error&&q.error()}})):this.doLoad(a,function(){this.success&&this.success()});c();$(window).bind("hashchange",function(){c()})}};
...
VVVV.Core.MainLoop=function(a,b){function g(){if(0==a.resourcesPending){e++;var k=(new Date).getTime();c&&(p.deltaT=k-c);c=k;f.populateInputConnectors();a.evaluate();f.processOutputConnectors();k=(new Date).getTime()-c;0==e%10&&(m&&(console.log(1/((c-h)/1E4)+"fps"),m=!1),h=c)}d&&window.setTimeout(g,Math.max(0,Math.round(1E3/VVVV.fps-k)))}VVVV.fps=b||60;var e=0,f=new VVVV.Core.DOMInterface(a),d=!0,c=void 0,h=0,m=!1;this.deltaT=1E3/VVVV.fps;a.setMainloop(this);var p=this;this.stop=function(){d=!1};
...
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Y Out",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=0,k=0;k<e.getSliceCount();k++)for(var b=0;b<g.getSliceCount();b++)0.5>f.getValue(0)&&k>b||(0.5>d.getValue(0)&&k<b||0.5>c.getValue(0)&&k==b)||(h.setValue(a,g.getValue(b)),m.setValue(a,e.getValue(k)),a++);h.setSliceCount(a);m.setSliceCount(a)}};VVVV.Nodes.Cross2d.prototype=new VVVV.Core.Node;
...
c;h++){var m=g.getValue(h),a=e.getValue(h);f.setValue(h,Math.atan2(a,m)/(2*Math.PI));d.setValue(h,Math.sqrt(m*m+a*a))}f.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Polar2d.prototype=new VVVV.Core.Node;
...
a=e.getValue(h);f.setValue(h,a*Math.cos(2*m*Math.PI));d.setValue(h,a*Math.sin(2*m*Math.PI))}f.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Cartesian2d.prototype=new VVVV.Core.Node;
...
m=this.addOutputPin("Length",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Angle",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=parseFloat(g.getValue(b)),q=parseFloat(e.getValue(b)),s=parseFloat(f.getValue(b)),v=parseFloat(d.getValue(b)),s=s-n,v=v-q;c.setValue(b,n+s/2);h.setValue(b,q+v/2);m.setValue(b,Math.sqrt(s*s+v*v));p.setValue(b,Math.atan2(v,s)/(2*Math.PI))}c.setSliceCount(a);h.setSliceCount(a);m.setSliceCount(a);p.setSliceCount(a)}};
...
m=this.addOutputPin("X2",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Y2",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=parseFloat(g.getValue(b)),q=parseFloat(e.getValue(b)),s=parseFloat(f.getValue(b)),v=2*parseFloat(d.getValue(b)-0.25)*Math.PI,u=s/2*Math.sin(v),s=-s/2*Math.cos(v);c.setValue(b,n+u);h.setValue(b,q+s);m.setValue(b,n-u);p.setValue(b,q-s)}c.setSliceCount(a);h.setSliceCount(a);m.setSliceCount(a);p.setSliceCount(a)}};
...
this.evaluate=function(){for(var m=this.getMaxInputSliceCount(),a=0;a<m;a++){var b=g.getValue(a),r=e.getValue(a),n=f.getValue(a),q=Math.sqrt(b*b+r*r+n*n);c.setValue(a,Math.atan2(-b,-n)/(2*Math.PI));d.setValue(a,0==q?0:Math.acos(-r/q)/(2*Math.PI)-0.25);h.setValue(a,q)}c.setSliceCount(m);d.setSliceCount(m);h.setSliceCount(m)}};VVVV.Nodes.Polar3d.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var m=this.getMaxInputSliceCount(),a=0;a<m;a++){var b=2*(g.getValue(a)+0.25)*Math.PI,r=2*e.getValue(a)*Math.PI,n=f.getValue(a);h.setValue(a,-n*Math.cos(r)*Math.sin(b));d.setValue(a,-n*Math.sin(r)*Math.sin(b));c.setValue(a,-n*Math.cos(b))}d.setSliceCount(m);c.setSliceCount(m);h.setSliceCount(m)}};VVVV.Nodes.Cartesian3d.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Input Length",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=g.getValue(b),n=e.getValue(b),q=f.getValue(b),s=Math.sqrt(r*r+n*n+q*q);d.setValue(b,0==s?0:r/s);c.setValue(b,0==s?0:n/s);h.setValue(b,0==s?0:q/s);m.setValue(b,s)}d.setSliceCount(a);c.setSliceCount(a);h.setSliceCount(a);m.setSliceCount(a)}};VVVV.Nodes.Normalize3d.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Cycles",[0],VVVV.PinTypes.Value),p=[],k=[],r=(new Date).getTime(),n=(new Date).getTime();this.evaluate=function(){var a=this.getMaxInputSliceCount();r=(new Date).getTime()-n;for(var b=0;b<a;b++){var v=g.getValue(b%g.values.length),u=e.getValue(b%e.values.length),t=f.getValue(b%f.values.length),w=d.getValue(b%d.values.length),l=c.getValue(b%c.values.length);void 0==p[b]&&(p[b]=0);void 0==k[b]&&(k[b]=0);0.5>u&&(0!=v&&isFinite(v))&&(dv=1/(1E3*v)*r,0<t&&(dv*=
-1),p[b]+=dv,0>p[b]&&(k[b]-=Math.ceil(-p[b]),p[b]=1+p[b]),1<p[b]&&(k[b]+=Math.floor(p[b])));n=(new Date).getTime();0.5<=w&&(p[b]=0,k[b]=0);if(0.5>u||0.5<=w)h.setValue(b,(p[b]+l)%1),m.setValue(b,k[b]);p[b]%=1}h.setSliceCount(a);m.setSliceCount(a);p.splice(a);k.splice(a)}};VVVV.Nodes.LFO.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],m=[],p=[],k=[],r=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||e.pinIsChanged(),s=0;s<a;s++){void 0==c[s]&&(c[s]=(new Date).getTime());var v=this.parentPatch.mainloop.deltaT,u=parseFloat(g.getValue(s)),t=parseFloat(e.getValue(s));void 0==p[s]&&(p[s]=u);if(u!=h[s]||t!=m[s])r[s]=void 0,h[s]=u,m[s]=t,k[s]=0<m[s]?(h[s]-p[s])/(1E3*m[s]):0;Math.abs(k[s]*v)>Math.abs(h[s]-p[s])?p[s]=h[s]:p[s]+=
k[s]*v;0!=r[s]&&(f.setValue(s,p[s]),d.setValue(s,k[s]));r[s]=h[s]-p[s];c[s]=(new Date).getTime()}b&&f.getSliceCount()!=a&&(h.splice(a),m.splice(a),c.splice(a),p.splice(a),k.splice(a),r.splice(a),f.setSliceCount(a))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
VVVV.Nodes.Damper=function(a,b){this.constructor(a,"Damper (Animation)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Cyclic Pin not implemented","Acceleration Out is not set yet"]};this.auto_evaluate=!0;var g=this.addInputPin("Go To Position",[0],VVVV.PinTypes.Value),e=this.addInputPin("FilterTime",[1],VVVV.PinTypes.Value),f=this.addOutputPin("Position Out",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Velocity Out",[0],VVVV.PinTypes.Value);
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],m=[],p=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||e.pinIsChanged(),n=0;n<a;n++){void 0==c[n]&&(c[n]=(new Date).getTime());var q=this.parentPatch.mainloop.deltaT,s=parseFloat(g.getValue(n)),v=parseFloat(e.getValue(n));void 0==h[n]&&(h[n]=s);m[n]=0<v?(s-h[n])/(1E3*v):0;Math.abs(m[n]*q)>Math.abs(s-h[n])?h[n]=s:h[n]+=m[n]*q;0!=p[n]&&(f.setValue(n,h[n]),d.setValue(n,m[n]));p[n]=s-h[n];
c[n]=(new Date).getTime()}b&&f.getSliceCount()!=a&&(c.splice(a),h.splice(a),m.splice(a),p.splice(a),f.setSliceCount(a))}};VVVV.Nodes.Damper.prototype=new VVVV.Core.Node;
...
h=this.addOutputPin("DayOfWeek",[0],VVVV.PinTypes.Value),m=this.addOutputPin("Day",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Month",[1],VVVV.PinTypes.Value),k=this.addOutputPin("Year",[2E3],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var q=864E5*(g.getValue(b)-25569),q=new Date(q);x=q.getMilliseconds();x!=e.values[b]&&e.setValue(b,x);x=q.getSeconds();x!=f.values[b]&&f.setValue(b,x);x=q.getMinutes();x!=d.values[b]&&d.setValue(b,x);x=q.getHours();
x!=c.values[b]&&c.setValue(b,x);x=q.getDay();x!=h.values[b]&&h.setValue(b,x);x=q.getDate();x!=m.values[b]&&m.setValue(b,x);x=q.getMonth()+1;x!=p.values[b]&&p.setValue(b,x);x=q.getYear()+1900;x!=k.values[b]&&k.setValue(b,x)}e.setSliceCount(a);f.setSliceCount(a);d.setSliceCount(a);c.setSliceCount(a);h.setSliceCount(a);m.setSliceCount(a);p.setSliceCount(a);k.setSliceCount(a)}};VVVV.Nodes.GregorianAstronomySplit.prototype=new VVVV.Core.Node;VVVV.Nodes.And=function(a,b){this.constructor(a,"AND (Boolean)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Output",[1],VVVV.PinTypes.Value);this.initialize=function(){var d=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,d,function(c){return this.addInputPin("Input "+(c+1),[1],VVVV.PinTypes.Value)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();for(var d=this.getMaxInputSliceCount(),c=e.length,h,a=0;a<d;a++){h=!0;for(var b=0;b<c;b++)h=h&&0.5<=e[b].getValue(a);f.setValue(a,h?1:0)}f.setSliceCount(d)}};VVVV.Nodes.And.prototype=new VVVV.Core.Node;
VVVV.Nodes.Or=function(a,b){this.constructor(a,"OR (Boolean)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Output",[1],VVVV.PinTypes.Value);this.initialize=function(){var d=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,d,function(c){return this.addInputPin("Input "+(c+1),[1],VVVV.PinTypes.Value)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();for(var d=this.getMaxInputSliceCount(),c=e.length,h,a=0;a<d;a++){h=!1;for(var b=0;b<c;b++)h=h||0.5<=e[b].getValue(a);f.setValue(a,h?1:0)}f.setSliceCount(d)}};VVVV.Nodes.Or.prototype=new VVVV.Core.Node;
...
["miter"],VVVV.PinTypes.Enum);c.enumOptions=["miter","round","bevel"];var h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.CanvasRenderState),m=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==m[b]&&(m[b]=new VVVV.Types.CanvasRenderState),m[b].copy_attributes(g.getValue(b)),m[b].strokeColor=e.getValue(b).split(","),m[b].lineWidth=f.getValue(b),m[b].capStyle=d.getValue(b),m[b].joinStyle=c.getValue(b),h.setValue(b,m[b]);h.setSliceCount(a)}};
...
h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.CanvasRenderState),m=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==m[b]&&(m[b]=new VVVV.Types.CanvasRenderState),m[b].copy_attributes(g.getValue(b)),m[b].shadowOffsetX=f.getValue(b),m[b].shadowOffsetY=d.getValue(b),m[b].shadowBlur=c.getValue(b),m[b].shadowColor=e.getValue(b).split(","),h.setValue(b,m[b]);h.setSliceCount(a)}};VVVV.Nodes.ShadowCanvas.prototype=new VVVV.Core.Node;
...
c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==k[b]&&(k[b]=new r),k[b].transform=e.getValue(b),k[b].radius=parseFloat(d.getValue(b)),k[b].startAngle=2*parseFloat(c.getValue(b))*Math.PI,k[b].endAngle=2*parseFloat(h.getValue(b))*Math.PI,k[b].renderState=g.getValue(b),k[b].drawSegment=1==m.getValue(b),f.isConnected()&&(k[b].clippingLayer=f.getValue(b));for(b=0;b<k.length;b++)p.setValue(b,
k[b]);p.setSliceCount(a)}};VVVV.Nodes.ArcCanvas.prototype=new VVVV.Core.Node;
...
c.strokeText(this.text,0,0);c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==k[a]&&(k[a]=new r),k[a].transform=e.getValue(a),k[a].text=d.getValue(a),k[a].font=c.getValue(a),k[a].align=h.getValue(a),k[a].baseline=m.getValue(a),k[a].renderState=g.getValue(a),k[a].clippingLayer=f.getValue(a);for(a=0;a<k.length;a++)p.setValue(a,k[a]);p.setSliceCount(b)}};VVVV.Nodes.TextCanvas.prototype=new VVVV.Core.Node;
...
c.beginPath();c.moveTo(this.x[0],this.y[0]);for(var h=1;h<this.x.length;h++)c.bezierCurveTo(this.c1x[h-1]+this.x[h-1],this.c1y[h-1]+this.y[h-1],this.c2x[h]+this.x[h],this.c2y[h]+this.y[h],this.x[h],this.y[h]);0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,w=0,l=0;l<a||0<n.getValue(0)&&(0<w||0<b%f.getSliceCount());l++)void 0==s[b]&&(s[b]=new v),0==w&&(s[b].renderState=
e.getValue(b),s[b].transform=f.getValue(b),d.isConnected()&&(s[b].clippingLayer=d.getValue(b))),s[b].x[w]=parseFloat(c.getValue(l)),s[b].y[w]=parseFloat(h.getValue(l)),s[b].c1x[w]=parseFloat(m.getValue(l)),s[b].c1y[w]=parseFloat(p.getValue(l)),s[b].c2x[w]=parseFloat(k.getValue(l)),s[b].c2y[w]=parseFloat(r.getValue(l)),w++,0<n.getValue(0)&&w>=n.getValue(b)&&(w<s[b].x.length&&g(s[b],w),b++,w=0);0>n.getValue(0)?w<s[b].x.length&&(g(s[b],w),s.splice(1)):s.splice(b);for(a=0;a<s.length;a++)q.setValue(a,
...
this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==h[b]&&(h[b]=new m),h[b].transform=g.getValue(b),h[b].texture=f.getValue(b),h[b].color=d.getValue(b).split(","),e.isConnected()&&(h[b].clippingLayer=e.getValue(b));for(b=0;b<h.length;b++)c.setValue(b,h[b]);c.setSliceCount(a)}};VVVV.Nodes.QuadCanvas.prototype=new VVVV.Core.Node;
VVVV.Nodes.GroupCanvas=function(a,b){this.constructor(a,"Group (Canvas VVVVjs)",b);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var g=this.addInvisiblePin("Layer Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Layer",[],VVVV.PinTypes.CanvasLayer);this.initialize=function(){var d=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,d,function(c){return this.addInputPin("Layer "+(c+1),[],VVVV.PinTypes.CanvasLayer)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();for(var d=0,c=e.length,h,b=0;b<c;b++)if(e[b].isConnected()){h=e[b].getSliceCount();for(var a=0;a<h;a++)f.setValue(d++,e[b].getValue(a))}f.setSliceCount(d)}};VVVV.Nodes.GroupCanvas.prototype=new VVVV.Core.Node;
...
e.pinIsChanged||f.pinIsChanged()||d.pinIsChanged()){for(var h=this.getMaxInputSliceCount(),b=0;b<h;b++){var a=g.getValue(b)||0,k=e.getValue(b)||0,r=f.getValue(b)||0,n=d.getValue(b)||0;c.setValue(b,a+","+k+","+r+","+n)}c.setSliceCount(h)}}};VVVV.Nodes.RGBJoin.prototype=new VVVV.Core.Node;
...
this.getMaxInputSliceCount(),b=0;b<h;b++){var a=g.getValue(b),k=e.getValue(b),r=f.getValue(b),n=d.getValue(b),a=VVVV.PinTypes.Color.hsvToRgb(a,k,r);c.setValue(b,a[0]+","+a[1]+","+a[2]+","+n)}c.setSliceCount(h)}};VVVV.Nodes.HSVJoin.prototype=new VVVV.Core.Node;
...
d.setSliceCount(b);c.setSliceCount(b)}if(m){for(a=0;a<b;a++)e.setValue(a,h[a]),f.setValue(a,h[a].width),d.setValue(a,h[a].height),c.setValue(a,h[a].loaded?1:0);m=!1}}};VVVV.Nodes.FileTextureCanvas.prototype=new VVVV.Core.Node;
...
l[a]=z[0];l[a].volume=0;z=function(c){return function(){}}(a);l[a].onprogress=z;l[a].oncanplay=z;l[a].oncanplaythrough=z}filename!=l[a].currentSrc&&($(l[a]).find("source").first().attr("src",filename),l[a].load(),0.5<f.getValue(a)?l[a].play():l[a].pause(),l[a].loaded=!0,r.setValue(a,l[a]),n.setValue(a,l[a]))}if(f.pinIsChanged())for(a=0;a<b;a++)0.5<f.getValue(a)?l[a].play():l[a].pause();if(m.pinIsChanged())for(a=0;a<b;a++)l[a%l.length].loaded&&0.5<=m.getValue(a)&&(l[a%l.length].currentTime=parseFloat(p.getValue(a)),
0.5<f.getValue(a)&&l[a].play());for(a=0;a<b;a++){if(!l[a].paused){r.setValue(a,l[a]);n.setValue(a,l[a]);q.getValue(a)!=l[a].duration&&q.setValue(a,l[a].duration);s.setValue(a,l[a].currentTime);var z=parseFloat(h.getValue(a)),F=parseFloat(c.getValue(a));l[a].currentTime<F&&(l[a].currentTime=F);if(l[a].currentTime>=l[a].duration||0<=z&&l[a].currentTime>=z)0.5<=d.getValue(a)?l[a].currentTime=F:l[a].pause()}if(l[a].videoWidth!=v.getValue(a)||l[a].videoHeight!=u.getValue(a))v.setValue(a,l[a].videoWidth),
u.setValue(a,l[a].videoHeight);t.getValue(a)!=g[l[a].networkState]&&t.setValue(a,g[l[a].networkState]);w.getValue(a)!=e[l[a].readyState]&&w.setValue(a,e[l[a].readyState])}r.setSliceCount(b);n.setSliceCount(b)}};VVVV.Nodes.FileStreamCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Value),c=this.addOutputPin("Status",[""],VVVV.PinTypes.String),h=this.addOutputPin("Body",[""],VVVV.PinTypes.String),m=this.addOutputPin("Fail",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Success",[0],VVVV.PinTypes.Value),k,r,n,q=!1;this.evaluate=function(){this.getMaxInputSliceCount();var a=g.pinIsChanged()||e.pinIsChanged()||f.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==p.getValue(0)&&p.setValue(0,0);1==m.getValue(0)&&m.setValue(0,0);q&&(h.setValue(0,k),c.setValue(0,void 0),
p.setValue(0,r),m.setValue(0,n),q=!1);a&&(void 0==g.getValue(0)?(h.setValue(0,""),c.setValue(0,"")):$.ajax({url:g.getValue(0),type:"get",success:function(c,d,h){k=c;r=1;q=!0},error:function(c,d){k="";n=1;q=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,k=0,r,n,q,s=0;s<a||0<e.getValue(0)&&(0<k||0!=b%e.values.length);s++)r=g.getValue(s),0==k&&(center=n=q=r,width=0),n=Math.min(r,n),q=Math.max(r,q),k++,0<e.getValue(0)&&k>=e.getValue(b)&&(c.setValue(b,n),h.setValue(b,q),f.setValue(b,n+(q-n)/2),d.setValue(b,Math.abs(q-n)),b++,k=0);-1==e.getValue(0)&&(c.setValue(b,n),h.setValue(b,q),f.setValue(b,n+(q-n)/2),d.setValue(b,Math.abs(q-n)));f.setSliceCount(b+(0<k));
d.setSliceCount(b+(0<k));c.setSliceCount(b+(0<k));h.setSliceCount(b+(0<k))}};VVVV.Nodes.BoundsSpectral.prototype=new VVVV.Core.Node;VVVV.Nodes.GetSliceSpreads=function(a,b){this.constructor(a,"GetSlice (Spreads)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addInputPin("Bin Size",[1],VVVV.PinTypes.Value),f=this.addInputPin("Index",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var c=f.getSliceCount(),a,b,p=0,k=0;k<c;k++)if(b=e.getValue(k),a=
g.getValue(Math.round(f.getValue(k)),b),1<b)for(b=0;b<a.length;b++)d.setValue(p,parseFloat(a[b])),p++;else d.setValue(k,parseFloat(a)),p++;d.setSliceCount(p)}};VVVV.Nodes.GetSliceSpreads.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),c=this.addInputPin("Spread Count",[1],VVVV.PinTypes.Value),h=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=parseInt(c.getValue(0)),b=parseFloat(e.getValue(0)),k=parseFloat(d.getValue(0)),r=parseFloat(g.getValue(0)),n=f.getValue(0);""==n&&(n="Centered");var q=b/a;"Block"==n&&(q=b/(a-1));var s=q/2;if("Block"==n||"LeftJustified"==n)s=0;"RightJustified"==n&&(s=q);for(var v,u=0;u<a;u++)v=u*q+s,"Block"!=n&&0!=b&&(v=(v+k*b)%b),v=r-b/2+v,h.setValue(u,
v.toFixed(4));h.setSliceCount(a)}};VVVV.Nodes.LinearSpread.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){f.pinIsChanged()&&this.initialize();for(var d=0,c=0;c<g.length;c++)for(var a=0;a<g[c].getSliceCount();a++)e.setValue(d++,g[c].getValue(a));e.setSliceCount(d)}};VVVV.Nodes.ConsSpreads.prototype=new VVVV.Core.Node;
...
d||void 0==d)d=e.getValue(b)}for(b=0;b<c;b++){var p=g.getValue(b);if(p<d)f.setValue(b,-1);else{for(var k=0;k<a-1&&!(p>=e.getValue(k)&&p<e.getValue(k+1));k++);f.setValue(b,k)}}f.setSliceCount(c)}};VVVV.Nodes.IntervalSpreads.prototype=new VVVV.Core.Node;
...
(c+1),["text"],VVVV.PinTypes.String)})};this.evaluate=function(){g.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),a=0;a<c;a++)d.setValue(a,f[Math.round(Math.abs(e.getValue(a)))%f.length].getValue(a));d.setSliceCount(c)}};VVVV.Nodes.SwitchStringInput.prototype=new VVVV.Core.Node;
...
UnderScore:"_",Minus:"-"},k={Single:"'",Double:'"'};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,q=0;q<a;q++){var s=g.getValue(q),v=p[e.getValue(q)],u=f.getValue(q),t=k[d.getValue(q)],w=c.getValue(q);""==v&&(v=u);""==v&&(v=void 0);s=s.split(v);v=s.length;t=RegExp("[^"+t+"]*","g");for(u=0;u<v;u++)1==w?h.setValue(b,s[u]):h.setValue(b,s[u].match(t).join("")),m.setValue(b,q),b++}h.setSliceCount(b);m.setSliceCount(b)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var p=g.getValue(b),k=e.getValue(b),r=f.getValue(b),p=p==k||0==r&&p.toLowerCase()==k.toLowerCase()?1:0;d.setValue(b,p);c.setValue(b,1-p)}d.setSliceCount(a);c.setSliceCount(a)}};VVVV.Nodes.EQString.prototype=new VVVV.Core.Node;
...
0;p<e.getValue(b);p++)f.setValue(a,g.getValue(b)),d.setValue(a,b),a++;f.setSliceCount(a);d.setSliceCount(a)}};VVVV.Nodes.SelectString.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var a=this.trIn.isConnected()?this.getMaxInputSliceCount():Math.max(this.xIn.getSliceCount(),this.yIn.getSliceCount(),this.zIn.getSliceCount()),b=0;b<a;b++){var f=this.inputPins["Transform In"].getValue(b),d=parseFloat(this.inputPins.X.getValue(b)),c=parseFloat(this.inputPins.Y.getValue(b)),h=parseFloat(this.inputPins.Z.getValue(b)),m=mat4.create();mat4.identity(m);mat4.rotate(m,2*c*Math.PI,[0,1,0]);mat4.rotate(m,2*d*Math.PI,[1,0,0]);mat4.rotate(m,2*h*Math.PI,[0,0,1]);
this.trIn.isConnected()&&mat4.multiply(f,m,m);this.trOut.setValue(b,m)}this.trOut.setSliceCount(a)}};VVVV.Nodes.Rotate.prototype=new VVVV.Core.Node;
...
mat4.multiply(g.getValue(c),mat4.inverse(a,a),a);f.setValue(c,a)}f.setSliceCount(d)}};VVVV.Nodes.InverseTransform.prototype=new VVVV.Core.Node;
...
[],VVVV.PinTypes.Transform),h=mat4.create();mat4.identity(h);mat4.scale(h,[-1,1,-1]);this.evaluate=function(){for(var a=Math.max(g.getSliceCount(),Math.max(e.getSliceCount()/3,Math.max(f.getSliceCount()/3,d.getSliceCount()/3))),a=Math.ceil(a),b=0;b<a;b++){var k=e.getValue(b,3),r=f.getValue(b,3),n=d.getValue(b,3),q=mat4.create();mat4.identity(q);mat4.lookAt(vec3.create(k),vec3.create(r),vec3.create(n),q);mat4.multiply(h,q,q);g.isConnected()&&mat4.multiply(g.getValue(b),q,q);c.setValue(b,q)}c.setSliceCount(a)}};
...
h=this.addInputPin("LookAt Y",[0],VVVV.PinTypes.Value),m=this.addInputPin("LookAt Z",[1],VVVV.PinTypes.Value),p=this.addInputPin("UpVector X",[0],VVVV.PinTypes.Value),k=this.addInputPin("UpVector Y",[1],VVVV.PinTypes.Value),r=this.addInputPin("UpVector Z",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Transform Out",[],VVVV.PinTypes.Transform),q=mat4.create();mat4.identity(q);mat4.scale(q,[-1,1,-1]);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var u=mat4.create();mat4.identity(u);
mat4.lookAt(vec3.create([e.getValue(b),f.getValue(b),d.getValue(b)]),vec3.create([c.getValue(b),h.getValue(b),m.getValue(b)]),vec3.create([p.getValue(b),k.getValue(b),r.getValue(b)]),u);mat4.multiply(q,u,u);g.isConnected()&&mat4.multiply(g.getValue(b),u,u);n.setValue(b,u)}n.setSliceCount(a)}};VVVV.Nodes.LookAtTransform.prototype=new VVVV.Core.Node;VVVV.Nodes.AddValue=function(a,b){this.constructor(a,"Add (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[""]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.initialize=function(){var a=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,a,function(c){return this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();var a=this.getMaxInputSliceCount();f.setSliceCount(a);for(var c=0;c<a;c++){for(var b=parseFloat(e[0].getValue(c)),m=1;m<e.length;m++)b+=parseFloat(e[m].getValue(c));f.setValue(c,b)}}};VVVV.Nodes.AddValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.SubtractValue=function(a,b){this.constructor(a,"Subtract (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[""]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.initialize=function(){var a=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,a,function(c){return this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();var a=this.getMaxInputSliceCount();f.setSliceCount(a);for(var c=0;c<a;c++){for(var b=parseFloat(e[0].getValue(c)),m=1;m<e.length;m++)b-=parseFloat(e[m].getValue(c));f.setValue(c,b)}}};VVVV.Nodes.SubtractValue.prototype=new VVVV.Core.Node;
...
b=0;b<a;b++){var p=0;Math.abs(g.getValue(b)-e.getValue(b))<=Math.abs(f.getValue(b))&&(p=1);d.setValue(b,p);c.setValue(b,1-p)}d.setSliceCount(a);c.setSliceCount(a)}};VVVV.Nodes.EqValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.GtValue=function(a,b){this.constructor(a,"GT (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["No dynamic pin count yet"]};var g=this.addInputPin("Input 1",[0],VVVV.PinTypes.Value),e=this.addInputPin("Input 2",[0],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){var b=0;parseFloat(g.getValue(c))>parseFloat(e.getValue(c))&&
(b=1);f.setValue(c,b)}f.setSliceCount(a)}};VVVV.Nodes.GtValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.LtValue=function(a,b){this.constructor(a,"LT (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["No dynamic pin count yet"]};var g=this.addInputPin("Input 1",[0],VVVV.PinTypes.Value),e=this.addInputPin("Input 2",[0],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){var b=0;parseFloat(g.getValue(c))<parseFloat(e.getValue(c))&&
(b=1);f.setValue(c,b)}f.setSliceCount(a)}};VVVV.Nodes.LtValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.MultiplyValue=function(a,b){this.constructor(a,"Multiply (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[""]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.initialize=function(){var a=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,a,function(c){return this.addInputPin("Input "+(c+1),[1],VVVV.PinTypes.Value)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();var a=this.getMaxInputSliceCount();f.setSliceCount(a);for(var c=0;c<a;c++){for(var b=parseFloat(e[0].getValue(c)),m=1;m<e.length;m++)b*=parseFloat(e[m].getValue(c));f.setValue(c,b)}}};VVVV.Nodes.MultiplyValue.prototype=new VVVV.Core.Node;
...
c=0;c<a;c++)0==e.getValue(c)?f.setValue(c,1):f.setValue(c,g.getValue(c)/e.getValue(c));f.setSliceCount(a)}};VVVV.Nodes.DivideValue.prototype=new VVVV.Core.Node;
...
var d=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){g.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),a=0;a<c;a++)d.setValue(a,f[Math.round(Math.abs(e.getValue(a)))%f.length].getValue(a));d.setSliceCount(c)}};VVVV.Nodes.SwitchValueInput.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){subtype=e.getValue(c).match(/^(.+) \(/)[1];var b;switch(subtype){case "Real":b=parseFloat(g.getValue(c)).toFixed(4);break;case "Integer":b=parseInt(g.getValue(c)).toFixed(0)}f.setValue(c,b)}f.setSliceCount(a)}};VVVV.Nodes.AsString.prototype=new VVVV.Core.Node;
VVVV.Nodes.Frac=function(a,b){this.constructor(a,"Frac (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addOutputPin("Whole Part",[0],VVVV.PinTypes.Value),f=this.addOutputPin("Real Part",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){var b=parseFloat(g.getValue(c));e.setValue(c,Math.floor(b));f.setValue(c,b-Math.floor(b))}e.setSliceCount(a);
...
[1],VVVV.PinTypes.Value),h=this.addInputPin("Mapping",["Float"],VVVV.PinTypes.Enum);h.enumOptions=["Float","Wrap","Clamp"];var m=this.addOutputPin("Output",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=parseFloat(g.getValue(b)),n=parseFloat(e.getValue(b)),q=parseFloat(f.getValue(b)),s=parseFloat(d.getValue(b)),v=parseFloat(c.getValue(b)),u=h.getValue(b);if(q==n)m.setValue(b,s);else{r-=n;q-=n;switch(u){case "Clamp":r=Math.max(0,Math.min(q,
r));break;case "Wrap":r=(q+r%q)%q}m.setValue(b,r*((v-s)/q)+s)}}m.setSliceCount(a)}};VVVV.Nodes.MapValue.prototype=new VVVV.Core.Node;
...
function(){e.pinIsChanged()&&this.initialize();for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){for(var b=void 0,m=0;m<f.length;m++)if(void 0==b||f[m].getValue(c)<b)b=f[m].getValue(c);g.setValue(c,b)}g.setSliceCount(a)}};VVVV.Nodes.MinValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.MaxValue=function(a,b){this.constructor(a,"Max (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addOutputPin("Output",[0],VVVV.PinTypes.Value),e=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),f=[];this.initialize=function(){var a=Math.max(2,e.getValue(0));VVVV.Helpers.dynamicPins(this,f,a,function(c){return this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=
function(){e.pinIsChanged()&&this.initialize();for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){for(var b=void 0,m=0;m<f.length;m++)if(void 0==b||f[m].getValue(c)>b)b=f[m].getValue(c);g.setValue(c,b)}g.setSliceCount(a)}};VVVV.Nodes.MaxValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.ModValue=function(a,b){this.constructor(a,"Mod (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addOutputPin("Output",[0],VVVV.PinTypes.Value),e=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),f=[];this.initialize=function(){var a=Math.max(2,e.getValue(0));VVVV.Helpers.dynamicPins(this,f,a,function(c){return this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=
function(){e.pinIsChanged()&&this.initialize();for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){for(var b=f[0].getValue(c),m=1;m<f.length;m++)b=0==f[m].getValue(c)?0:b%f[m].getValue(c);g.setValue(c,b)}g.setSliceCount(a)}};VVVV.Nodes.ModValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.PowerValue=function(a,b){this.constructor(a,"Power (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addOutputPin("Output",[0],VVVV.PinTypes.Value),e=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),f=[];this.initialize=function(){var a=Math.max(2,e.getValue(0));VVVV.Helpers.dynamicPins(this,f,a,function(c){return this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=
function(){e.pinIsChanged()&&this.initialize();for(var a=this.getMaxInputSliceCount(),c=0;c<a;c++){for(var b=f[0].getValue(c),m=1;m<f.length;m++)b=Math.pow(b,f[m].getValue(c));g.setValue(c,b)}g.setSliceCount(a)}};VVVV.Nodes.PowerValue.prototype=new VVVV.Core.Node;
...
(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=function(){f.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),a,b,p,k=0;k<c;k++)a=g.getValue(k),b=((Math.floor(a)+d.length)%d.length+d.length)%d.length,p=((Math.ceil(a)+d.length)%d.length+d.length)%d.length,a-=Math.floor(a),e.setValue(k,(1-a)*d[b].getValue(k)+a*d[p].getValue(k));e.setSliceCount(c)}};VVVV.Nodes.InputMorphValue.prototype=new VVVV.Core.Node;
...
h=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),m=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value);this.evaluate=function(){this.getMaxInputSliceCount();var a=g.getSliceCount(),b=e.getSliceCount();c.setSliceCount(a);f.getValue(0);for(var r=d.getValue(0),n={},q=0,s=0;s<a;s++){var v=s;"Last"==r&&(v=a-s-1);var u=g.getValue(v);if("All"!=r){if(n[u]){c.setValue(v,0);continue}n[u]=!0}for(var t=0,w=0;w<b;w++)if(filter=e.getValue(w),filter==u){h.setValue(q,v);m.setValue(q,w);q++;t++;break}c.setValue(v,
...
f=this.addInvisiblePin("Type",["Texture"],VVVV.PinTypes.Enum);f.enumOptions=["Texture","Cube Texture"];var d=[];this.evaluate=function(){if(this.renderContexts){var c=this.renderContexts[0];if(c){if(this.contextChanged){for(var a=0;a<d.length;a++)d[a].context.deleteTexture(d[a]);d=[]}if(g.pinIsChanged()||f.pinIsChanged()||this.contextChanged){for(var b=f.getValue(0),p=this.getMaxInputSliceCount(),a=0;a<p;a++){var k=VVVV.Helpers.prepareFilePath(g.getValue(a),this.parentPatch);0===k.indexOf("http://")&&
...
d[a].image.src=k);e.setValue(a,VVVV.defaultTexture)}e.setSliceCount(p)}this.contextChanged=!1}}};this.destroy=function(){for(var c=0;c<d.length;c++)d[c].context.deleteTexture(d[c])}};VVVV.Nodes.FileTexture.prototype=new VVVV.Core.Node;
...
m=parseInt(e.getValue(0)),p=[],k=[],r=[],n=0,q=0;q<m;q++)for(var s=0;s<b;s++)p.push(parseFloat(s)/(b-1)-0.5),p.push(0.5-parseFloat(q)/(m-1)),p.push(0),n++,k.push(0),k.push(0),k.push(1),r.push(parseFloat(s)/(b-1)),r.push(parseFloat(q)/(m-1));p=new VVVV.Types.VertexBuffer(a,p);p.setSubBuffer("TEXCOORD0",2,r);p.setSubBuffer("NORMAL",3,k);p.create();k=[];for(q=0;q<m-1;q++)for(s=0;s<b-1;s++)r=s+b*q,k.push(r),k.push(r+1),k.push(r+b+1),k.push(r+b+1),k.push(r+b),k.push(r);d=new VVVV.Types.Mesh(a,p,k);f.setValue(0,
...
this.renderContexts[0];if(a){for(var b=parseInt(e.getValue(0)),p=parseInt(f.getValue(0)),k=parseFloat(g.getValue(0)),r=[],n=[],q=[],s=0;s<p+1;s++)for(var v=Math.cos(-parseFloat(s)/p*Math.PI),u=0;u<b;u++){var t=Math.cos(2*(parseFloat(u)/b)*Math.PI)*Math.cos(Math.asin(v)),w=Math.sin(2*(parseFloat(u)/b)*Math.PI)*Math.cos(Math.asin(v));r.push(t*k);r.push(v*k);r.push(w*k);n.push(t);n.push(v);n.push(w);q.push(parseFloat(u)/b);q.push(parseFloat(s)/p)}k=new VVVV.Types.VertexBuffer(a,r);k.setSubBuffer("TEXCOORD0",
2,q);k.setSubBuffer("NORMAL",3,n);k.create();n=[];for(s=0;s<p;s++)for(u=0;u<b;u++)q=b*s,r=u+q,n.push(r),n.push((r+1)%b+q),n.push((r+1)%b+b+q),n.push((r+1)%b+b+q),n.push(r+b),n.push(r);c=new VVVV.Types.Mesh(a,k,n);d.setValue(0,c)}}}};VVVV.Nodes.Sphere.prototype=new VVVV.Core.Node;
...
[15],VVVV.PinTypes.Value),m=this.addInputPin("Resolution Y",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Mesh",[],VVVV.PinTypes.WebGlResource),k=null;this.evaluate=function(){if(this.renderContexts){var a=this.renderContexts[0];if(a){var b=parseInt(h.getValue(0)),q=parseInt(m.getValue(0)),s=parseFloat(g.getValue(0)),v=parseFloat(e.getValue(0)),u=parseFloat(f.getValue(0)),t=parseFloat(d.getValue(0)),w=[],l=[],A=[];w.push(0);w.push(u/2);w.push(0);l.push(0);l.push(1);l.push(0);A.push(0);A.push(0);w.push(0);
w.push(-u/2);w.push(0);l.push(0);l.push(-1);l.push(0);A.push(0);A.push(0);for(var y=0;y<q+1;y++)for(var z=parseFloat(y)/q,F=(z-0.5)*-u,C=0;C<b+1;C++){var I=Math.cos(2*(parseFloat(C)/b)*Math.PI*t-Math.PI*t-Math.PI/2),J=Math.sin(2*(parseFloat(C)/b)*Math.PI*t-Math.PI*t-Math.PI/2),H=z*v+(1-z)*s;w.push(I*H);w.push(F);w.push(J*H);l.push(I);l.push(0);l.push(J);A.push(parseFloat(C)/b);A.push(parseFloat(y)/q)}s=new VVVV.Types.VertexBuffer(a,w);s.setSubBuffer("TEXCOORD0",2,A);s.setSubBuffer("NORMAL",3,l);s.create();
l=[];if(0.5<c.getValue(0))for(z=0;2>z;z++)for(C=0;C<b;C++)l.push(z),l.push(2+C+z+z*q*(b+1)),l.push(2+C+(1-z)+z*q*(b+1));for(y=0;y<q;y++)for(C=0;C<b;C++)z=C+(b+1)*y+2,l.push(z),l.push(z+1),l.push(z+b+2),l.push(z+b+2),l.push(z+b+1),l.push(z);k=new VVVV.Types.Mesh(a,s,l);p.setValue(0,k)}}}};VVVV.Nodes.Cylinder.prototype=new VVVV.Core.Node;
...
var h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.WebGlRenderState),m=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==m[b]&&(m[b]=new VVVV.Types.WebGlRenderState),m[b].copy_attributes(e.getValue(b)),m[b].alphaBlending=0.5<parseFloat(f.getValue(b)),m[b].srcBlendMode=g(d.getValue(b)),m[b].destBlendMode=g(c.getValue(b)),h.setValue(b,m[b]);h.setSliceCount(a)}};VVVV.Nodes.BlendWebGLAdvanced.prototype=new VVVV.Core.Node;
...
m[b].copy_attributes(e.getValue(b)),m[b].enableZWrite=0.5<parseFloat(f.getValue(b)),m[b].depthFunc=g(d.getValue(b)),m[b].depthOffset=parseFloat(c.getValue(0)),h.setValue(b,m[b]);h.setSliceCount(a)}};VVVV.Nodes.ZWriteEnableWebGL.prototype=new VVVV.Core.Node;
...
VVVV.Editors.BrowserEditor.PatchWindow=function(a,b,g){function e(){c.selectAll(".vvvv-node.selected").attr("class",function(a){return a.isIOBox?"vvvv-node vvvv-iobox":"vvvv-node"});n=[]}this.state=UIState.Idle;var f=0,d=0,c,h,m,p,k,r=void 0,n=[],q=a,s=0,v=VVVV.Root+"/patch.html",u=!1,t=0,w=0,l=0,A=0;a.isSubpatch&&(v="patch.html");this.window=g?window:window.open(v,a.nodename,"location=no, width="+a.windowWidth+", height="+a.windowHeight+", toolbar=no");var y=this;window.setTimeout(function(){for(var k=
...
y.state=UIState.Idle);d3.event.stopPropagation();d3.event.preventDefault();return!1}).on("mouseup",function(){if(y.state==UIState.Moving){y.state=UIState.Idle;for(var a=d3.event.pageX-f,e=d3.event.pageY-d,h="<PATCH>",g=0;g<n.length;g++)var k=n[g],h=h+("<NODE componentmode='Node' id='"+k.id+"'><BOUNDS type='Node' left='"+15*(a+k.x)+"' top='"+15*(e+k.y)+"' width='"+k.width+"' height='"+k.height+"'/></NODE>");b.update(q,h+"</PATCH>")}y.state==UIState.AreaSelecting&&(c.select(".selection-area").remove(),
y.state=UIState.Idle)}).on("dblclick",function(){function a(b){h.empty();var c=e.find("#node_filter").val().toLowerCase();b=VVVV.NodeNames.concat(_(VVVV.ShaderCodeResources).map(function(a,b){return b.replace("%VVVV%/effects/","")}));b=_(_(b).filter(function(a){return 0<=VVVV.Helpers.translateOperators(a).toLowerCase().indexOf(c)})).sortBy(function(a){return a.toLowerCase().indexOf(c)});for(var d=0;d<b.length;d++)h.append($("<option>"+b[d]+"</option>"));h.find("option").first().attr("selected",!0)}
function c(){var a=e.find("#node_filter").val(),h=e.find("#new_node option:selected").val(),g="";h||!a.match(/\.fx$/)&&!a.match(/\.v4p$/)||(h=a);if(h){var k;(k=h.match(/^(.*\/([^\.]+))(\.vvvvjs)?\.fx$/))?(h=k[2]+" (EX9.Effect)",g=k[1]+".fx"):(k=h.match("([^.]+)(.vvvvjs)?.fx$"))?(h=k[1]+" (EX9.Effect)",g="%VVVV%/effects/"+k[1]+".fx"):h.match("[^/].v4p$")&&(g=h=a);s++;h="<PATCH>"+("<NODE componentmode='Hidden' id='"+s+"' nodename='"+h+"' systemname='"+h+"' "+(""!=g?"filename='"+g+"'":"")+">");h+="<BOUNDS type='Node' left='"+
15*d+"' top='"+15*f+"' width='100' height='100'/>"}else s++,h="<PATCH>"+("<NODE componentmode='Hidden' id='"+s+"' nodename='IOBox (String)' systemname='IOBox (String)'>"),h+="<BOUNDS type='Node' left='"+15*d+"' top='"+15*f+"' width='100' height='100'/>",h+="<PIN pinname='Input String' values='|"+a+"|'/>";b.update(q,h+"</NODE>");e.remove()}$("#node_selection",y.window.document).remove();var d=d3.event.pageX,f=d3.event.pageY,e=$('<div id="node_selection"><input type="text" id="node_filter"/></div>'),
h=$('<select id="new_node" size="8">');e.append(h);e.css("left",d);e.css("top",f);$("body",y.window.document).append(e);e.find("input").bind("paste",function(a){a.stopPropagation()});e.find("#node_filter").get(0).focus();a();e.find("#node_filter").keyup(function(b){if(13==b.which)c();else{if(40==b.which)return h.find("option:selected").first().next().attr("selected",!0),!1;if(38==b.which)return h.find("option:selected").first().prev().attr("selected",!0),!1;a()}});e.find("#new_node").click(c)}).on("mousedown",
...
function(a){u=a.shiftKey},$(y.window.document).keydown(k),$(y.window.document).keyup(k),$(y.window.document).keydown(function(a){if(46==a.which&&0<n.length){var d="<PATCH>";for(a=0;a<n.length;a++){var f=n[a],d=d+("<NODE id='"+n[a].id+"' deleteme='pronto'/>");_(f.inputPins).each(function(a){_(a.links).each(function(a){d+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(f.outputPins).each(function(a){_(a.links).each(function(a){d+=
"<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})})}d+="</PATCH>";b.update(q,d);n=[]}else if(115!=a.which&&83!=a.which||!a.ctrlKey){if(73==a.which&&a.ctrlKey)return b.inspector?b.inspector.win.focus():b.openInspector("."),a.preventDefault(),!1;if(65==a.which&&a.ctrlKey)return e(),c.selectAll(".vvvv-node").each(function(a){n.push(a);d3.select(this).attr("class","vvvv-node selected")}),
a.preventDefault(),!1}else return b.save(q.nodename,q.toXML()),a.preventDefault(),!1}),$(y.window).resize(function(){q.windowWidth=$(this).width();q.windowHeight=$(this).height();q.boundingBox.width=Math.max(q.windowWidth,q.boundingBox.width);q.boundingBox.height=Math.max(q.windowHeight,q.boundingBox.height);q.afterUpdate()}));y.drawComplete();q.afterUpdate=function(){y.drawComplete();b.inspector&&window.setTimeout(b.inspector.update,100)};q.afterEvaluate=function(){h.filter(function(a){return a.isIOBox&&
...
Math.max(q.windowWidth,q.boundingBox.width)).attr("height",Math.max(q.windowHeight,q.boundingBox.height));var a=c.append("svg:g").attr("class","link-group");h=c.selectAll("g.vvvv-node").data(q.nodeList).enter().append("svg:g").attr("class",function(a){var b="vvvv-node";a.isIOBox&&(b+=" vvvv-iobox");0<=n.indexOf(a)&&(b+=" selected");return b}).attr("id",function(a){return"vvvv-node-"+a.id}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"});h.append("svg:rect").attr("class","vvvv-node-background").attr("height",
...
c=0;c<b.length;c++)this.removePatch(b[c]);b=VVVV.Helpers.prepareFilePath(a.nodename,a.parentPatch);g[b].splice(g[b].indexOf(a),1);0==g[b].length&&delete g[b];console.log(g)};this.openPatch=function(a,d){b.push(new VVVV.Editors.BrowserEditor.PatchWindow(a,this,d))};this.update=function(a,b){for(var c=VVVV.Helpers.prepareFilePath(a.nodename,a.parentPatch),e=g[c].length,m=0;m<e;m++)g[c][m].doLoad(b),g[c][m].afterUpdate()};this.disable=function(){for(var f=0;f<b.length;f++)b[f].close();this.inspector&&
