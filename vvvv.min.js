...
$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var n=void 0!=p.nodeMap[$(this).attr("id")];if(n)e=p.nodeMap[$(this).attr("id")];else{nodename=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==nodename)return;if(void 0!=VVVV.NodeLibrary[nodename.toLowerCase()])e=new (VVVV.NodeLibrary[nodename.toLowerCase()])($(this).attr("id"),p),VVVV.NodeLibrary[nodename.toLowerCase()].requirements&&(p.resourcesPending++,_(VVVV.NodeLibrary[nodename.toLowerCase()].requirements).each(function(c){void 0===
...
a--;"development"==VVVV_ENV&&console.log(this.nodename+"invoking update links");f(c);p.editor&&p.editor.addPatch(this);if(this.auto_evaluate){var d=p;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(p.mainloop);p.afterUpdate();0>=p.resourcesPending&&h&&(h(),h=void 0)},function(){this.not_implemented=!0;VVVV.onNotImplemented(nodename)},p,$(k).attr("id"));e.isSubpatch=!0;p.editor&&!e.editor&&p.editor.addPatch(e);p.nodeMap[e.id]=e}else e=new VVVV.Core.Node($(this).attr("id"),nodename,p),
e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&p.editor.sendUndo(),VVVV.onNotImplemented(nodename);"development"==VVVV_ENV&&"complete"!=g&&console.log(p.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){k=p;do k.auto_evaluate=!0;while(k=k.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);k=e.getSubPatches();k.push(e);for(var b,m=0;m<k.length;m++)b=VVVV.Helpers.prepareFilePath(k[m].nodename,
k[m].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(e),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}p.nodeList.splice(p.nodeList.indexOf(e),1);e.destroy();delete p.nodeMap[e.id]}0<q.length&&(q.attr("left")&&(e.x=q.attr("left")/15,e.y=q.attr("top")/15,p.boundingBox.width=Math.max(p.boundingBox.width,e.x+100),p.boundingBox.height=Math.max(p.boundingBox.height,e.y+100)),q.attr("width")&&(e.width=q.attr("width"),e.height=q.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&
(e.isIOBox=!0);n||e.setup();k=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var g=0;g<f.length;g++)e.invisiblePins[h].values[g]!=f[g]&&e.invisiblePins[h].setValue(g,f[g]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==$(this).attr("visible")&&
