var VVVV_ENV="production";$.ajaxPrefilter(function(a,b,g){if("script"==a.dataType||"script"==b.dataType)a.cache=!0});window.console||(window.console={log:function(a){}});VVVV={Config:{}};VVVV.Config.auto_undo=!1;VVVV.Nodes={};VVVV.PinTypes={};VVVV.NodeLibrary={};VVVV.NodeNames=[];VVVV.Patches={};VVVV.onNotImplemented=function(a){console.log("Warning: "+a+" is not implemented.")};VVVV.loadCounter=0;
...
d});"function"===typeof g&&g.call()}VVVV.Root=a||"./";"development"==VVVV_ENV&&console.log("loading vvvv.js ...");if("development"==VVVV_ENV){document.getElementsByTagName("head");var e=function(d){d.target.removeEventListener("load",e);0>=--VVVV.loadCounter&&f()};0==$("script[src*=thirdparty]").length&&VVVV.loadScript("thirdparty.js",e);0==$("script[src*=underscore]").length&&VVVV.loadScript("lib/underscore/underscore-min.js",e);0!=$('script[src*="d3.js"]').length||"full"!=b&&"vvvviewer"!=b||VVVV.loadScript("lib/d3-v3/d3.v3.min.js",
...
VVVV.Core={Pin:function(a,b,g,f,e){this.direction=b;this.pinname=a;this.links=[];this.values=[];this.node=f;this.changed=!0;this.reset_on_disconnect=this.active=!1;this.masterPin=this.slavePin=void 0;this.connectionChangedHandlers={};this.enumOptions=[];void 0==e&&"development"==VVVV_ENV&&console.warn(f.nodename+" / "+a+" -> Generic");this.getValue=function(d,c){if(!c||1==c)return this.values[d%this.values.length];for(var h=[],e=0;e<c;e++)h.push(this.values[(d*c+e)%this.values.length]);return h};
...
h.attr("values",_(d.defaultPinValues[c.pinname]).map(function(c){return"|"+c+"|"}).join(","));f.append(h)});_(this.invisiblePins).each(function(c){var d=$("<PIN>");d.attr("pinname",c.pinname);d.attr("visible","0");VVVV.PinTypes[c.typeName].primitive&&d.attr("values",_(c.values).map(function(c){return"|"+c+"|"}).join(","));f.append(d)});return f}},Link:function(a,b){this.fromPin=a;this.toPin=b;this.fromPin.links.push(this);this.toPin.links.push(this);this.destroy=function(){this.fromPin.links.splice(this.fromPin.links.indexOf(this),
1);this.toPin.links.splice(this.toPin.links.indexOf(this),1);this.fromPin.node.parentPatch.linkList.splice(this.fromPin.node.parentPatch.linkList.indexOf(this),1)};this.serialize=function(){var g=$("<LONK>");g.attr("srcnodeid",this.fromPin.node.id);g.attr("srcpinname",this.fromPin.pinname);g.attr("dstnodeid",this.toPin.node.id);g.attr("dstpinname",this.toPin.pinname);return g}},Patch:function(a,b,g,f,e){function d(c){if(void 0==c)return[];if("45_26">=this.vvvv_version)return separator=/\|/.test(c)?
"|":",",c.split(separator).filter(function(c,d){return""!=c});for(var d=[],h="",e=!1,f=c.length,g=0;g<f;g++)","!=c[g]||e?"|"==c[g]?e=!e:h+=c[g]:(d.push(h),h="");d.push(h);return d}function c(){var c=window.location.hash.match("#([^/]+)/"+m.ressource+"$");c&&(console.log("launching editor ..."),(new VVVV.Editors[c[1]]).enable(m))}this.ressource=a;this.vvvv_version="45_26.1";this.boundingBox={width:0,height:0};this.windowHeight=this.windowWidth=500;this.pinMap={};this.nodeMap={};this.nodeList=[];this.linkList=
[];this.success=b;this.error=g;this.XMLCode="";this.editor=void 0;this.setupObject();f&&(this.parentPatch=f);e&&(this.id=e);var h=!1;this.getAbsolutePath=function(){var c=this.getRelativePath();this.parentPatch&&(c=this.parentPatch.getRelativePath()+c);return c};this.getRelativePath=function(){return this.nodename.match(/(.*\/)?[^/]+\.v4p/)[1]||""};this.destroy=function(){for(var c=0;c<this.nodeList.length;c++)this.nodeList[c].destroy(),delete this.nodeMap[this.nodeList[c].id],delete this.nodeList[c]};
if("45_26">=this.vvvv_version)var q={},n={},k={},r={};var m=this;this.doLoad=function(c,h){function e(c){"complete"==g&&(n={});$(c).find('link[deleteme="pronto"]').each(function(){for(var c=!1,d=0;d<m.linkList.length;d++)m.linkList[d].fromPin.node.id==$(this).attr("srcnodeid")&&(m.linkList[d].fromPin.pinname==$(this).attr("srcpinname")&&m.linkList[d].toPin.node.id==$(this).attr("dstnodeid")&&m.linkList[d].toPin.pinname==$(this).attr("dstpinname"))&&(c=m.linkList[d]);if(c){"development"==VVVV_ENV&&
console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var d=c.fromPin,h=c.toPin;c.destroy();d.connectionChanged();h.connectionChanged();h.markPinAsChanged();h.reset_on_disconnect&&h.reset()}});$(c).find('link[deleteme!="pronto"]').each(function(){var c=m.pinMap[$(this).attr("srcnodeid")+"_out_"+$(this).attr("srcpinname")],d=m.pinMap[$(this).attr("dstnodeid")+"_in_"+$(this).attr("dstpinname")];void 0==c&&m.nodeMap[$(this).attr("srcnodeid")]&&(c=m.nodeMap[$(this).attr("srcnodeid")].addOutputPin($(this).attr("srcpinname"),
void 0));void 0==d&&m.nodeMap[$(this).attr("dstnodeid")]&&(d=m.nodeMap[$(this).attr("dstnodeid")].addInputPin($(this).attr("dstpinname"),void 0));if(c&&d){c.typeName!=d.typeName&&"Node"!=c.typeName&&"Node"!=d.typeName&&"development"==VVVV_ENV&&console.warn("Type Mismatch: "+c.node.nodename+"/"+c.pinname+" <-> "+d.node.nodename+"/"+d.pinname);for(var h=!1,e=0;e<m.linkList.length;e++)m.linkList[e].fromPin.node.id==c.node.id&&(m.linkList[e].fromPin.pinname==c.pinname&&m.linkList[e].toPin.node.id==d.node.id&&
m.linkList[e].toPin.pinname==d.pinname)&&(h=m.linkList[e]);if(!h){h=new VVVV.Core.Link(c,d);c.connectionChanged();d.connectionChanged();m.linkList.push(h);for(e=0;e<c.values.length;e++)d.setValue(e,c.getValue(e));d.setSliceCount(c.getSliceCount())}"complete"==g&&(n[c.node.id+"_"+c.pinname+"-"+d.node.id+"_"+d.pinname]=h)}});"complete"==g&&(_(q).each(function(c,d){if(void 0==n[d]){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var h=c.fromPin,e=c.toPin;c.destroy();
h.connectionChanged();e.connectionChanged();e.markPinAsChanged();e.reset_on_disconnect&&e.reset()}}),q={},_(n).each(function(c,d){q[d]=c}))}var f=this;do f.dirty=!0;while(f=f.parentPatch);if(f=/^<!DOCTYPE\s+PATCH\s+SYSTEM\s+"(.+)\\(.+)\.dtd/.exec(c))m.vvvv_version=f[2].replace(/[a-zA-Z]+/,"_");var g="diff";if(/\s<PATCH/.test(c)||"45_26">=m.vvvv_version)g="complete","development"==VVVV_ENV&&console.log("complete: "+this.nodename),m.XMLCode=c;f=$(c).find('bounds[type="Window"]').first();0<f.length&&
(m.windowWidth=f.attr("width")/15,m.windowHeight=f.attr("height")/15);"complete"==g&&(r={});var a=0;$(c).find("node").each(function(){if("pronto"==$(this).attr("createme")&&void 0!=m.nodeMap[$(this).attr("id")]){var f=m.nodeMap[$(this).attr("id")];"development"==VVVV_ENV&&console.log("node renamed, so deleting node "+f.id+" / "+f.nodename);_(f.inputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.fromPin.connectionChanged()})});_(f.outputPins).each(function(c){_(c.links).each(function(c){c.destroy();
c.toPin.connectionChanged();c.toPin.markPinAsChanged();c.toPin.reset_on_disconnect&&c.toPin.reset()})});m.nodeList.splice(m.nodeList.indexOf(f),1);delete m.nodeMap[f.id]}var q;q="InABox"==$(this).attr("componentmode")?$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var n=void 0!=m.nodeMap[$(this).attr("id")];if(n)f=m.nodeMap[$(this).attr("id")];else{nodename=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==nodename)return;
if(void 0!=VVVV.NodeLibrary[nodename.toLowerCase()])f=new (VVVV.NodeLibrary[nodename.toLowerCase()])($(this).attr("id"),m),VVVV.NodeLibrary[nodename.toLowerCase()].requirements&&(m.resourcesPending++,_(VVVV.NodeLibrary[nodename.toLowerCase()].requirements).each(function(c){void 0===VVVV.LoadedLibs[c]&&VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){m.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))f=new VVVV.Nodes.GenericShader($(this).attr("id"),m),f.isShader=!0,f.shaderFile=
$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),f.nodename=nodename;else if(/.v4p$/.test($(this).attr("filename"))){m.resourcesPending++;a++;var k=this,f=new VVVV.Core.Patch($(this).attr("filename"),function(){m.resourcesPending--;a--;"development"==VVVV_ENV&&console.log(this.nodename+"invoking update links");e(c);m.editor&&m.editor.addPatch(this);if(this.auto_evaluate){var d=m;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(m.mainloop);
m.afterUpdate();0>=m.resourcesPending&&h&&(h(),h=void 0)},function(){this.not_implemented=!0;VVVV.onNotImplemented(nodename)},m,$(k).attr("id"));f.isSubpatch=!0;m.editor&&!f.editor&&m.editor.addPatch(f);m.nodeMap[f.id]=f}else f=new VVVV.Core.Node($(this).attr("id"),nodename,m),f.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&m.editor.sendUndo(),VVVV.onNotImplemented(nodename);"development"==VVVV_ENV&&"complete"!=g&&console.log(m.nodename+": inserted new node "+f.nodename)}if(f.auto_evaluate){k=
m;do k.auto_evaluate=!0;while(k=k.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+f.id);if(f.isSubpatch){f.editor&&f.editor.removePatch(f);k=f.getSubPatches();k.push(f);for(var b,l=0;l<k.length;l++)b=VVVV.Helpers.prepareFilePath(k[l].nodename,k[l].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(f),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}m.nodeList.splice(m.nodeList.indexOf(f),1);f.destroy();delete m.nodeMap[f.id]}0<
q.length&&(q.attr("left")&&(f.x=q.attr("left")/15,f.y=q.attr("top")/15,m.boundingBox.width=Math.max(m.boundingBox.width,f.x+100),m.boundingBox.height=Math.max(m.boundingBox.height,f.y+100)),q.attr("width")&&(f.width=q.attr("width"),f.height=q.attr("height")));/^iobox/.test(f.nodename.toLowerCase())&&(f.isIOBox=!0);n||f.setup();k=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),e=d($(this).attr("values"));void 0!=e&&0<e.length&&f.addDefault(h,e);if(void 0==f.outputPins[h])if(void 0!=
...
f.addOutputPin(h,e):void 0==f.inputPins[h]&&void 0==f.invisiblePins[h]&&f.addInputPin(h,e):void 0==f.inputPins[h]&&void 0==f.invisiblePins[h]&&f.addInvisiblePin(h,e)});n||(f.initialize(),m.nodeList.push(f));"complete"==g&&(r[f.id]=f)});"complete"==g&&(_(k).each(function(c,d){void 0==r[d]&&("development"==VVVV_ENV&&console.log("removing node "+c.id),m.nodeList.splice(m.nodeList.indexOf(c),1),delete m.nodeMap[c.id])}),k={},_(r).each(function(c,d){k[d]=c}));0===a&&e(c);0>=this.resourcesPending&&h&&(h(),
h=void 0)};this.getSubPatches=function(){for(var c=[],d=0;d<this.nodeList.length;d++)this.nodeList[d].isSubpatch&&(c.push(this.nodeList[d]),c=c.concat(this.nodeList[d].getSubPatches()));return c};this.setMainloop=function(c){this.mainloop=c;for(var d=0;d<this.nodeList.length;d++)this.nodeList[d].isSubpatch&&this.nodeList[d].setMainloop(c)};this.afterEvaluate=function(){};this.afterUpdate=function(){};this.toXML=function(){var c=$("<PATCH>"),d=$("<BOUNDS>");d.attr("type","Window");d.attr("width",parseInt(15*
this.windowWidth));d.attr("height",parseInt(15*this.windowHeight));c.append(d);for(d=0;d<this.nodeList.length;d++)c.append(this.nodeList[d].serialize());for(d=0;d<this.linkList.length;d++)c.append(this.linkList[d].serialize());c='<!DOCTYPE PATCH  SYSTEM "http://vvvv.org/versions/vvvv45beta28.1.dtd" >\r\n  '+c.wrapAll("<d></d>").parent().html();c=c.replace(/<patch/g,"<PATCH");c=c.replace(/<\/patch>/g,"\n  </PATCH>");c=c.replace(/<node/g,"\n  <NODE");c=c.replace(/<\/node>/g,"\n  </NODE>");c=c.replace(/<bounds/g,
"\n  <BOUNDS");c=c.replace(/<\/bounds>/g,"\n  </BOUNDS>");c=c.replace(/<pin/g,"\n  <PIN");c=c.replace(/<\/pin>/g,"\n  </PIN>");c=c.replace(/<lonk/g,"\n  <LINK");return c=c.replace(/<\/lonk>/g,"\n  </LINK>")};this.evaluate=function(){function c(d){var f=d.getUpstreamNodes();_(f).each(function(d){void 0==e[d.id]||d.delays_output||c(d)});if(d.dirty||d.auto_evaluate||d.isSubpatch){if(h)var g=(new Date).getTime();if(d.auto_nil&&!d.isSubpatch&&d.hasNilInputs())_(d.outputPins).each(function(c){c.setSliceCount(0)});
else{try{d.evaluate()}catch(q){console.log("VVVV.Js / Error evaluating "+d.nodename+": "+q.message)}h&&console.log(d.nodename+" / "+d.id+": "+((new Date).getTime()-g)+"ms");d.dirty=!1;_(d.inputPins).each(function(c){c.changed=!1})}}delete e[d.id];return!0}if(!(0<this.resourcesPending)){if(h)var d=(new Date).getTime();for(var e={},f={},g=0;g<this.nodeList.length;g++){if(0==this.nodeList[g].getDownstreamNodes().length||this.nodeList[g].auto_evaluate||this.nodeList[g].delays_output)f[this.nodeList[g].id]=
this.nodeList[g];e[this.nodeList[g].id]=this.nodeList[g]}h&&console.log("building node maps: "+((new Date).getTime()-d)+"ms");_(f).each(function(d,h,f){void 0!=e[d.id]&&c(d)});h&&(d=(new Date).getTime());this.afterEvaluate();h&&console.log("patch rendering: "+((new Date).getTime()-d)+"ms");h=!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(h=!0)});if(/\.v4p[^<>\s]*$/.test(a)){this.nodename=a;var l=this,t=a;this.parentPatch&&(t=VVVV.Helpers.prepareFilePath(a,this.parentPatch));
VVVV.Patches[t]?l.doLoad(VVVV.Patches[t][0].toXML(),function(){VVVV.Patches[t].push(l);l.success&&l.success();l.afterUpdate()}):$.ajax({url:t,type:"get",dataType:"text",success:function(c){l.doLoad(c,function(){VVVV.Patches[t]=VVVV.Patches[t]||[];VVVV.Patches[t].push(l);l.success&&l.success();l.afterUpdate()})}})}else/^ws:\/\//.test(a)?(l=this,VVVV.Editors.Connector.host=a,VVVV.Editors.Connector.enable(this,{success:function(){l.success&&l.success()},error:function(){l.error&&l.error()}})):this.doLoad(a,
function(){this.success&&this.success()});c();$(window).bind("hashchange",function(){c()})}};VVVV.Core.Patch.prototype=new VVVV.Core.Node;if(!WebSocket&&MozWebSocket)var WebSocket=MozWebSocket;
...
VVVV.Core.MainLoop=function(a,b){function g(){if(0==a.resourcesPending){f++;var q=(new Date).getTime();e.populateInputConnectors();a.evaluate();e.processOutputConnectors();var b=(new Date).getTime()-q;0==f%10&&(h&&(console.log(1/((q-c)/1E4)+"fps"),h=!1),c=q)}this.deltaT=Math.max(0,Math.round(1E3/VVVV.fps-b));d&&window.setTimeout(g,this.deltaT)}VVVV.fps=b||60;var f=0,e=new VVVV.Core.DOMInterface(a),d=!0,c=0,h=!1;this.deltaT=1E3/VVVV.fps;a.setMainloop(this);this.stop=function(){d=!1};this.start=function(){d||
...
[0],VVVV.PinTypes.Value),q=this.addOutputPin("Y Out",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=0,b=0;b<f.getSliceCount();b++)for(var r=0;r<g.getSliceCount();r++)0.5>e.getValue(0)&&b>r||(0.5>d.getValue(0)&&b<r||0.5>c.getValue(0)&&b==r)||(h.setValue(a,g.getValue(r)),q.setValue(a,f.getValue(b)),a++);h.setSliceCount(a);q.setSliceCount(a)}};VVVV.Nodes.Cross2d.prototype=new VVVV.Core.Node;
...
c;h++){var q=g.getValue(h),a=f.getValue(h);e.setValue(h,Math.atan2(a,q)/(2*Math.PI));d.setValue(h,Math.sqrt(q*q+a*a))}e.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Polar2d.prototype=new VVVV.Core.Node;
...
a=f.getValue(h);e.setValue(h,a*Math.cos(2*q*Math.PI));d.setValue(h,a*Math.sin(2*q*Math.PI))}e.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Cartesian2d.prototype=new VVVV.Core.Node;
...
q=this.addOutputPin("Length",[1],VVVV.PinTypes.Value),n=this.addOutputPin("Angle",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var m=parseFloat(g.getValue(b)),l=parseFloat(f.getValue(b)),t=parseFloat(e.getValue(b)),s=parseFloat(d.getValue(b)),t=t-m,s=s-l;c.setValue(b,m+t/2);h.setValue(b,l+s/2);q.setValue(b,Math.sqrt(t*t+s*s));n.setValue(b,Math.atan2(s,t)/(2*Math.PI))}c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a);n.setSliceCount(a)}};
...
q=this.addOutputPin("X2",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Y2",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var m=parseFloat(g.getValue(b)),l=parseFloat(f.getValue(b)),t=parseFloat(e.getValue(b)),s=2*parseFloat(d.getValue(b)-0.25)*Math.PI,v=t/2*Math.sin(s),t=-t/2*Math.cos(s);c.setValue(b,m+v);h.setValue(b,l+t);q.setValue(b,m-v);n.setValue(b,l-t)}c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a);n.setSliceCount(a)}};
...
this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var k=g.getValue(b),r=f.getValue(b),m=e.getValue(b),l=Math.sqrt(k*k+r*r+m*m);c.setValue(b,Math.atan2(-k,-m)/(2*Math.PI));d.setValue(b,0==l?0:Math.acos(-r/l)/(2*Math.PI)-0.25);h.setValue(b,l)}c.setSliceCount(a);d.setSliceCount(a);h.setSliceCount(a)}};VVVV.Nodes.Polar3d.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),q=this.addOutputPin("Input Length",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=g.getValue(b),m=f.getValue(b),l=e.getValue(b),t=Math.sqrt(r*r+m*m+l*l);d.setValue(b,0==t?0:r/t);c.setValue(b,0==t?0:m/t);h.setValue(b,0==t?0:l/t);q.setValue(b,t)}d.setSliceCount(a);c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a)}};VVVV.Nodes.Normalize3d.prototype=new VVVV.Core.Node;
...
h[0]+h[1]*h[1]+h[2]*h[2]);f.setValue(3*c,0==a?0:h[0]/a);f.setValue(3*c+1,0==a?0:h[1]/a);f.setValue(3*c+2,0==a?0:h[2]/a);e.setValue(c,a)}f.setSliceCount(3*Math.ceil(d/3));e.setSliceCount(Math.ceil(d/3))}};VVVV.Nodes.Normalize3dVector.prototype=new VVVV.Core.Node;
...
3);e.setValue(3*c+0,h[1]*a[2]-h[2]*a[1]);e.setValue(3*c+1,-(h[0]*a[2]-h[2]*a[0]));e.setValue(3*c+2,h[0]*a[1]-h[1]*a[0])}e.setSliceCount(3*Math.ceil(d/3))}};VVVV.Nodes.Multiply3dCross.prototype=new VVVV.Core.Node;
VVVV.Nodes.Multiply3dDot=function(a,b){this.constructor(a,"Multiply (3d Dot)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Input1 XYZ",[],VVVV.PinTypes.Value),f=this.addInputPin("Input2 XYZ",[],VVVV.PinTypes.Value),e=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0;c<d/3;c++){var h=g.getValue(c,3),a=f.getValue(c,3);e.setValue(c,
h[0]*a[0]+h[1]*a[1]+h[2]*a[2])}e.setSliceCount(Math.ceil(d/3))}};VVVV.Nodes.Multiply3dDot.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],q=[],n=[],k=[],r=[],m=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||f.pinIsChanged(),s=0;s<a;s++){void 0==c[s]&&(c[s]=(new Date).getTime());var v=this.parentPatch.mainloop.deltaT,u=parseFloat(g.getValue(s)),w=parseFloat(f.getValue(s));void 0==n[s]&&(n[s]=u);if(u!=h[s]||w!=q[s])r[s]=void 0,h[s]=u,q[s]=w,m[s]=0<=h[s]-n[s],k[s]=0<q[s]?(h[s]-n[s])/(1E3*q[s]):0;m[s]==0<=h[s]-n[s]&&(n[s]+=
k[s]*v);void 0!=r[s]&&0<=h[s]-n[s]!=0<=r[s]&&(k[s]=0,n[s]=h[s]);0!=r[s]&&(e.setValue(s,n[s]),d.setValue(s,k[s]));r[s]=h[s]-n[s];c[s]=(new Date).getTime()}b&&e.getSliceCount()!=a&&(h.splice(a),q.splice(a),c.splice(a),n.splice(a),k.splice(a),r.splice(a),e.setSliceCount(a))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
VVVV.Nodes.Damper=function(a,b){this.constructor(a,"Damper (Animation)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Cyclic Pin not implemented","Acceleration Out is not set yet"]};this.auto_evaluate=!0;var g=this.addInputPin("Go To Position",[0],VVVV.PinTypes.Value),f=this.addInputPin("FilterTime",[1],VVVV.PinTypes.Value),e=this.addOutputPin("Position Out",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Velocity Out",[0],VVVV.PinTypes.Value);
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],q=[],n=[],k=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||f.pinIsChanged(),l=0;l<a;l++){void 0==c[l]&&(c[l]=(new Date).getTime());var t=this.parentPatch.mainloop.deltaT,s=parseFloat(g.getValue(l)),v=parseFloat(f.getValue(l));void 0==h[l]&&(h[l]=s);n[l]=void 0;k[l]=0<=s-h[l];q[l]=0<v?(s-h[l])/(1E3*v):0;k[l]==0<=s-h[l]&&(h[l]+=q[l]*t);void 0!=n[l]&&0<=s-h[l]!=0<=n[l]&&(q[l]=0,h[l]=s);
0!=n[l]&&(e.setValue(l,h[l]),d.setValue(l,q[l]));n[l]=s-h[l];c[l]=(new Date).getTime()}b&&e.getSliceCount()!=a&&(c.splice(a),h.splice(a),q.splice(a),n.splice(a),e.setSliceCount(a))}};VVVV.Nodes.Damper.prototype=new VVVV.Core.Node;
...
["miter"],VVVV.PinTypes.Enum);c.enumOptions=["miter","round","bevel"];var h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.CanvasRenderState),q=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==q[b]&&(q[b]=new VVVV.Types.CanvasRenderState),q[b].copy_attributes(g.getValue(b)),q[b].strokeColor=f.getValue(b).split(","),q[b].lineWidth=e.getValue(b),q[b].capStyle=d.getValue(b),q[b].joinStyle=c.getValue(b),h.setValue(b,q[b]);h.setSliceCount(a)}};
...
h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.CanvasRenderState),q=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==q[b]&&(q[b]=new VVVV.Types.CanvasRenderState),q[b].copy_attributes(g.getValue(b)),q[b].shadowOffsetX=e.getValue(b),q[b].shadowOffsetY=d.getValue(b),q[b].shadowBlur=c.getValue(b),q[b].shadowColor=f.getValue(b).split(","),h.setValue(b,q[b]);h.setSliceCount(a)}};VVVV.Nodes.ShadowCanvas.prototype=new VVVV.Core.Node;
...
k[b].gradient.startX=f.getValue(b);k[b].gradient.startY=e.getValue(b);k[b].gradient.endX=d.getValue(b);k[b].gradient.endY=c.getValue(b);k[b].gradient.colors=[];k[b].gradient.colorPositions=[];for(var l=Math.max(h.values.length,q.values.length),t=0;t<l;t++)k[b].gradient.colors.push(h.getValue(t).split(",")),k[b].gradient.colorPositions.push(q.getValue(t));n.setValue(b,k[b])}n.setSliceCount(a)}};VVVV.Nodes.LinearGradientCanvas.prototype=new VVVV.Core.Node;
...
c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==k[a]&&(k[a]=new r),k[a].transform=f.getValue(a),k[a].radius=parseFloat(d.getValue(a)),k[a].startAngle=2*parseFloat(c.getValue(a))*Math.PI,k[a].endAngle=2*parseFloat(h.getValue(a))*Math.PI,k[a].renderState=g.getValue(a),k[a].drawSegment=1==q.getValue(a),e.isConnected()&&(k[a].clippingLayer=e.getValue(a));for(a=0;a<k.length;a++)n.setValue(a,
k[a]);n.setSliceCount(b)}};VVVV.Nodes.ArcCanvas.prototype=new VVVV.Core.Node;
...
this.renderState.apply(c);var e=this.width/2,f=e-this.cornerRadius,h=this.height/2,a=h-this.cornerRadius,b=-e,g=-f,q=-h,k=-a;c.moveTo(g,q);c.lineTo(f,q);c.arc(f,k,this.cornerRadius,1.5*Math.PI,0,!1);c.lineTo(e,a);c.arc(f,a,this.cornerRadius,0,0.5*Math.PI,!1);c.lineTo(g,h);c.arc(g,a,this.cornerRadius,0.5*Math.PI,Math.PI,!1);c.lineTo(b,k);c.arc(g,k,this.cornerRadius,Math.PI,1.5*Math.PI,!1);c.closePath();0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();
c.restore()}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==n[b]&&(n[b]=new k),n[b].transform=f.getValue(b),n[b].width=d.getValue(b),n[b].height=c.getValue(b),n[b].cornerRadius=h.getValue(b),n[b].renderState=g.getValue(b),e.isConnected()&&(n[b].clippingLayer=e.getValue(b));for(b=0;b<n.length;b++)q.setValue(b,n[b]);q.setSliceCount(a)}};VVVV.Nodes.RectangleCanvas.prototype=new VVVV.Core.Node;
...
c.strokeText(this.text,0,0);c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==k[a]&&(k[a]=new r),k[a].transform=f.getValue(a),k[a].text=d.getValue(a),k[a].font=c.getValue(a),k[a].align=h.getValue(a),k[a].baseline=q.getValue(a),k[a].renderState=g.getValue(a),k[a].clippingLayer=e.getValue(a);for(a=0;a<k.length;a++)n.setValue(a,k[a]);n.setSliceCount(b)}};VVVV.Nodes.TextCanvas.prototype=new VVVV.Core.Node;
...
c.beginPath();c.moveTo(this.x[0],this.y[0]);for(var e=1;e<this.x.length;e++)c.bezierCurveTo(this.c1x[e-1]+this.x[e-1],this.c1y[e-1]+this.y[e-1],this.c2x[e]+this.x[e],this.c2y[e]+this.y[e],this.x[e],this.y[e]);0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,w=0,p=0;p<a||0<m.getValue(0)&&(0<w||0<b%e.getSliceCount());p++)void 0==t[b]&&(t[b]=new s),0==w&&(t[b].renderState=
f.getValue(b),t[b].transform=e.getValue(b),d.isConnected()&&(t[b].clippingLayer=d.getValue(b))),t[b].x[w]=parseFloat(c.getValue(p)),t[b].y[w]=parseFloat(h.getValue(p)),t[b].c1x[w]=parseFloat(q.getValue(p)),t[b].c1y[w]=parseFloat(n.getValue(p)),t[b].c2x[w]=parseFloat(k.getValue(p)),t[b].c2y[w]=parseFloat(r.getValue(p)),w++,0<m.getValue(0)&&w>=m.getValue(b)&&(w<t[b].x.length&&g(t[b],w),b++,w=0);0>m.getValue(0)&&w<t[b].x.length?(g(t[b],w),t.splice(1)):t.splice(b);for(a=0;a<t.length;a++)l.setValue(a,
...
this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==h[b]&&(h[b]=new q),h[b].transform=g.getValue(b),h[b].texture=e.getValue(b),h[b].color=d.getValue(b).split(","),f.isConnected()&&(h[b].clippingLayer=f.getValue(b));for(b=0;b<h.length;b++)c.setValue(b,h[b]);c.setSliceCount(a)}};VVVV.Nodes.QuadCanvas.prototype=new VVVV.Core.Node;
VVVV.Nodes.GroupCanvas=function(a,b){this.constructor(a,"Group (Canvas VVVVjs)",b);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var g=[],f=this.addOutputPin("Layer",[],VVVV.PinTypes.CanvasLayer);this.initialize=function(){for(var e=0;2>e;e++)g[e]=this.addInputPin("Layer "+(e+1),[],VVVV.PinTypes.CanvasLayer)};this.evaluate=function(){for(var e=0,d=0;2>d;d++)if(g[d].isConnected())for(var c=0;c<g[d].values.length;c++)f.setValue(e++,g[d].getValue(c));
...
VVVV.PinTypes.Color),d=this.addInputPin("Backbuffer Width",[0],VVVV.PinTypes.Value),c=this.addInputPin("Backbuffer Height",[0],VVVV.PinTypes.Value),h=this.addInputPin("View",[],VVVV.PinTypes.Transform),q=this.addOutputPin("Canvas Out",[],this),n,k,r,m=[0,0,0,1],l;this.getContext=function(){var e=this.invisiblePins["Descriptive Name"].getValue(0);if((e=$(e).get(0))&&"CANVAS"==e.nodeName)l=e;else{var f=parseInt(d.getValue(0)),h=parseInt(c.getValue(0));l=$('<canvas width="'+(0<f?f:512)+'" height="'+
(0<h?h:512)+'" id="vvvv-js-generated-renderer-'+(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');e||(e="body");$(e).append(l);l=l.get(0)}k=l.width;r=l.height;l&&l.getContext&&(g(l),n=l.getContext("2d"),l.ctx=n)};this.destroy=function(){$(l).remove()};this.evaluate=function(){this.invisiblePins["Descriptive Name"].pinIsChanged()&&($(l).hasClass("vvvv-js-generated-renderer")&&$(l).remove(),this.getContext());if(n){if(d.pinIsChanged()){var b=parseInt(d.getValue(0));0<b&&(k=b,$(l).attr("width",
k))}c.pinIsChanged()&&(b=parseInt(c.getValue(0)),0<b&&(r=b,$(l).attr("height",r)));e.pinIsChanged()&&(m=e.getValue(0).split(","),m[0]=parseInt(255*m[0]),m[1]=parseInt(255*m[1]),m[2]=parseInt(255*m[2]));defaultRenderState.apply(n);n.clearRect(0,0,k,r);n.save();n.translate(k/2,r/2);n.scale(k/2,-r/2);h.isConnected()&&(b=h.getValue(0),n.transform(b[0],b[1],b[4],b[5],b[12],b[13]));if(f.isConnected())for(b=0;b<f.values.length;b++)f.getValue(b).draw(n);n.restore();defaultRenderState.apply(n);n.globalCompositeOperation=
"destination-over";n.fillStyle="rgba("+m[0]+","+m[1]+","+m[2]+","+m[3]+")";n.fillRect(0,0,k,r);q.setValue(0,l);l.loaded=!0}}};VVVV.Nodes.RendererCanvas.prototype=new VVVV.Core.Node;VVVV.PinTypes.Color={typeName:"Color",reset_on_disconnect:!1,defaultValue:function(){return"1.0, 1.0, 1.0, 1.0"},primitive:!0,hsvToRgb:function(a,b,g){a*=360;var f=Math.floor(a/60),e=a/60-f;a=g*(1-b);var d=g*(1-b*e);b=g*(1-b*(1-e));switch(f){case 1:return[d,g,a];case 2:return[a,g,b];case 3:return[a,d,g];case 4:return[b,a,g];case 5:return[g,a,d];default:return[g,b,a]}},rgbToHsv:function(a,b,g){var f=Math.max(a,Math.max(b,g)),e=Math.min(a,Math.min(b,g)),d;f==e?d=0:f==a?d=60*(0+(b-g)/(f-e)):f==b?d=60*
(2+(g-a)/(f-e)):f==g&&(d=60*(4+(a-b)/(f-e)));0>d&&(d+=360);a=0;0!=f&&(a=(f-e)/f);return[d/360,a,f]}};
...
f.pinIsChanged||e.pinIsChanged()||d.pinIsChanged()){for(var h=this.getMaxInputSliceCount(),b=0;b<h;b++){var a=g.getValue(b)||0,k=f.getValue(b)||0,r=e.getValue(b)||0,m=d.getValue(b)||0;c.setValue(b,a+","+k+","+r+","+m)}c.setSliceCount(h)}}};VVVV.Nodes.RGBJoin.prototype=new VVVV.Core.Node;
...
this.getMaxInputSliceCount(),b=0;b<h;b++){var a=g.getValue(b),k=f.getValue(b),r=e.getValue(b),m=d.getValue(b),a=VVVV.PinTypes.Color.hsvToRgb(a,k,r);c.setValue(b,a[0]+","+a[1]+","+a[2]+","+m)}c.setSliceCount(h)}};VVVV.Nodes.HSVJoin.prototype=new VVVV.Core.Node;
...
d.setSliceCount(b);c.setSliceCount(b)}if(q){for(a=0;a<b;a++)f.setValue(a,h[a]),e.setValue(a,h[a].width),d.setValue(a,h[a].height),c.setValue(a,h[a].loaded?1:0);q=!1}}};VVVV.Nodes.FileTextureCanvas.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),v=this.addOutputPin("Video Height",[0],VVVV.PinTypes.Value),u=this.addOutputPin("Network Status",[""],VVVV.PinTypes.String),w=this.addOutputPin("Ready Status",[""],VVVV.PinTypes.String),p=[];this.evaluate=function(){var b=this.getMaxInputSliceCount();if(k.pinIsChanged())for(var a=0;a<b;a++){filename=VVVV.Helpers.prepareFilePath(k.getValue(a),this.parentPatch);if(void 0==p[a]){var z=$('<video style="display:none"><source src="" type=video/ogg></video>');$("body").append(z);
p[a]=z[0];p[a].volume=0;z=function(c){return function(){}}(a);p[a].onprogress=z;p[a].oncanplay=z;p[a].oncanplaythrough=z}filename!=p[a].currentSrc&&($(p[a]).find("source").first().attr("src",filename),p[a].load(),0.5<e.getValue(a)?p[a].play():p[a].pause(),p[a].loaded=!0,r.setValue(a,p[a]),m.setValue(a,p[a]))}if(e.pinIsChanged())for(a=0;a<b;a++)0.5<e.getValue(a)?p[a].play():p[a].pause();if(q.pinIsChanged())for(a=0;a<b;a++)p[a%p.length].loaded&&0.5<=q.getValue(a)&&(p[a%p.length].currentTime=parseFloat(n.getValue(a)),
0.5<e.getValue(a)&&p[a].play());for(a=0;a<b;a++){if(!p[a].paused){r.setValue(a,p[a]);m.setValue(a,p[a]);l.getValue(a)!=p[a].duration&&l.setValue(a,p[a].duration);t.setValue(a,p[a].currentTime);var z=parseFloat(h.getValue(a)),E=parseFloat(c.getValue(a));p[a].currentTime<E&&(p[a].currentTime=E);if(p[a].currentTime>=p[a].duration||0<=z&&p[a].currentTime>=z)0.5<=d.getValue(a)?p[a].currentTime=E:p[a].pause()}if(p[a].videoWidth!=s.getValue(a)||p[a].videoHeight!=v.getValue(a))s.setValue(a,p[a].videoWidth),
v.setValue(a,p[a].videoHeight);u.getValue(a)!=g[p[a].networkState]&&u.setValue(a,g[p[a].networkState]);w.getValue(a)!=f[p[a].readyState]&&w.setValue(a,f[p[a].readyState])}r.setSliceCount(b);m.setSliceCount(b)}};VVVV.Nodes.FileStreamCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Value),c=this.addOutputPin("Status",[""],VVVV.PinTypes.String),h=this.addOutputPin("Body",[""],VVVV.PinTypes.String),q=this.addOutputPin("Fail",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Success",[0],VVVV.PinTypes.Value),k,r,m,l=!1;this.evaluate=function(){this.getMaxInputSliceCount();var a=g.pinIsChanged()||f.pinIsChanged()||e.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==n.getValue(0)&&n.setValue(0,0);1==q.getValue(0)&&q.setValue(0,0);l&&(h.setValue(0,k),c.setValue(0,void 0),
n.setValue(0,r),q.setValue(0,m),l=!1);a&&(void 0==g.getValue(0)?(h.setValue(0,""),c.setValue(0,"")):$.ajax({url:g.getValue(0),type:"get",success:function(c,d,e){k=c;r=1;l=!0},error:function(c,d){k="";m=1;l=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,k=0,r,m,l,t=0;t<a||0<f.getValue(0)&&(0<k||0!=b%f.values.length);t++)r=g.getValue(t),0==k&&(center=m=l=r,width=0),m=Math.min(r,m),l=Math.max(r,l),k++,0<f.getValue(0)&&k>=f.getValue(b)&&(c.setValue(b,m),h.setValue(b,l),e.setValue(b,m+(l-m)/2),d.setValue(b,Math.abs(l-m)),b++,k=0);-1==f.getValue(0)&&(c.setValue(b,m),h.setValue(b,l),e.setValue(b,m+(l-m)/2),d.setValue(b,Math.abs(l-m)));e.setSliceCount(b+(0<k));
...
[0],VVVV.PinTypes.Value),c=this.addInputPin("Spread Count",[1],VVVV.PinTypes.Value),h=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=parseInt(c.getValue(0)),b=parseFloat(f.getValue(0)),k=parseFloat(d.getValue(0)),r=parseFloat(g.getValue(0)),m=e.getValue(0);""==m&&(m="Centered");var l=b/a;"Block"==m&&(l=b/(a-1));var t=l/2;if("Block"==m||"LeftJustified"==m)t=0;"RightJustified"==m&&(t=l);for(var s,v=0;v<a;v++)s=v*l+t,"Block"!=m&&0!=b&&(s=(s+k*b)%b),s=r-b/2+s,h.setValue(v,
...
c=parseInt(this.sprcntin.getValue(0)),h=2*Math.PI,d=h*d,q=0,n=Math.max(this.inpyin.getSliceCount(),this.inpxin.getSliceCount()),k=0;k<n*c;k++)for(var r=parseFloat(this.inpxin.getValue(k)),m=parseFloat(this.inpyin.getValue(k)),l=0;l<c;l++){var t=d+h*(l/c)*e,s=r+Math.cos(t)*a,t=m+Math.sin(t)*b;this.xout.setValue(q,s);this.yout.setValue(q,t);q++}this.xout.setSliceCount(c*n);this.yout.setSliceCount(c*n)}};VVVV.Nodes.CircularSpread.prototype=new VVVV.Core.Node;
...
d||void 0==d)d=f.getValue(b)}for(b=0;b<c;b++){var n=g.getValue(b);if(n<d)e.setValue(b,-1);else{for(var k=0;k<a-1&&!(n>=f.getValue(k)&&n<f.getValue(k+1));k++);e.setValue(b,k)}}e.setSliceCount(c)}};VVVV.Nodes.IntervalSpreads.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0;c<d;c++){var a=[],b=f.getValue(c);void 0==b&&(b="");for(var n=0;n<g.length;n++)a.push(g[n].getValue(c));e.setValue(c,a.join(b))}e.setSliceCount(d)}};VVVV.Nodes.AddString.prototype=new VVVV.Core.Node;
...
c=this.addOutputPin("Hits",[0],VVVV.PinTypes.Value),h=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),q=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Found At Position",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=this.getMaxInputSliceCount();c.setSliceCount(a);var b=e.getValue(0);d.getValue(0);for(var m=0,l=0;l<a;l++){var t=g.getValue(l),s=1;if("MatchesAny"==b||"ContainsAny"==b)s=f.getSliceCount();for(var v=0,u=0;u<s;u++){var w=f.getValue(u+l),
p=[];if("Matches"==b||"MatchesAny"==b)w==t&&p.push(-1);else for(var w=RegExp(w,"gi"),A;A=w.exec(t);)p.push(A.index);for(w=0;w<p.length;w++)h.setValue(m,l%g.getSliceCount()),q.setValue(m,(l+u)%f.getSliceCount()),n.setValue(m,p[w]+1),m++,v++}c.setValue(l,v)}h.setSliceCount(m);q.setSliceCount(m);n.setSliceCount(m)}};VVVV.Nodes.SiftString.prototype=new VVVV.Core.Node;
...
UnderScore:"_",Minus:"-"},k={Single:"'",Double:'"'};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,l=0;l<a;l++){var t=g.getValue(l),s=n[f.getValue(l)],v=e.getValue(l),u=k[d.getValue(l)],w=c.getValue(l);""==s&&(s=v);""==s&&(s=void 0);t=t.split(s);s=t.length;u=RegExp("[^"+u+"]*","g");for(v=0;v<s;v++)1==w?h.setValue(b,t[v]):h.setValue(b,t[v].match(u).join("")),q.setValue(b,l),b++}h.setSliceCount(b);q.setSliceCount(b)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=g.getValue(b),k=f.getValue(b),r=e.getValue(b),n=n==k||0==r&&n.toLowerCase()==k.toLowerCase()?1:0;d.setValue(b,n);c.setValue(b,1-n)}d.setSliceCount(a);c.setSliceCount(a)}};VVVV.Nodes.EQString.prototype=new VVVV.Core.Node;
...
0;n<f.getValue(b);n++)e.setValue(a,g.getValue(b)),d.setValue(a,b),a++;e.setSliceCount(a);d.setSliceCount(a)}};VVVV.Nodes.SelectString.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var a=this.trIn.isConnected()?this.getMaxInputSliceCount():Math.max(this.xIn.getSliceCount(),this.yIn.getSliceCount(),this.zIn.getSliceCount()),b=0;b<a;b++){var e=this.inputPins["Transform In"].getValue(b),d=parseFloat(this.inputPins.X.getValue(b)),c=parseFloat(this.inputPins.Y.getValue(b)),h=parseFloat(this.inputPins.Z.getValue(b)),q=mat4.create();mat4.identity(q);mat4.rotate(q,2*c*Math.PI,[0,1,0]);mat4.rotate(q,2*d*Math.PI,[1,0,0]);mat4.rotate(q,2*h*Math.PI,[0,0,1]);
this.trIn.isConnected()&&mat4.multiply(e,q,q);this.trOut.setValue(b,q)}this.trOut.setSliceCount(a)}};VVVV.Nodes.Rotate.prototype=new VVVV.Core.Node;
...
b=0;b<a;b++){var n=0;Math.abs(g.getValue(b)-f.getValue(b))<=Math.abs(e.getValue(b))&&(n=1);d.setValue(b,n);c.setValue(b,1-n)}d.setSliceCount(a);c.setSliceCount(a)}};VVVV.Nodes.EqValue.prototype=new VVVV.Core.Node;
...
[1],VVVV.PinTypes.Value),h=this.addInputPin("Mapping",["Float"],VVVV.PinTypes.Enum);h.enumOptions=["Float","Wrap","Clamp"];var q=this.addOutputPin("Output",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=parseFloat(g.getValue(b)),m=parseFloat(f.getValue(b)),l=parseFloat(e.getValue(b)),t=parseFloat(d.getValue(b)),s=parseFloat(c.getValue(b)),v=h.getValue(b);if(l==m)q.setValue(b,t);else{r-=m;l-=m;switch(v){case "Clamp":r=Math.max(0,Math.min(l,
r));break;case "Wrap":r=(l+r%l)%l}q.setValue(b,r*((s-t)/l)+t)}}q.setSliceCount(a)}};VVVV.Nodes.MapValue.prototype=new VVVV.Core.Node;
...
function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=g.getValue(b),k=f.getValue(b),r=e.getValue(b),m=d.getValue(b),l=c.values[b];void 0==l&&c.setValue(b,0);if(0.5<=n){l=c.values[b];do n=Math.random()*r,0.5<=k&&(n=Math.round(n));while(0.5<=m&&n==l);c.setValue(b,n)}}c.setSliceCount(a)}};VVVV.Nodes.RandomValue.prototype=new VVVV.Core.Node;
...
b=parseInt(f.getValue(0)),n=[],k=[],r=[],m=0,l=0;l<b;l++)for(var t=0;t<a;t++)n.push(parseFloat(t)/(a-1)-0.5),n.push(0.5-parseFloat(l)/(b-1)),n.push(0),m++,k.push(0),k.push(0),k.push(1),r.push(parseFloat(t)/(a-1)),r.push(parseFloat(l)/(b-1));n=new VVVV.Types.VertexBuffer(c,n);n.setSubBuffer("TEXCOORD0",2,r);n.setSubBuffer("NORMAL",3,k);n.create();k=[];for(l=0;l<b-1;l++)for(t=0;t<a-1;t++)r=t+a*l,k.push(r),k.push(r+1),k.push(r+a+1),k.push(r+a+1),k.push(r+a),k.push(r);d=new VVVV.Types.Mesh(c,n,k);e.setValue(0,
...
this.renderContexts[0];if(a){for(var b=parseInt(f.getValue(0)),n=parseInt(e.getValue(0)),k=parseFloat(g.getValue(0)),r=[],m=[],l=[],t=0;t<n+1;t++)for(var s=Math.cos(-parseFloat(t)/n*Math.PI),v=0;v<b;v++){var u=Math.cos(2*(parseFloat(v)/b)*Math.PI)*Math.cos(Math.asin(s)),w=Math.sin(2*(parseFloat(v)/b)*Math.PI)*Math.cos(Math.asin(s));r.push(u*k);r.push(s*k);r.push(w*k);m.push(u);m.push(s);m.push(w);l.push(parseFloat(v)/b);l.push(parseFloat(t)/n)}k=new VVVV.Types.VertexBuffer(a,r);k.setSubBuffer("TEXCOORD0",
2,l);k.setSubBuffer("NORMAL",3,m);k.create();m=[];for(t=0;t<n;t++)for(v=0;v<b;v++)l=b*t,r=v+l,m.push(r),m.push((r+1)%b+l),m.push((r+1)%b+b+l),m.push((r+1)%b+b+l),m.push(r+b),m.push(r);c=new VVVV.Types.Mesh(a,k,m);d.setValue(0,c)}}}};VVVV.Nodes.Sphere.prototype=new VVVV.Core.Node;
...
[15],VVVV.PinTypes.Value),q=this.addInputPin("Resolution Y",[1],VVVV.PinTypes.Value),n=this.addOutputPin("Mesh",[],VVVV.PinTypes.WebGlResource),k=null;this.evaluate=function(){if(this.renderContexts){var a=this.renderContexts[0];if(a){var b=parseInt(h.getValue(0)),l=parseInt(q.getValue(0)),t=parseFloat(g.getValue(0)),s=parseFloat(f.getValue(0)),v=parseFloat(e.getValue(0)),u=parseFloat(d.getValue(0)),w=[],p=[],A=[];w.push(0);w.push(v/2);w.push(0);p.push(0);p.push(1);p.push(0);A.push(0);A.push(0);w.push(0);
w.push(-v/2);w.push(0);p.push(0);p.push(-1);p.push(0);A.push(0);A.push(0);for(var y=0;y<l+1;y++)for(var z=parseFloat(y)/l,E=(z-0.5)*-v,C=0;C<b+1;C++){var H=Math.cos(2*(parseFloat(C)/b)*Math.PI*u-Math.PI*u-Math.PI/2),J=Math.sin(2*(parseFloat(C)/b)*Math.PI*u-Math.PI*u-Math.PI/2),I=z*s+(1-z)*t;w.push(H*I);w.push(E);w.push(J*I);p.push(H);p.push(0);p.push(J);A.push(parseFloat(C)/b);A.push(parseFloat(y)/l)}t=new VVVV.Types.VertexBuffer(a,w);t.setSubBuffer("TEXCOORD0",2,A);t.setSubBuffer("NORMAL",3,p);t.create();
p=[];if(0.5<c.getValue(0))for(z=0;2>z;z++)for(C=0;C<b;C++)p.push(z),p.push(2+C+z+z*l*(b+1)),p.push(2+C+(1-z)+z*l*(b+1));for(y=0;y<l;y++)for(C=0;C<b;C++)z=C+(b+1)*y+2,p.push(z),p.push(z+1),p.push(z+b+2),p.push(z+b+2),p.push(z+b+1),p.push(z);k=new VVVV.Types.Mesh(a,t,p);n.setValue(0,k)}}}};VVVV.Nodes.Cylinder.prototype=new VVVV.Core.Node;
...
var h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.WebGlRenderState),q=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==q[b]&&(q[b]=new VVVV.Types.WebGlRenderState),q[b].copy_attributes(f.getValue(b)),q[b].alphaBlending=0.5<parseFloat(e.getValue(b)),q[b].srcBlendMode=g(d.getValue(b)),q[b].destBlendMode=g(c.getValue(b)),h.setValue(b,q[b]);h.setSliceCount(a)}};VVVV.Nodes.BlendWebGLAdvanced.prototype=new VVVV.Core.Node;
...
q[b].copy_attributes(f.getValue(b)),q[b].enableZWrite=0.5<parseFloat(e.getValue(b)),q[b].depthFunc=g(d.getValue(b)),q[b].depthOffset=parseFloat(c.getValue(0)),h.setValue(b,q[b]);h.setSliceCount(a)}};VVVV.Nodes.ZWriteEnableWebGL.prototype=new VVVV.Core.Node;
...
(b=0);var q=c.length;if(this.contextChanged||n)q=0;b<q&&c.splice(b,q-b);for(var s=q;s<b;s++)c[s]=new VVVV.Types.Layer,c[s].mesh=f.getValue(0),c[s].shader=h,_(h.uniformSpecs).each(function(a){c[s].uniformNames.push(a.varname);c[s].uniforms[a.varname]={uniformSpec:a,value:void 0}});if(f.pinIsChanged())for(s=0;s<b;s++)c[s].mesh=f.getValue(0);for(s=0;s<b;s++)c[s].shader=h;for(a=0;a<k.length;a++){var v=k[a].pinname.replace(/ /g,"_");if(k[a].pinIsChanged()||q<b)for(s=0;s<b;s++)if("vec"==h.uniformSpecs[v].type){var u=
"COLOR"==h.uniformSpecs[v].semantic?_(k[a].getValue(s).split(",")).map(function(a){return parseFloat(a)}):k[a].getValue(s,k[a].dimensions);c[s].uniforms[v].value=new Float32Array(u)}else u=k[a].getValue(s),c[s].uniforms[v].value=u}if(g.pinIsChanged()||q<b)for(a=0;a<b;a++)g.isConnected()?c[a].renderState=g.getValue(a):c[a].renderState=VVVV.DefaultRenderState;if(e.pinIsChanged()||q<b)for(a=0;a<b;a++)q=this.inputPins.Transform.getValue(a),c[a].uniforms[c[a].shader.uniformSemanticMap.WORLD].value=q;this.outputPins.Layer.setSliceCount(b);
...
d.setFragmentShader(b);d.setVertexShader("attribute vec3 PosO : POSITION; attribute vec2 TexCd : TEXCOORD0; uniform mat4 tW : WORLD; uniform mat4 tV : VIEW; uniform mat4 tP : PROJECTION; uniform mat4 tTex; varying vec2 vs2psTexCd; void main(void) { gl_Position = tP * tV * tW * vec4(PosO, 1.0); vs2psTexCd = (tTex * vec4(TexCd.xy-.5, 0.0, 1.0)).xy+.5; }");d.setup(a)}a=this.getMaxInputSliceCount();b=f.length;this.contextChanged&&(b=0);a<b&&f.splice(a,b-a);for(var q=b;q<a;q++)f[q]=new VVVV.Types.Layer,
f[q].mesh=e,f[q].shader=d,_(d.uniformSpecs).each(function(a){f[q].uniformNames.push(a.varname);f[q].uniforms[a.varname]={uniformSpec:a,value:void 0}});var n=this.inputPins.Color.pinIsChanged(),k=this.inputPins.Transform.pinIsChanged(),r=this.inputPins.Texture.pinIsChanged(),m=this.inputPins["Texture Transform"].pinIsChanged();if(n||b<a)for(n=0;n<a;n++){var l=this.inputPins.Color.getValue(n),l=_(l.split(",")).map(function(a){return parseFloat(a)});f[n].uniforms.col.value=new Float32Array(l)}if(g.pinIsChanged()||
b<a)for(n=0;n<a;n++)g.isConnected()?f[n].renderState=g.getValue(n):f[n].renderState=VVVV.DefaultRenderState;if(k||b<a)for(n=0;n<a;n++)k=this.inputPins.Transform.getValue(n),f[n].uniforms[f[n].shader.uniformSemanticMap.WORLD].value=k;if(r||b<a)for(n=0;n<a;n++)f[n].uniforms.Samp0.value=this.inputPins.Texture.getValue(n);if(m||b<a)for(n=0;n<a;n++)k=this.inputPins["Texture Transform"].getValue(n),f[n].uniforms.tTex.value=k;this.outputPins.Layer.setSliceCount(a);for(n=0;n<a;n++)this.outputPins.Layer.setValue(n,
f[n]);this.contextChanged=!1}}}};VVVV.Nodes.Quad.prototype=new VVVV.Core.Node;
VVVV.Nodes.Group=function(a,b){this.constructor(a,"Group (EX9)",b);this.auto_nil=!1;this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=[],f=this.addInputPin("Enabled",[1],VVVV.PinTypes.Value),e=this.addInvisiblePin("Layer Template Count",[2],VVVV.PinTypes.Value),d=this.addOutputPin("Layer",[],VVVV.PinTypes.WebGlResource);this.initialize=function(){for(var a=e.getValue(0),d=g.length;d<a;d++)g[d]=this.addInputPin("Layer "+(d+1),[],VVVV.PinTypes.WebGlResource);
for(d=a;d<g.length;d++)this.removeInputPin(g[d].pinname);g.length=a;b.afterUpdate()};this.evaluate=function(){e.pinIsChanged()&&this.initialize();var a=0;if(0.5<f.getValue(0))for(var b=0;b<g.length;b++)for(var q=0;q<g[b].getSliceCount();q++)d.setValue(a++,g[b].getValue(q));d.setSliceCount(a)}};VVVV.Nodes.Group.prototype=new VVVV.Core.Node;
VVVV.Nodes.RendererWebGL=function(a,b){function g(){$(u).detach("mousemove");$(u).detach("mousedown");$(u).detach("mouseup");VVVV.MousePositions[u.id]={x:0,y:0,wheel:0,lb:0,mb:0,rb:0};$(u).mousemove(function(a){var b=2*(a.pageX-$(this).offset().left)/$(this).width()-1;a=-(2*(a.pageY-$(this).offset().top)/$(this).height()-1);VVVV.MousePositions._all.x=b;VVVV.MousePositions._all.y=a;VVVV.MousePositions[u.id].x=b;VVVV.MousePositions[u.id].y=a});$(u).bind("mousewheel",function(a){a=a.originalEvent.wheelDelta/
...
["NONE","DX16"];var k=this.addOutputPin("Actual Backbuffer Width",[0],VVVV.PinTypes.Value),r=this.addOutputPin("Actual Backbuffer Height",[0],VVVV.PinTypes.Value),m=this.addOutputPin("EX9 Out",[],VVVV.PinTypes.WebGlResource),l=0,t=0,s,v,u,w=this.ctxt=void 0,p,A,y;this.getContexts=function(){if(this.invisiblePins["Descriptive Name"]){var a=this.invisiblePins["Descriptive Name"].getValue(0);if((a=$(a).get(0))&&"CANVAS"==a.nodeName)u=$(a);else{var b=parseInt(d.getValue(0)),k=parseInt(c.getValue(0));
u=$('<canvas width="'+(0<b?b:512)+'" height="'+(0<k?k:512)+'" id="vvvv-js-generated-renderer-'+(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');a||(a="body");$(a).append(u)}if(u){g();try{w=u.get(0).getContext("experimental-webgl",{preserveDrawingBuffer:!0}),w.viewportWidth=parseInt(u.get(0).width),w.viewportHeight=parseInt(u.get(0).height)}catch(n){console.log(n)}this.ctxt=w;m.isConnected()&&this.renderContexts&&this.renderContexts[0]?(p=this.ctxt=this.renderContexts[0],A=p.createFramebuffer(),
p.bindFramebuffer(p.FRAMEBUFFER,A),A.width=u.get(0).width,A.height=u.get(0).height,y=p.createTexture(),p.bindTexture(p.TEXTURE_2D,y),p.pixelStorei(p.UNPACK_FLIP_Y_WEBGL,!0),p.texParameteri(p.TEXTURE_2D,p.TEXTURE_MAG_FILTER,p.LINEAR),p.texParameteri(p.TEXTURE_2D,p.TEXTURE_MIN_FILTER,p.LINEAR_MIPMAP_NEAREST),p.texImage2D(p.TEXTURE_2D,0,p.RGBA,A.width,A.height,0,p.RGBA,p.UNSIGNED_BYTE,null),p.generateMipmap(p.TEXTURE_2D),a=p.createRenderbuffer(),p.bindRenderbuffer(p.RENDERBUFFER,a),p.renderbufferStorage(p.RENDERBUFFER,
p.DEPTH_COMPONENT16,A.width,A.height),p.framebufferTexture2D(p.FRAMEBUFFER,p.COLOR_ATTACHMENT0,p.TEXTURE_2D,y,0),p.framebufferRenderbuffer(p.FRAMEBUFFER,p.DEPTH_ATTACHMENT,p.RENDERBUFFER,a),p.bindTexture(p.TEXTURE_2D,null),p.bindRenderbuffer(p.RENDERBUFFER,null),p.bindFramebuffer(p.FRAMEBUFFER,null)):this.renderContexts&&this.renderContexts[0]&&(this.renderContexts[0].deleteTexture(y),y=void 0);this.ctxt&&(l=parseInt(u.get(0).width),t=parseInt(u.get(0).height),p=this.ctxt,a=new Uint8Array([255,255,
255]),p.DefaultTexture=p.createTexture(),p.bindTexture(p.TEXTURE_2D,p.DefaultTexture),p.texParameteri(p.TEXTURE_2D,p.TEXTURE_WRAP_S,p.CLAMP_TO_EDGE),p.texParameteri(p.TEXTURE_2D,p.TEXTURE_WRAP_T,p.CLAMP_TO_EDGE),p.texParameteri(p.TEXTURE_2D,p.TEXTURE_MIN_FILTER,p.NEAREST),p.texParameteri(p.TEXTURE_2D,p.TEXTURE_MAG_FILTER,p.NEAREST),p.pixelStorei(p.UNPACK_ALIGNMENT,1),p.texImage2D(p.TEXTURE_2D,0,p.RGB,1,1,0,p.RGB,p.UNSIGNED_BYTE,a),p.bindTexture(p.TEXTURE_2D,null),this.inputPins.Layers.markPinAsChanged(),
f.markPinAsChanged(),e.markPinAsChanged(),h.markPinAsChanged(),q.markPinAsChanged())}}};this.destroy=function(){$(u).remove()};var z=!1;this.evaluate=function(){p=this.ctxt;if(this.invisiblePins["Descriptive Name"].pinIsChanged()||this.contextChanged)w&&$(w.canvas).hasClass("vvvv-js-generated-renderer")&&$(w.canvas).remove(),this.getContexts(),this.inputPins.Layers.isConnected()&&this.inputPins.Layers.links[0].fromPin.connectionChanged();z||(k.setValue(0,l),r.setValue(0,t),z=!0);if(void 0!=p){if(d.pinIsChanged()&&
(!this.renderContexts||!this.renderContexts[0])){var a=parseInt(d.getValue(0));0<a&&(l=a,$(w.canvas).attr("width",l),k.setValue(0,l))}!c.pinIsChanged()||this.renderContexts&&this.renderContexts[0]||(a=parseInt(c.getValue(0)),0<a&&(t=a,$(w.canvas).attr("height",t),r.setValue(0,t)));this.renderContexts&&this.renderContexts[0]&&p==this.renderContexts[0]?p.bindFramebuffer(p.FRAMEBUFFER,A):p.bindFramebuffer(p.FRAMEBUFFER,null);a=_(e.getValue(0).split(",")).map(function(a){return parseFloat(a)});p.clearColor(a[0],
a[1],a[2],a[3]);p.clear(p.COLOR_BUFFER_BIT|p.DEPTH_BUFFER_BIT);"NONE"==n.getValue(0)?p.disable(p.DEPTH_TEST):p.enable(p.DEPTH_TEST);q.pinIsChanged()&&(q.isConnected()?(s=mat4.create(),mat4.set(q.getValue(0),s)):(s=mat4.create(),mat4.ortho(-1,1,-1,1,-100,100,s)),mat4.scale(s,[1,1,-1]),this.renderContexts&&this.renderContexts[0]&&mat4.scale(s,[1,-1,1]));h.pinIsChanged()&&(v=h.getValue(0));if(this.contextChanged)this.contextChanged=!1;else{p.viewport(0,0,l,t);var b=a=null,f=null;if(this.inputPins.Layers.isConnected())for(var g=
this.inputPins.Layers.values,u=0;u<g.length;u++)if(layer=g[u],void 0!=layer.shader){a!=layer.shader.shaderProgram&&(p.useProgram(layer.shader.shaderProgram),p.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.PROJECTION].position,!1,s),p.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.VIEW].position,!1,v));var D=layer.renderState;D||(D=defaultWebGlRenderState);D!=b&&D.apply(p);if(layer.mesh!=f||layer.shader.shaderProgram!=a)p.bindBuffer(p.ARRAY_BUFFER,
layer.mesh.vertexBuffer.vbo),_(layer.mesh.vertexBuffer.subBuffers).each(function(a){layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]]&&-1!=layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position&&(p.enableVertexAttribArray(layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position),p.vertexAttribPointer(layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position,a.size,p.FLOAT,!1,0,a.offset))}),p.bindBuffer(p.ELEMENT_ARRAY_BUFFER,
layer.mesh.indexBuffer);for(var b=layer.uniformNames.length,F=f=0;F<b;F++){var B=layer.uniforms[layer.uniformNames[F]];if(void 0!=B.value&&!(0<u&&layer.shader.shaderProgram==a&&g[u-1].uniforms[layer.uniformNames[F]]&&B.value==g[u-1].uniforms[layer.uniformNames[F]].value)){start=(new Date).getTime();switch(B.uniformSpec.type){case "mat":p["uniformMatrix"+B.uniformSpec.dimension+"fv"](B.uniformSpec.position,!1,B.value);break;case "vec":p["uniform"+B.uniformSpec.dimension+"fv"](B.uniformSpec.position,
B.value);break;case "int":p["uniform"+B.uniformSpec.dimension+"i"](B.uniformSpec.position,B.value);break;case "float":p["uniform"+B.uniformSpec.dimension+"f"](B.uniformSpec.position,B.value);break;case "sampler":var G=B.value;G==VVVV.DefaultTexture&&(G=p.DefaultTexture);p.activeTexture(p["TEXTURE"+f]);p.bindTexture(p["TEXTURE_"+B.uniformSpec.dimension],G);p.uniform1i(B.uniformSpec.position,f);f++}loopstart=(new Date).getTime()}}p.drawElements(p[D.polygonDrawMode],layer.mesh.numIndices,p.UNSIGNED_SHORT,
0);a=layer.shader.shaderProgram;b=D;f=layer.mesh}this.renderContexts&&this.renderContexts[0]&&(p.bindTexture(p.TEXTURE_2D,y),p.generateMipmap(p.TEXTURE_2D),p.bindTexture(this.renderContexts[0].TEXTURE_2D,null));m.setValue(0,y)}}}};VVVV.Nodes.RendererWebGL.prototype=new VVVV.Core.Node;
...
b);a.preventDefault();return!1});var n=0,q=0;$("body",y.window.document).mousemove(function(a){n=a.pageX;q=a.pageY});$("body",y.window.document).on("paste",function(a){a=a.originalEvent.clipboardData.getData("text/plain");if(!a.match(/^<!DOCTYPE PATCH/))return!1;$patch=$(a);var d=void 0,e=void 0;$patch.find("node bounds").each(function(){var a=parseInt($(this).attr("left")),b=parseInt($(this).attr("top"));if(!d||a<d)d=a;if(!e||b<e)e=b});var g={};$patch.find("node").each(function(){t++;g[$(this).attr("id")]=
t;$(this).attr("createme","pronto");$(this).attr("id",t);var a=parseInt($(this).find("bounds").attr("left"))-d+15*n,b=parseInt($(this).find("bounds").attr("top"))-e+15*q;$(this).find("bounds").attr("left",a);$(this).find("bounds").attr("top",b)});$patch.find("link").each(function(){$(this).attr("createme","pronto");var a=$(this).attr("srcnodeid"),b=$(this).attr("dstnodeid");$(this).attr("srcnodeid",g[a]);$(this).attr("dstnodeid",g[b])});a=$patch.wrapAll("<d></d>").parent().html();b.update(l,a);f();
var h=_(g).map(function(a){return a});c.selectAll(".vvvv-node").each(function(a){0<=h.indexOf(parseInt(a.id))&&(m.push(a),d3.select(this).attr("class","vvvv-node selected"))})})}c=root.append("svg:svg").attr("class","chart").attr("width",Math.max(l.windowWidth,l.boundingBox.width)).attr("height",Math.max(l.windowHeight,l.boundingBox.height));k=c.append("svg:rect").attr("class","background").attr("width",Math.max(l.windowWidth,l.boundingBox.width)).attr("height",Math.max(l.windowHeight,l.boundingBox.height));
...
m[g];c.selectAll(".vvvv-link line").filter(function(a){return a.fromPin.node.id==h.id}).attr("x1",function(b){return b.fromPin.x+b.fromPin.node.x+a}).attr("y1",function(a){return a.fromPin.y+a.fromPin.node.y+b});c.selectAll(".vvvv-link line").filter(function(a){return a.toPin.node.id==h.id}).attr("x2",function(b){return b.toPin.x+b.toPin.node.x+a}).attr("y2",function(a){return a.toPin.y+a.toPin.node.y+b})}}else y.state==UIState.AreaSelecting&&(p=d3.event.pageX,A=d3.event.pageY,c.select(".selection-area").attr("transform",
"translate("+Math.min(u,p)+", "+Math.min(w,A)+")").attr("width",Math.abs(p-u)).attr("height",Math.abs(A-w)),f(),c.selectAll(".vvvv-node").each(function(a){for(var b=[a.x,a.x+a.getWidth()],c=[a.y,a.y+a.getHeight()],d=!1,e=0;2>e;e++)for(var f=0;2>f;f++)b[e]>=Math.min(u,p)&&(b[e]<=Math.max(u,p)&&c[f]>=Math.min(w,A)&&c[f]<=Math.max(w,A))&&(d=!0);d&&(d3.select(this).attr("class","vvvv-node selected"),m.push(a))}))}).on("contextmenu",function(){y.state==UIState.Connecting&&(c.select(".vvvv-link.current-link").remove(),
y.state=UIState.Idle);d3.event.stopPropagation();d3.event.preventDefault();return!1}).on("mouseup",function(){if(y.state==UIState.Moving){y.state=UIState.Idle;for(var a=d3.event.pageX-e,f=d3.event.pageY-d,g="<PATCH>",h=0;h<m.length;h++)var k=m[h],g=g+("<NODE componentmode='Node' id='"+k.id+"'><BOUNDS type='Node' left='"+15*(a+k.x)+"' top='"+15*(f+k.y)+"' width='"+k.width+"' height='"+k.height+"'/></NODE>");b.update(l,g+"</PATCH>")}y.state==UIState.AreaSelecting&&(c.select(".selection-area").remove(),
y.state=UIState.Idle)}).on("dblclick",function(){function a(b){g.empty();var c=f.find("#node_filter").val().toLowerCase();b=VVVV.NodeNames.concat(_(VVVV.ShaderCodeResources).map(function(a,b){return b.replace("%VVVV%/effects/","")}));b=_(_(b).filter(function(a){return 0<=VVVV.Helpers.translateOperators(a).toLowerCase().indexOf(c)})).sortBy(function(a){return a.toLowerCase().indexOf(c)});for(var d=0;d<b.length;d++)g.append($("<option>"+b[d]+"</option>"));g.find("option").first().attr("selected",!0)}
function c(){var a=f.find("#node_filter").val(),g=f.find("#new_node option:selected").val(),h="";g||!a.match(/\.fx$/)&&!a.match(/\.v4p$/)||(g=a);if(g){var k;(k=g.match(/^(.*\/([^\.]+))(\.vvvvjs)?\.fx$/))?(g=k[2]+" (EX9.Effect)",h=k[1]+".fx"):(k=g.match("([^.]+)(.vvvvjs)?.fx$"))?(g=k[1]+" (EX9.Effect)",h="%VVVV%/effects/"+k[1]+".fx"):g.match("[^/].v4p$")&&(h=g=a);t++;g="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='"+g+"' systemname='"+g+"' "+(""!=h?"filename='"+h+"'":"")+">");g+="<BOUNDS type='Node' left='"+
15*d+"' top='"+15*e+"' width='100' height='100'/>"}else t++,g="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='IOBox (String)' systemname='IOBox (String)'>"),g+="<BOUNDS type='Node' left='"+15*d+"' top='"+15*e+"' width='100' height='100'/>",g+="<PIN pinname='Input String' values='|"+a+"|'/>";b.update(l,g+"</NODE>");f.remove()}$("#node_selection",y.window.document).remove();var d=d3.event.pageX,e=d3.event.pageY,f=$('<div id="node_selection"><input type="text" id="node_filter"/></div>'),
...
function(){y.state==UIState.Idle&&(y.state=UIState.AreaSelecting,u=p=d3.event.pageX+1,w=A=d3.event.pageY+1,c.append("svg:rect").attr("class","selection-area").attr("stroke","#000").attr("stroke-dasharray","2,2").attr("stroke-width",1).attr("fill","rgba(0,0,0,0)").attr("transform","translate("+u+", "+w+")").attr("width",0).attr("height",0),f())}),k.on("click",function(){y.state=UIState.Idle;$(".resettable",y.window.document).remove();$("#node_selection",y.window.document).remove();f();r=void 0}),k=
function(a){v=a.shiftKey},$(y.window.document).keydown(k),$(y.window.document).keyup(k),$(y.window.document).keydown(function(a){if(46==a.which&&0<m.length){var d="<PATCH>";for(a=0;a<m.length;a++){var e=m[a],d=d+("<NODE id='"+m[a].id+"' deleteme='pronto'/>");_(e.inputPins).each(function(a){_(a.links).each(function(a){d+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(e.outputPins).each(function(a){_(a.links).each(function(a){d+=
"<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})})}d+="</PATCH>";b.update(l,d);m=[]}else if(115!=a.which&&83!=a.which||!a.ctrlKey){if(73==a.which&&a.ctrlKey)return b.inspector?b.inspector.win.focus():b.openInspector("."),a.preventDefault(),!1;if(65==a.which&&a.ctrlKey)return f(),c.selectAll(".vvvv-node").each(function(a){m.push(a);d3.select(this).attr("class","vvvv-node selected")}),
a.preventDefault(),!1}else return b.save(l.nodename,l.toXML()),a.preventDefault(),!1}),$(y.window).resize(function(){l.windowWidth=$(this).width();l.windowHeight=$(this).height();l.boundingBox.width=Math.max(l.windowWidth,l.boundingBox.width);l.boundingBox.height=Math.max(l.windowHeight,l.boundingBox.height);l.afterUpdate()}));y.drawComplete();l.afterUpdate=function(){y.drawComplete();b.inspector&&window.setTimeout(b.inspector.update,100)};l.afterEvaluate=function(){h.filter(function(a){return a.isIOBox&&
VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel}).each(function(a){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a)})}},1E3);this.close=function(){this.window.close()};this.drawComplete=function(){h&&h.remove();k&&k.remove();g||l.windowWidth==$(y.window).width()&&l.windowHeight==$(y.window).height()||y.window.resizeTo(l.windowWidth,l.windowHeight);c.attr("width",Math.max(l.windowWidth,l.boundingBox.width)).attr("height",Math.max(l.windowHeight,l.boundingBox.height));c.selectAll(".background").attr("width",
Math.max(l.windowWidth,l.boundingBox.width)).attr("height",Math.max(l.windowHeight,l.boundingBox.height));var a=c.append("svg:g").attr("class","link-group");h=c.selectAll("g.vvvv-node").data(l.nodeList).enter().append("svg:g").attr("class",function(a){var b="vvvv-node";a.isIOBox&&(b+=" vvvv-iobox");0<=m.indexOf(a)&&(b+=" selected");return b}).attr("id",function(a){return"vvvv-node-"+a.id}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"});h.append("svg:rect").attr("class","vvvv-node-background").attr("height",
...
function(a,d){if(y.state!=UIState.Connecting){r=a;y.state=UIState.Connecting;c.append("svg:line").attr("class","vvvv-link current-link resettable").attr("stroke","#000").attr("stroke-width",1).attr("x1",a.x+a.node.x+2+0.5).attr("y1",a.y+a.node.y+2+0.5).attr("x2",a.x+a.node.x+2+0.5).attr("y2",a.y+a.node.y+2+0.5);var e=r.direction==PinDirection.Output?"input":"output",f=r.node.getUpstreamNodes();c.selectAll("g.vvvv-"+e+"-pin").filter(function(a){return a.typeName!=r.typeName&&("Node"!=r.typeName||VVVV.PinTypes[a.typeName].primitive&&
("Node"!=a.typeName||VVVV.PinTypes[r.typeName].primitive))||0<=f.indexOf(a.node)||a.node==r.node?!1:!0}).append("svg:rect").attr("class","vvvv-connection-highlight").attr("width",4).attr("height",3).attr("fill","rgba(0,0,0,1)").attr("y",function(a){return a.direction==PinDirection.Input?-3:3})}else{if(r.direction==PinDirection.Input)var e=a,g=r;else e=r,g=a;if(e.typeName==g.typeName||"Node"==e.typeName&&!VVVV.PinTypes[g.typeName].primitive||"Node"==g.typeName&&!VVVV.PinTypes[e.typeName].primitive){var h=
"<PATCH>";_(g.links).each(function(a){h+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"});h+="<LINK createme='pronto' srcnodeid='"+e.node.id+"' srcpinname='"+e.pinname+"' dstnodeid='"+g.node.id+"' dstpinname='"+g.pinname+"'/>";h+="</PATCH>";b.update(l,h);c.select(".vvvv-link.current-link").remove();c.select(".vvvv-connection-highlight").remove();y.state=UIState.Idle}l.afterUpdate()}d3.event.stopPropagation()}).on("mousedown",
function(){d3.event.stopPropagation()}),c.selectAll("g.vvvv-link").on("contextmenu",function(a){b.update(l,"<PATCH><LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/></PATCH>");d3.event.preventDefault()}),h.on("mousedown",function(a){$(".resettable",y.window.document).remove();y.state=UIState.Moving;0>m.indexOf(a)&&(v||f(),d3.select(this).attr("class","vvvv-node selected"),m.push(a));e=d3.event.pageX;
...
VVVV.PinTypes[d.typeName].defaultValue());d.setSliceCount(b);d.node.parentPatch.afterUpdate();a.preventDefault();return!1});for(f=f=0;f<e;f++)if(g=$(c.win.document).find("#values .row").eq(f+1),b||!g.children().first().hasClass("pininputbox")){if(VVVV.PinTypes[a.typeName].openInputBox&&a.direction!=PinDirection.Output&&!a.isConnected()&&void 0!=a.getValue(0)){var m=$('<div class="row value"><div style="height:100%">'+a.getValue(f)+"</div></div>");(function(b){m.find("div").click(function(){VVVV.PinTypes[a.typeName].openInputBox(c.win,
$(this),a,b)})})(f)}else m=$('<div class="row value readonlyvalue"><div style="height:100%">'+a.getValue(f)+"</div></div>");0<g.length?g.replaceWith(m):$(c.win.document).find("#values").append(m);"Color"==a.typeName&&m.find("div").css("background-color","rgba("+_(a.getValue(f).split(",")).map(function(a){return parseInt(255*a)}).join(",")+")")}f++;for(g=$(c.win.document).find("#values .row");f<g.length;f++)g.eq(f).remove()}this.win=window.open(a+"/inspektor.html","inspektor","location=no, width=250, height=600, toolbar=no");
...
c=0;c<b.length;c++)this.removePatch(b[c]);b=VVVV.Helpers.prepareFilePath(a.nodename,a.parentPatch);g[b].splice(g[b].indexOf(a),1);0==g[b].length&&delete g[b];console.log(g)};this.openPatch=function(a,d){b.push(new VVVV.Editors.BrowserEditor.PatchWindow(a,this,d))};this.update=function(a,b){for(var c=VVVV.Helpers.prepareFilePath(a.nodename,a.parentPatch),f=g[c].length,q=0;q<f;q++)g[c][q].doLoad(b),g[c][q].afterUpdate()};this.disable=function(){for(var e=0;e<b.length;e++)b[e].close();this.inspector&&
