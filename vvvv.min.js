...
VVVV={Nodes:{},PinTypes:{},Types:{},NodeLibrary:{},NodeNames:[],Patches:{},Editors:{},onNotImplemented:function(b){console.log("Warning: "+b+" is not implemented.")},loadCounter:0,loadScript:function(b,a){var g=document.getElementsByTagName("head")[0],e=document.createElement("script");e.async=!1;e.src=VVVV.Root+"/"+b;a&&e.addEventListener("load",a);VVVV.loadCounter++;g.appendChild(e)},init:function(b,a,g){function e(){var d=new VVVV.Core.Patch("");_(VVVV.Nodes).each(function(c){var h=new c(0,d);
...
a,g,e){for(var f=a.length;f<g;f++)a[f]=e.call(b,f);for(f=g;f<a.length;f++)b.removeInputPin(a[f].pinname);a.length=g;b.parentPatch.afterUpdate()}};
VVVV.Core={Pin:function(b,a,g,e,f){this.direction=a;this.pinname=b;this.links=[];this.values=[];this.node=e;this.changed=!0;this.reset_on_disconnect=this.active=!1;this.masterPin=this.slavePin=void 0;this.connectionChangedHandlers={};this.enumOptions=[];this.getValue=function(d,c){if(!c||1==c)return this.values[d%this.values.length];for(var h=[],f=0;f<c;f++)h.push(this.values[(d*c+f)%this.values.length]);return h};this.setValue=function(d,c,h){h=h||!1;this.values[d]=c;this.changed=!0;this.node.dirty=
...
"invoking update links");f(c);n.editor&&n.editor.addPatch(this);if(this.auto_evaluate){var d=n;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(n.mainloop);n.afterUpdate();0>=n.resourcesPending&&h&&(h(),h=void 0)},function(){n.resourcesPending--;this.not_implemented=!0;VVVV.onNotImplemented(a);f(c);n.afterUpdate();0>=n.resourcesPending&&h&&(h(),h=void 0)},n,$(k).attr("id"));e.isSubpatch=!0;n.editor&&!e.editor&&n.editor.addPatch(e);n.nodeMap[e.id]=e}else e=new VVVV.Core.Node($(this).attr("id"),
a,n),e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&n.editor.sendUndo(),VVVV.onNotImplemented(a);"development"==VVVV_ENV&&"complete"!=g&&console.log(n.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){k=n;do k.auto_evaluate=!0;while(k=k.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);k=e.getSubPatches();k.push(e);for(var p,q=0;q<k.length;q++)p=VVVV.Helpers.prepareFilePath(k[q].nodename,
k[q].parentPatch),VVVV.Patches[p].splice(VVVV.Patches[p].indexOf(e),1),0==VVVV.Patches[p].length&&delete VVVV.Patches[p]}n.nodeList.splice(n.nodeList.indexOf(e),1);e.destroy();delete n.nodeMap[e.id]}0<m.length&&(m.attr("left")&&(e.x=m.attr("left")/15,e.y=m.attr("top")/15,n.boundingBox.width=Math.max(n.boundingBox.width,e.x+100),n.boundingBox.height=Math.max(n.boundingBox.height,e.y+100)),m.attr("width")&&(e.width=m.attr("width"),e.height=m.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&
(e.isIOBox=!0);b||e.setup();k=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var m=0;m<f.length;m++)e.invisiblePins[h].values[m]!=f[m]&&e.invisiblePins[h].setValue(m,f[m]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==$(this).attr("visible")&&
"Configuration"!=$(this).attr("pintype")||e.isSubpatch?"Output"==$(this).attr("pintype")||0<$(c).find("link[srcnodeid="+e.id+"]").filter("link[srcpinname='"+h.replace(/[\[\]]/,"")+"']").length?void 0==e.outputPins[h]&&e.addOutputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInvisiblePin(h,f)});b||(e.initialize(),n.nodeList.push(e));"complete"==g&&(r[e.id]=e)});"complete"==g&&(_(k).each(function(c,d){void 0==
...
c.wrapAll("<d></d>").parent().html();c=c.replace(/<patch/g,"<PATCH");c=c.replace(/<\/patch>/g,"\n  </PATCH>");c=c.replace(/<node/g,"\n  <NODE");c=c.replace(/<\/node>/g,"\n  </NODE>");c=c.replace(/<bounds/g,"\n  <BOUNDS");c=c.replace(/<\/bounds>/g,"\n  </BOUNDS>");c=c.replace(/<pin/g,"\n  <PIN");c=c.replace(/<\/pin>/g,"\n  </PIN>");c=c.replace(/<lonk/g,"\n  <LINK");return c=c.replace(/<\/lonk>/g,"\n  </LINK>")};this.evaluate=function(){function c(f){var g=f.getUpstreamNodes();_(g).each(function(d){void 0==
m[d.id]||d.delays_output||c(d)});if(f.dirty||f.auto_evaluate||f.isSubpatch){if(h)var b=(new Date).getTime();if(f.auto_nil&&!f.isSubpatch&&f.hasNilInputs())_(f.outputPins).each(function(c){c.setSliceCount(0)});else{try{f.evaluate()}catch(a){console.log("VVVV.Js / Error evaluating "+f.nodename+": "+a.message)}f.dirty=!1;_(f.inputPins).each(function(c){c.changed=!1})}h&&(d[f.nodename]||(d[f.nodename]={count:0,dt:0}),e=(new Date).getTime()-b,d[f.nodename].count++,d[f.nodename].dt+=e,console.log(f.nodename+
" / "+f.id+": "+e+"ms"))}delete m[f.id];return!0}if(!(0<this.resourcesPending)){if(h)var d={},f=(new Date).getTime(),e=0;for(var m={},g={},b=0;b<this.nodeList.length;b++){if(0==this.nodeList[b].getDownstreamNodes().length||this.nodeList[b].auto_evaluate||this.nodeList[b].delays_output)g[this.nodeList[b].id]=this.nodeList[b];m[this.nodeList[b].id]=this.nodeList[b]}h&&console.log("building node maps: "+((new Date).getTime()-f)+"ms");_(g).each(function(d,h,f){void 0!=m[d.id]&&c(d)});h&&(_(d).each(function(c,
d){console.log(c.count+"x "+d+": "+c.dt+"ms")}),f=(new Date).getTime());this.afterEvaluate();h&&console.log("patch rendering: "+((new Date).getTime()-f)+"ms");h=!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(h=!0)});if(/\.v4p[^<>\s]*$/.test(b)){this.nodename=b;var q=this,s=b;this.parentPatch&&(s=VVVV.Helpers.prepareFilePath(b,this.parentPatch));VVVV.Patches[s]?q.doLoad(VVVV.Patches[s][0].toXML(),function(){VVVV.Patches[s].push(q);q.success&&q.success();q.afterUpdate()}):$.ajax({url:s,
type:"get",dataType:"text",success:function(c){q.doLoad(c,function(){VVVV.Patches[s]=VVVV.Patches[s]||[];VVVV.Patches[s].push(q);q.success&&q.success();q.afterUpdate()})},error:function(){q.error&&q.error()}})}else this.doLoad(b,function(){this.success&&this.success()});c();$(window).bind("hashchange",function(){c()})}};VVVV.Core.Patch.prototype=new VVVV.Core.Node;VVVV.Core.DOMInterface=function(b){function a(d){if("event"==d.property_class){var c=d.selector;"window"==c&&(c=window);"document"==c&&(c=document);$(c).each(function(c){d.values[c]=0;$(this).bind(d.property,function(){d.values[c]=1;return!1})})}}var g={},e={};b.domInterface=this;this.connect=function(d){_(g).each(function(c,h){c.node.id==d.id&&("event"==c.property_class&&$(c.selector).unbind(c.property),delete g[h])});_(e).each(function(c,h){c.node.id==d.id&&delete e[h]});var c=/([^\/]+)(\/(event|attribute|style)\/(.+))?/.exec(d.invisiblePins["Descriptive Name"].getValue(0));
if(null!=c){var h={selector:c[1],property_class:c[3],property:c[4],values:[],node:d};0!=$(h.selector).length&&(0==d.getUpstreamNodes().length?g[c[0]]=h:0==d.getDownstreamNodes().length&&(e[c[0]]=h),a(h))}};var f=this;_(b.nodeList).each(function(d){d.isIOBox&&void 0!=d.invisiblePins["Descriptive Name"]&&(f.connect(d),d.IOBoxInputPin().connectionChanged=function(){f.connect(d)})});this.populateInputConnectors=function(){var d=this;_(g).each(function(c){switch(c.property_class){case "event":c.values.length!=
...
c[g]);break;default:b.html($(d.selector).eq(e).html()+c[g])}"attribute"==d.property_class&&b.attr(d.property,c[g]);"style"==d.property_class&&b.css(d.property,c[g])}}}}};VVVV.fps=60;
VVVV.Core.MainLoop=function(b,a){function g(){if(0==b.resourcesPending){e++;var a=(new Date).getTime();c&&(p.deltaT=a-c);c=a;f.populateInputConnectors();b.evaluate();f.processOutputConnectors();a=(new Date).getTime()-c;0==e%10&&(m&&(console.log(1/((c-h)/1E4)+"fps"),m=!1),h=c)}d&&window.setTimeout(g,Math.max(0,Math.round(1E3/VVVV.fps-a)))}VVVV.fps=a||60;var e=0,f=new VVVV.Core.DOMInterface(b),d=!0,c=void 0,h=0,m=!1;this.deltaT=1E3/VVVV.fps;b.setMainloop(this);var p=this;this.stop=function(){d=!1};
this.start=function(){d||(d=!0,g())};this.isRunning=function(){return d};g();$(window).keydown(function(c){(102==c.which||70==c.which)&&(c.altKey&&c.ctrlKey)&&(m=!0)})};VVVV.Nodes.Cross2d=function(b,a){this.constructor(b,"Cross (2d)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("X In",[0],VVVV.PinTypes.Value),e=this.addInputPin("Y In",[0],VVVV.PinTypes.Value),f=this.addInputPin("Include Upper",[1],VVVV.PinTypes.Value),d=this.addInputPin("Include Lower",[1],VVVV.PinTypes.Value),c=this.addInputPin("Include Equal",[1],VVVV.PinTypes.Value),h=this.addOutputPin("X Out",
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Y Out",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=0,a=0;a<e.getSliceCount();a++)for(var r=0;r<g.getSliceCount();r++)0.5>f.getValue(0)&&a>r||(0.5>d.getValue(0)&&a<r||0.5>c.getValue(0)&&a==r)||(h.setValue(b,g.getValue(r)),m.setValue(b,e.getValue(a)),b++);h.setSliceCount(b);m.setSliceCount(b)}};VVVV.Nodes.Cross2d.prototype=new VVVV.Core.Node;
...
c;h++){var m=g.getValue(h),b=e.getValue(h);f.setValue(h,Math.atan2(b,m)/(2*Math.PI));d.setValue(h,Math.sqrt(m*m+b*b))}f.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Polar2d.prototype=new VVVV.Core.Node;
...
b=e.getValue(h);f.setValue(h,b*Math.cos(2*m*Math.PI));d.setValue(h,b*Math.sin(2*m*Math.PI))}f.setSliceCount(c);d.setSliceCount(c)}};VVVV.Nodes.Cartesian2d.prototype=new VVVV.Core.Node;
...
m=this.addOutputPin("Length",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Angle",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var n=parseFloat(g.getValue(a)),q=parseFloat(e.getValue(a)),s=parseFloat(f.getValue(a)),v=parseFloat(d.getValue(a)),s=s-n,v=v-q;c.setValue(a,n+s/2);h.setValue(a,q+v/2);m.setValue(a,Math.sqrt(s*s+v*v));p.setValue(a,Math.atan2(v,s)/(2*Math.PI))}c.setSliceCount(b);h.setSliceCount(b);m.setSliceCount(b);p.setSliceCount(b)}};
...
m=this.addOutputPin("X2",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Y2",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var n=parseFloat(g.getValue(a)),q=parseFloat(e.getValue(a)),s=parseFloat(f.getValue(a)),v=2*parseFloat(d.getValue(a)-0.25)*Math.PI,u=s/2*Math.sin(v),s=-s/2*Math.cos(v);c.setValue(a,n+u);h.setValue(a,q+s);m.setValue(a,n-u);p.setValue(a,q-s)}c.setSliceCount(b);h.setSliceCount(b);m.setSliceCount(b);p.setSliceCount(b)}};
...
this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var k=g.getValue(a),r=e.getValue(a),n=f.getValue(a),q=Math.sqrt(k*k+r*r+n*n);c.setValue(a,Math.atan2(-k,-n)/(2*Math.PI));d.setValue(a,0==q?0:Math.acos(-r/q)/(2*Math.PI)-0.25);h.setValue(a,q)}c.setSliceCount(b);d.setSliceCount(b);h.setSliceCount(b)}};VVVV.Nodes.Polar3d.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var k=2*(g.getValue(a)+0.25)*Math.PI,r=2*e.getValue(a)*Math.PI,n=f.getValue(a);h.setValue(a,-n*Math.cos(r)*Math.sin(k));d.setValue(a,-n*Math.sin(r)*Math.sin(k));c.setValue(a,-n*Math.cos(k))}d.setSliceCount(b);c.setSliceCount(b);h.setSliceCount(b)}};VVVV.Nodes.Cartesian3d.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Input Length",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var r=g.getValue(a),n=e.getValue(a),q=f.getValue(a),s=Math.sqrt(r*r+n*n+q*q);d.setValue(a,0==s?0:r/s);c.setValue(a,0==s?0:n/s);h.setValue(a,0==s?0:q/s);m.setValue(a,s)}d.setSliceCount(b);c.setSliceCount(b);h.setSliceCount(b);m.setSliceCount(b)}};VVVV.Nodes.Normalize3d.prototype=new VVVV.Core.Node;
...
h[0]+h[1]*h[1]+h[2]*h[2]);e.setValue(3*c,0==b?0:h[0]/b);e.setValue(3*c+1,0==b?0:h[1]/b);e.setValue(3*c+2,0==b?0:h[2]/b);f.setValue(c,b)}e.setSliceCount(3*Math.ceil(d/3));f.setSliceCount(Math.ceil(d/3))}};VVVV.Nodes.Normalize3dVector.prototype=new VVVV.Core.Node;
VVVV.Nodes.Multiply3dCross=function(b,a){this.constructor(b,"Multiply (3d Cross)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Input1 XYZ",[],VVVV.PinTypes.Value),e=this.addInputPin("Input2 XYZ",[],VVVV.PinTypes.Value),f=this.addOutputPin("Output XYZ",[],VVVV.PinTypes.Value);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0;c<d/3;c++){var h=g.getValue(c,3),b=e.getValue(c,
3);f.setValue(3*c+0,h[1]*b[2]-h[2]*b[1]);f.setValue(3*c+1,-(h[0]*b[2]-h[2]*b[0]));f.setValue(3*c+2,h[0]*b[1]-h[1]*b[0])}f.setSliceCount(3*Math.ceil(d/3))}};VVVV.Nodes.Multiply3dCross.prototype=new VVVV.Core.Node;
VVVV.Nodes.Multiply3dDot=function(b,a){this.constructor(b,"Multiply (3d Dot)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Input1 XYZ",[],VVVV.PinTypes.Value),e=this.addInputPin("Input2 XYZ",[],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0;c<d/3;c++){var h=g.getValue(c,3),b=e.getValue(c,3);f.setValue(c,
h[0]*b[0]+h[1]*b[1]+h[2]*b[2])}f.setSliceCount(Math.ceil(d/3))}};VVVV.Nodes.Multiply3dDot.prototype=new VVVV.Core.Node;
...
d;c++){var h=e.getValue(3*c,3),b=g.getValue(c);mat4.multiplyVec3(b,h);f.setValue(3*c+0,h[0]);f.setValue(3*c+1,h[1]);f.setValue(3*c+2,h[2])}f.setSliceCount(Math.ceil(3*d))}};VVVV.Nodes.Multiply3dVector.prototype=new VVVV.Core.Node;VVVV.Nodes.LFO=function(b,a){this.constructor(b,"LFO (Animation)",a);this.meta={authors:["Matthias Zauner, sebl"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Not spreadable yet"]};this.auto_evaluate=!0;var g=this.addInputPin("Period",[1],VVVV.PinTypes.Value),e=this.addInputPin("Pause",[0],VVVV.PinTypes.Value),f=this.addInputPin("Reverse",[0],VVVV.PinTypes.Value),d=this.addInputPin("Reset",[0],VVVV.PinTypes.Value),c=this.addInputPin("Phase",[0],VVVV.PinTypes.Value),h=this.addOutputPin("Output",
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Cycles",[0],VVVV.PinTypes.Value),p=[],k=[],r=(new Date).getTime(),n=(new Date).getTime();this.evaluate=function(){var b=this.getMaxInputSliceCount();r=(new Date).getTime()-n;for(var a=0;a<b;a++){var v=g.getValue(a%g.values.length),u=e.getValue(a%e.values.length),t=f.getValue(a%f.values.length),w=d.getValue(a%d.values.length),l=c.getValue(a%c.values.length);void 0==p[a]&&(p[a]=0);void 0==k[a]&&(k[a]=0);0.5>u&&(0!=v&&isFinite(v))&&(dv=1/(1E3*v)*r,0<t&&(dv*=
-1),p[a]+=dv,0>p[a]&&(k[a]-=Math.ceil(-p[a]),p[a]=1+p[a]),1<p[a]&&(k[a]+=Math.floor(p[a])));n=(new Date).getTime();0.5<=w&&(p[a]=0,k[a]=0);if(0.5>u||0.5<=w)h.setValue(a,(p[a]+l)%1),m.setValue(a,k[a]);p[a]%=1}h.setSliceCount(b);m.setSliceCount(b);p.splice(b);k.splice(b)}};VVVV.Nodes.LFO.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],m=[],p=[],k=[],r=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=g.pinIsChanged()||e.pinIsChanged(),s=0;s<b;s++){void 0==c[s]&&(c[s]=(new Date).getTime());var v=this.parentPatch.mainloop.deltaT,u=parseFloat(g.getValue(s)),t=parseFloat(e.getValue(s));void 0==p[s]&&(p[s]=u);if(u!=h[s]||t!=m[s])r[s]=void 0,h[s]=u,m[s]=t,k[s]=0<m[s]?(h[s]-p[s])/(1E3*m[s]):0;Math.abs(k[s]*v)>Math.abs(h[s]-p[s])?p[s]=h[s]:p[s]+=
k[s]*v;0!=r[s]&&(f.setValue(s,p[s]),d.setValue(s,k[s]));r[s]=h[s]-p[s];c[s]=(new Date).getTime()}a&&f.getSliceCount()!=b&&(h.splice(b),m.splice(b),c.splice(b),p.splice(b),k.splice(b),r.splice(b),f.setSliceCount(b))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],m=[],p=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=g.pinIsChanged()||e.pinIsChanged(),n=0;n<b;n++){void 0==c[n]&&(c[n]=(new Date).getTime());var q=this.parentPatch.mainloop.deltaT,s=parseFloat(g.getValue(n)),v=parseFloat(e.getValue(n));void 0==h[n]&&(h[n]=s);m[n]=0<v?(s-h[n])/(1E3*v):0;Math.abs(m[n]*q)>Math.abs(s-h[n])?h[n]=s:h[n]+=m[n]*q;0!=p[n]&&(f.setValue(n,h[n]),d.setValue(n,m[n]));p[n]=s-h[n];
c[n]=(new Date).getTime()}a&&f.getSliceCount()!=b&&(c.splice(b),h.splice(b),m.splice(b),p.splice(b),f.setSliceCount(b))}};VVVV.Nodes.Damper.prototype=new VVVV.Core.Node;
...
function(){var b=this.getMaxInputSliceCount();now=(new Date).getTime();var a=g.pinIsChanged();if(void 0==g.getValue(0))a&&d.setValue(0,void 0);else{if(1==f.getValue(0)&&(a||e.pinIsChanged()))for(h.pop(),h.unshift(now),a=0;a<b;a++){if(void 0==c[a]){c[a]=Array(1024);for(var k=1024;k--;)c[a][k]=0}1024<=c[a].length&&c[a].pop();c[a].unshift(g.getValue(a))}for(a=0;a<b;a++){for(var r=now-1E3*e.getValue(a),n=!1,k=0;1024>k;k++)if(h[k]<=r){d.values[a]!=c[a][k]&&d.setValue(a,c[a][k]);n=!0;break}n||0==d.values[a]||
...
b=f.getSliceCount();if(h>b)for(;b<h;b++)f.setValue(b,0),d.setValue(b,1);for(b=0;b<h;b++){var a=void 0;1<=Math.round(e.getValue(b))&&(a=0);1<=Math.round(g.getValue(b))&&(a=1-parseFloat(f.getValue(b)));void 0!=a&&f.getValue(b)!=a?(f.setValue(b,a),d.setValue(b,1-a)):c||(f.setValue(b,0),d.setValue(b,1))}f.setSliceCount(h);d.setSliceCount(h);c=!0}};VVVV.Nodes.Toggle.prototype=new VVVV.Core.Node;
...
h=this.addOutputPin("DayOfWeek",[0],VVVV.PinTypes.Value),m=this.addOutputPin("Day",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Month",[1],VVVV.PinTypes.Value),k=this.addOutputPin("Year",[2E3],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var q=864E5*(g.getValue(a)-25569),q=new Date(q);x=q.getMilliseconds();x!=e.values[a]&&e.setValue(a,x);x=q.getSeconds();x!=f.values[a]&&f.setValue(a,x);x=q.getMinutes();x!=d.values[a]&&d.setValue(a,x);x=q.getHours();
x!=c.values[a]&&c.setValue(a,x);x=q.getDay();x!=h.values[a]&&h.setValue(a,x);x=q.getDate();x!=m.values[a]&&m.setValue(a,x);x=q.getMonth()+1;x!=p.values[a]&&p.setValue(a,x);x=q.getYear()+1900;x!=k.values[a]&&k.setValue(a,x)}e.setSliceCount(b);f.setSliceCount(b);d.setSliceCount(b);c.setSliceCount(b);h.setSliceCount(b);m.setSliceCount(b);p.setSliceCount(b);k.setSliceCount(b)}};VVVV.Nodes.GregorianAstronomySplit.prototype=new VVVV.Core.Node;VVVV.Nodes.And=function(b,a){this.constructor(b,"AND (Boolean)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Output",[1],VVVV.PinTypes.Value);this.initialize=function(){var d=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,d,function(c){return this.addInputPin("Input "+(c+1),[1],VVVV.PinTypes.Value)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();for(var d=this.getMaxInputSliceCount(),c=e.length,h,b=0;b<d;b++){h=!0;for(var a=0;a<c;a++)h=h&&0.5<=e[a].getValue(b);f.setValue(b,h?1:0)}f.setSliceCount(d)}};VVVV.Nodes.And.prototype=new VVVV.Core.Node;
...
this.initialize();for(var d=this.getMaxInputSliceCount(),c=e.length,h,b=0;b<d;b++){h=!1;for(var a=0;a<c;a++)h=h||0.5<=e[a].getValue(b);f.setValue(b,h?1:0)}f.setSliceCount(d)}};VVVV.Nodes.Or.prototype=new VVVV.Core.Node;
...
VVVV.Nodes.OrSpectral=function(b,a){this.constructor(b,"OR (Boolean Spectral)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addInputPin("Bin Size",[-1],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0,h=0,b=!1,a=0;a<d||0<e.getValue(0)&&(0<h||0!=c%e.values.length);a++)0==h&&(b=!1),
b=b||0.5<=g.getValue(a),h++,0<e.getValue(0)?h>=e.getValue(c)&&(f.setValue(c,b?1:0),c++,h=0):f.setValue(0,b?1:0);f.setSliceCount(c+(0<h))}};VVVV.Nodes.OrSpectral.prototype=new VVVV.Core.Node;
VVVV.Nodes.AndSpectral=function(b,a){this.constructor(b,"AND (Boolean Spectral)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addInputPin("Bin Size",[-1],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0,h=0,b=!1,a=0;a<d||0<e.getValue(0)&&(0<h||0!=c%e.values.length);a++)0==h&&(b=
!0),b=b&&0.5<=g.getValue(a),h++,0<e.getValue(0)?h>=e.getValue(c)&&(f.setValue(c,b?1:0),c++,h=0):f.setValue(0,b?1:0);f.setSliceCount(c+(0<h))}};VVVV.Nodes.AndSpectral.prototype=new VVVV.Core.Node;VVVV.Types.CanvasTexture={imageObject:void 0,loaded:!1};
...
["miter"],VVVV.PinTypes.Enum);c.enumOptions=["miter","round","bevel"];var h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.CanvasRenderState),m=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==m[a]&&(m[a]=new VVVV.Types.CanvasRenderState),m[a].copy_attributes(g.getValue(a)),m[a].strokeColor=e.getValue(a).split(","),m[a].lineWidth=f.getValue(a),m[a].capStyle=d.getValue(a),m[a].joinStyle=c.getValue(a),h.setValue(a,m[a]);h.setSliceCount(b)}};
...
h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.CanvasRenderState),m=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==m[a]&&(m[a]=new VVVV.Types.CanvasRenderState),m[a].copy_attributes(g.getValue(a)),m[a].shadowOffsetX=f.getValue(a),m[a].shadowOffsetY=d.getValue(a),m[a].shadowBlur=c.getValue(a),m[a].shadowColor=e.getValue(a).split(","),h.setValue(a,m[a]);h.setSliceCount(b)}};VVVV.Nodes.ShadowCanvas.prototype=new VVVV.Core.Node;
...
c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==k[a]&&(k[a]=new r),k[a].transform=e.getValue(a),k[a].radius=parseFloat(d.getValue(a)),k[a].startAngle=2*parseFloat(c.getValue(a))*Math.PI,k[a].endAngle=2*parseFloat(h.getValue(a))*Math.PI,k[a].renderState=g.getValue(a),k[a].drawSegment=1==m.getValue(a),f.isConnected()&&(k[a].clippingLayer=f.getValue(a));for(a=0;a<k.length;a++)p.setValue(a,
...
this.renderState.apply(c);var h=this.width/2,f=h-this.cornerRadius,e=this.height/2,a=e-this.cornerRadius,b=-h,g=-f,m=-e,k=-a;c.moveTo(g,m);c.lineTo(f,m);c.arc(f,k,this.cornerRadius,1.5*Math.PI,0,!1);c.lineTo(h,a);c.arc(f,a,this.cornerRadius,0,0.5*Math.PI,!1);c.lineTo(g,e);c.arc(g,a,this.cornerRadius,0.5*Math.PI,Math.PI,!1);c.lineTo(b,k);c.arc(g,k,this.cornerRadius,Math.PI,1.5*Math.PI,!1);c.closePath();0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();
c.restore()}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==p[b]&&(p[b]=new k),p[b].transform=e.getValue(b),p[b].width=d.getValue(b),p[b].height=c.getValue(b),p[b].cornerRadius=h.getValue(b),p[b].renderState=g.getValue(b),f.isConnected()&&(p[b].clippingLayer=f.getValue(b));for(b=0;b<p.length;b++)m.setValue(b,p[b]);m.setSliceCount(a)}};VVVV.Nodes.RectangleCanvas.prototype=new VVVV.Core.Node;
...
c.strokeText(this.text,0,0);c.restore();c.restore()}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++)void 0==k[b]&&(k[b]=new r),k[b].transform=e.getValue(b),k[b].text=d.getValue(b),k[b].font=c.getValue(b),k[b].align=h.getValue(b),k[b].baseline=m.getValue(b),k[b].renderState=g.getValue(b),k[b].clippingLayer=f.getValue(b);for(b=0;b<k.length;b++)p.setValue(b,k[b]);p.setSliceCount(a)}};VVVV.Nodes.TextCanvas.prototype=new VVVV.Core.Node;
...
e.getValue(a),s[a].transform=f.getValue(a),d.isConnected()&&(s[a].clippingLayer=d.getValue(a))),s[a].x[w]=parseFloat(c.getValue(l)),s[a].y[w]=parseFloat(h.getValue(l)),s[a].c1x[w]=parseFloat(m.getValue(l)),s[a].c1y[w]=parseFloat(p.getValue(l)),s[a].c2x[w]=parseFloat(k.getValue(l)),s[a].c2y[w]=parseFloat(r.getValue(l)),w++,0<n.getValue(0)&&w>=n.getValue(a)&&(w<s[a].x.length&&g(s[a],w),a++,w=0);0>n.getValue(0)?w<s[a].x.length&&(g(s[a],w),s.splice(1)):s.splice(a);for(b=0;b<s.length;b++)q.setValue(b,
s[b]);q.setSliceCount(s.length)}};VVVV.Nodes.BezierCurveCanvas.prototype=new VVVV.Core.Node;
...
this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==h[a]&&(h[a]=new m),h[a].transform=g.getValue(a),h[a].texture=f.getValue(a),h[a].color=d.getValue(a).split(","),e.isConnected()&&(h[a].clippingLayer=e.getValue(a));for(a=0;a<h.length;a++)c.setValue(a,h[a]);c.setSliceCount(b)}};VVVV.Nodes.QuadCanvas.prototype=new VVVV.Core.Node;
VVVV.Nodes.GroupCanvas=function(b,a){this.constructor(b,"Group (Canvas VVVVjs)",a);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var g=this.addInvisiblePin("Layer Count",[2],VVVV.PinTypes.Value),e=[],f=this.addOutputPin("Layer",[],VVVV.PinTypes.CanvasLayer);this.initialize=function(){var d=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,e,d,function(c){return this.addInputPin("Layer "+(c+1),[],VVVV.PinTypes.CanvasLayer)})};this.evaluate=function(){g.pinIsChanged()&&
this.initialize();for(var d=0,c=e.length,h,a=0;a<c;a++)if(e[a].isConnected()){h=e[a].getSliceCount();for(var b=0;b<h;b++)f.setValue(d++,e[a].getValue(b))}f.setSliceCount(d)}};VVVV.Nodes.GroupCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Color),d=this.addInputPin("Backbuffer Width",[0],VVVV.PinTypes.Value),c=this.addInputPin("Backbuffer Height",[0],VVVV.PinTypes.Value),h=this.addInputPin("View",[],VVVV.PinTypes.Transform),m=this.addOutputPin("Canvas Out",[],this),p,k,r,n=[0,0,0,1],q;this.getContext=function(){var h=this.invisiblePins["Descriptive Name"].getValue(0);if((h=$(h).get(0))&&"CANVAS"==h.nodeName)q=h;else{var a=parseInt(d.getValue(0)),f=parseInt(c.getValue(0));q=$('<canvas width="'+(0<a?a:512)+'" height="'+
(0<f?f:512)+'" id="vvvv-js-generated-renderer-'+(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');h||(h="body");$(h).append(q);q=q.get(0)}k=q.width;r=q.height;q&&q.getContext&&(g(q),p=q.getContext("2d"),q.ctx=p)};this.destroy=function(){$(q).remove()};this.evaluate=function(){this.invisiblePins["Descriptive Name"].pinIsChanged()&&($(q).hasClass("vvvv-js-generated-renderer")&&$(q).remove(),this.getContext());if(p){if(d.pinIsChanged()){var a=parseInt(d.getValue(0));0<a&&(k=a,$(q).attr("width",
k))}c.pinIsChanged()&&(a=parseInt(c.getValue(0)),0<a&&(r=a,$(q).attr("height",r)));f.pinIsChanged()&&(n=f.getValue(0).split(","),n[0]=parseInt(255*n[0]),n[1]=parseInt(255*n[1]),n[2]=parseInt(255*n[2]));defaultRenderState.apply(p);p.clearRect(0,0,k,r);p.save();p.translate(k/2,r/2);p.scale(k/2,-r/2);h.isConnected()&&(a=h.getValue(0),p.transform(a[0],a[1],a[4],a[5],a[12],a[13]));if(e.isConnected())for(a=0;a<e.values.length;a++)e.getValue(a).draw(p);p.restore();defaultRenderState.apply(p);p.globalCompositeOperation=
"destination-over";p.fillStyle="rgba("+n[0]+","+n[1]+","+n[2]+","+n[3]+")";p.fillRect(0,0,k,r);m.setValue(0,q);q.loaded=!0}}};VVVV.Nodes.RendererCanvas.prototype=new VVVV.Core.Node;VVVV.PinTypes.Color={typeName:"Color",reset_on_disconnect:!1,defaultValue:function(){return"1.0, 1.0, 1.0, 1.0"},primitive:!0,hsvToRgb:function(b,a,g){b=360*((b%1+1)%1);var e=Math.floor(b/60),f=b/60-e;b=g*(1-a);var d=g*(1-a*f);a=g*(1-a*(1-f));switch(e){case 1:return[d,g,b];case 2:return[b,g,a];case 3:return[b,d,g];case 4:return[a,b,g];case 5:return[g,b,d];default:return[g,a,b]}},rgbToHsv:function(b,a,g){var e=Math.max(b,Math.max(a,g)),f=Math.min(b,Math.min(a,g)),d;e==f?d=0:e==b?d=60*(0+(a-g)/(e-f)):
e==a?d=60*(2+(g-b)/(e-f)):e==g&&(d=60*(4+(b-a)/(e-f)));0>d&&(d+=360);b=0;0!=e&&(b=(e-f)/e);return[d/360,b,e]}};
...
e.pinIsChanged||f.pinIsChanged()||d.pinIsChanged()){for(var h=this.getMaxInputSliceCount(),a=0;a<h;a++){var b=g.getValue(a)||0,k=e.getValue(a)||0,r=f.getValue(a)||0,n=d.getValue(a)||0;c.setValue(a,b+","+k+","+r+","+n)}c.setSliceCount(h)}}};VVVV.Nodes.RGBJoin.prototype=new VVVV.Core.Node;
...
this.getMaxInputSliceCount(),a=0;a<h;a++){var b=g.getValue(a),k=e.getValue(a),r=f.getValue(a),n=d.getValue(a),b=VVVV.PinTypes.Color.hsvToRgb(b,k,r);c.setValue(a,b[0]+","+b[1]+","+b[2]+","+n)}c.setSliceCount(h)}};VVVV.Nodes.HSVJoin.prototype=new VVVV.Core.Node;
...
h=[],m=!1;this.evaluate=function(){var a=this.getMaxInputSliceCount();if(g.pinIsChanged()){for(var b=0;b<a;b++){var r=VVVV.Helpers.prepareFilePath(g.getValue(b),this.parentPatch);0===r.indexOf("http://")&&void 0!==VVVV.ImageProxyPrefix&&(r=VVVV.ImageProxyPrefix+encodeURI(r));if(void 0==h[b]||h[b].origSrc!=r)h[b]=new Image,h[b].loaded=!1,h[b].origSrc=r,h[b].onload=function(c){return function(){m=h[c].loaded=!0}}(b),h[b].src=r,c.setValue(b,0),e.setValue(b,h[b])}h.length=a;e.setSliceCount(a);f.setSliceCount(a);
d.setSliceCount(a);c.setSliceCount(a)}if(m){for(b=0;b<a;b++)e.setValue(b,h[b]),f.setValue(b,h[b].width),d.setValue(b,h[b].height),c.setValue(b,h[b].loaded?1:0);m=!1}}};VVVV.Nodes.FileTextureCanvas.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),u=this.addOutputPin("Video Height",[0],VVVV.PinTypes.Value),t=this.addOutputPin("Network Status",[""],VVVV.PinTypes.String),w=this.addOutputPin("Ready Status",[""],VVVV.PinTypes.String),l=[];this.evaluate=function(){var a=this.getMaxInputSliceCount();if(k.pinIsChanged())for(var b=0;b<a;b++){filename=VVVV.Helpers.prepareFilePath(k.getValue(b),this.parentPatch);if(void 0==l[b]){var z=$('<video style="display:none"><source src="" type=video/ogg></video>');$("body").append(z);
l[b]=z[0];l[b].volume=0;z=function(c){return function(){}}(b);l[b].onprogress=z;l[b].oncanplay=z;l[b].oncanplaythrough=z}filename!=l[b].currentSrc&&($(l[b]).find("source").first().attr("src",filename),l[b].load(),0.5<f.getValue(b)?l[b].play():l[b].pause(),l[b].loaded=!0,r.setValue(b,l[b]),n.setValue(b,l[b]))}if(f.pinIsChanged())for(b=0;b<a;b++)0.5<f.getValue(b)?l[b].play():l[b].pause();if(m.pinIsChanged())for(b=0;b<a;b++)l[b%l.length].loaded&&0.5<=m.getValue(b)&&(l[b%l.length].currentTime=parseFloat(p.getValue(b)),
0.5<f.getValue(b)&&l[b].play());for(b=0;b<a;b++){if(!l[b].paused){r.setValue(b,l[b]);n.setValue(b,l[b]);q.getValue(b)!=l[b].duration&&q.setValue(b,l[b].duration);s.setValue(b,l[b].currentTime);var z=parseFloat(h.getValue(b)),F=parseFloat(c.getValue(b));l[b].currentTime<F&&(l[b].currentTime=F);if(l[b].currentTime>=l[b].duration||0<=z&&l[b].currentTime>=z)0.5<=d.getValue(b)?l[b].currentTime=F:l[b].pause()}if(l[b].videoWidth!=v.getValue(b)||l[b].videoHeight!=u.getValue(b))v.setValue(b,l[b].videoWidth),
u.setValue(b,l[b].videoHeight);t.getValue(b)!=g[l[b].networkState]&&t.setValue(b,g[l[b].networkState]);w.getValue(b)!=e[l[b].readyState]&&w.setValue(b,e[l[b].readyState])}r.setSliceCount(a);n.setSliceCount(a)}};VVVV.Nodes.FileStreamCanvas.prototype=new VVVV.Core.Node;
VVVV.Nodes.AudioOutHTML5=function(b,a){this.constructor(b,"AudioOut (HTML5 VVVVjs)",a);this.meta={authors:["Matthias Zauner"],original_authors:[],credits:[],compatibility_issues:[]};var g=this.addInputPin("Audio",[],this),e=this.addInputPin("Volume",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){var b=this.getMaxInputSliceCount();if(e.pinIsChanged())for(var d=0;d<b;d++)g.getValue(d).volume=Math.max(0,Math.min(1,parseFloat(e.getValue(d))))}};VVVV.Nodes.AudioOutHTML5.prototype=new VVVV.Core.Node;VVVV.Nodes.HTTPGet=function(b,a){this.constructor(b,"HTTP (Network Get)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Doesnt handle get variables yet","Doesnt support proxies","No header output pin yet"]};this.auto_evaluate=!0;var g=this.addInputPin("URL",["http://localhost"],VVVV.PinTypes.String),e=this.addInputPin("Name",[""],VVVV.PinTypes.String),f=this.addInputPin("Value",[""],VVVV.PinTypes.String),d=this.addInputPin("Refresh",[0],
VVVV.PinTypes.Value),c=this.addOutputPin("Status",[""],VVVV.PinTypes.String),h=this.addOutputPin("Body",[""],VVVV.PinTypes.String),m=this.addOutputPin("Fail",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Success",[0],VVVV.PinTypes.Value),k,r,n,q=!1;this.evaluate=function(){this.getMaxInputSliceCount();var b=g.pinIsChanged()||e.pinIsChanged()||f.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==p.getValue(0)&&p.setValue(0,0);1==m.getValue(0)&&m.setValue(0,0);q&&(h.setValue(0,k),c.setValue(0,void 0),
p.setValue(0,r),m.setValue(0,n),q=!1);b&&(void 0==g.getValue(0)?(h.setValue(0,""),c.setValue(0,"")):$.ajax({url:g.getValue(0),type:"get",success:function(c,d,b){k=c;r=1;q=!0},error:function(c,d){k="";n=1;q=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
(this.setType(VVVV.PinTypes.Node),g.setType(VVVV.PinTypes.Node),g.masterPin&&g.masterPin.setType(VVVV.PinTypes.Node))}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),d=0;d<b;d++)e.setValue(d,g.getValue(d));e.setSliceCount(b)}};VVVV.Nodes.IOBoxNode.prototype=new VVVV.Core.Node;
...
[],VVVV.PinTypes.Node);f.length=c};this.evaluate=function(){e.pinIsChanged()&&this.initialize();if(void 0==g.getValue(0))d.setValue(0,void 0);else{for(var c=f[Math.round(Math.abs(g.getValue(0)))%f.length],b=c.getSliceCount(),a=0;a<b;a++)d.setValue(a,c.getValue(a));d.setSliceCount(b)}}};VVVV.Nodes.SwitchNodeInput.prototype=new VVVV.Core.Node;VVVV.Nodes.BoundsSpectral=function(b,a){this.constructor(b,"Bounds (Spectral)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addInputPin("Bin Size",[-1],VVVV.PinTypes.Value),f=this.addOutputPin("Center",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Width",[0],VVVV.PinTypes.Value),c=this.addOutputPin("Minimum",[0],VVVV.PinTypes.Value),h=this.addOutputPin("Maximum",
[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0,k=0,r,n,q,s=0;s<b||0<e.getValue(0)&&(0<k||0!=a%e.values.length);s++)r=g.getValue(s),0==k&&(center=n=q=r,width=0),n=Math.min(r,n),q=Math.max(r,q),k++,0<e.getValue(0)&&k>=e.getValue(a)&&(c.setValue(a,n),h.setValue(a,q),f.setValue(a,n+(q-n)/2),d.setValue(a,Math.abs(q-n)),a++,k=0);-1==e.getValue(0)&&(c.setValue(a,n),h.setValue(a,q),f.setValue(a,n+(q-n)/2),d.setValue(a,Math.abs(q-n)));f.setSliceCount(a+(0<k));
d.setSliceCount(a+(0<k));c.setSliceCount(a+(0<k));h.setSliceCount(a+(0<k))}};VVVV.Nodes.BoundsSpectral.prototype=new VVVV.Core.Node;VVVV.Nodes.GetSliceSpreads=function(b,a){this.constructor(b,"GetSlice (Spreads)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addInputPin("Bin Size",[1],VVVV.PinTypes.Value),f=this.addInputPin("Index",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var c=f.getSliceCount(),b,a,p=0,k=0;k<c;k++)if(a=e.getValue(k),b=
g.getValue(Math.round(f.getValue(k)),a),1<a)for(a=0;a<b.length;a++)d.setValue(p,parseFloat(b[a])),p++;else d.setValue(k,parseFloat(b)),p++;d.setSliceCount(p)}};VVVV.Nodes.GetSliceSpreads.prototype=new VVVV.Core.Node;
VVVV.Nodes.SetSliceSpreads=function(b,a){this.constructor(b,"SetSlice (Spreads)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Spread",[0],VVVV.PinTypes.Value),e=this.addInputPin("Input",[0],VVVV.PinTypes.Value),f=this.addInputPin("Index",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var c=g.getSliceCount(),b=0;b<c;b++)d.setValue(b,
g.getValue(b));size=Math.max(e.getSliceCount(),f.getSliceCount());for(b=0;b<size;b++)d.setValue(f.getValue(b)%c,e.getValue(b));d.setSliceCount(c)}};VVVV.Nodes.SetSliceSpreads.prototype=new VVVV.Core.Node;
VVVV.Nodes.RandomSpread=function(b,a){function g(b){var a=[],d=0,c=0;(function(c){for(var d=0;256>d;d++)a[d]=d;for(var b=0,d=0;256>d;d++){var b=(b+a[d]+c.charCodeAt(d%c.length))%256,e=a[d];a[d]=a[b];a[b]=e}})(b);this.getRandomNumber=function(){for(var b=0,e=1,g=0;8>g;g++){d=(d+1)%256;c=(c+a[d])%256;var k=a[d];a[d]=a[c];a[c]=k;b+=a[(a[d]+a[c])%256]*e;e*=256}return b/1.8446744073709552E19}}this.constructor(b,"RandomSpread (Spreads)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],
...
parseInt(this.inputPins["Spread Count"].getValue(0)),a=parseFloat(this.inputPins.Input.getValue(0)),d=parseFloat(this.inputPins.Width.getValue(0)),c=parseInt(this.inputPins["Random Seed"].getValue(0));rng=new g(c.toString());this.outputPins.Output.setSliceCount(b);for(c=0;c<b;c++)this.outputPins.Output.setValue(c,rng.getRandomNumber()*d-d/2+a)}};VVVV.Nodes.RandomSpread.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),c=this.addInputPin("Spread Count",[1],VVVV.PinTypes.Value),h=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){var b=parseInt(c.getValue(0)),a=parseFloat(e.getValue(0)),k=parseFloat(d.getValue(0)),r=parseFloat(g.getValue(0)),n=f.getValue(0);""==n&&(n="Centered");var q=a/b;"Block"==n&&(q=a/(b-1));var s=q/2;if("Block"==n||"LeftJustified"==n)s=0;"RightJustified"==n&&(s=q);for(var v,u=0;u<b;u++)v=u*q+s,"Block"!=n&&0!=a&&(v=(v+k*a)%a),v=r-a/2+v,h.setValue(u,
v.toFixed(4));h.setSliceCount(b)}};VVVV.Nodes.LinearSpread.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){f.pinIsChanged()&&this.initialize();for(var d=0,c=0;c<g.length;c++)for(var b=0;b<g[c].getSliceCount();b++)e.setValue(d++,g[c].getValue(b));e.setSliceCount(d)}};VVVV.Nodes.ConsSpreads.prototype=new VVVV.Core.Node;
VVVV.Nodes.IntervalSpreads=function(b,a){this.constructor(b,"Interval (Spreads)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Input",[0.5],VVVV.PinTypes.Value),e=this.addInputPin("Intervals",[0],VVVV.PinTypes.Value),f=this.addOutputPin("Index",[0],VVVV.PinTypes.Value),d=0;this.evaluate=function(){var c=g.getSliceCount(),b=e.getSliceCount();if(e.pinIsChanged()){d=void 0;for(var a=0;a<b;a++)if(e.getValue(a)<
d||void 0==d)d=e.getValue(a)}for(a=0;a<c;a++){var p=g.getValue(a);if(p<d)f.setValue(a,-1);else{for(var k=0;k<b-1&&!(p>=e.getValue(k)&&p<e.getValue(k+1));k++);f.setValue(a,k)}}f.setSliceCount(c)}};VVVV.Nodes.IntervalSpreads.prototype=new VVVV.Core.Node;
...
[],VVVV.PinTypes.Value);this.evaluate=function(){var b=e.getValue(0),a=f.getValue(0),k=d.getValue(0),r=!1;if(0.5<=b&&0.5>k){b=g.getSliceCount();for(r=b-1;0<=r;r--)c.values.unshift(g.getValue(r));h.values.unshift(b);r=!0}if(f.pinIsChanged()||r){for(r=currFrameCount=h.values.length;r>a;r--)c.values.splice(-h.values[r-1]);h.values.splice(a);r=!0}if(r){for(r=0;r<c.values.length;r++)c.setValue(r,c.values[r]);for(r=0;r<h.values.length;r++)h.setValue(r,h.values[r])}0.5<=k&&(c.setSliceCount(0),h.setSliceCount(0))}};
...
VVVV.Nodes.SwitchStringInput=function(b,a){this.constructor(b,"Switch (String Input)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=this.addInputPin("Switch",[0],VVVV.PinTypes.Value),f=[],d=this.addOutputPin("Output",["text"],VVVV.PinTypes.String);this.initialize=function(){var c=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,f,c,function(c){return this.addInputPin("Input "+
(c+1),["text"],VVVV.PinTypes.String)})};this.evaluate=function(){g.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),b=0;b<c;b++)d.setValue(b,f[Math.round(Math.abs(e.getValue(b)))%f.length].getValue(b));d.setSliceCount(c)}};VVVV.Nodes.SwitchStringInput.prototype=new VVVV.Core.Node;
...
g.values;for(var b=_(g.values).map(function(c,b){return[c,b]}),b=_(b).sortBy(function(c){return c[0]}),c=0;c<b.length;c++)e.setValue(c,b[c][0]),f.setValue(c,b[c][1]);e.setSliceCount(b.length);f.setSliceCount(b.length)}};VVVV.Nodes.SortString.prototype=new VVVV.Core.Node;
VVVV.Nodes.LengthString=function(b,a){this.constructor(b,"Length (String)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addInputPin("Input",["text"],VVVV.PinTypes.String),e=this.addOutputPin("Count",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),d=0;d<b;d++){var c=g.getValue(d);e.setValue(d,c.length)}e.setSliceCount(b)}};VVVV.Nodes.LengthString.prototype=new VVVV.Core.Node;
...
c=this.addOutputPin("Hits",[0],VVVV.PinTypes.Value),h=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),m=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Found At Position",[0],VVVV.PinTypes.Value);this.evaluate=function(){var b=this.getMaxInputSliceCount();c.setSliceCount(b);var a=f.getValue(0);d.getValue(0);for(var n=0,q=0;q<b;q++){var s=g.getValue(q),v=1;if("MatchesAny"==a||"ContainsAny"==a)v=e.getSliceCount();for(var u=0,t=0;t<v;t++){var w=e.getValue(t+q),
l=[];if("Matches"==a||"MatchesAny"==a)w==s&&l.push(-1);else for(var w=RegExp(w,"gi"),A;A=w.exec(s);)l.push(A.index);for(w=0;w<l.length;w++)h.setValue(n,q%g.getSliceCount()),m.setValue(n,(q+t)%e.getSliceCount()),p.setValue(n,l[w]+1),n++,u++}c.setValue(q,u)}h.setSliceCount(n);m.setSliceCount(n);p.setSliceCount(n)}};VVVV.Nodes.SiftString.prototype=new VVVV.Core.Node;
...
UnderScore:"_",Minus:"-"},k={Single:"'",Double:'"'};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0,q=0;q<b;q++){var s=g.getValue(q),v=p[e.getValue(q)],u=f.getValue(q),t=k[d.getValue(q)],w=c.getValue(q);""==v&&(v=u);""==v&&(v=void 0);s=s.split(v);v=s.length;t=RegExp("[^"+t+"]*","g");for(u=0;u<v;u++)1==w?h.setValue(a,s[u]):h.setValue(a,s[u].match(t).join("")),m.setValue(a,q),a++}h.setSliceCount(a);m.setSliceCount(a)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var p=g.getValue(a),k=e.getValue(a),r=f.getValue(a),p=p==k||0==r&&p.toLowerCase()==k.toLowerCase()?1:0;d.setValue(a,p);c.setValue(a,1-p)}d.setSliceCount(b);c.setSliceCount(b)}};VVVV.Nodes.EQString.prototype=new VVVV.Core.Node;
...
0;p<e.getValue(a);p++)f.setValue(b,g.getValue(a)),d.setValue(b,a),b++;f.setSliceCount(b);d.setSliceCount(b)}};VVVV.Nodes.SelectString.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){if(1<=Math.round(e.getValue(0))){var b=eval(g.getValue(0));b instanceof Array||b instanceof Object||(b=[b]);if(void 0!=b)for(var c=0;c<b.length;c++)f.setValue(c,b[c])}}};VVVV.Nodes.ShellExecute.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var b=this.trIn.isConnected()?this.getMaxInputSliceCount():Math.max(this.xIn.getSliceCount(),this.yIn.getSliceCount(),this.zIn.getSliceCount()),a=0;a<b;a++){var f=this.inputPins["Transform In"].getValue(a),d=parseFloat(this.inputPins.X.getValue(a)),c=parseFloat(this.inputPins.Y.getValue(a)),h=parseFloat(this.inputPins.Z.getValue(a)),m=mat4.create();mat4.identity(m);mat4.rotate(m,2*c*Math.PI,[0,1,0]);mat4.rotate(m,2*d*Math.PI,[1,0,0]);mat4.rotate(m,2*h*Math.PI,[0,0,1]);
...
this.evaluate=function(){for(var b=this.trIn.isConnected()?this.getMaxInputSliceCount():Math.max(this.xIn.getSliceCount(),this.yIn.getSliceCount(),this.zIn.getSliceCount()),a=0;a<b;a++){var f=parseFloat(this.inputPins.X.getValue(a)),d=parseFloat(this.inputPins.Y.getValue(a)),c=parseFloat(this.inputPins.Z.getValue(a)),h=mat4.create();mat4.identity(h);mat4.scale(h,[f,d,c]);this.inputPins["Transform In"].isConnected()&&(f=this.inputPins["Transform In"].getValue(a),mat4.multiply(f,h,h));this.trOut.setValue(a,
...
mat4.multiply(g.getValue(c),mat4.inverse(a,a),a);f.setValue(c,a)}f.setSliceCount(b)}};VVVV.Nodes.InverseTransform.prototype=new VVVV.Core.Node;
...
[],VVVV.PinTypes.Transform),h=mat4.create();mat4.identity(h);mat4.scale(h,[-1,1,-1]);this.evaluate=function(){for(var b=Math.max(g.getSliceCount(),Math.max(e.getSliceCount()/3,Math.max(f.getSliceCount()/3,d.getSliceCount()/3))),b=Math.ceil(b),a=0;a<b;a++){var k=e.getValue(a,3),r=f.getValue(a,3),n=d.getValue(a,3),q=mat4.create();mat4.identity(q);mat4.lookAt(vec3.create(k),vec3.create(r),vec3.create(n),q);mat4.multiply(h,q,q);g.isConnected()&&mat4.multiply(g.getValue(a),q,q);c.setValue(a,q)}c.setSliceCount(b)}};
...
a=0;a<b;a++){var p=0;Math.abs(g.getValue(a)-e.getValue(a))<=Math.abs(f.getValue(a))&&(p=1);d.setValue(a,p);c.setValue(a,1-p)}d.setSliceCount(b);c.setSliceCount(b)}};VVVV.Nodes.EqValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.GtValue=function(b,a){this.constructor(b,"GT (Value)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["No dynamic pin count yet"]};var g=this.addInputPin("Input 1",[0],VVVV.PinTypes.Value),e=this.addInputPin("Input 2",[0],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){var a=0;parseFloat(g.getValue(c))>parseFloat(e.getValue(c))&&
...
c=0;c<b;c++)0==e.getValue(c)?f.setValue(c,1):f.setValue(c,g.getValue(c)/e.getValue(c));f.setSliceCount(b)}};VVVV.Nodes.DivideValue.prototype=new VVVV.Core.Node;
...
VVVV.Nodes.SwitchValueInput=function(b,a){this.constructor(b,"Switch (Value Input)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["No dynamic pin count yet"]};var g=this.addInvisiblePin("Input Count",[2],VVVV.PinTypes.Value),e=this.addInputPin("Switch",[0],VVVV.PinTypes.Value),f=[];this.initialize=function(){var c=Math.max(2,g.getValue(0));VVVV.Helpers.dynamicPins(this,f,c,function(c){return this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value)})};
var d=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){g.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),b=0;b<c;b++)d.setValue(b,f[Math.round(Math.abs(e.getValue(b)))%f.length].getValue(b));d.setSliceCount(c)}};VVVV.Nodes.SwitchValueInput.prototype=new VVVV.Core.Node;
VVVV.Nodes.SelectValue=function(b,a){this.constructor(b,"Select (Value)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addInputPin("Select",[1],VVVV.PinTypes.Value),f=this.addOutputPin("Output",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Former Slice",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var c=this.getMaxInputSliceCount(),b=0,a=0;a<c;a++)for(var p=0;p<e.getValue(a);p++)f.setValue(b,
parseFloat(g.getValue(a))),d.setValue(b,a),b++;f.setSliceCount(b);d.setSliceCount(b)}};VVVV.Nodes.SelectValue.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){subtype=e.getValue(c).match(/^(.+) \(/)[1];var a;switch(subtype){case "Real":a=parseFloat(g.getValue(c)).toFixed(4);break;case "Integer":a=parseInt(g.getValue(c)).toFixed(0)}f.setValue(c,a)}f.setSliceCount(b)}};VVVV.Nodes.AsString.prototype=new VVVV.Core.Node;
VVVV.Nodes.Frac=function(b,a){this.constructor(b,"Frac (Value)",a);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Input",[0],VVVV.PinTypes.Value),e=this.addOutputPin("Whole Part",[0],VVVV.PinTypes.Value),f=this.addOutputPin("Real Part",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){var a=parseFloat(g.getValue(c));e.setValue(c,Math.floor(a));f.setValue(c,a-Math.floor(a))}e.setSliceCount(b);
...
[1],VVVV.PinTypes.Value),h=this.addInputPin("Mapping",["Float"],VVVV.PinTypes.Enum);h.enumOptions=["Float","Wrap","Clamp"];var m=this.addOutputPin("Output",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var r=parseFloat(g.getValue(a)),n=parseFloat(e.getValue(a)),q=parseFloat(f.getValue(a)),s=parseFloat(d.getValue(a)),v=parseFloat(c.getValue(a)),u=h.getValue(a);if(q==n)m.setValue(a,s);else{r-=n;q-=n;switch(u){case "Clamp":r=Math.max(0,Math.min(q,
r));break;case "Wrap":r=(q+r%q)%q}m.setValue(a,r*((v-s)/q)+s)}}m.setSliceCount(b)}};VVVV.Nodes.MapValue.prototype=new VVVV.Core.Node;
...
function(){e.pinIsChanged()&&this.initialize();for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){for(var a=void 0,m=0;m<f.length;m++)if(void 0==a||f[m].getValue(c)<a)a=f[m].getValue(c);g.setValue(c,a)}g.setSliceCount(b)}};VVVV.Nodes.MinValue.prototype=new VVVV.Core.Node;
...
function(){e.pinIsChanged()&&this.initialize();for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){for(var a=void 0,m=0;m<f.length;m++)if(void 0==a||f[m].getValue(c)>a)a=f[m].getValue(c);g.setValue(c,a)}g.setSliceCount(b)}};VVVV.Nodes.MaxValue.prototype=new VVVV.Core.Node;
...
function(){e.pinIsChanged()&&this.initialize();for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){for(var a=f[0].getValue(c),m=1;m<f.length;m++)a=0==f[m].getValue(c)?0:a%f[m].getValue(c);g.setValue(c,a)}g.setSliceCount(b)}};VVVV.Nodes.ModValue.prototype=new VVVV.Core.Node;
...
function(){e.pinIsChanged()&&this.initialize();for(var b=this.getMaxInputSliceCount(),c=0;c<b;c++){for(var a=f[0].getValue(c),m=1;m<f.length;m++)a=Math.pow(a,f[m].getValue(c));g.setValue(c,a)}g.setSliceCount(b)}};VVVV.Nodes.PowerValue.prototype=new VVVV.Core.Node;
...
(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=function(){f.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),b,a,p,k=0;k<c;k++)b=g.getValue(k),a=((Math.floor(b)+d.length)%d.length+d.length)%d.length,p=((Math.ceil(b)+d.length)%d.length+d.length)%d.length,b-=Math.floor(b),e.setValue(k,(1-b)*d[a].getValue(k)+b*d[p].getValue(k));e.setSliceCount(c)}};VVVV.Nodes.InputMorphValue.prototype=new VVVV.Core.Node;
...
h=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),m=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value);this.evaluate=function(){this.getMaxInputSliceCount();var b=g.getSliceCount(),a=e.getSliceCount();c.setSliceCount(b);f.getValue(0);for(var r=d.getValue(0),n={},q=0,s=0;s<b;s++){var v=s;"Last"==r&&(v=b-s-1);var u=g.getValue(v);if("All"!=r){if(n[u]){c.setValue(v,0);continue}n[u]=!0}for(var t=0,w=0;w<a;w++)if(filter=e.getValue(w),filter==u){h.setValue(q,v);m.setValue(q,w);q++;t++;break}c.setValue(v,
t)}h.setSliceCount(q);m.setSliceCount(q)}};VVVV.Nodes.Sift.prototype=new VVVV.Core.Node;VVVV.Nodes.Join2dVector=function(b,a){this.constructor(b,"Vector (2d Join)",a);this.meta={authors:["Julien Vulliet"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[""]};var g=this.addInputPin("X",[0],VVVV.PinTypes.Value),e=this.addInputPin("Y",[0],VVVV.PinTypes.Value),f=this.addOutputPin("XY",[0,0],VVVV.PinTypes.Value);this.evaluate=function(){var b=this.getMaxInputSliceCount();f.setSliceCount(2*b);for(var c=0;c<b;c++)f.setValue(2*c,g.getValue(c)),f.setValue(2*c+1,e.getValue(c))}};
...
b,g.getValue(b)),d.setValue(3*b+1,e.getValue(b)),d.setValue(3*b+2,f.getValue(b))}};VVVV.Nodes.Join3dVector.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.WebGlResource={typeName:"WebGlResource",reset_on_disconnect:!0,connectionChangedHandlers:{webglresource:function(){function b(b){return"WebGlResource"==b||"WebGlTexture"==b}if(this.direction!=PinDirection.Input){var a=this.node,g=a.findDownstreamNodes("Renderer (EX9)");a.renderContexts||(a.renderContexts=[]);void 0==a.contextChanged&&(a.contextChanged=!1);for(var e=0;e<g.length;e++)a.contextChanged|=!a.renderContexts[e]||a.renderContexts[e].canvas.id!=g[e].ctxt.id,a.renderContexts[e]=
g[e].ctxt,a.dirty=!0;a.renderContexts.length!=g.length&&(a.renderContexts.length=g.length,a.contextChanged=!0,a.dirty=!0);a.isSubpatch||_(a.inputPins).each(function(f){var d;f.markPinAsChanged();"Renderer (EX9)"!=a.nodename&&f.isConnected()&&(0<f.links.length?d=f.links[0].fromPin:0<f.masterPin.links[0].fromPin.links.length&&(d=f.masterPin.links[0].fromPin),d&&b(d.typeName)&&d.connectionChanged())});this.masterPin&&b(this.masterPin.typeName)&&this.masterPin.connectionChanged()}}},defaultValue:function(){return new VVVV.Types.Layer}};
...
VVVV.Types.ShaderProgram=function(){this.uniformSpecs={};this.attributeSpecs={};this.attribSemanticMap={};this.uniformSemanticMap={};var b="",a="",g,e;this.isSetup=!1;this.shaderProgram=void 0;this.log="";var f=this;this.extractSemantics=function(b){f.attributeSpecs={};f.attribSemanticMap={};f.uniformSpecs={};f.uniformSemanticMap={};for(var c=/(uniform|attribute) ([a-zA-Z]+)([0-9xD]*) ([a-zA-Z0-9_]+)( : ([A-Z0-9]+))?( = \{?([^;\}]+)\}?)?;/g,a;a=c.exec(b);)"attribute"!=a[1]||f.attributeSpecs[a[4]]?
...
f=this.addInvisiblePin("Type",["Texture"],VVVV.PinTypes.Enum);f.enumOptions=["Texture","Cube Texture"];var d=[];this.evaluate=function(){if(this.renderContexts){var b=this.renderContexts[0];if(b){if(this.contextChanged){for(var a=0;a<d.length;a++)d[a].context.deleteTexture(d[a]);d=[]}if(g.pinIsChanged()||f.pinIsChanged()||this.contextChanged){for(var m=f.getValue(0),p=this.getMaxInputSliceCount(),a=0;a<p;a++){var k=VVVV.Helpers.prepareFilePath(g.getValue(a),this.parentPatch);0===k.indexOf("http://")&&
...
this.width/4+'" height="'+this.height/3+'"></canvas>');$("body").append(h);for(var g=h.get(0).getContext("2d"),k=0;6>k;k++)g.save(),g.translate(-this.width/4*f[k].offset[0],-this.height/3*f[k].offset[1]),g.drawImage(this,0,0),b.texImage2D(f[k].face,0,b.RGBA,b.RGBA,b.UNSIGNED_BYTE,h.get(0)),b.texParameteri(b.TEXTURE_CUBE_MAP,b.TEXTURE_MAG_FILTER,b.NEAREST),b.texParameteri(b.TEXTURE_CUBE_MAP,b.TEXTURE_MIN_FILTER,b.NEAREST),g.restore();b.bindTexture(b.TEXTURE_CUBE_MAP,null);h.remove();e.setValue(a,d[a])}}(a),
d[a].image.src=k);e.setValue(a,VVVV.defaultTexture)}e.setSliceCount(p)}this.contextChanged=!1}}};this.destroy=function(){for(var b=0;b<d.length;b++)d[b].context.deleteTexture(d[b])}};VVVV.Nodes.FileTexture.prototype=new VVVV.Core.Node;
...
b&&0.5<=f.getValue(0)&&(g.isConnected()?(c=new VVVV.Types.Mesh(b,g.getValue(0),e.values),d.setValue(0,c)):(d.setValue(0,void 0),delete c))}};VVVV.Nodes.MeshJoin.prototype=new VVVV.Core.Node;
...
m=parseInt(e.getValue(0)),p=[],k=[],r=[],n=0,q=0;q<m;q++)for(var s=0;s<a;s++)p.push(parseFloat(s)/(a-1)-0.5),p.push(0.5-parseFloat(q)/(m-1)),p.push(0),n++,k.push(0),k.push(0),k.push(1),r.push(parseFloat(s)/(a-1)),r.push(parseFloat(q)/(m-1));p=new VVVV.Types.VertexBuffer(b,p);p.setSubBuffer("TEXCOORD0",2,r);p.setSubBuffer("NORMAL",3,k);p.create();k=[];for(q=0;q<m-1;q++)for(s=0;s<a-1;s++)r=s+a*q,k.push(r),k.push(r+1),k.push(r+a+1),k.push(r+a+1),k.push(r+a),k.push(r);d=new VVVV.Types.Mesh(b,p,k);f.setValue(0,
...
this.renderContexts[0];if(b){for(var a=parseInt(e.getValue(0)),p=parseInt(f.getValue(0)),k=parseFloat(g.getValue(0)),r=[],n=[],q=[],s=0;s<p+1;s++)for(var v=Math.cos(-parseFloat(s)/p*Math.PI),u=0;u<a;u++){var t=Math.cos(2*(parseFloat(u)/a)*Math.PI)*Math.cos(Math.asin(v)),w=Math.sin(2*(parseFloat(u)/a)*Math.PI)*Math.cos(Math.asin(v));r.push(t*k);r.push(v*k);r.push(w*k);n.push(t);n.push(v);n.push(w);q.push(parseFloat(u)/a);q.push(parseFloat(s)/p)}k=new VVVV.Types.VertexBuffer(b,r);k.setSubBuffer("TEXCOORD0",
2,q);k.setSubBuffer("NORMAL",3,n);k.create();n=[];for(s=0;s<p;s++)for(u=0;u<a;u++)q=a*s,r=u+q,n.push(r),n.push((r+1)%a+q),n.push((r+1)%a+a+q),n.push((r+1)%a+a+q),n.push(r+a),n.push(r);c=new VVVV.Types.Mesh(b,k,n);d.setValue(0,c)}}}};VVVV.Nodes.Sphere.prototype=new VVVV.Core.Node;
...
[15],VVVV.PinTypes.Value),m=this.addInputPin("Resolution Y",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Mesh",[],VVVV.PinTypes.WebGlResource),k=null;this.evaluate=function(){if(this.renderContexts){var b=this.renderContexts[0];if(b){var a=parseInt(h.getValue(0)),q=parseInt(m.getValue(0)),s=parseFloat(g.getValue(0)),v=parseFloat(e.getValue(0)),u=parseFloat(f.getValue(0)),t=parseFloat(d.getValue(0)),w=[],l=[],A=[];w.push(0);w.push(u/2);w.push(0);l.push(0);l.push(1);l.push(0);A.push(0);A.push(0);w.push(0);
w.push(-u/2);w.push(0);l.push(0);l.push(-1);l.push(0);A.push(0);A.push(0);for(var y=0;y<q+1;y++)for(var z=parseFloat(y)/q,F=(z-0.5)*-u,C=0;C<a+1;C++){var I=Math.cos(2*(parseFloat(C)/a)*Math.PI*t-Math.PI*t-Math.PI/2),J=Math.sin(2*(parseFloat(C)/a)*Math.PI*t-Math.PI*t-Math.PI/2),H=z*v+(1-z)*s;w.push(I*H);w.push(F);w.push(J*H);l.push(I);l.push(0);l.push(J);A.push(parseFloat(C)/a);A.push(parseFloat(y)/q)}s=new VVVV.Types.VertexBuffer(b,w);s.setSubBuffer("TEXCOORD0",2,A);s.setSubBuffer("NORMAL",3,l);s.create();
l=[];if(0.5<c.getValue(0))for(z=0;2>z;z++)for(C=0;C<a;C++)l.push(z),l.push(2+C+z+z*q*(a+1)),l.push(2+C+(1-z)+z*q*(a+1));for(y=0;y<q;y++)for(C=0;C<a;C++)z=C+(a+1)*y+2,l.push(z),l.push(z+1),l.push(z+a+2),l.push(z+a+2),l.push(z+a+1),l.push(z);k=new VVVV.Types.Mesh(b,s,l);p.setValue(0,k)}}}};VVVV.Nodes.Cylinder.prototype=new VVVV.Core.Node;
VVVV.Nodes.BlendWebGLAdvanced=function(b,a){function g(b){switch(b){case "One":return"ONE";case "Zero":return"ZERO";case "SrcAlpha":return"SRC_ALPHA";case "InvSrcAlpha":return"ONE_MINUS_SRC_ALPHA";case "DestAlpha":return"DST_ALPHA";case "InvDestAlpha":return"ONE_MINUS_DST_ALPHA";case "SrcColor":return"SRC_COLOR";case "InvSrcColor":return"ONE_MINUS_SRC_COLOR";case "DestColor":return"DST_COLOR";case "InvDestColor":return"ONE_MINUS_DST_COLOR"}return null}this.constructor(b,"Blend (EX9.RenderState Advanced)",
...
var h=this.addOutputPin("Render State Out",[],VVVV.PinTypes.WebGlRenderState),m=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==m[a]&&(m[a]=new VVVV.Types.WebGlRenderState),m[a].copy_attributes(e.getValue(a)),m[a].alphaBlending=0.5<parseFloat(f.getValue(a)),m[a].srcBlendMode=g(d.getValue(a)),m[a].destBlendMode=g(c.getValue(a)),h.setValue(a,m[a]);h.setSliceCount(b)}};VVVV.Nodes.BlendWebGLAdvanced.prototype=new VVVV.Core.Node;
...
var f=this.addOutputPin("Render State Out",[],VVVV.PinTypes.WebGlRenderState),d=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){void 0==d[a]&&(d[a]=new VVVV.Types.WebGlRenderState);d[a].copy_attributes(g.getValue(a));switch(e.getValue(a)){case "Point":d[a].polygonDrawMode="POINTS";break;case "Solid":d[a].polygonDrawMode="TRIANGLES";break;case "WireFrame":d[a].polygonDrawMode="LINES"}f.setValue(a,d[a])}f.setSliceCount(b)}};VVVV.Nodes.FillWebGL.prototype=new VVVV.Core.Node;
...
m[a].copy_attributes(e.getValue(a)),m[a].enableZWrite=0.5<parseFloat(f.getValue(a)),m[a].depthFunc=g(d.getValue(a)),m[a].depthOffset=parseFloat(c.getValue(0)),h.setValue(a,m[a]);h.setSliceCount(b)}};VVVV.Nodes.ZWriteEnableWebGL.prototype=new VVVV.Core.Node;
...
(b=0);var m=c.length;if(this.contextChanged||p)m=0;b<m&&c.splice(b,m-b);for(var v=m;v<b;v++)c[v]=new VVVV.Types.Layer,c[v].mesh=e.getValue(0),c[v].shader=h,_(h.uniformSpecs).each(function(a){c[v].uniformNames.push(a.varname);c[v].uniforms[a.varname]={uniformSpec:a,value:void 0}});if(e.pinIsChanged())for(v=0;v<b;v++)c[v].mesh=e.getValue(0);for(v=0;v<b;v++)c[v].shader=h;for(a=0;a<k.length;a++){var u=k[a].pinname.replace(/ /g,"_");if(k[a].pinIsChanged()||m<b)for(v=0;v<b;v++)if("vec"==h.uniformSpecs[u].type){var t=
"COLOR"==h.uniformSpecs[u].semantic?_(k[a].getValue(v).split(",")).map(function(a){return parseFloat(a)}):k[a].getValue(v,k[a].dimensions);c[v].uniforms[u].value=new Float32Array(t)}else t=k[a].getValue(v),c[v].uniforms[u].value=t}if(g.pinIsChanged()||m<b)for(a=0;a<b;a++)g.isConnected()?c[a].renderState=g.getValue(a):c[a].renderState=VVVV.DefaultRenderState;if(f.pinIsChanged()||m<b)for(a=0;a<b;a++)m=this.inputPins.Transform.getValue(a),c[a].uniforms[c[a].shader.uniformSemanticMap.WORLD].value=m;this.outputPins.Layer.setSliceCount(b);
...
e[m].mesh=f,e[m].shader=d,_(d.uniformSpecs).each(function(a){e[m].uniformNames.push(a.varname);e[m].uniforms[a.varname]={uniformSpec:a,value:void 0}});var p=this.inputPins.Color.pinIsChanged(),k=this.inputPins.Transform.pinIsChanged(),r=this.inputPins.Texture.pinIsChanged(),n=this.inputPins["Texture Transform"].pinIsChanged();if(p||b<a)for(p=0;p<a;p++){var q=this.inputPins.Color.getValue(p),q=_(q.split(",")).map(function(a){return parseFloat(a)});e[p].uniforms.col.value=new Float32Array(q)}if(g.pinIsChanged()||
b<a)for(p=0;p<a;p++)g.isConnected()?e[p].renderState=g.getValue(p):e[p].renderState=VVVV.DefaultRenderState;if(k||b<a)for(p=0;p<a;p++)k=this.inputPins.Transform.getValue(p),e[p].uniforms[e[p].shader.uniformSemanticMap.WORLD].value=k;if(r||b<a)for(p=0;p<a;p++)e[p].uniforms.Samp0.value=this.inputPins.Texture.getValue(p);if(n||b<a)for(p=0;p<a;p++)k=this.inputPins["Texture Transform"].getValue(p),e[p].uniforms.tTex.value=k;this.outputPins.Layer.setSliceCount(a);for(p=0;p<a;p++)this.outputPins.Layer.setValue(p,
e[p]);this.contextChanged=!1}}}};VVVV.Nodes.Quad.prototype=new VVVV.Core.Node;
VVVV.Nodes.Group=function(b,a){this.constructor(b,"Group (EX9)",a);this.auto_nil=!1;this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=[],e=this.addInputPin("Enabled",[1],VVVV.PinTypes.Value),f=this.addInvisiblePin("Layer Template Count",[2],VVVV.PinTypes.Value),d=this.addOutputPin("Layer",[],VVVV.PinTypes.WebGlResource);this.initialize=function(){var a=Math.max(2,f.getValue(0));VVVV.Helpers.dynamicPins(this,g,a,function(a){return this.addInputPin("Layer "+
(a+1),[],VVVV.PinTypes.WebGlResource)})};this.evaluate=function(){f.pinIsChanged()&&this.initialize();var a=0;if(0.5<e.getValue(0))for(var b=0;b<g.length;b++)for(var m=0;m<g[b].getSliceCount();m++)d.setValue(a++,g[b].getValue(m));d.setSliceCount(a)}};VVVV.Nodes.Group.prototype=new VVVV.Core.Node;
...
l.DEPTH_COMPONENT16,A.width,A.height),l.framebufferTexture2D(l.FRAMEBUFFER,l.COLOR_ATTACHMENT0,l.TEXTURE_2D,y,0),l.framebufferRenderbuffer(l.FRAMEBUFFER,l.DEPTH_ATTACHMENT,l.RENDERBUFFER,a),l.bindTexture(l.TEXTURE_2D,null),l.bindRenderbuffer(l.RENDERBUFFER,null),l.bindFramebuffer(l.FRAMEBUFFER,null)):this.renderContexts&&this.renderContexts[0]&&(this.renderContexts[0].deleteTexture(y),y=void 0);this.ctxt&&(q=parseInt(t.get(0).width),s=parseInt(t.get(0).height),l=this.ctxt,a=new Uint8Array([255,255,
...
m.markPinAsChanged())}}};this.destroy=function(){$(t).remove()};var z=!1;this.evaluate=function(){l=this.ctxt;if(this.invisiblePins["Descriptive Name"].pinIsChanged()||this.contextChanged)w&&$(w.canvas).hasClass("vvvv-js-generated-renderer")&&$(w.canvas).remove(),this.getContexts(),this.inputPins.Layers.isConnected()&&this.inputPins.Layers.links[0].fromPin.connectionChanged();z||(k.setValue(0,q),r.setValue(0,s),z=!0);if(void 0!=l){if(d.pinIsChanged()&&(!this.renderContexts||!this.renderContexts[0])){var a=
parseInt(d.getValue(0));0<a&&(q=a,$(w.canvas).attr("width",q),k.setValue(0,q))}!c.pinIsChanged()||this.renderContexts&&this.renderContexts[0]||(a=parseInt(c.getValue(0)),0<a&&(s=a,$(w.canvas).attr("height",s),r.setValue(0,s)));this.renderContexts&&this.renderContexts[0]&&l==this.renderContexts[0]?l.bindFramebuffer(l.FRAMEBUFFER,A):l.bindFramebuffer(l.FRAMEBUFFER,null);a=_(f.getValue(0).split(",")).map(function(a){return parseFloat(a)});l.clearColor(a[0],a[1],a[2],a[3]);l.clear(l.COLOR_BUFFER_BIT|
l.DEPTH_BUFFER_BIT);"NONE"==p.getValue(0)?l.disable(l.DEPTH_TEST):l.enable(l.DEPTH_TEST);m.pinIsChanged()&&(m.isConnected()?(v=mat4.create(),mat4.set(m.getValue(0),v)):(v=mat4.create(),mat4.ortho(-1,1,-1,1,-100,100,v)),mat4.scale(v,[1,1,-1]),this.renderContexts&&this.renderContexts[0]&&mat4.scale(v,[1,-1,1]));h.pinIsChanged()&&(u=h.getValue(0));if(this.contextChanged)this.contextChanged=!1;else{l.viewport(0,0,q,s);var b=a=null,e=null;if(this.inputPins.Layers.isConnected()){for(var g=this.inputPins.Layers.values,
t=0;t<g.length;t++)if(layer=g[t],void 0!=layer.shader){a!=layer.shader.shaderProgram&&(l.useProgram(layer.shader.shaderProgram),l.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.PROJECTION].position,!1,v),l.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.VIEW].position,!1,u));var E=layer.renderState;E||(E=defaultWebGlRenderState);E!=b&&E.apply(l);if(layer.mesh!=e||layer.shader.shaderProgram!=a)l.bindBuffer(l.ARRAY_BUFFER,layer.mesh.vertexBuffer.vbo),
...
VVVV.PinTypes.Value.makeLabel=VVVV.PinTypes.String.makeLabel=function(b,a){var g=a.IOBoxRows();a.IOBoxInputPin().getSliceCount();d3.select(b).selectAll(".vvvv-node-label").remove();for(var e=0;e<g;e++)d3.select(b).append("svg:text").text(a.IOBoxInputPin().getValue(e)).attr("class","vvvv-node-label").attr("shape-rendering","crispEdges").attr("dy",function(a){return 12*e+12}).attr("dx",4).attr("font-size",10).attr("font-family","Lucida Sans Unicode")};
VVVV.PinTypes.Value.openInputBox=VVVV.PinTypes.String.openInputBox=function(b,a,g,e){$inputbox=$("<input type='text' value='"+g.getValue(e)+"' class='pininputbox value resettable'/>");$inputbox.css("position",a.css("position"));$inputbox.css("width",a.css("width"));$inputbox.css("height",a.css("height"));$inputbox.css("left",a.css("left"));$inputbox.css("top",a.css("top"));a.replaceWith($inputbox);$inputbox.get(0).select();$inputbox.change(function(){g.setValue(e,$(this).val());g.node.parentPatch.editor.update(g.node.parentPatch,
...
VVVV.PinTypes.Enum.openInputBox=function(b,a,g,e){$inputbox=$("<select class='pininputbox value resettable'>");for(b=0;b<g.enumOptions.length;b++)$opt=$('<option value="'+g.enumOptions[b]+'">'+g.enumOptions[b]+"</option>"),g.getValue(e)==g.enumOptions[b]&&$opt.attr("selected",!0),$inputbox.append($opt);$inputbox.css("position",a.css("position"));$inputbox.css("width",a.css("width"));$inputbox.css("height",a.css("height"));$inputbox.css("left",a.css("left"));$inputbox.css("top",a.css("top"));a.replaceWith($inputbox);
...
VVVV.PinTypes.Color.makeLabel=function(b,a){var g=a.IOBoxRows();a.IOBoxInputPin().getSliceCount();d3.select(b).selectAll(".vvvv-node-label").remove();for(var e=0;e<g;e++)d3.select(b).append("svg:rect").attr("class","vvvv-node-label").attr("height",a.getHeight()/g-8).attr("width",a.getWidth()).attr("y",12*e+4).attr("fill",function(b){b=a.IOBoxInputPin().getValue(e).split(",");for(var d=0;d<b.length;d++)b[d]=parseInt(256*b[d]);return"rgba("+b.join(",")+")"})};
VVVV.PinTypes.Color.openInputBox=function(b,a,g,e){function f(a){c=a.altKey&&a.shiftKey?3:a.altKey?1:a.shiftKey?2:0}function d(a){var b=g.getValue(e).split(","),d=VVVV.PinTypes.Color.rgbToHsv(b[0],b[1],b[2]);d[3]=parseFloat(b[3]);d[c]+=0.01*a;0!=c&&(d[c]=Math.min(1,Math.max(0,d[c])));b=VVVV.PinTypes.Color.hsvToRgb(d[0],d[1],d[2]);b[3]=d[3];$inputbox.css("background-color","rgba("+_(b).map(function(a){return parseInt(255*a)}).join(",")+")");ibx=$inputbox;b.join(",");g.setValue(e,b.join(","));g.node.parentPatch.editor.update(g.node.parentPatch,
"<PATCH><NODE id='"+g.node.id+"'><PIN pinname='"+g.pinname+"' values='"+_(g.values).map(function(a){return"|"+a+"|"}).join(",")+"'/></NODE>")}var c=0;$(b.window.document).keydown(f).keyup(f);$inputbox=$('<div class="pininputbox color resettable"></div>');$inputbox.css("position",a.css("position"));$inputbox.css("width",a.css("width"));$inputbox.css("height",a.css("height"));$inputbox.css("left",a.css("left"));$inputbox.css("top",a.offset().top-8+"px");b=g.getValue(e).split(",");for(var h=0;h<b.length;h++)b[h]=
...
b);a.preventDefault();return!1});var m=0,p=0;$("body",y.window.document).mousemove(function(a){m=a.pageX;p=a.pageY});$("body",y.window.document).on("paste",function(b){b=b.originalEvent.clipboardData.getData("text/plain");if(!b.match(/^<!DOCTYPE PATCH/))return!1;$patch=$(b);var d=void 0,f=void 0;$patch.find("node bounds").each(function(){var a=parseInt($(this).attr("left")),b=parseInt($(this).attr("top"));if(!d||a<d)d=a;if(!f||b<f)f=b});var h={};$patch.find("node").each(function(){s++;h[$(this).attr("id")]=
s;$(this).attr("createme","pronto");$(this).attr("id",s);var a=parseInt($(this).find("bounds").attr("left"))-d+15*m,b=parseInt($(this).find("bounds").attr("top"))-f+15*p;$(this).find("bounds").attr("left",a);$(this).find("bounds").attr("top",b)});$patch.find("link").each(function(){$(this).attr("createme","pronto");var a=$(this).attr("srcnodeid"),b=$(this).attr("dstnodeid");$(this).attr("srcnodeid",h[a]);$(this).attr("dstnodeid",h[b])});b=$patch.wrapAll("<d></d>").parent().html();a.update(q,b);e();
var g=_(h).map(function(a){return a});c.selectAll(".vvvv-node").each(function(a){0<=g.indexOf(parseInt(a.id))&&(n.push(a),d3.select(this).attr("class","vvvv-node selected"))})})}c=root.append("svg:svg").attr("class","chart").attr("width",Math.max(q.windowWidth,q.boundingBox.width)).attr("height",Math.max(q.windowHeight,q.boundingBox.height));k=c.append("svg:rect").attr("class","background").attr("width",Math.max(q.windowWidth,q.boundingBox.width)).attr("height",Math.max(q.windowHeight,q.boundingBox.height));
g||(c.on("mousemove",function(){if(y.state==UIState.Connecting)c.select(".vvvv-link.current-link").attr("x2",function(a){return d3.event.pageX+(d3.select(this).attr("x1")<d3.event.pageX?-1:1)}).attr("y2",function(a){return d3.event.pageY+(d3.select(this).attr("y1")<d3.event.pageY?-1:1)});else if(y.state==UIState.Moving){var a=d3.event.pageX-f,b=d3.event.pageY-d;c.selectAll(".vvvv-node.selected").attr("transform",function(c){return"translate("+(c.x+a)+","+(c.y+b)+")"});for(var h=0;h<n.length;h++){var g=
n[h];c.selectAll(".vvvv-link line").filter(function(a){return a.fromPin.node.id==g.id}).attr("x1",function(b){return b.fromPin.x+b.fromPin.node.x+a}).attr("y1",function(a){return a.fromPin.y+a.fromPin.node.y+b});c.selectAll(".vvvv-link line").filter(function(a){return a.toPin.node.id==g.id}).attr("x2",function(b){return b.toPin.x+b.toPin.node.x+a}).attr("y2",function(a){return a.toPin.y+a.toPin.node.y+b})}}else y.state==UIState.AreaSelecting&&(l=d3.event.pageX,A=d3.event.pageY,c.select(".selection-area").attr("transform",
"translate("+Math.min(t,l)+", "+Math.min(w,A)+")").attr("width",Math.abs(l-t)).attr("height",Math.abs(A-w)),e(),c.selectAll(".vvvv-node").each(function(a){for(var b=[a.x,a.x+a.getWidth()],c=[a.y,a.y+a.getHeight()],d=!1,f=0;2>f;f++)for(var e=0;2>e;e++)b[f]>=Math.min(t,l)&&(b[f]<=Math.max(t,l)&&c[e]>=Math.min(w,A)&&c[e]<=Math.max(w,A))&&(d=!0);d&&(d3.select(this).attr("class","vvvv-node selected"),n.push(a))}))}).on("contextmenu",function(){y.state==UIState.Connecting&&(c.select(".vvvv-link.current-link").remove(),
y.state=UIState.Idle);d3.event.stopPropagation();d3.event.preventDefault();return!1}).on("mouseup",function(){if(y.state==UIState.Moving){y.state=UIState.Idle;for(var b=d3.event.pageX-f,e=d3.event.pageY-d,h="<PATCH>",g=0;g<n.length;g++)var k=n[g],h=h+("<NODE componentmode='Node' id='"+k.id+"'><BOUNDS type='Node' left='"+15*(b+k.x)+"' top='"+15*(e+k.y)+"' width='"+k.width+"' height='"+k.height+"'/></NODE>");a.update(q,h+"</PATCH>")}y.state==UIState.AreaSelecting&&(c.select(".selection-area").remove(),
y.state=UIState.Idle)}).on("dblclick",function(){function b(a){h.empty();var c=e.find("#node_filter").val().toLowerCase();a=VVVV.NodeNames.concat(_(VVVV.ShaderCodeResources).map(function(a,b){return b.replace("%VVVV%/effects/","")}));a=_(_(a).filter(function(a){return 0<=VVVV.Helpers.translateOperators(a).toLowerCase().indexOf(c)})).sortBy(function(a){return a.toLowerCase().indexOf(c)});for(var d=0;d<a.length;d++)h.append($("<option>"+a[d]+"</option>"));h.find("option").first().attr("selected",!0)}
function c(){var b=e.find("#node_filter").val(),h=e.find("#new_node option:selected").val(),g="";h||!b.match(/\.fx$/)&&!b.match(/\.v4p$/)||(h=b);if(h){var k;(k=h.match(/^(.*\/([^\.]+))(\.vvvvjs)?\.fx$/))?(h=k[2]+" (EX9.Effect)",g=k[1]+".fx"):(k=h.match("([^.]+)(.vvvvjs)?.fx$"))?(h=k[1]+" (EX9.Effect)",g="%VVVV%/effects/"+k[1]+".fx"):h.match("[^/].v4p$")&&(g=h=b);s++;h="<PATCH>"+("<NODE componentmode='Hidden' id='"+s+"' nodename='"+h+"' systemname='"+h+"' "+(""!=g?"filename='"+g+"'":"")+">");h+="<BOUNDS type='Node' left='"+
15*d+"' top='"+15*f+"' width='100' height='100'/>"}else s++,h="<PATCH>"+("<NODE componentmode='Hidden' id='"+s+"' nodename='IOBox (String)' systemname='IOBox (String)'>"),h+="<BOUNDS type='Node' left='"+15*d+"' top='"+15*f+"' width='100' height='100'/>",h+="<PIN pinname='Input String' values='|"+b+"|'/>";a.update(q,h+"</NODE>");e.remove()}$("#node_selection",y.window.document).remove();var d=d3.event.pageX,f=d3.event.pageY,e=$('<div id="node_selection"><input type="text" id="node_filter"/></div>'),
h=$('<select id="new_node" size="8">');e.append(h);e.css("left",d);e.css("top",f);$("body",y.window.document).append(e);e.find("input").bind("paste",function(a){a.stopPropagation()});e.find("#node_filter").get(0).focus();b();e.find("#node_filter").keyup(function(a){if(13==a.which)c();else{if(40==a.which)return h.find("option:selected").first().next().attr("selected",!0),!1;if(38==a.which)return h.find("option:selected").first().prev().attr("selected",!0),!1;b()}});e.find("#new_node").click(c)}).on("mousedown",
...
function(a){u=a.shiftKey},$(y.window.document).keydown(k),$(y.window.document).keyup(k),$(y.window.document).keydown(function(b){if(46==b.which&&0<n.length){var d="<PATCH>";for(b=0;b<n.length;b++){var f=n[b],d=d+("<NODE id='"+n[b].id+"' deleteme='pronto'/>");_(f.inputPins).each(function(a){_(a.links).each(function(a){d+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(f.outputPins).each(function(a){_(a.links).each(function(a){d+=
"<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})})}d+="</PATCH>";a.update(q,d);n=[]}else if(115!=b.which&&83!=b.which||!b.ctrlKey){if(73==b.which&&b.ctrlKey)return a.inspector?a.inspector.win.focus():a.openInspector("."),b.preventDefault(),!1;if(65==b.which&&b.ctrlKey)return e(),c.selectAll(".vvvv-node").each(function(a){n.push(a);d3.select(this).attr("class","vvvv-node selected")}),
b.preventDefault(),!1}else return a.save(q.nodename,q.toXML()),b.preventDefault(),!1}),$(y.window).resize(function(){q.windowWidth=$(this).width();q.windowHeight=$(this).height();q.boundingBox.width=Math.max(q.windowWidth,q.boundingBox.width);q.boundingBox.height=Math.max(q.windowHeight,q.boundingBox.height);q.afterUpdate()}));y.drawComplete();q.afterUpdate=function(){y.drawComplete();a.inspector&&window.setTimeout(a.inspector.update,100)};q.afterEvaluate=function(){h.filter(function(a){return a.isIOBox&&
VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel}).each(function(a){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a)})}},1E3);this.close=function(){this.window.close()};this.drawComplete=function(){h&&h.remove();k&&k.remove();g||q.windowWidth==$(y.window).width()&&q.windowHeight==$(y.window).height()||y.window.resizeTo(q.windowWidth,q.windowHeight);c.attr("width",Math.max(q.windowWidth,q.boundingBox.width)).attr("height",Math.max(q.windowHeight,q.boundingBox.height));c.selectAll(".background").attr("width",
Math.max(q.windowWidth,q.boundingBox.width)).attr("height",Math.max(q.windowHeight,q.boundingBox.height));var b=c.append("svg:g").attr("class","link-group");h=c.selectAll("g.vvvv-node").data(q.nodeList).enter().append("svg:g").attr("class",function(a){var b="vvvv-node";a.isIOBox&&(b+=" vvvv-iobox");0<=n.indexOf(a)&&(b+=" selected");return b}).attr("id",function(a){return"vvvv-node-"+a.id}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"});h.append("svg:rect").attr("class","vvvv-node-background").attr("height",
...
("Node"!=a.typeName||VVVV.PinTypes[r.typeName].primitive))||0<=e.indexOf(a.node)||a.node==r.node?!1:!0}).append("svg:rect").attr("class","vvvv-connection-highlight resettable").attr("width",4).attr("height",3).attr("fill","rgba(0,0,0,1)").attr("y",function(a){return a.direction==PinDirection.Input?-3:3})}else{if(r.direction==PinDirection.Input)var f=b,h=r;else f=r,h=b;if(f.typeName==h.typeName&&f.direction!=h.direction||"Node"==f.typeName&&!VVVV.PinTypes[h.typeName].primitive||"Node"==h.typeName&&
!VVVV.PinTypes[f.typeName].primitive){var g="<PATCH>";_(h.links).each(function(a){g+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"});g+="<LINK createme='pronto' srcnodeid='"+f.node.id+"' srcpinname='"+f.pinname+"' dstnodeid='"+h.node.id+"' dstpinname='"+h.pinname+"'/>";g+="</PATCH>";a.update(q,g);c.select(".vvvv-link.current-link").remove();c.select(".vvvv-connection-highlight").remove();
y.state=UIState.Idle}q.afterUpdate()}d3.event.stopPropagation()}).on("mousedown",function(){d3.event.stopPropagation()}),c.selectAll("g.vvvv-link").on("contextmenu",function(b){a.update(q,"<PATCH><LINK deleteme='pronto' srcnodeid='"+b.fromPin.node.id+"' srcpinname='"+b.fromPin.pinname+"' dstnodeid='"+b.toPin.node.id+"' dstpinname='"+b.toPin.pinname+"'/></PATCH>");d3.event.preventDefault()}),h.on("mousedown",function(b){$(".resettable",y.window.document).remove();y.state=UIState.Moving;0>n.indexOf(b)&&
(u||e(),d3.select(this).attr("class","vvvv-node selected"),n.push(b));f=d3.event.pageX;d=d3.event.pageY;a.inspector&&a.inspector.setNode(b);d3.event.preventDefault();d3.event.stopPropagation();return!1}).on("contextmenu",function(b){b.isSubpatch?a.openPatch(b):b.openUIWindow&&b.openUIWindow();d3.event.preventDefault();d3.event.stopPropagation();return!1}))}};
...
VVVV.PinTypes[d.typeName].defaultValue());d.setSliceCount(b);d.node.parentPatch.afterUpdate();a.preventDefault();return!1});for(e=e=0;e<f;e++)if(g=$(c.win.document).find("#values .row").eq(e+1),b||!g.children().first().hasClass("pininputbox")){if(VVVV.PinTypes[a.typeName].openInputBox&&a.direction!=PinDirection.Output&&!a.isConnected()&&void 0!=a.getValue(0)){var n=$('<div class="row value"><div style="height:100%">'+a.getValue(e)+"</div></div>");(function(b){n.find("div").click(function(){VVVV.PinTypes[a.typeName].openInputBox(c.win,
$(this),a,b)})})(e)}else n=$('<div class="row value readonlyvalue"><div style="height:100%">'+a.getValue(e)+"</div></div>");0<g.length?g.replaceWith(n):$(c.win.document).find("#values").append(n);"Color"==a.typeName&&n.find("div").css("background-color","rgba("+_(a.getValue(e).split(",")).map(function(a){return parseInt(255*a)}).join(",")+")")}e++;for(g=$(c.win.document).find("#values .row");e<g.length;e++)g.eq(e).remove()}this.win=window.open(b+"/inspektor.html","inspektor","location=no, width=250, height=600, toolbar=no");
