...
a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(e.outputPins).each(function(a){_(a.links).each(function(a){d+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})})}d+="</PATCH>";b.update(m.nodename,d);p=[]}else if(115!=a.which&&83!=a.which||!a.ctrlKey){if(73==a.which&&a.ctrlKey)return b.inspector?b.inspector.win.focus():b.openInspector("."),
a.preventDefault(),!1;if(65==a.which&&a.ctrlKey)return f(),c.selectAll(".vvvv-node").each(function(a){p.push(a);d3.select(this).attr("class","vvvv-node selected")}),a.preventDefault(),!1}else return b.save(m.nodename,m.toXML()),a.preventDefault(),!1});$(y.window).resize(function(){m.width=$(this).width();m.height=$(this).height();m.boundingBox.width=Math.max(m.width,m.boundingBox.width);m.boundingBox.height=Math.max(m.height,m.boundingBox.height);m.afterUpdate()});y.drawComplete();m.afterUpdate=function(){y.drawComplete();
b.inspector&&window.setTimeout(b.inspector.update,100)};m.afterEvaluate=function(){h.filter(function(a){return a.isIOBox&&VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel}).each(function(a){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a)})}},1E3);this.close=function(){this.window.close()};this.drawComplete=function(){h&&h.remove();k&&k.remove();m.width==$(y.window).width()&&m.height==$(y.window).height()||y.window.resizeTo(m.width,m.height);c.attr("width",Math.max(m.width,m.boundingBox.width)).attr("height",
Math.max(m.height,m.boundingBox.height));c.selectAll(".background").attr("width",Math.max(m.width,m.boundingBox.width)).attr("height",Math.max(m.height,m.boundingBox.height));var a=c.append("svg:g").attr("class","link-group");h=c.selectAll("g.vvvv-node").data(m.nodeList).enter().append("svg:g").attr("class",function(a){var b="vvvv-node";a.isIOBox&&(b+=" vvvv-iobox");0<=p.indexOf(a)&&(b+=" selected");return b}).attr("id",function(a){return"vvvv-node-"+a.id}).attr("transform",function(a){return"translate("+
...
"vvvv-node-pinbar").attr("y",function(a){return a.isIOBox?a.getHeight()-2:a.getHeight()-4}).attr("height",function(a){return a.isIOBox?2:4}).attr("fill",function(a){return a.isIOBox?"#dddddd":"#9a9a9a"}).attr("width",function(a){return a.getWidth()});h.append("svg:text").text(function(a){return a.invisiblePins["Descriptive Name"]?a.invisiblePins["Descriptive Name"].getValue(0):null}).attr("class","vvvv-node-descriptive-name").attr("shape-rendering","crispEdges").attr("dy",function(a){return a.getHeight()+
12}).attr("font-size",10).attr("font-family","Lucida Sans Unicode");h.each(function(a){if(a.isIOBox&&VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a);for(var b=a.IOBoxRows(),c=a.IOBoxInputPin().getSliceCount(),d=this,e=0;e<b;e++)(function(e){d3.select(d).append("svg:rect").attr("width",a.getWidth()+2).attr("height",a.getHeight()/b-4).attr("y",12*e+2).attr("fill","rgba(0,0,0,0)").on("contextmenu",function(a){if(void 0!=a.IOBoxInputPin().getValue(0)){$(".resettable",
y.window.document).remove();var d=$("<input type='text'/>");$("body",y.window.document).append(d);d.css("position","absolute");d.css("left",$(this).offset().left);d.css("top",$(this).offset().top+2);d.css("width",a.getWidth());d.css("height",a.getHeight()/b-4);VVVV.PinTypes[a.IOBoxInputPin().typeName].openInputBox(y.window,d,a.IOBoxInputPin(),e%c)}d3.event.stopPropagation();d3.event.preventDefault();return!1})})(e)}else d3.select(this).append("svg:text").text(function(a){return a.label()}).attr("class",
...
function(a,d){if(y.state!=UIState.Connecting){r=a;y.state=UIState.Connecting;c.append("svg:line").attr("class","vvvv-link current-link resettable").attr("stroke","#000").attr("stroke-width",1).attr("x1",a.x+a.node.x+2+0.5).attr("y1",a.y+a.node.y+2+0.5).attr("x2",a.x+a.node.x+2+0.5).attr("y2",a.y+a.node.y+2+0.5);var e=r.direction==PinDirection.Output?"input":"output",f=r.node.getUpstreamNodes();c.selectAll("g.vvvv-"+e+"-pin").filter(function(a){return a.typeName!=r.typeName&&("Node"!=r.typeName||0<=
q.indexOf(a.typeName))&&("Node"!=a.typeName||0<=q.indexOf(r.typeName))||0<=f.indexOf(a.node)||a.node==r.node?!1:!0}).append("svg:rect").attr("class","vvvv-connection-highlight").attr("width",4).attr("height",3).attr("fill","rgba(0,0,0,1)").attr("y",function(a){return a.direction==PinDirection.Input?-3:3})}else{if(r.direction==PinDirection.Input)var e=a,g=r;else e=r,g=a;if(e.typeName==g.typeName||"Node"==e.typeName&&0>q.indexOf(g.typeName)||"Node"==g.typeName&&0>q.indexOf(e.typeName)){var h="<PATCH>";
_(g.links).each(function(a){h+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"});h+="<LINK createme='pronto' srcnodeid='"+e.node.id+"' srcpinname='"+e.pinname+"' dstnodeid='"+g.node.id+"' dstpinname='"+g.pinname+"'/>";h+="</PATCH>";b.update(m.nodename,h);c.select(".vvvv-link.current-link").remove();c.select(".vvvv-connection-highlight").remove();y.state=UIState.Idle}m.afterUpdate()}d3.event.stopPropagation()}).on("mousedown",
function(){d3.event.stopPropagation()});c.selectAll("g.vvvv-link").on("contextmenu",function(a){b.update(m.nodename,"<PATCH><LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/></PATCH>");d3.event.preventDefault()});h.on("mousedown",function(a){$(".resettable",y.window.document).remove();y.state=UIState.Moving;0>p.indexOf(a)&&(g||f(),d3.select(this).attr("class","vvvv-node selected"),p.push(a));
e=d3.event.pageX;d=d3.event.pageY;b.inspector&&b.inspector.setNode(a);d3.event.preventDefault();d3.event.stopPropagation();return!1}).on("contextmenu",function(a){a.isSubpatch?b.openPatch(a):a.openUIWindow&&a.openUIWindow();d3.event.preventDefault();d3.event.stopPropagation();return!1})}};
