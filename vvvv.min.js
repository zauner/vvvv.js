...
d.delays_output||c(d)});if(e.dirty||e.auto_evaluate||e.isSubpatch){if(d)var g=(new Date).getTime();e.auto_nil&&!e.isSubpatch&&e.hasNilInputs()?_(e.outputPins).each(function(c){c.setSliceCount(0)}):(e.evaluate(),d&&console.log(e.nodename+" / "+e.id+": "+((new Date).getTime()-g)+"ms"),e.dirty=!1,_(e.inputPins).each(function(c){c.changed=!1}))}delete k[e.id];return!0}if(!(0<this.resourcesPending)){if(d)var e=(new Date).getTime();for(var k={},g={},h=0;h<this.nodeList.length;h++){if(0==this.nodeList[h].getDownstreamNodes().length||
this.nodeList[h].auto_evaluate||this.nodeList[h].delays_output)g[this.nodeList[h].id]=this.nodeList[h];k[this.nodeList[h].id]=this.nodeList[h]}d&&console.log("building node maps: "+((new Date).getTime()-e)+"ms");_(g).each(function(d,e,g){void 0!=k[d.id]&&c(d)});d&&(e=(new Date).getTime());this.afterEvaluate();d&&console.log("patch rendering: "+((new Date).getTime()-e)+"ms");d=!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(d=!0)});if(/\.v4p[^<>\s]*$/.test(b)){this.nodename=
b;var r=this;$.ajax({url:b,type:"get",dataType:"text",success:function(c){r.doLoad(c,function(){r.success&&r.success();r.afterUpdate()})}})}else/^ws:\/\//.test(b)?(r=this,VVVV.Editors.Connector.host=b,VVVV.Editors.Connector.enable(this,{success:function(){r.success&&r.success()},error:function(){r.error&&r.error()}})):this.doLoad(b,function(){this.success&&this.success()});e();$(window).bind("hashchange",function(){e()})}};VVVV.Core.Patch.prototype=new VVVV.Core.Node;if(!WebSocket&&MozWebSocket)var WebSocket=MozWebSocket;
...
VVVV.Helpers.translateOperators(a).toLowerCase().indexOf(c)})).sortBy(function(a){return a.toLowerCase().indexOf(c)});for(var d=0;d<a.length;d++)h.append($("<option>"+a[d]+"</option>"));h.find("option").first().attr("selected",!0)}function c(){var b=g.find("#node_filter").val(),h=g.find("#new_node option:selected").val(),k="";h||!b.match(/\.fx$/)&&!b.match(/\.v4p$/)||(h=b);h?((b=h.match(/^(.*\/([^\.]+))(\.vvvvjs)?\.fx$/))?(h=b[2]+" (EX9.Effect)",k=b[1]+".fx"):(b=h.match("([^.]+)(.vvvvjs)?.fx$"))?
(h=b[1]+" (EX9.Effect)",k="%VVVV%/effects/"+b[1]+".fx"):h.match("[^/].v4p$")&&(k=h=f),t++,h="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='"+h+"' systemname='"+h+"' "+(""!=k?"filename='"+k+"'":"")+">"),h+="<BOUNDS type='Node' left='"+15*d+"' top='"+15*e+"' width='100' height='100'/>"):(t++,h="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='IOBox (String)' systemname='IOBox (String)'>"),h+="<BOUNDS type='Node' left='"+15*d+"' top='"+15*e+"' width='100' height='100'/>",h+=
...
c,a,0)}d3.event.stopPropagation();d3.event.preventDefault();return!1});var s=["Generic","Color","Enum"];c.selectAll("g.vvvv-input-pin, g.vvvv-output-pin").on("click",function(b,d){if(z.state!=UIState.Connecting){r=b;z.state=UIState.Connecting;c.append("svg:line").attr("class","vvvv-link current-link resettable").attr("stroke","#000").attr("stroke-width",1).attr("x1",b.x+b.node.x+2+0.5).attr("y1",b.y+b.node.y+2+0.5).attr("x2",b.x+b.node.x+2+0.5).attr("y2",b.y+b.node.y+2+0.5);var e=r.direction==PinDirection.Output?
"input":"output",g=r.node.getUpstreamNodes();c.selectAll("g.vvvv-"+e+"-pin").filter(function(a){return a.typeName!=r.typeName&&("Node"!=r.typeName||0<=s.indexOf(a.typeName))&&("Node"!=a.typeName||0<=s.indexOf(r.typeName))||0<=g.indexOf(a.node)||a.node==r.node?!1:!0}).append("svg:rect").attr("class","vvvv-connection-highlight").attr("width",4).attr("height",3).attr("fill","rgba(0,0,0,1)").attr("y",function(a){return a.direction==PinDirection.Input?-3:3})}else{if(r.direction==PinDirection.Input)var e=
b,h=r;else e=r,h=b;if(e.typeName==h.typeName||"Node"==e.typeName&&0>s.indexOf(h.typeName)||"Node"==h.typeName&&0>s.indexOf(e.typeName)){var k="<PATCH>";_(h.links).each(function(a){k+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"});k+="<LINK createme='pronto' srcnodeid='"+e.node.id+"' srcpinname='"+e.pinname+"' dstnodeid='"+h.node.id+"' dstpinname='"+h.pinname+"'/>";k+="</PATCH>";a.update(n.nodename,
