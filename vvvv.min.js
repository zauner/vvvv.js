...
1);this.fromPin.node.parentPatch.linkList.splice(this.fromPin.node.parentPatch.linkList.indexOf(this),1)};this.serialize=function(){var g=$("<LONK>");g.attr("srcnodeid",this.fromPin.node.id);g.attr("srcpinname",this.fromPin.pinname);g.attr("dstnodeid",this.toPin.node.id);g.attr("dstpinname",this.toPin.pinname);return g}},Patch:function(a,b,g,e,f){function d(c){if(void 0==c)return[];if("45_26">=this.vvvv_version)return separator=/\|/.test(c)?"|":",",c.split(separator).filter(function(c,d){return""!=
c});for(var d=[],h="",f=!1,e=c.length,g=0;g<e;g++)","!=c[g]||f?"|"==c[g]?f=!f:h+=c[g]:(d.push(h),h="");d.push(h);return d}function c(){var c=window.location.hash.match("#([^/]+)/"+n.ressource+"$");c&&(console.log("launching editor ..."),(new VVVV.Editors[c[1]]).enable(n))}this.ressource=a;this.vvvv_version="45_26.1";this.boundingBox={width:0,height:0};this.windowHeight=this.windowWidth=500;this.pinMap={};this.nodeMap={};this.nodeList=[];this.linkList=[];this.success=b;this.error=g;this.XMLCode="";
this.editor=void 0;this.setupObject();e&&(this.parentPatch=e);f&&(this.id=f);var h=!1;this.getAbsolutePath=function(){var c=this.getRelativePath();this.parentPatch&&(c=this.parentPatch.getRelativePath()+c);return c};this.getRelativePath=function(){return this.nodename.match(/(.*\/)?[^/]+\.v4p/)[1]||""};this.destroy=function(){for(var c=0;c<this.nodeList.length;c++)this.nodeList[c].destroy(),delete this.nodeMap[this.nodeList[c].id],delete this.nodeList[c]};if("45_26">=this.vvvv_version)var q={},p=
{},k={},r={};var n=this;this.doLoad=function(c,h){function f(c){"complete"==g&&(p={});$(c).find('link[deleteme="pronto"]').each(function(){for(var c=!1,d=0;d<n.linkList.length;d++)n.linkList[d].fromPin.node.id==$(this).attr("srcnodeid")&&(n.linkList[d].fromPin.pinname==$(this).attr("srcpinname")&&n.linkList[d].toPin.node.id==$(this).attr("dstnodeid")&&n.linkList[d].toPin.pinname==$(this).attr("dstpinname"))&&(c=n.linkList[d]);if(c){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+
" -> "+c.toPin.pinname);var d=c.fromPin,h=c.toPin;c.destroy();d.connectionChanged();h.connectionChanged();h.markPinAsChanged();h.reset_on_disconnect&&h.reset()}});$(c).find('link[deleteme!="pronto"]').each(function(){var c=n.pinMap[$(this).attr("srcnodeid")+"_out_"+$(this).attr("srcpinname")],d=n.pinMap[$(this).attr("dstnodeid")+"_in_"+$(this).attr("dstpinname")];void 0==c&&n.nodeMap[$(this).attr("srcnodeid")]&&(c=n.nodeMap[$(this).attr("srcnodeid")].addOutputPin($(this).attr("srcpinname"),void 0));
void 0==d&&n.nodeMap[$(this).attr("dstnodeid")]&&(d=n.nodeMap[$(this).attr("dstnodeid")].addInputPin($(this).attr("dstpinname"),void 0));if(c&&d){for(var h=!1,f=0;f<n.linkList.length;f++)n.linkList[f].fromPin.node.id==c.node.id&&(n.linkList[f].fromPin.pinname==c.pinname&&n.linkList[f].toPin.node.id==d.node.id&&n.linkList[f].toPin.pinname==d.pinname)&&(h=n.linkList[f]);if(!h){h=new VVVV.Core.Link(c,d);c.connectionChanged();d.connectionChanged();n.linkList.push(h);for(f=0;f<c.values.length;f++)d.setValue(f,
c.getValue(f));d.setSliceCount(c.getSliceCount())}"complete"==g&&(p[c.node.id+"_"+c.pinname+"-"+d.node.id+"_"+d.pinname]=h)}});"complete"==g&&(_(q).each(function(c,d){if(void 0==p[d]){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var h=c.fromPin,f=c.toPin;c.destroy();h.connectionChanged();f.connectionChanged();f.markPinAsChanged();f.reset_on_disconnect&&f.reset()}}),q={},_(p).each(function(c,d){q[d]=c}))}var e=this;do e.dirty=!0;while(e=e.parentPatch);
if(e=/^<!DOCTYPE\s+PATCH\s+SYSTEM\s+"(.+)\\(.+)\.dtd/.exec(c))n.vvvv_version=e[2].replace(/[a-zA-Z]+/,"_");var g="diff";if(/\s<PATCH/.test(c)||"45_26">=n.vvvv_version)g="complete","development"==VVVV_ENV&&console.log("complete: "+this.nodename),n.XMLCode=c;e=$(c).find('bounds[type="Window"]').first();0<e.length&&(n.windowWidth=e.attr("width")/15,n.windowHeight=e.attr("height")/15);"complete"==g&&(r={});var a=0;$(c).find("node").each(function(){if("pronto"==$(this).attr("createme")&&void 0!=n.nodeMap[$(this).attr("id")]){var e=
n.nodeMap[$(this).attr("id")];"development"==VVVV_ENV&&console.log("node renamed, so deleting node "+e.id+" / "+e.nodename);_(e.inputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.fromPin.connectionChanged()})});_(e.outputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.toPin.connectionChanged();c.toPin.markPinAsChanged();c.toPin.reset_on_disconnect&&c.toPin.reset()})});n.nodeList.splice(n.nodeList.indexOf(e),1);delete n.nodeMap[e.id]}var q;q="InABox"==$(this).attr("componentmode")?
$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var p=void 0!=n.nodeMap[$(this).attr("id")];if(p)e=n.nodeMap[$(this).attr("id")];else{nodename=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==nodename)return;if(void 0!=VVVV.NodeLibrary[nodename.toLowerCase()])e=new (VVVV.NodeLibrary[nodename.toLowerCase()])($(this).attr("id"),n),VVVV.NodeLibrary[nodename.toLowerCase()].requirements&&(n.resourcesPending++,_(VVVV.NodeLibrary[nodename.toLowerCase()].requirements).each(function(c){void 0===
VVVV.LoadedLibs[c]&&VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){n.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))e=new VVVV.Nodes.GenericShader($(this).attr("id"),n),e.isShader=!0,e.shaderFile=$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),e.nodename=nodename;else if(/.v4p$/.test($(this).attr("filename"))){n.resourcesPending++;a++;var k=this,e=new VVVV.Core.Patch($(this).attr("filename"),function(){n.resourcesPending--;
a--;"development"==VVVV_ENV&&console.log(this.nodename+"invoking update links");f(c);n.editor&&n.editor.addPatch(this);if(this.auto_evaluate){var d=n;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(n.mainloop);n.afterUpdate();0>=n.resourcesPending&&h&&(h(),h=void 0)},function(){n.resourcesPending--;a--;this.not_implemented=!0;VVVV.onNotImplemented(nodename);n.afterUpdate();0>=n.resourcesPending&&h&&(h(),h=void 0)},n,$(k).attr("id"));e.isSubpatch=!0;n.editor&&!e.editor&&n.editor.addPatch(e);
n.nodeMap[e.id]=e}else e=new VVVV.Core.Node($(this).attr("id"),nodename,n),e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&n.editor.sendUndo(),VVVV.onNotImplemented(nodename);"development"==VVVV_ENV&&"complete"!=g&&console.log(n.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){k=n;do k.auto_evaluate=!0;while(k=k.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);
k=e.getSubPatches();k.push(e);for(var b,m=0;m<k.length;m++)b=VVVV.Helpers.prepareFilePath(k[m].nodename,k[m].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(e),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}n.nodeList.splice(n.nodeList.indexOf(e),1);e.destroy();delete n.nodeMap[e.id]}0<q.length&&(q.attr("left")&&(e.x=q.attr("left")/15,e.y=q.attr("top")/15,n.boundingBox.width=Math.max(n.boundingBox.width,e.x+100),n.boundingBox.height=Math.max(n.boundingBox.height,e.y+100)),q.attr("width")&&
(e.width=q.attr("width"),e.height=q.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&(e.isIOBox=!0);p||e.setup();k=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var g=0;g<f.length;g++)e.invisiblePins[h].values[g]!=f[g]&&e.invisiblePins[h].setValue(g,
f[g]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==$(this).attr("visible")&&"Configuration"!=$(this).attr("pintype")||e.isSubpatch?"Output"==$(this).attr("pintype")||0<$(c).find("link[srcnodeid="+e.id+"]").filter("link[srcpinname='"+h.replace(/[\[\]]/,"")+"']").length?void 0==e.outputPins[h]&&e.addOutputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInvisiblePin(h,f)});p||(e.initialize(),n.nodeList.push(e));
"complete"==g&&(r[e.id]=e)});"complete"==g&&(_(k).each(function(c,d){void 0==r[d]&&("development"==VVVV_ENV&&console.log("removing node "+c.id),n.nodeList.splice(n.nodeList.indexOf(c),1),delete n.nodeMap[c.id])}),k={},_(r).each(function(c,d){k[d]=c}));0===a&&f(c);0>=this.resourcesPending&&h&&(h(),h=void 0)};this.getSubPatches=function(){for(var c=[],d=0;d<this.nodeList.length;d++)this.nodeList[d].isSubpatch&&(c.push(this.nodeList[d]),c=c.concat(this.nodeList[d].getSubPatches()));return c};this.setMainloop=
...
function(){function c(d){var e=d.getUpstreamNodes();_(e).each(function(d){void 0==f[d.id]||d.delays_output||c(d)});if(d.dirty||d.auto_evaluate||d.isSubpatch){if(h)var g=(new Date).getTime();if(d.auto_nil&&!d.isSubpatch&&d.hasNilInputs())_(d.outputPins).each(function(c){c.setSliceCount(0)});else{try{d.evaluate()}catch(q){console.log("VVVV.Js / Error evaluating "+d.nodename+": "+q.message)}h&&console.log(d.nodename+" / "+d.id+": "+((new Date).getTime()-g)+"ms");d.dirty=!1;_(d.inputPins).each(function(c){c.changed=
!1})}}delete f[d.id];return!0}if(!(0<this.resourcesPending)){if(h)var d=(new Date).getTime();for(var f={},e={},g=0;g<this.nodeList.length;g++){if(0==this.nodeList[g].getDownstreamNodes().length||this.nodeList[g].auto_evaluate||this.nodeList[g].delays_output)e[this.nodeList[g].id]=this.nodeList[g];f[this.nodeList[g].id]=this.nodeList[g]}h&&console.log("building node maps: "+((new Date).getTime()-d)+"ms");_(e).each(function(d,h,e){void 0!=f[d.id]&&c(d)});h&&(d=(new Date).getTime());this.afterEvaluate();
h&&console.log("patch rendering: "+((new Date).getTime()-d)+"ms");h=!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(h=!0)});if(/\.v4p[^<>\s]*$/.test(a)){this.nodename=a;var m=this,t=a;this.parentPatch&&(t=VVVV.Helpers.prepareFilePath(a,this.parentPatch));VVVV.Patches[t]?m.doLoad(VVVV.Patches[t][0].toXML(),function(){VVVV.Patches[t].push(m);m.success&&m.success();m.afterUpdate()}):$.ajax({url:t,type:"get",dataType:"text",success:function(c){m.doLoad(c,function(){VVVV.Patches[t]=
VVVV.Patches[t]||[];VVVV.Patches[t].push(m);m.success&&m.success();m.afterUpdate()})},error:function(){m.error&&m.error()}})}else/^ws:\/\//.test(a)?(m=this,VVVV.Editors.Connector.host=a,VVVV.Editors.Connector.enable(this,{success:function(){m.success&&m.success()},error:function(){m.error&&m.error()}})):this.doLoad(a,function(){this.success&&this.success()});c();$(window).bind("hashchange",function(){c()})}};VVVV.Core.Patch.prototype=new VVVV.Core.Node;VVVV.Core.DOMInterface=function(a){function b(d){if("event"==d.property_class){var c=d.selector;"window"==c&&(c=window);"document"==c&&(c=document);$(c).each(function(c){d.values[c]=0;$(this).bind(d.property,function(){d.values[c]=1;return!1})})}}var g={},e={};a.domInterface=this;this.connect=function(d){_(g).each(function(c,h){c.node.id==d.id&&("event"==c.property_class&&$(c.selector).unbind(c.property),delete g[h])});_(e).each(function(c,h){c.node.id==d.id&&delete e[h]});var c=/([^\/]+)(\/(event|attribute|style)\/(.+))?/.exec(d.invisiblePins["Descriptive Name"].getValue(0));
...
VVVV.Core.MainLoop=function(a,b){function g(){if(0==a.resourcesPending){e++;var q=(new Date).getTime();f.populateInputConnectors();a.evaluate();f.processOutputConnectors();var p=(new Date).getTime()-q;0==e%10&&(h&&(console.log(1/((q-c)/1E4)+"fps"),h=!1),c=q)}this.deltaT=Math.max(0,Math.round(1E3/VVVV.fps-p));d&&window.setTimeout(g,this.deltaT)}VVVV.fps=b||60;var e=0,f=new VVVV.Core.DOMInterface(a),d=!0,c=0,h=!1;this.deltaT=1E3/VVVV.fps;a.setMainloop(this);this.stop=function(){d=!1};this.start=function(){d||
...
q=this.addOutputPin("Length",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Angle",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=parseFloat(g.getValue(b)),m=parseFloat(e.getValue(b)),t=parseFloat(f.getValue(b)),s=parseFloat(d.getValue(b)),t=t-n,s=s-m;c.setValue(b,n+t/2);h.setValue(b,m+s/2);q.setValue(b,Math.sqrt(t*t+s*s));p.setValue(b,Math.atan2(s,t)/(2*Math.PI))}c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a);p.setSliceCount(a)}};
...
q=this.addOutputPin("X2",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Y2",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=parseFloat(g.getValue(b)),m=parseFloat(e.getValue(b)),t=parseFloat(f.getValue(b)),s=2*parseFloat(d.getValue(b)-0.25)*Math.PI,v=t/2*Math.sin(s),t=-t/2*Math.cos(s);c.setValue(b,n+v);h.setValue(b,m+t);q.setValue(b,n-v);p.setValue(b,m-t)}c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a);p.setSliceCount(a)}};
...
this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var k=2*(g.getValue(b)+0.25)*Math.PI,r=2*e.getValue(b)*Math.PI,n=f.getValue(b);h.setValue(b,-n*Math.cos(r)*Math.sin(k));d.setValue(b,-n*Math.sin(r)*Math.sin(k));c.setValue(b,-n*Math.cos(k))}d.setSliceCount(a);c.setSliceCount(a);h.setSliceCount(a)}};VVVV.Nodes.Cartesian3d.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),q=this.addOutputPin("Cycles",[0],VVVV.PinTypes.Value),p=[],k=[],r=(new Date).getTime(),n=(new Date).getTime();this.evaluate=function(){var a=this.getMaxInputSliceCount();r=(new Date).getTime()-n;for(var b=0;b<a;b++){var s=g.getValue(b%g.values.length),v=e.getValue(b%e.values.length),u=f.getValue(b%f.values.length),w=d.getValue(b%d.values.length),l=c.getValue(b%c.values.length);void 0==p[b]&&(p[b]=0);void 0==k[b]&&(k[b]=0);0.5>v&&(0!=s&&isFinite(s))&&(dv=1/(1E3*s)*r,0<u&&(dv*=
-1),p[b]+=dv,0>p[b]&&(k[b]-=Math.ceil(-p[b]),p[b]=1+p[b]),1<p[b]&&(k[b]+=Math.floor(p[b])));n=(new Date).getTime();0.5<=w&&(p[b]=0,k[b]=0);if(0.5>v||0.5<=w)h.setValue(b,(p[b]+l)%1),q.setValue(b,k[b]);p[b]%=1}h.setSliceCount(a);q.setSliceCount(a);p.splice(a);k.splice(a)}};VVVV.Nodes.LFO.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],q=[],p=[],k=[],r=[],n=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||e.pinIsChanged(),s=0;s<a;s++){void 0==c[s]&&(c[s]=(new Date).getTime());var v=this.parentPatch.mainloop.deltaT,u=parseFloat(g.getValue(s)),w=parseFloat(e.getValue(s));void 0==p[s]&&(p[s]=u);if(u!=h[s]||w!=q[s])r[s]=void 0,h[s]=u,q[s]=w,n[s]=0<=h[s]-p[s],k[s]=0<q[s]?(h[s]-p[s])/(1E3*q[s]):0;n[s]==0<=h[s]-p[s]&&(p[s]+=
k[s]*v);void 0!=r[s]&&0<=h[s]-p[s]!=0<=r[s]&&(k[s]=0,p[s]=h[s]);0!=r[s]&&(f.setValue(s,p[s]),d.setValue(s,k[s]));r[s]=h[s]-p[s];c[s]=(new Date).getTime()}b&&f.getSliceCount()!=a&&(h.splice(a),q.splice(a),c.splice(a),p.splice(a),k.splice(a),r.splice(a),f.setSliceCount(a))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
VVVV.Nodes.Damper=function(a,b){this.constructor(a,"Damper (Animation)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Cyclic Pin not implemented","Acceleration Out is not set yet"]};this.auto_evaluate=!0;var g=this.addInputPin("Go To Position",[0],VVVV.PinTypes.Value),e=this.addInputPin("FilterTime",[1],VVVV.PinTypes.Value),f=this.addOutputPin("Position Out",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Velocity Out",[0],VVVV.PinTypes.Value);
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],q=[],p=[],k=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||e.pinIsChanged(),m=0;m<a;m++){void 0==c[m]&&(c[m]=(new Date).getTime());var t=this.parentPatch.mainloop.deltaT,s=parseFloat(g.getValue(m)),v=parseFloat(e.getValue(m));void 0==h[m]&&(h[m]=s);p[m]=void 0;k[m]=0<=s-h[m];q[m]=0<v?(s-h[m])/(1E3*v):0;k[m]==0<=s-h[m]&&(h[m]+=q[m]*t);void 0!=p[m]&&0<=s-h[m]!=0<=p[m]&&(q[m]=0,h[m]=s);
0!=p[m]&&(f.setValue(m,h[m]),d.setValue(m,q[m]));p[m]=s-h[m];c[m]=(new Date).getTime()}b&&f.getSliceCount()!=a&&(c.splice(a),h.splice(a),q.splice(a),p.splice(a),f.setSliceCount(a))}};VVVV.Nodes.Damper.prototype=new VVVV.Core.Node;
...
p.pinIsChanged(),v=0;v<a;v++)1!=n.getValue(v)&&m||n.setValue(v,0),1!=r.getValue(v)&&m||r.setValue(v,0),m||k.setValue(v,0),b=b||0.5<=g.getValue(v)||0.5<=e.getValue(v);if(b){for(v=0;v<a;v++){var b=parseFloat(c.getValue(v)),u=v>=k.getSliceCount()?0:parseFloat(k.getValue(v)),w=parseFloat(d.getValue(v)),l=parseFloat(f.getValue(v)),A=0;"Unlimited"==p.getValue(v)&&(A=1);"Clamp"==p.getValue(v)&&(A=2);switch(A){case 1:0.5<=g.getValue(v)&&(u+=b);0.5<=e.getValue(v)&&(u-=b);break;case 2:0.5<=g.getValue(v)&&(u+=
b);0.5<=e.getValue(v)&&(u-=b);u>w&&(u=w,n.setValue(v,1));u<l&&(u=l,r.setValue(v,1));break;default:0.5<=g.getValue(v)&&(u+=b),0.5<=e.getValue(v)&&(u-=b),u>w&&(u=l,n.setValue(v,1)),u<l&&(u=w,r.setValue(v,1))}k.getValue(v)!=u&&k.setValue(v,u);0.5<=q.getValue(v)&&k.setValue(v,h.getValue(v))}k.setSliceCount(a);r.setSliceCount(a);n.setSliceCount(a)}m=!0}};VVVV.Nodes.Counter.prototype=new VVVV.Core.Node;VVVV.Nodes.CurrentTimeAstronomy=function(a,b){this.constructor(a,"CurrentTime (Astronomy)",b);this.meta={authors:["Zauner"],original_authors:["VVVV Group"],credits:["http://javascript.about.com/library/bldst.htm"],compatibility_issues:["Time Zone index differs from classic VVVV","Time Zone Name does not work in Firefox"]};this.auto_evaluate=!0;var g=new Date,e=this.addOutputPin("GMT",[],VVVV.PinTypes.Value),f=this.addOutputPin("Time Zone",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Time Zone Name",
...
c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==p[a]&&(p[a]=new k),p[a].transform=e.getValue(a),p[a].width=d.getValue(a),p[a].height=c.getValue(a),p[a].cornerRadius=h.getValue(a),p[a].renderState=g.getValue(a),f.isConnected()&&(p[a].clippingLayer=f.getValue(a));for(a=0;a<p.length;a++)q.setValue(a,p[a]);q.setSliceCount(b)}};VVVV.Nodes.RectangleCanvas.prototype=new VVVV.Core.Node;
...
c.strokeText(this.text,0,0);c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==k[a]&&(k[a]=new r),k[a].transform=e.getValue(a),k[a].text=d.getValue(a),k[a].font=c.getValue(a),k[a].align=h.getValue(a),k[a].baseline=q.getValue(a),k[a].renderState=g.getValue(a),k[a].clippingLayer=f.getValue(a);for(a=0;a<k.length;a++)p.setValue(a,k[a]);p.setSliceCount(b)}};VVVV.Nodes.TextCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Value),c=this.addOutputPin("Status",[""],VVVV.PinTypes.String),h=this.addOutputPin("Body",[""],VVVV.PinTypes.String),q=this.addOutputPin("Fail",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Success",[0],VVVV.PinTypes.Value),k,r,n,m=!1;this.evaluate=function(){this.getMaxInputSliceCount();var a=g.pinIsChanged()||e.pinIsChanged()||f.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==p.getValue(0)&&p.setValue(0,0);1==q.getValue(0)&&q.setValue(0,0);m&&(h.setValue(0,k),c.setValue(0,void 0),
p.setValue(0,r),q.setValue(0,n),m=!1);a&&(void 0==g.getValue(0)?(h.setValue(0,""),c.setValue(0,"")):$.ajax({url:g.getValue(0),type:"get",success:function(c,d,h){k=c;r=1;m=!0},error:function(c,d){k="";n=1;m=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
[0],VVVV.PinTypes.Value),c=this.addInputPin("Spread Count",[1],VVVV.PinTypes.Value),h=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=parseInt(c.getValue(0)),b=parseFloat(e.getValue(0)),k=parseFloat(d.getValue(0)),r=parseFloat(g.getValue(0)),n=f.getValue(0);""==n&&(n="Centered");var m=b/a;"Block"==n&&(m=b/(a-1));var t=m/2;if("Block"==n||"LeftJustified"==n)t=0;"RightJustified"==n&&(t=m);for(var s,v=0;v<a;v++)s=v*m+t,"Block"!=n&&0!=b&&(s=(s+k*b)%b),s=r-b/2+s,h.setValue(v,
...
c=parseInt(this.sprcntin.getValue(0)),h=2*Math.PI,d=h*d,q=0,p=Math.max(this.inpyin.getSliceCount(),this.inpxin.getSliceCount()),k=0;k<p*c;k++)for(var r=parseFloat(this.inpxin.getValue(k)),n=parseFloat(this.inpyin.getValue(k)),m=0;m<c;m++){var t=d+h*(m/c)*f,s=r+Math.cos(t)*a,t=n+Math.sin(t)*b;this.xout.setValue(q,s);this.yout.setValue(q,t);q++}this.xout.setSliceCount(c*p);this.yout.setSliceCount(c*p)}};VVVV.Nodes.CircularSpread.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var d=this.getMaxInputSliceCount(),c=0;c<d;c++){var a=[],b=e.getValue(c);void 0==b&&(b="");for(var p=0;p<g.length;p++)a.push(g[p].getValue(c));f.setValue(c,a.join(b))}f.setSliceCount(d)}};VVVV.Nodes.AddString.prototype=new VVVV.Core.Node;
...
c=this.addOutputPin("Hits",[0],VVVV.PinTypes.Value),h=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),q=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Found At Position",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=this.getMaxInputSliceCount();c.setSliceCount(a);var b=f.getValue(0);d.getValue(0);for(var n=0,m=0;m<a;m++){var t=g.getValue(m),s=1;if("MatchesAny"==b||"ContainsAny"==b)s=e.getSliceCount();for(var v=0,u=0;u<s;u++){var w=e.getValue(u+m),
l=[];if("Matches"==b||"MatchesAny"==b)w==t&&l.push(-1);else for(var w=RegExp(w,"gi"),A;A=w.exec(t);)l.push(A.index);for(w=0;w<l.length;w++)h.setValue(n,m%g.getSliceCount()),q.setValue(n,(m+u)%e.getSliceCount()),p.setValue(n,l[w]+1),n++,v++}c.setValue(m,v)}h.setSliceCount(n);q.setSliceCount(n);p.setSliceCount(n)}};VVVV.Nodes.SiftString.prototype=new VVVV.Core.Node;
...
UnderScore:"_",Minus:"-"},k={Single:"'",Double:'"'};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,m=0;m<a;m++){var t=g.getValue(m),s=p[e.getValue(m)],v=f.getValue(m),u=k[d.getValue(m)],w=c.getValue(m);""==s&&(s=v);""==s&&(s=void 0);t=t.split(s);s=t.length;u=RegExp("[^"+u+"]*","g");for(v=0;v<s;v++)1==w?h.setValue(b,t[v]):h.setValue(b,t[v].match(u).join("")),q.setValue(b,m),b++}h.setSliceCount(b);q.setSliceCount(b)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
[1],VVVV.PinTypes.Value),h=this.addInputPin("Mapping",["Float"],VVVV.PinTypes.Enum);h.enumOptions=["Float","Wrap","Clamp"];var q=this.addOutputPin("Output",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=parseFloat(g.getValue(b)),n=parseFloat(e.getValue(b)),m=parseFloat(f.getValue(b)),t=parseFloat(d.getValue(b)),s=parseFloat(c.getValue(b)),v=h.getValue(b);if(m==n)q.setValue(b,t);else{r-=n;m-=n;switch(v){case "Clamp":r=Math.max(0,Math.min(m,
...
function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var p=g.getValue(b),k=e.getValue(b),r=f.getValue(b),n=d.getValue(b),m=c.values[b];void 0==m&&c.setValue(b,0);if(0.5<=p){m=c.values[b];do p=Math.random()*r,0.5<=k&&(p=Math.round(p));while(0.5<=n&&p==m);c.setValue(b,p)}}c.setSliceCount(a)}};VVVV.Nodes.RandomValue.prototype=new VVVV.Core.Node;
...
b=parseInt(e.getValue(0)),p=[],k=[],r=[],n=0,m=0;m<b;m++)for(var t=0;t<a;t++)p.push(parseFloat(t)/(a-1)-0.5),p.push(0.5-parseFloat(m)/(b-1)),p.push(0),n++,k.push(0),k.push(0),k.push(1),r.push(parseFloat(t)/(a-1)),r.push(parseFloat(m)/(b-1));p=new VVVV.Types.VertexBuffer(c,p);p.setSubBuffer("TEXCOORD0",2,r);p.setSubBuffer("NORMAL",3,k);p.create();k=[];for(m=0;m<b-1;m++)for(t=0;t<a-1;t++)r=t+a*m,k.push(r),k.push(r+1),k.push(r+a+1),k.push(r+a+1),k.push(r+a),k.push(r);d=new VVVV.Types.Mesh(c,p,k);f.setValue(0,
...
this.renderContexts[0];if(a){for(var b=parseInt(e.getValue(0)),p=parseInt(f.getValue(0)),k=parseFloat(g.getValue(0)),r=[],n=[],m=[],t=0;t<p+1;t++)for(var s=Math.cos(-parseFloat(t)/p*Math.PI),v=0;v<b;v++){var u=Math.cos(2*(parseFloat(v)/b)*Math.PI)*Math.cos(Math.asin(s)),w=Math.sin(2*(parseFloat(v)/b)*Math.PI)*Math.cos(Math.asin(s));r.push(u*k);r.push(s*k);r.push(w*k);n.push(u);n.push(s);n.push(w);m.push(parseFloat(v)/b);m.push(parseFloat(t)/p)}k=new VVVV.Types.VertexBuffer(a,r);k.setSubBuffer("TEXCOORD0",
2,m);k.setSubBuffer("NORMAL",3,n);k.create();n=[];for(t=0;t<p;t++)for(v=0;v<b;v++)m=b*t,r=v+m,n.push(r),n.push((r+1)%b+m),n.push((r+1)%b+b+m),n.push((r+1)%b+b+m),n.push(r+b),n.push(r);c=new VVVV.Types.Mesh(a,k,n);d.setValue(0,c)}}}};VVVV.Nodes.Sphere.prototype=new VVVV.Core.Node;
...
l=[];if(0.5<c.getValue(0))for(z=0;2>z;z++)for(C=0;C<b;C++)l.push(z),l.push(2+C+z+z*m*(b+1)),l.push(2+C+(1-z)+z*m*(b+1));for(y=0;y<m;y++)for(C=0;C<b;C++)z=C+(b+1)*y+2,l.push(z),l.push(z+1),l.push(z+b+2),l.push(z+b+2),l.push(z+b+1),l.push(z);k=new VVVV.Types.Mesh(a,t,l);p.setValue(0,k)}}}};VVVV.Nodes.Cylinder.prototype=new VVVV.Core.Node;
...
p=!0;this.parentPatch.afterUpdate()};this.addUniformPins=function(){h.extractSemantics(q);for(var a=[],c=0;c<k.length;c++)h.uniformSpecs[k[c].pinname.replace(/ /g,"_")]||(r.removeInputPin(k[c].pinname),a.push(c));for(c=0;c<a.length;c++)k.splice(a[c],1);_(h.uniformSpecs).each(function(a){if("VIEW"!=a.semantic&&"PROJECTION"!=a.semantic&&"WORLD"!=a.semantic){var c=VVVV.PinTypes.Value,b=[];switch(a.type){case "mat":c=VVVV.PinTypes.Transform;break;case "samplerCube":case "sampler":c=VVVV.PinTypes.WebGlTexture;
...
VVVV.Editors.BrowserEditor.PatchWindow=function(a,b,g){function e(){c.selectAll(".vvvv-node.selected").attr("class",function(a){return a.isIOBox?"vvvv-node vvvv-iobox":"vvvv-node"});n=[]}this.state=UIState.Idle;var f=0,d=0,c,h,q,p,k,r=void 0,n=[],m=a,t=0,s=VVVV.Root+"/patch.html",v=!1,u=0,w=0,l=0,A=0;a.isSubpatch&&(s="patch.html");this.window=g?window:window.open(s,a.nodename,"location=no, width="+a.windowWidth+", height="+a.windowHeight+", toolbar=no");var y=this;window.setTimeout(function(){for(var k=
...
t;$(this).attr("createme","pronto");$(this).attr("id",t);var a=parseInt($(this).find("bounds").attr("left"))-d+15*p,b=parseInt($(this).find("bounds").attr("top"))-f+15*q;$(this).find("bounds").attr("left",a);$(this).find("bounds").attr("top",b)});$patch.find("link").each(function(){$(this).attr("createme","pronto");var a=$(this).attr("srcnodeid"),b=$(this).attr("dstnodeid");$(this).attr("srcnodeid",g[a]);$(this).attr("dstnodeid",g[b])});a=$patch.wrapAll("<d></d>").parent().html();b.update(m,a);e();
...
y.state=UIState.Idle);d3.event.stopPropagation();d3.event.preventDefault();return!1}).on("mouseup",function(){if(y.state==UIState.Moving){y.state=UIState.Idle;for(var a=d3.event.pageX-f,e=d3.event.pageY-d,g="<PATCH>",h=0;h<n.length;h++)var k=n[h],g=g+("<NODE componentmode='Node' id='"+k.id+"'><BOUNDS type='Node' left='"+15*(a+k.x)+"' top='"+15*(e+k.y)+"' width='"+k.width+"' height='"+k.height+"'/></NODE>");b.update(m,g+"</PATCH>")}y.state==UIState.AreaSelecting&&(c.select(".selection-area").remove(),
...
function(a){v=a.shiftKey},$(y.window.document).keydown(k),$(y.window.document).keyup(k),$(y.window.document).keydown(function(a){if(46==a.which&&0<n.length){var d="<PATCH>";for(a=0;a<n.length;a++){var f=n[a],d=d+("<NODE id='"+n[a].id+"' deleteme='pronto'/>");_(f.inputPins).each(function(a){_(a.links).each(function(a){d+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(f.outputPins).each(function(a){_(a.links).each(function(a){d+=
"<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})})}d+="</PATCH>";b.update(m,d);n=[]}else if(115!=a.which&&83!=a.which||!a.ctrlKey){if(73==a.which&&a.ctrlKey)return b.inspector?b.inspector.win.focus():b.openInspector("."),a.preventDefault(),!1;if(65==a.which&&a.ctrlKey)return e(),c.selectAll(".vvvv-node").each(function(a){n.push(a);d3.select(this).attr("class","vvvv-node selected")}),
...
Math.max(m.windowWidth,m.boundingBox.width)).attr("height",Math.max(m.windowHeight,m.boundingBox.height));var a=c.append("svg:g").attr("class","link-group");h=c.selectAll("g.vvvv-node").data(m.nodeList).enter().append("svg:g").attr("class",function(a){var b="vvvv-node";a.isIOBox&&(b+=" vvvv-iobox");0<=n.indexOf(a)&&(b+=" selected");return b}).attr("id",function(a){return"vvvv-node-"+a.id}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"});h.append("svg:rect").attr("class","vvvv-node-background").attr("height",
...
function(){d3.event.stopPropagation()}),c.selectAll("g.vvvv-link").on("contextmenu",function(a){b.update(m,"<PATCH><LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/></PATCH>");d3.event.preventDefault()}),h.on("mousedown",function(a){$(".resettable",y.window.document).remove();y.state=UIState.Moving;0>n.indexOf(a)&&(v||e(),d3.select(this).attr("class","vvvv-node selected"),n.push(a));f=d3.event.pageX;
