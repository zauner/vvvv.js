...
delete this.nodeList[c]};if("45_26">=this.vvvv_version)var m={},p={},k={},r={};var n=this;this.doLoad=function(c,g){function f(c){"complete"==h&&(p={});$(c).find('link[deleteme="pronto"]').each(function(){for(var c=!1,d=0;d<n.linkList.length;d++)n.linkList[d].fromPin.node.id==$(this).attr("srcnodeid")&&(n.linkList[d].fromPin.pinname==$(this).attr("srcpinname")&&n.linkList[d].toPin.node.id==$(this).attr("dstnodeid")&&n.linkList[d].toPin.pinname==$(this).attr("dstpinname"))&&(c=n.linkList[d]);if(c){"development"==
...
c.getValue(f));d.setSliceCount(c.getSliceCount())}"complete"==h&&(p[c.node.id+"_"+c.pinname+"-"+d.node.id+"_"+d.pinname]=g)}});"complete"==h&&(_(m).each(function(c,d){if(void 0==p[d]){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var g=c.fromPin,f=c.toPin;c.destroy();g.connectionChanged();f.connectionChanged();f.markPinAsChanged();f.reset_on_disconnect&&f.reset()}}),m={},_(p).each(function(c,d){m[d]=c}))}var e=this;do e.dirty=!0;while(e=e.parentPatch);
...
VVVV.Core.MainLoop=function(b,a){function h(){if(0==b.resourcesPending){e++;var a=(new Date).getTime();c&&(p.deltaT=a-c);c=a;f.populateInputConnectors();b.evaluate();f.processOutputConnectors();a=(new Date).getTime()-c;0==e%10&&(m&&(console.log(1/((c-g)/1E4)+"fps"),m=!1),g=c)}d&&window.setTimeout(h,Math.max(0,Math.round(1E3/VVVV.fps-a)))}VVVV.fps=a||60;var e=0,f=new VVVV.Core.DOMInterface(b),d=!0,c=void 0,g=0,m=!1;this.deltaT=1E3/VVVV.fps;b.setMainloop(this);var p=this;this.stop=function(){d=!1};
...
m=this.addOutputPin("Length",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Angle",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var n=parseFloat(h.getValue(a)),q=parseFloat(e.getValue(a)),s=parseFloat(f.getValue(a)),v=parseFloat(d.getValue(a)),s=s-n,v=v-q;c.setValue(a,n+s/2);g.setValue(a,q+v/2);m.setValue(a,Math.sqrt(s*s+v*v));p.setValue(a,Math.atan2(v,s)/(2*Math.PI))}c.setSliceCount(b);g.setSliceCount(b);m.setSliceCount(b);p.setSliceCount(b)}};
...
m=this.addOutputPin("X2",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Y2",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var n=parseFloat(h.getValue(a)),q=parseFloat(e.getValue(a)),s=parseFloat(f.getValue(a)),v=2*parseFloat(d.getValue(a)-0.25)*Math.PI,u=s/2*Math.sin(v),s=-s/2*Math.cos(v);c.setValue(a,n+u);g.setValue(a,q+s);m.setValue(a,n-u);p.setValue(a,q-s)}c.setSliceCount(b);g.setSliceCount(b);m.setSliceCount(b);p.setSliceCount(b)}};
...
[0],VVVV.PinTypes.Value),m=this.addOutputPin("Cycles",[0],VVVV.PinTypes.Value),p=[],k=[],r=(new Date).getTime(),n=(new Date).getTime();this.evaluate=function(){var b=this.getMaxInputSliceCount();r=(new Date).getTime()-n;for(var a=0;a<b;a++){var v=h.getValue(a%h.values.length),u=e.getValue(a%e.values.length),t=f.getValue(a%f.values.length),w=d.getValue(a%d.values.length),l=c.getValue(a%c.values.length);void 0==p[a]&&(p[a]=0);void 0==k[a]&&(k[a]=0);0.5>u&&(0!=v&&isFinite(v))&&(dv=1/(1E3*v)*r,0<t&&(dv*=
-1),p[a]+=dv,0>p[a]&&(k[a]-=Math.ceil(-p[a]),p[a]=1+p[a]),1<p[a]&&(k[a]+=Math.floor(p[a])));n=(new Date).getTime();0.5<=w&&(p[a]=0,k[a]=0);if(0.5>u||0.5<=w)g.setValue(a,(p[a]+l)%1),m.setValue(a,k[a]);p[a]%=1}g.setSliceCount(b);m.setSliceCount(b);p.splice(b);k.splice(b)}};VVVV.Nodes.LFO.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],g=[],m=[],p=[],k=[],r=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=h.pinIsChanged()||e.pinIsChanged(),s=0;s<b;s++){void 0==c[s]&&(c[s]=(new Date).getTime());var v=this.parentPatch.mainloop.deltaT,u=parseFloat(h.getValue(s)),t=parseFloat(e.getValue(s));void 0==p[s]&&(p[s]=u);if(u!=g[s]||t!=m[s])r[s]=void 0,g[s]=u,m[s]=t,k[s]=0<m[s]?(g[s]-p[s])/(1E3*m[s]):0;Math.abs(k[s]*v)>Math.abs(g[s]-p[s])?p[s]=g[s]:p[s]+=
k[s]*v;0!=r[s]&&(f.setValue(s,p[s]),d.setValue(s,k[s]));r[s]=g[s]-p[s];c[s]=(new Date).getTime()}a&&f.getSliceCount()!=b&&(g.splice(b),m.splice(b),c.splice(b),p.splice(b),k.splice(b),r.splice(b),f.setSliceCount(b))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],g=[],m=[],p=[];this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=h.pinIsChanged()||e.pinIsChanged(),n=0;n<b;n++){void 0==c[n]&&(c[n]=(new Date).getTime());var q=this.parentPatch.mainloop.deltaT,s=parseFloat(h.getValue(n)),v=parseFloat(e.getValue(n));void 0==g[n]&&(g[n]=s);m[n]=0<v?(s-g[n])/(1E3*v):0;Math.abs(m[n]*q)>Math.abs(s-g[n])?g[n]=s:g[n]+=m[n]*q;0!=p[n]&&(f.setValue(n,g[n]),d.setValue(n,m[n]));p[n]=s-g[n];
c[n]=(new Date).getTime()}a&&f.getSliceCount()!=b&&(c.splice(b),g.splice(b),m.splice(b),p.splice(b),f.setSliceCount(b))}};VVVV.Nodes.Damper.prototype=new VVVV.Core.Node;
...
p.pinIsChanged(),u=0;u<b;u++)1!=n.getValue(u)&&q||n.setValue(u,0),1!=r.getValue(u)&&q||r.setValue(u,0),q||k.setValue(u,0),a=a||0.5<=h.getValue(u)||0.5<=e.getValue(u);if(a){for(u=0;u<b;u++){var a=parseFloat(c.getValue(u)),t=u>=k.getSliceCount()?0:parseFloat(k.getValue(u)),w=parseFloat(d.getValue(u)),l=parseFloat(f.getValue(u)),A=0;"Unlimited"==p.getValue(u)&&(A=1);"Clamp"==p.getValue(u)&&(A=2);switch(A){case 1:0.5<=h.getValue(u)&&(t+=a);0.5<=e.getValue(u)&&(t-=a);break;case 2:0.5<=h.getValue(u)&&(t+=
a);0.5<=e.getValue(u)&&(t-=a);t>w&&(t=w,n.setValue(u,1));t<l&&(t=l,r.setValue(u,1));break;default:0.5<=h.getValue(u)&&(t+=a),0.5<=e.getValue(u)&&(t-=a),t>w&&(t=l,n.setValue(u,1)),t<l&&(t=w,r.setValue(u,1))}k.getValue(u)!=t&&k.setValue(u,t);0.5<=m.getValue(u)&&k.setValue(u,g.getValue(u))}k.setSliceCount(b);r.setSliceCount(b);n.setSliceCount(b)}q=!0}};VVVV.Nodes.Counter.prototype=new VVVV.Core.Node;VVVV.Nodes.CurrentTimeAstronomy=function(b,a){this.constructor(b,"CurrentTime (Astronomy)",a);this.meta={authors:["Zauner"],original_authors:["VVVV Group"],credits:["http://javascript.about.com/library/bldst.htm"],compatibility_issues:["Time Zone index differs from classic VVVV","Time Zone Name does not work in Firefox"]};this.auto_evaluate=!0;var h=new Date,e=this.addOutputPin("GMT",[],VVVV.PinTypes.Value),f=this.addOutputPin("Time Zone",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Time Zone Name",
...
e.getValue(a),s[a].transform=f.getValue(a),d.isConnected()&&(s[a].clippingLayer=d.getValue(a))),s[a].x[w]=parseFloat(c.getValue(l)),s[a].y[w]=parseFloat(g.getValue(l)),s[a].c1x[w]=parseFloat(m.getValue(l)),s[a].c1y[w]=parseFloat(p.getValue(l)),s[a].c2x[w]=parseFloat(k.getValue(l)),s[a].c2y[w]=parseFloat(r.getValue(l)),w++,0<n.getValue(0)&&w>=n.getValue(a)&&(w<s[a].x.length&&h(s[a],w),a++,w=0);0>n.getValue(0)?w<s[a].x.length&&(h(s[a],w),s.splice(1)):s.splice(a);for(b=0;b<s.length;b++)q.setValue(b,
...
VVVV.PinTypes.Color),d=this.addInputPin("Backbuffer Width",[0],VVVV.PinTypes.Value),c=this.addInputPin("Backbuffer Height",[0],VVVV.PinTypes.Value),g=this.addInputPin("View",[],VVVV.PinTypes.Transform),m=this.addOutputPin("Canvas Out",[],this),p,k,r,n=[0,0,0,1],q;this.getContext=function(){var g=this.invisiblePins["Descriptive Name"].getValue(0);if((g=$(g).get(0))&&"CANVAS"==g.nodeName)q=g;else{var a=parseInt(d.getValue(0)),f=parseInt(c.getValue(0));q=$('<canvas width="'+(0<a?a:512)+'" height="'+
(0<f?f:512)+'" id="vvvv-js-generated-renderer-'+(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');g||(g="body");$(g).append(q);q=q.get(0)}k=q.width;r=q.height;q&&q.getContext&&(h(q),p=q.getContext("2d"),q.ctx=p)};this.destroy=function(){$(q).remove()};this.evaluate=function(){this.invisiblePins["Descriptive Name"].pinIsChanged()&&($(q).hasClass("vvvv-js-generated-renderer")&&$(q).remove(),this.getContext());if(p){if(d.pinIsChanged()){var a=parseInt(d.getValue(0));0<a&&(k=a,$(q).attr("width",
k))}c.pinIsChanged()&&(a=parseInt(c.getValue(0)),0<a&&(r=a,$(q).attr("height",r)));f.pinIsChanged()&&(n=f.getValue(0).split(","),n[0]=parseInt(255*n[0]),n[1]=parseInt(255*n[1]),n[2]=parseInt(255*n[2]));defaultRenderState.apply(p);p.clearRect(0,0,k,r);p.save();p.translate(k/2,r/2);p.scale(k/2,-r/2);g.isConnected()&&(a=g.getValue(0),p.transform(a[0],a[1],a[4],a[5],a[12],a[13]));if(e.isConnected())for(a=0;a<e.values.length;a++)e.getValue(a).draw(p);p.restore();defaultRenderState.apply(p);p.globalCompositeOperation=
...
0.5<f.getValue(b)&&l[b].play());for(b=0;b<a;b++){if(!l[b].paused){r.setValue(b,l[b]);n.setValue(b,l[b]);q.getValue(b)!=l[b].duration&&q.setValue(b,l[b].duration);s.setValue(b,l[b].currentTime);var z=parseFloat(g.getValue(b)),D=parseFloat(c.getValue(b));l[b].currentTime<D&&(l[b].currentTime=D);if(l[b].currentTime>=l[b].duration||0<=z&&l[b].currentTime>=z)0.5<=d.getValue(b)?l[b].currentTime=D:l[b].pause()}if(l[b].videoWidth!=v.getValue(b)||l[b].videoHeight!=u.getValue(b))v.setValue(b,l[b].videoWidth),
u.setValue(b,l[b].videoHeight);t.getValue(b)!=h[l[b].networkState]&&t.setValue(b,h[l[b].networkState]);w.getValue(b)!=e[l[b].readyState]&&w.setValue(b,e[l[b].readyState])}r.setSliceCount(a);n.setSliceCount(a)}};VVVV.Nodes.FileStreamCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Value),c=this.addOutputPin("Status",[""],VVVV.PinTypes.String),g=this.addOutputPin("Body",[""],VVVV.PinTypes.String),m=this.addOutputPin("Fail",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Success",[0],VVVV.PinTypes.Value),k,r,n,q=!1;this.evaluate=function(){this.getMaxInputSliceCount();var b=h.pinIsChanged()||e.pinIsChanged()||f.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==p.getValue(0)&&p.setValue(0,0);1==m.getValue(0)&&m.setValue(0,0);q&&(g.setValue(0,k),c.setValue(0,void 0),
p.setValue(0,r),m.setValue(0,n),q=!1);b&&(void 0==h.getValue(0)?(g.setValue(0,""),c.setValue(0,"")):$.ajax({url:h.getValue(0),type:"get",success:function(c,d,b){k=c;r=1;q=!0},error:function(c,d){k="";n=1;q=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0,k=0,r,n,q,s=0;s<b||0<e.getValue(0)&&(0<k||0!=a%e.values.length);s++)r=h.getValue(s),0==k&&(center=n=q=r,width=0),n=Math.min(r,n),q=Math.max(r,q),k++,0<e.getValue(0)&&k>=e.getValue(a)&&(c.setValue(a,n),g.setValue(a,q),f.setValue(a,n+(q-n)/2),d.setValue(a,Math.abs(q-n)),a++,k=0);-1==e.getValue(0)&&(c.setValue(a,n),g.setValue(a,q),f.setValue(a,n+(q-n)/2),d.setValue(a,Math.abs(q-n)));f.setSliceCount(a+(0<k));
...
h.getValue(Math.round(f.getValue(k)),a),1<a)for(a=0;a<b.length;a++)d.setValue(p,parseFloat(b[a])),p++;else d.setValue(k,parseFloat(b)),p++;d.setSliceCount(p)}};VVVV.Nodes.GetSliceSpreads.prototype=new VVVV.Core.Node;
...
a=c*b,p=0;p<a;p++)d.setValue(p%c*b+parseInt(p/c),h.getValue(p));d.setSliceCount(a)}};VVVV.Nodes.SwapDim.prototype=new VVVV.Core.Node;
...
c=parseInt(this.sprcntin.getValue(0)),g=2*Math.PI,d=g*d,m=0,p=Math.max(this.inpyin.getSliceCount(),this.inpxin.getSliceCount()),k=0;k<p*c;k++)for(var r=parseFloat(this.inpxin.getValue(k)),n=parseFloat(this.inpyin.getValue(k)),q=0;q<c;q++){var s=d+g*(q/c)*f,v=r+Math.cos(s)*b,s=n+Math.sin(s)*a;this.xout.setValue(m,v);this.yout.setValue(m,s);m++}this.xout.setSliceCount(c*p);this.yout.setSliceCount(c*p)}};VVVV.Nodes.CircularSpread.prototype=new VVVV.Core.Node;
...
d||void 0==d)d=e.getValue(a)}for(a=0;a<c;a++){var p=h.getValue(a);if(p<d)f.setValue(a,-1);else{for(var k=0;k<b-1&&!(p>=e.getValue(k)&&p<e.getValue(k+1));k++);f.setValue(a,k)}}f.setSliceCount(c)}};VVVV.Nodes.IntervalSpreads.prototype=new VVVV.Core.Node;
...
c=this.addOutputPin("Hits",[0],VVVV.PinTypes.Value),g=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),m=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value),p=this.addOutputPin("Found At Position",[0],VVVV.PinTypes.Value);this.evaluate=function(){var b=this.getMaxInputSliceCount();c.setSliceCount(b);var a=f.getValue(0);d.getValue(0);for(var n=0,q=0;q<b;q++){var s=h.getValue(q),v=1;if("MatchesAny"==a||"ContainsAny"==a)v=e.getSliceCount();for(var u=0,t=0;t<v;t++){var w=e.getValue(t+q),
l=[];if("Matches"==a||"MatchesAny"==a)w==s&&l.push(-1);else for(var w=RegExp(w,"gi"),A;A=w.exec(s);)l.push(A.index);for(w=0;w<l.length;w++)g.setValue(n,q%h.getSliceCount()),m.setValue(n,(q+t)%e.getSliceCount()),p.setValue(n,l[w]+1),n++,u++}c.setValue(q,u)}g.setSliceCount(n);m.setSliceCount(n);p.setSliceCount(n)}};VVVV.Nodes.SiftString.prototype=new VVVV.Core.Node;
...
UnderScore:"_",Minus:"-"},k={Single:"'",Double:'"'};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0,q=0;q<b;q++){var s=h.getValue(q),v=p[e.getValue(q)],u=f.getValue(q),t=k[d.getValue(q)],w=c.getValue(q);""==v&&(v=u);""==v&&(v=void 0);s=s.split(v);v=s.length;t=RegExp("[^"+t+"]*","g");for(u=0;u<v;u++)1==w?g.setValue(a,s[u]):g.setValue(a,s[u].match(t).join("")),m.setValue(a,q),a++}g.setSliceCount(a);m.setSliceCount(a)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var p=h.getValue(a),k=e.getValue(a),r=f.getValue(a),p=p==k||0==r&&p.toLowerCase()==k.toLowerCase()?1:0;d.setValue(a,p);c.setValue(a,1-p)}d.setSliceCount(b);c.setSliceCount(b)}};VVVV.Nodes.EQString.prototype=new VVVV.Core.Node;
...
0;p<e.getValue(a);p++)f.setValue(b,h.getValue(a)),d.setValue(b,a),b++;f.setSliceCount(b);d.setSliceCount(b)}};VVVV.Nodes.SelectString.prototype=new VVVV.Core.Node;
...
[],VVVV.PinTypes.Transform),g=mat4.create();mat4.identity(g);mat4.scale(g,[-1,1,-1]);this.evaluate=function(){for(var b=Math.max(h.getSliceCount(),Math.max(e.getSliceCount()/3,Math.max(f.getSliceCount()/3,d.getSliceCount()/3))),b=Math.ceil(b),a=0;a<b;a++){var k=e.getValue(a,3),r=f.getValue(a,3),n=d.getValue(a,3),q=mat4.create();mat4.identity(q);mat4.lookAt(vec3.create(k),vec3.create(r),vec3.create(n),q);mat4.multiply(g,q,q);h.isConnected()&&mat4.multiply(h.getValue(a),q,q);c.setValue(a,q)}c.setSliceCount(b)}};
...
a=0;a<b;a++){var p=0;Math.abs(h.getValue(a)-e.getValue(a))<=Math.abs(f.getValue(a))&&(p=1);d.setValue(a,p);c.setValue(a,1-p)}d.setSliceCount(b);c.setSliceCount(b)}};VVVV.Nodes.EqValue.prototype=new VVVV.Core.Node;
...
[1],VVVV.PinTypes.Value),g=this.addInputPin("Mapping",["Float"],VVVV.PinTypes.Enum);g.enumOptions=["Float","Wrap","Clamp"];var m=this.addOutputPin("Output",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++){var r=parseFloat(h.getValue(a)),n=parseFloat(e.getValue(a)),q=parseFloat(f.getValue(a)),s=parseFloat(d.getValue(a)),v=parseFloat(c.getValue(a)),u=g.getValue(a);if(q==n)m.setValue(a,s);else{r-=n;q-=n;switch(u){case "Clamp":r=Math.max(0,Math.min(q,
...
(c+1),[0],VVVV.PinTypes.Value)})};this.evaluate=function(){f.pinIsChanged()&&this.initialize();for(var c=this.getMaxInputSliceCount(),b,a,p,k=0;k<c;k++)b=h.getValue(k),a=((Math.floor(b)+d.length)%d.length+d.length)%d.length,p=((Math.ceil(b)+d.length)%d.length+d.length)%d.length,b-=Math.floor(b),e.setValue(k,(1-b)*d[a].getValue(k)+b*d[p].getValue(k));e.setSliceCount(c)}};VVVV.Nodes.InputMorphValue.prototype=new VVVV.Core.Node;
...
f=this.addInvisiblePin("Type",["Texture"],VVVV.PinTypes.Enum);f.enumOptions=["Texture","Cube Texture"];var d=[];this.evaluate=function(){if(this.renderContexts){var b=this.renderContexts[0];if(b){if(this.contextChanged){for(var a=0;a<d.length;a++)d[a].context.deleteTexture(d[a]);d=[]}if(h.pinIsChanged()||f.pinIsChanged()||this.contextChanged){for(var m=f.getValue(0),p=this.getMaxInputSliceCount(),a=0;a<p;a++){var k=VVVV.Helpers.prepareFilePath(h.getValue(a),this.parentPatch);0===k.indexOf("http://")&&
...
[15],VVVV.PinTypes.Value),m=this.addInputPin("Resolution Y",[1],VVVV.PinTypes.Value),p=this.addOutputPin("Mesh",[],VVVV.PinTypes.WebGlResource),k=null;this.evaluate=function(){if(this.renderContexts){var b=this.renderContexts[0];if(b){var a=parseInt(g.getValue(0)),q=parseInt(m.getValue(0)),s=parseFloat(h.getValue(0)),v=parseFloat(e.getValue(0)),u=parseFloat(f.getValue(0)),t=parseFloat(d.getValue(0)),w=[],l=[],A=[];w.push(0);w.push(u/2);w.push(0);l.push(0);l.push(1);l.push(0);A.push(0);A.push(0);w.push(0);
w.push(-u/2);w.push(0);l.push(0);l.push(-1);l.push(0);A.push(0);A.push(0);for(var y=0;y<q+1;y++)for(var z=parseFloat(y)/q,D=(z-0.5)*-u,C=0;C<a+1;C++){var I=Math.cos(2*(parseFloat(C)/a)*Math.PI*t-Math.PI*t-Math.PI/2),J=Math.sin(2*(parseFloat(C)/a)*Math.PI*t-Math.PI*t-Math.PI/2),H=z*v+(1-z)*s;w.push(I*H);w.push(D);w.push(J*H);l.push(I);l.push(0);l.push(J);A.push(parseFloat(C)/a);A.push(parseFloat(y)/q)}s=new VVVV.Types.VertexBuffer(b,w);s.setSubBuffer("TEXCOORD0",2,A);s.setSubBuffer("NORMAL",3,l);s.create();
...
(b=0);var m=c.length;if(this.contextChanged||p)m=0;b<m&&c.splice(b,m-b);for(var v=m;v<b;v++)c[v]=new VVVV.Types.Layer,c[v].mesh=e.getValue(0),c[v].shader=g,_(g.uniformSpecs).each(function(a){c[v].uniformNames.push(a.varname);c[v].uniforms[a.varname]={uniformSpec:a,value:void 0}});if(e.pinIsChanged())for(v=0;v<b;v++)c[v].mesh=e.getValue(0);for(v=0;v<b;v++)c[v].shader=g;for(a=0;a<k.length;a++){var u=k[a].pinname.replace(/ /g,"_");if(k[a].pinIsChanged()||m<b)for(v=0;v<b;v++)if("vec"==g.uniformSpecs[u].type){var t=
"COLOR"==g.uniformSpecs[u].semantic?_(k[a].getValue(v).split(",")).map(function(a){return parseFloat(a)}):k[a].getValue(v,k[a].dimensions);c[v].uniforms[u].value=new Float32Array(t)}else t=k[a].getValue(v),c[v].uniforms[u].value=t}if(h.pinIsChanged()||m<b)for(a=0;a<b;a++)h.isConnected()?c[a].renderState=h.getValue(a):c[a].renderState=VVVV.DefaultRenderState;if(f.pinIsChanged()||m<b)for(a=0;a<b;a++)m=this.inputPins.Transform.getValue(a),c[a].uniforms[c[a].shader.uniformSemanticMap.WORLD].value=m;this.outputPins.Layer.setSliceCount(b);
...
l.DEPTH_BUFFER_BIT);"NONE"==p.getValue(0)?l.disable(l.DEPTH_TEST):l.enable(l.DEPTH_TEST);m.pinIsChanged()&&(m.isConnected()?(v=mat4.create(),mat4.set(m.getValue(0),v)):(v=mat4.create(),mat4.ortho(-1,1,-1,1,-100,100,v)),mat4.scale(v,[1,1,-1]),this.renderContexts&&this.renderContexts[0]&&mat4.scale(v,[1,-1,1]));g.pinIsChanged()&&(u=g.getValue(0));if(this.contextChanged)this.contextChanged=!1;else{l.viewport(0,0,q,s);var b=a=null,e=null;if(this.inputPins.Layers.isConnected()){for(var h=this.inputPins.Layers.values,
t=0;t<h.length;t++)if(layer=h[t],void 0!=layer.shader){a!=layer.shader.shaderProgram&&10>D&&(l.useProgram(layer.shader.shaderProgram),l.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.PROJECTION].position,!1,v),l.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.VIEW].position,!1,u));var F=layer.renderState;F||(F=defaultWebGlRenderState);F!=b&&F.apply(l);if(layer.mesh!=e||layer.shader.shaderProgram!=a)l.bindBuffer(l.ARRAY_BUFFER,layer.mesh.vertexBuffer.vbo),
_(layer.mesh.vertexBuffer.subBuffers).each(function(a){layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]]&&-1!=layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position&&10>D&&(l.enableVertexAttribArray(layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position),l.vertexAttribPointer(layer.shader.attributeSpecs[layer.shader.attribSemanticMap[a.usage]].position,a.size,l.FLOAT,!1,0,a.offset))}),l.bindBuffer(l.ELEMENT_ARRAY_BUFFER,layer.mesh.indexBuffer);
...
B.uniformSpec.dimension+"i"](B.uniformSpec.position,B.value);break;case "float":l["uniform"+B.uniformSpec.dimension+"f"](B.uniformSpec.position,B.value);break;case "sampler":var E=B.value;E==VVVV.DefaultTexture&&(E=l.DefaultTexture["2D"]);l.activeTexture(l["TEXTURE"+e]);l.bindTexture(l["TEXTURE_"+B.uniformSpec.dimension],E);l.uniform1i(B.uniformSpec.position,e);e++;break;case "samplerCube":E=B.value,E==VVVV.DefaultTexture&&(E=l.DefaultTexture.CUBE),l.activeTexture(l["TEXTURE"+e]),l.bindTexture(l.TEXTURE_CUBE_MAP,
E),l.uniform1i(B.uniformSpec.position,e),e++}loopstart=(new Date).getTime()}}l.drawElements(l[F.polygonDrawMode],layer.mesh.numIndices,l.UNSIGNED_SHORT,0);a=layer.shader.shaderProgram;b=F;e=layer.mesh}l.bindTexture(l.TEXTURE_2D,null);l.bindTexture(l.TEXTURE_CUBE_MAP,null);D++}this.renderContexts&&this.renderContexts[0]&&(l.bindTexture(l.TEXTURE_2D,y),l.generateMipmap(l.TEXTURE_2D),l.bindTexture(this.renderContexts[0].TEXTURE_2D,null));n.setValue(0,y)}}}};VVVV.Nodes.RendererWebGL.prototype=new VVVV.Core.Node;
