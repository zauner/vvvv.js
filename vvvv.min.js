...
VVVV.Core={Pin:function(a,b,g,e,f){this.direction=b;this.pinname=a;this.links=[];this.values=[];this.node=e;this.changed=!0;this.reset_on_disconnect=this.active=!1;this.masterPin=this.slavePin=void 0;this.connectionChangedHandlers={};this.enumOptions=[];void 0==f&&"development"==VVVV_ENV&&console.warn(e.nodename+" / "+a+" -> Generic");this.getValue=function(d,c){if(!c||1==c)return this.values[d%this.values.length];for(var h=[],f=0;f<c;f++)h.push(this.values[(d*c+f)%this.values.length]);return h};
this.setValue=function(d,c,h){h=h||!1;this.values[d]=c;this.changed=!0;this.node.dirty=!0;if(this.direction==PinDirection.Output)for(var f=this.links.length,e=0;e<f;e++)this.links[e].toPin.setValue(d,c);this.slavePin&&!h&&this.slavePin.setValue(d,c);this.direction==PinDirection.Input&&(this.masterPin&&!this.isConnected())&&this.masterPin.setValue(d,c,!0);this.node.isIOBox&&"Descriptive Name"==this.pinname&&(this.node.parentPatch.domInterface?this.node.parentPatch.domInterface.connect(this.node):this.node.parentPatch.parentPatch&&
...
d.connectionChangedHandlers[f]});this.typeName=d.typeName;this.defaultValue=d.defaultValue;this.direction==PinDirection.Input&&this.defaultValue&&(this.setValue(0,this.defaultValue()),this.setSliceCount(1));void 0!=d.reset_on_disconnect&&(this.reset_on_disconnect=d.reset_on_disconnect)}};void 0==f&&(f=VVVV.PinTypes.Generic,this.unvalidated=!0);this.setType(f);g&&0<g.length&&(this.values=g.slice(0));this.reset=function(){console.log("resetting "+this.pinname);this.defaultValue?(this.setValue(0,this.defaultValue()),
...
!1;this.dirty=!0;(this.parentPatch=g)&&(this.parentPatch.nodeMap[a]=this);this.addDefault=function(e,f){this.defaultPinValues[e]=f};this.addInputPin=function(e,f,d){f=new VVVV.Core.Pin(e,PinDirection.Input,f,this,d);this.inputPins[e]=f;this.parentPatch&&(this.parentPatch.pinMap[this.id+"_in_"+e]=f);this.applyPinValuesFromXML(e);return f};this.addOutputPin=function(e,f,d){f=new VVVV.Core.Pin(e,PinDirection.Output,f,this,d);this.outputPins[e]=f;this.parentPatch&&(this.parentPatch.pinMap[this.id+"_out_"+
e]=f);return f};this.removeInputPin=function(e){if(this.inputPins[e]){var f=this.inputPins[e].links[0];f&&(f.fromPin.connectionChanged(),f.destroy());delete this.inputPins[e];this.dirty=!0}};this.removeOutputPin=function(e){if(this.outputPins[e]){for(var f=this.outputPins[e].links.length,d=0;d<f;d++){var c=this.outputPins[e].links[d];c.toPin.connectionChanged();c.destroy()}delete this.outputPins[e];this.dirty=!0}};this.addInvisiblePin=function(e,f,d){f=new VVVV.Core.Pin(e,PinDirection.Configuration,
f,this,d);this.invisiblePins[e]=f;this.parentPatch.pinMap[this.id+"_inv_"+e]=f;void 0!=this.defaultPinValues[e]&&(f.values=this.defaultPinValues[e]);return f};this.IOBoxType=function(){var e=/^IOBox \((.*)\)/.exec(this.nodename);return e&&1<e.length?e[1]:""};this.IOBoxInputPin=function(){switch(this.IOBoxType()){case "Value Advanced":return this.inputPins["Y Input Value"];case "String":return this.inputPins["Input String"];case "Color":return this.inputPins["Color Input"];case "Node":return this.inputPins["Input Node"]}};
...
f=d.links[c].toPin.slavePin?f.concat(d.links[c].toPin.slavePin.node.findDownstreamNodes(e)):d.links[c].toPin.node.isIOBox&&d.links[c].toPin.node.IOBoxOutputPin().slavePin?f.concat(d.links[c].toPin.node.IOBoxOutputPin().slavePin.node.findDownstreamNodes(e)):f.concat(d.links[c].toPin.node.findDownstreamNodes(e))});return f};this.hasNilInputs=function(){var e=!1;_(this.inputPins).each(function(f){0==f.getSliceCount()&&(e=!0)});return e};this.getMaxInputSliceCount=function(){var e=0;_(this.inputPins).each(function(f){f.getSliceCount()>
e&&(e=f.values.length)});return e};this.applyPinValuesFromXML=function(e){if(this.inputPins[e]){var f=this.inputPins[e];e=this.defaultPinValues[e];if(void 0!=e&&(!f.reset_on_disconnect||1<e.length||""!=e[0])){for(var d=0;d<e.length;d++)f.values[d]!=e[d]&&f.setValue(d,e[d]);f.setSliceCount(e.length)}}};this.registerInterfacePin=function(){var e=this;if(this.isIOBox&&this.parentPatch.parentPatch&&""!=this.invisiblePins["Descriptive Name"].getValue(0)){var f=this.invisiblePins["Descriptive Name"].getValue(0);
this.IOBoxInputPin().connectionChangedHandlers.subpatchpins=function(){if(0<this.links.length&&this.masterPin){"development"==VVVV_ENV&&console.log("deleting "+f+" input pin because node has input connection...");for(var d=0;d<this.masterPin.links.length;d++)this.masterPin.links[d].destroy();e.parentPatch.removeInputPin(f);this.masterPin=void 0}e.IOBoxOutputPin().slavePin?e.IOBoxOutputPin().slavePin.pinname!=f&&("development"==VVVV_ENV&&console.log("renaming "+e.IOBoxOutputPin().slavePin.pinname+
" to "+f),e.parentPatch.outputPins[f]=e.parentPatch.outputPins[e.IOBoxOutputPin().slavePin.pinname],e.parentPatch.removeOutputPin(e.IOBoxOutputPin().slavePin.pinname),e.IOBoxOutputPin().slavePin.pinname=f):("development"==VVVV_ENV&&console.log("interfacing output pin detected: "+f),d=e.parentPatch.outputPins[f],void 0==d&&(d=e.parentPatch.addOutputPin(f,e.IOBoxOutputPin().values)),d.setType(VVVV.PinTypes[e.IOBoxOutputPin().typeName]),e.IOBoxOutputPin().slavePin=d,d.masterPin=e.IOBoxOutputPin())};
this.IOBoxInputPin().connectionChanged();this.IOBoxOutputPin().connectionChangedHandlers.subpatchpins=function(){if(0<this.links.length&&this.slavePin){"development"==VVVV_ENV&&console.log("deleting "+f+" output pin because node "+e.id+" has output connection...");for(var d=0;d<this.slavePin.links.length;d++)this.slavePin.links[d].destroy();e.parentPatch.removeOutputPin(f);this.slavePin=void 0}if(e.IOBoxInputPin().masterPin)e.IOBoxInputPin().masterPin.pinname!=f&&(console.log("renaming "+e.IOBoxInputPin().masterPin.pinname+
" to "+f),e.parentPatch.inputPins[f]=e.parentPatch.inputPins[e.IOBoxInputPin().masterPin.pinname],e.parentPatch.removeInputPin(e.IOBoxInputPin().masterPin.pinname),e.IOBoxInputPin().masterPin.pinname=f);else{"development"==VVVV_ENV&&console.log("interfacing input pin detected: "+f);var c=e.parentPatch.inputPins[f];if(void 0==c)"development"==VVVV_ENV&&console.log("creating new input pin at parent patch, using IOBox values"),c=e.parentPatch.addInputPin(f,e.IOBoxInputPin().values);else{slicecount=c.getSliceCount();
for(d=0;d<slicecount;d++)e.IOBoxInputPin().setValue(d,c.getValue(d));0<slicecount&&e.IOBoxInputPin().setSliceCount(c.getSliceCount())}d=c.values.slice();c.setType(VVVV.PinTypes[e.IOBoxInputPin().typeName]);if(c.unvalidated&&VVVV.PinTypes[c.typeName].primitive||c.isConnected())c.values=d,c.unvalidated=!1;c.slavePin=e.IOBoxInputPin();e.IOBoxInputPin().masterPin=c}};this.IOBoxOutputPin().connectionChanged()}};this.setup=function(){this.addInvisiblePin("Descriptive Name",[""],VVVV.PinTypes.String)};this.initialize=
...
"|":",",c.split(separator).filter(function(c,d){return""!=c});for(var d=[],h="",f=!1,e=c.length,g=0;g<e;g++)","!=c[g]||f?"|"==c[g]?f=!f:h+=c[g]:(d.push(h),h="");d.push(h);return d}function c(){var c=window.location.hash.match("#([^/]+)/"+p.ressource+"$");c&&(console.log("launching editor ..."),(new VVVV.Editors[c[1]]).enable(p))}this.ressource=a;this.vvvv_version="45_26.1";this.boundingBox={width:0,height:0};this.windowHeight=this.windowWidth=500;this.pinMap={};this.nodeMap={};this.nodeList=[];this.linkList=
...
if("45_26">=this.vvvv_version)var q={},n={},k={},r={};var p=this;this.doLoad=function(c,h){function f(c){"complete"==g&&(n={});$(c).find('link[deleteme="pronto"]').each(function(){for(var c=!1,d=0;d<p.linkList.length;d++)p.linkList[d].fromPin.node.id==$(this).attr("srcnodeid")&&(p.linkList[d].fromPin.pinname==$(this).attr("srcpinname")&&p.linkList[d].toPin.node.id==$(this).attr("dstnodeid")&&p.linkList[d].toPin.pinname==$(this).attr("dstpinname"))&&(c=p.linkList[d]);if(c){"development"==VVVV_ENV&&
console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var d=c.fromPin,h=c.toPin;c.destroy();d.connectionChanged();h.connectionChanged();h.markPinAsChanged();h.reset_on_disconnect&&h.reset()}});$(c).find('link[deleteme!="pronto"]').each(function(){var c=p.pinMap[$(this).attr("srcnodeid")+"_out_"+$(this).attr("srcpinname")],d=p.pinMap[$(this).attr("dstnodeid")+"_in_"+$(this).attr("dstpinname")];void 0==c&&p.nodeMap[$(this).attr("srcnodeid")]&&(c=p.nodeMap[$(this).attr("srcnodeid")].addOutputPin($(this).attr("srcpinname"),
void 0));void 0==d&&p.nodeMap[$(this).attr("dstnodeid")]&&(d=p.nodeMap[$(this).attr("dstnodeid")].addInputPin($(this).attr("dstpinname"),void 0));if(c&&d){c.typeName!=d.typeName&&"Node"!=c.typeName&&"Node"!=d.typeName&&"development"==VVVV_ENV&&console.warn("Type Mismatch: "+c.node.nodename+"/"+c.pinname+" <-> "+d.node.nodename+"/"+d.pinname);for(var h=!1,f=0;f<p.linkList.length;f++)p.linkList[f].fromPin.node.id==c.node.id&&(p.linkList[f].fromPin.pinname==c.pinname&&p.linkList[f].toPin.node.id==d.node.id&&
p.linkList[f].toPin.pinname==d.pinname)&&(h=p.linkList[f]);if(!h){h=new VVVV.Core.Link(c,d);c.connectionChanged();d.connectionChanged();p.linkList.push(h);for(f=0;f<c.values.length;f++)d.setValue(f,c.getValue(f));d.setSliceCount(c.getSliceCount())}"complete"==g&&(n[c.node.id+"_"+c.pinname+"-"+d.node.id+"_"+d.pinname]=h)}});"complete"==g&&(_(q).each(function(c,d){if(void 0==n[d]){"development"==VVVV_ENV&&console.log("removing "+c.fromPin.pinname+" -> "+c.toPin.pinname);var h=c.fromPin,f=c.toPin;c.destroy();
h.connectionChanged();f.connectionChanged();f.markPinAsChanged();f.reset_on_disconnect&&f.reset()}}),q={},_(n).each(function(c,d){q[d]=c}))}var e=this;do e.dirty=!0;while(e=e.parentPatch);if(e=/^<!DOCTYPE\s+PATCH\s+SYSTEM\s+"(.+)\\(.+)\.dtd/.exec(c))p.vvvv_version=e[2].replace(/[a-zA-Z]+/,"_");var g="diff";if(/\s<PATCH/.test(c)||"45_26">=p.vvvv_version)g="complete","development"==VVVV_ENV&&console.log("complete: "+this.nodename),p.XMLCode=c;e=$(c).find('bounds[type="Window"]').first();0<e.length&&
(p.windowWidth=e.attr("width")/15,p.windowHeight=e.attr("height")/15);"complete"==g&&(r={});var a=0;$(c).find("node").each(function(){if("pronto"==$(this).attr("createme")&&void 0!=p.nodeMap[$(this).attr("id")]){var e=p.nodeMap[$(this).attr("id")];"development"==VVVV_ENV&&console.log("node renamed, so deleting node "+e.id+" / "+e.nodename);_(e.inputPins).each(function(c){_(c.links).each(function(c){c.destroy();c.fromPin.connectionChanged()})});_(e.outputPins).each(function(c){_(c.links).each(function(c){c.destroy();
c.toPin.connectionChanged();c.toPin.markPinAsChanged();c.toPin.reset_on_disconnect&&c.toPin.reset()})});p.nodeList.splice(p.nodeList.indexOf(e),1);delete p.nodeMap[e.id]}var q;q="InABox"==$(this).attr("componentmode")?$(this).find('bounds[type="Box"]').first():$(this).find('bounds[type="Node"]').first();var n=void 0!=p.nodeMap[$(this).attr("id")];if(n)e=p.nodeMap[$(this).attr("id")];else{nodename=""!=$(this).attr("systemname")?$(this).attr("systemname"):$(this).attr("nodename");if(void 0==nodename)return;
if(void 0!=VVVV.NodeLibrary[nodename.toLowerCase()])e=new (VVVV.NodeLibrary[nodename.toLowerCase()])($(this).attr("id"),p),VVVV.NodeLibrary[nodename.toLowerCase()].requirements&&(p.resourcesPending++,_(VVVV.NodeLibrary[nodename.toLowerCase()].requirements).each(function(c){void 0===VVVV.LoadedLibs[c]&&VVVV.loadScript(VVVV.ThirdPartyLibs[c],function(){p.resourcesPending--})}));else if(/.fx$/.test($(this).attr("filename")))e=new VVVV.Nodes.GenericShader($(this).attr("id"),p),e.isShader=!0,e.shaderFile=
$(this).attr("filename").replace(/\\/g,"/").replace(/\.fx$/,".vvvvjs.fx").replace("lib/nodes/",""),e.nodename=nodename;else if(/.v4p$/.test($(this).attr("filename"))){p.resourcesPending++;a++;var k=this,e=new VVVV.Core.Patch($(this).attr("filename"),function(){p.resourcesPending--;a--;"development"==VVVV_ENV&&console.log(this.nodename+"invoking update links");f(c);p.editor&&p.editor.addPatch(this);if(this.auto_evaluate){var d=p;do d.auto_evaluate=!0;while(d=d.parentPatch)}this.setMainloop(p.mainloop);
p.afterUpdate();0>=p.resourcesPending&&h&&(h(),h=void 0)},function(){this.not_implemented=!0;VVVV.onNotImplemented(nodename)},p,$(k).attr("id"));e.isSubpatch=!0;p.editor&&!e.editor&&p.editor.addPatch(e);p.nodeMap[e.id]=e}else e=new VVVV.Core.Node($(this).attr("id"),nodename,p),e.not_implemented=!0,"diff"==g&&!0==VVVV.Config.auto_undo&&p.editor.sendUndo(),VVVV.onNotImplemented(nodename);"development"==VVVV_ENV&&"complete"!=g&&console.log(p.nodename+": inserted new node "+e.nodename)}if(e.auto_evaluate){k=
p;do k.auto_evaluate=!0;while(k=k.parentPatch)}if("pronto"==$(this).attr("deleteme")){"development"==VVVV_ENV&&console.log("removing node "+e.id);if(e.isSubpatch){e.editor&&e.editor.removePatch(e);k=e.getSubPatches();k.push(e);for(var b,m=0;m<k.length;m++)b=VVVV.Helpers.prepareFilePath(k[m].nodename,k[m].parentPatch),VVVV.Patches[b].splice(VVVV.Patches[b].indexOf(e),1),0==VVVV.Patches[b].length&&delete VVVV.Patches[b]}p.nodeList.splice(p.nodeList.indexOf(e),1);e.destroy();delete p.nodeMap[e.id]}0<
q.length&&(q.attr("left")&&(e.x=q.attr("left")/15,e.y=q.attr("top")/15,p.boundingBox.width=Math.max(p.boundingBox.width,e.x+100),p.boundingBox.height=Math.max(p.boundingBox.height,e.y+100)),q.attr("width")&&(e.width=q.attr("width"),e.height=q.attr("height")));/^iobox/.test(e.nodename.toLowerCase())&&(e.isIOBox=!0);n||e.setup();k=this;$(this).find("pin").each(function(){var h=$(this).attr("pinname"),f=d($(this).attr("values"));void 0!=f&&0<f.length&&e.addDefault(h,f);if(void 0==e.outputPins[h])if(void 0!=
e.inputPins[h])e.inputPins[h].isConnected()||e.applyPinValuesFromXML(h);else if(void 0!=e.invisiblePins[h]){if(void 0!=f){for(var g=0;g<f.length;g++)e.invisiblePins[h].values[g]!=f[g]&&e.invisiblePins[h].setValue(g,f[g]);e.invisiblePins[h].setSliceCount(f.length)}}else 1==$(this).attr("visible")&&"Configuration"!=$(this).attr("pintype")||e.isSubpatch?"Output"==$(this).attr("pintype")||0<$(c).find("link[srcnodeid="+e.id+"]").filter("link[srcpinname='"+h.replace(/[\[\]]/,"")+"']").length?void 0==e.outputPins[h]&&
e.addOutputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInputPin(h,f):void 0==e.inputPins[h]&&void 0==e.invisiblePins[h]&&e.addInvisiblePin(h,f)});n||(e.initialize(),p.nodeList.push(e));"complete"==g&&(r[e.id]=e)});"complete"==g&&(_(k).each(function(c,d){void 0==r[d]&&("development"==VVVV_ENV&&console.log("removing node "+c.id),p.nodeList.splice(p.nodeList.indexOf(c),1),delete p.nodeMap[c.id])}),k={},_(r).each(function(c,d){k[d]=c}));0===a&&f(c);0>=this.resourcesPending&&h&&(h(),
...
"\n  <BOUNDS");c=c.replace(/<\/bounds>/g,"\n  </BOUNDS>");c=c.replace(/<pin/g,"\n  <PIN");c=c.replace(/<\/pin>/g,"\n  </PIN>");c=c.replace(/<lonk/g,"\n  <LINK");return c=c.replace(/<\/lonk>/g,"\n  </LINK>")};this.evaluate=function(){function c(d){var e=d.getUpstreamNodes();_(e).each(function(d){void 0==f[d.id]||d.delays_output||c(d)});if(d.dirty||d.auto_evaluate||d.isSubpatch){if(h)var g=(new Date).getTime();if(d.auto_nil&&!d.isSubpatch&&d.hasNilInputs())_(d.outputPins).each(function(c){c.setSliceCount(0)});
else{try{d.evaluate()}catch(q){console.log("VVVV.Js / Error evaluating "+d.nodename+": "+q.message)}h&&console.log(d.nodename+" / "+d.id+": "+((new Date).getTime()-g)+"ms");d.dirty=!1;_(d.inputPins).each(function(c){c.changed=!1})}}delete f[d.id];return!0}if(!(0<this.resourcesPending)){if(h)var d=(new Date).getTime();for(var f={},e={},g=0;g<this.nodeList.length;g++){if(0==this.nodeList[g].getDownstreamNodes().length||this.nodeList[g].auto_evaluate||this.nodeList[g].delays_output)e[this.nodeList[g].id]=
this.nodeList[g];f[this.nodeList[g].id]=this.nodeList[g]}h&&console.log("building node maps: "+((new Date).getTime()-d)+"ms");_(e).each(function(d,h,e){void 0!=f[d.id]&&c(d)});h&&(d=(new Date).getTime());this.afterEvaluate();h&&console.log("patch rendering: "+((new Date).getTime()-d)+"ms");h=!1}};$(window).keydown(function(c){84==c.which&&(c.altKey&&c.ctrlKey)&&(h=!0)});if(/\.v4p[^<>\s]*$/.test(a)){this.nodename=a;var m=this,t=a;this.parentPatch&&(t=VVVV.Helpers.prepareFilePath(a,this.parentPatch));
VVVV.Patches[t]?m.doLoad(VVVV.Patches[t][0].toXML(),function(){VVVV.Patches[t].push(m);m.success&&m.success();m.afterUpdate()}):$.ajax({url:t,type:"get",dataType:"text",success:function(c){m.doLoad(c,function(){VVVV.Patches[t]=VVVV.Patches[t]||[];VVVV.Patches[t].push(m);m.success&&m.success();m.afterUpdate()})}})}else/^ws:\/\//.test(a)?(m=this,VVVV.Editors.Connector.host=a,VVVV.Editors.Connector.enable(this,{success:function(){m.success&&m.success()},error:function(){m.error&&m.error()}})):this.doLoad(a,
...
VVVV.Core.MainLoop=function(a,b){function g(){if(0==a.resourcesPending){e++;var q=(new Date).getTime();f.populateInputConnectors();a.evaluate();f.processOutputConnectors();var n=(new Date).getTime()-q;0==e%10&&(h&&(console.log(1/((q-c)/1E4)+"fps"),h=!1),c=q)}this.deltaT=Math.max(0,Math.round(1E3/VVVV.fps-n));d&&window.setTimeout(g,this.deltaT)}VVVV.fps=b||60;var e=0,f=new VVVV.Core.DOMInterface(a),d=!0,c=0,h=!1;this.deltaT=1E3/VVVV.fps;a.setMainloop(this);this.stop=function(){d=!1};this.start=function(){d||
...
q=this.addOutputPin("Length",[1],VVVV.PinTypes.Value),n=this.addOutputPin("Angle",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var p=parseFloat(g.getValue(b)),m=parseFloat(e.getValue(b)),t=parseFloat(f.getValue(b)),s=parseFloat(d.getValue(b)),t=t-p,s=s-m;c.setValue(b,p+t/2);h.setValue(b,m+s/2);q.setValue(b,Math.sqrt(t*t+s*s));n.setValue(b,Math.atan2(s,t)/(2*Math.PI))}c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a);n.setSliceCount(a)}};
...
q=this.addOutputPin("X2",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Y2",[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var p=parseFloat(g.getValue(b)),m=parseFloat(e.getValue(b)),t=parseFloat(f.getValue(b)),s=2*parseFloat(d.getValue(b)-0.25)*Math.PI,v=t/2*Math.sin(s),t=-t/2*Math.cos(s);c.setValue(b,p+v);h.setValue(b,m+t);q.setValue(b,p-v);n.setValue(b,m-t)}c.setSliceCount(a);h.setSliceCount(a);q.setSliceCount(a);n.setSliceCount(a)}};
...
this.evaluate=function(){for(var q=this.getMaxInputSliceCount(),a=0;a<q;a++){var b=g.getValue(a),r=e.getValue(a),p=f.getValue(a),m=Math.sqrt(b*b+r*r+p*p);c.setValue(a,Math.atan2(-b,-p)/(2*Math.PI));d.setValue(a,0==m?0:Math.acos(-r/m)/(2*Math.PI)-0.25);h.setValue(a,m)}c.setSliceCount(q);d.setSliceCount(q);h.setSliceCount(q)}};VVVV.Nodes.Polar3d.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var k=2*(g.getValue(b)+0.25)*Math.PI,r=2*e.getValue(b)*Math.PI,p=f.getValue(b);h.setValue(b,-p*Math.cos(r)*Math.sin(k));d.setValue(b,-p*Math.sin(r)*Math.sin(k));c.setValue(b,-p*Math.cos(k))}d.setSliceCount(a);c.setSliceCount(a);h.setSliceCount(a)}};VVVV.Nodes.Cartesian3d.prototype=new VVVV.Core.Node;
...
[0],VVVV.PinTypes.Value),q=this.addOutputPin("Cycles",[0],VVVV.PinTypes.Value),n=[],k=[],r=(new Date).getTime(),p=(new Date).getTime();this.evaluate=function(){var a=this.getMaxInputSliceCount();r=(new Date).getTime()-p;for(var b=0;b<a;b++){var s=g.getValue(b%g.values.length),v=e.getValue(b%e.values.length),u=f.getValue(b%f.values.length),w=d.getValue(b%d.values.length),l=c.getValue(b%c.values.length);void 0==n[b]&&(n[b]=0);void 0==k[b]&&(k[b]=0);0.5>v&&(0!=s&&isFinite(s))&&(dv=1/(1E3*s)*r,0<u&&(dv*=
-1),n[b]+=dv,0>n[b]&&(k[b]-=Math.ceil(-n[b]),n[b]=1+n[b]),1<n[b]&&(k[b]+=Math.floor(n[b])));p=(new Date).getTime();0.5<=w&&(n[b]=0,k[b]=0);if(0.5>v||0.5<=w)h.setValue(b,(n[b]+l)%1),q.setValue(b,k[b]);n[b]%=1}h.setSliceCount(a);q.setSliceCount(a);n.splice(a);k.splice(a)}};VVVV.Nodes.LFO.prototype=new VVVV.Core.Node;
...
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],q=[],n=[],k=[],r=[],p=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||e.pinIsChanged(),s=0;s<a;s++){void 0==c[s]&&(c[s]=(new Date).getTime());var v=this.parentPatch.mainloop.deltaT,u=parseFloat(g.getValue(s)),w=parseFloat(e.getValue(s));void 0==n[s]&&(n[s]=u);if(u!=h[s]||w!=q[s])r[s]=void 0,h[s]=u,q[s]=w,p[s]=0<=h[s]-n[s],k[s]=0<q[s]?(h[s]-n[s])/(1E3*q[s]):0;p[s]==0<=h[s]-n[s]&&(n[s]+=
k[s]*v);void 0!=r[s]&&0<=h[s]-n[s]!=0<=r[s]&&(k[s]=0,n[s]=h[s]);0!=r[s]&&(f.setValue(s,n[s]),d.setValue(s,k[s]));r[s]=h[s]-n[s];c[s]=(new Date).getTime()}b&&f.getSliceCount()!=a&&(h.splice(a),q.splice(a),c.splice(a),n.splice(a),k.splice(a),r.splice(a),f.setSliceCount(a))}};VVVV.Nodes.LinearFilter.prototype=new VVVV.Core.Node;
VVVV.Nodes.Damper=function(a,b){this.constructor(a,"Damper (Animation)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:["Cyclic Pin not implemented","Acceleration Out is not set yet"]};this.auto_evaluate=!0;var g=this.addInputPin("Go To Position",[0],VVVV.PinTypes.Value),e=this.addInputPin("FilterTime",[1],VVVV.PinTypes.Value),f=this.addOutputPin("Position Out",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Velocity Out",[0],VVVV.PinTypes.Value);
this.addOutputPin("Acceleration Out",[0],VVVV.PinTypes.Value);var c=[],h=[],q=[],n=[],k=[];this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=g.pinIsChanged()||e.pinIsChanged(),m=0;m<a;m++){void 0==c[m]&&(c[m]=(new Date).getTime());var t=this.parentPatch.mainloop.deltaT,s=parseFloat(g.getValue(m)),v=parseFloat(e.getValue(m));void 0==h[m]&&(h[m]=s);n[m]=void 0;k[m]=0<=s-h[m];q[m]=0<v?(s-h[m])/(1E3*v):0;k[m]==0<=s-h[m]&&(h[m]+=q[m]*t);void 0!=n[m]&&0<=s-h[m]!=0<=n[m]&&(q[m]=0,h[m]=s);
0!=n[m]&&(f.setValue(m,h[m]),d.setValue(m,q[m]));n[m]=s-h[m];c[m]=(new Date).getTime()}b&&f.getSliceCount()!=a&&(c.splice(a),h.splice(a),q.splice(a),n.splice(a),f.setSliceCount(a))}};VVVV.Nodes.Damper.prototype=new VVVV.Core.Node;
...
n.pinIsChanged(),v=0;v<a;v++)1!=p.getValue(v)&&m||p.setValue(v,0),1!=r.getValue(v)&&m||r.setValue(v,0),m||k.setValue(v,0),b=b||0.5<=g.getValue(v)||0.5<=e.getValue(v);if(b){for(v=0;v<a;v++){var b=parseFloat(c.getValue(v)),u=v>=k.getSliceCount()?0:parseFloat(k.getValue(v)),w=parseFloat(d.getValue(v)),l=parseFloat(f.getValue(v)),A=0;"Unlimited"==n.getValue(v)&&(A=1);"Clamp"==n.getValue(v)&&(A=2);switch(A){case 1:0.5<=g.getValue(v)&&(u+=b);0.5<=e.getValue(v)&&(u-=b);break;case 2:0.5<=g.getValue(v)&&(u+=
b);0.5<=e.getValue(v)&&(u-=b);u>w&&(u=w,p.setValue(v,1));u<l&&(u=l,r.setValue(v,1));break;default:0.5<=g.getValue(v)&&(u+=b),0.5<=e.getValue(v)&&(u-=b),u>w&&(u=l,p.setValue(v,1)),u<l&&(u=w,r.setValue(v,1))}k.getValue(v)!=u&&k.setValue(v,u);0.5<=q.getValue(v)&&k.setValue(v,h.getValue(v))}k.setSliceCount(a);r.setSliceCount(a);p.setSliceCount(a)}m=!0}};VVVV.Nodes.Counter.prototype=new VVVV.Core.Node;VVVV.Nodes.CurrentTimeAstronomy=function(a,b){this.constructor(a,"CurrentTime (Astronomy)",b);this.meta={authors:["Zauner"],original_authors:["VVVV Group"],credits:["http://javascript.about.com/library/bldst.htm"],compatibility_issues:["Time Zone index differs from classic VVVV","Time Zone Name does not work in Firefox"]};this.auto_evaluate=!0;var g=new Date,e=this.addOutputPin("GMT",[],VVVV.PinTypes.Value),f=this.addOutputPin("Time Zone",[0],VVVV.PinTypes.Value),d=this.addOutputPin("Time Zone Name",
...
c.strokeText(this.text,0,0);c.restore();c.restore()}};this.evaluate=function(){for(var b=this.getMaxInputSliceCount(),a=0;a<b;a++)void 0==k[a]&&(k[a]=new r),k[a].transform=e.getValue(a),k[a].text=d.getValue(a),k[a].font=c.getValue(a),k[a].align=h.getValue(a),k[a].baseline=q.getValue(a),k[a].renderState=g.getValue(a),k[a].clippingLayer=f.getValue(a);for(a=0;a<k.length;a++)n.setValue(a,k[a]);n.setSliceCount(b)}};VVVV.Nodes.TextCanvas.prototype=new VVVV.Core.Node;
...
c.beginPath();c.moveTo(this.x[0],this.y[0]);for(var h=1;h<this.x.length;h++)c.bezierCurveTo(this.c1x[h-1]+this.x[h-1],this.c1y[h-1]+this.y[h-1],this.c2x[h]+this.x[h],this.c2y[h]+this.y[h],this.x[h],this.y[h]);0<this.renderState.fillColor[3]&&c.fill();0<this.renderState.strokeColor[3]&&c.stroke();c.restore();c.restore()}}};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,w=0,l=0;l<a||0<p.getValue(0)&&(0<w||0<b%f.getSliceCount());l++)void 0==t[b]&&(t[b]=new s),0==w&&(t[b].renderState=
e.getValue(b),t[b].transform=f.getValue(b),d.isConnected()&&(t[b].clippingLayer=d.getValue(b))),t[b].x[w]=parseFloat(c.getValue(l)),t[b].y[w]=parseFloat(h.getValue(l)),t[b].c1x[w]=parseFloat(q.getValue(l)),t[b].c1y[w]=parseFloat(n.getValue(l)),t[b].c2x[w]=parseFloat(k.getValue(l)),t[b].c2y[w]=parseFloat(r.getValue(l)),w++,0<p.getValue(0)&&w>=p.getValue(b)&&(w<t[b].x.length&&g(t[b],w),b++,w=0);0>p.getValue(0)&&w<t[b].x.length?(g(t[b],w),t.splice(1)):t.splice(b);for(a=0;a<t.length;a++)m.setValue(a,
t[a]);m.setSliceCount(t.length)}};VVVV.Nodes.BezierCurveCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Color),d=this.addInputPin("Backbuffer Width",[0],VVVV.PinTypes.Value),c=this.addInputPin("Backbuffer Height",[0],VVVV.PinTypes.Value),h=this.addInputPin("View",[],VVVV.PinTypes.Transform),q=this.addOutputPin("Canvas Out",[],this),n,k,r,p=[0,0,0,1],m;this.getContext=function(){var h=this.invisiblePins["Descriptive Name"].getValue(0);if((h=$(h).get(0))&&"CANVAS"==h.nodeName)m=h;else{var f=parseInt(d.getValue(0)),e=parseInt(c.getValue(0));m=$('<canvas width="'+(0<f?f:512)+'" height="'+
(0<e?e:512)+'" id="vvvv-js-generated-renderer-'+(new Date).getTime()+'" class="vvvv-js-generated-renderer"></canvas>');h||(h="body");$(h).append(m);m=m.get(0)}k=m.width;r=m.height;m&&m.getContext&&(g(m),n=m.getContext("2d"),m.ctx=n)};this.destroy=function(){$(m).remove()};this.evaluate=function(){this.invisiblePins["Descriptive Name"].pinIsChanged()&&($(m).hasClass("vvvv-js-generated-renderer")&&$(m).remove(),this.getContext());if(n){if(d.pinIsChanged()){var b=parseInt(d.getValue(0));0<b&&(k=b,$(m).attr("width",
k))}c.pinIsChanged()&&(b=parseInt(c.getValue(0)),0<b&&(r=b,$(m).attr("height",r)));f.pinIsChanged()&&(p=f.getValue(0).split(","),p[0]=parseInt(255*p[0]),p[1]=parseInt(255*p[1]),p[2]=parseInt(255*p[2]));defaultRenderState.apply(n);n.clearRect(0,0,k,r);n.save();n.translate(k/2,r/2);n.scale(k/2,-r/2);h.isConnected()&&(b=h.getValue(0),n.transform(b[0],b[1],b[4],b[5],b[12],b[13]));if(e.isConnected())for(b=0;b<e.values.length;b++)e.getValue(b).draw(n);n.restore();defaultRenderState.apply(n);n.globalCompositeOperation=
"destination-over";n.fillStyle="rgba("+p[0]+","+p[1]+","+p[2]+","+p[3]+")";n.fillRect(0,0,k,r);q.setValue(0,m);m.loaded=!0}}};VVVV.Nodes.RendererCanvas.prototype=new VVVV.Core.Node;VVVV.PinTypes.Color={typeName:"Color",reset_on_disconnect:!1,defaultValue:function(){return"1.0, 1.0, 1.0, 1.0"},primitive:!0,hsvToRgb:function(a,b,g){a*=360;var e=Math.floor(a/60),f=a/60-e;a=g*(1-b);var d=g*(1-b*f);b=g*(1-b*(1-f));switch(e){case 1:return[d,g,a];case 2:return[a,g,b];case 3:return[a,d,g];case 4:return[b,a,g];case 5:return[g,a,d];default:return[g,b,a]}},rgbToHsv:function(a,b,g){var e=Math.max(a,Math.max(b,g)),f=Math.min(a,Math.min(b,g)),d;e==f?d=0:e==a?d=60*(0+(b-g)/(e-f)):e==b?d=60*
(2+(g-a)/(e-f)):e==g&&(d=60*(4+(a-b)/(e-f)));0>d&&(d+=360);a=0;0!=e&&(a=(e-f)/e);return[d/360,a,e]}};
...
e.pinIsChanged||f.pinIsChanged()||d.pinIsChanged()){for(var h=this.getMaxInputSliceCount(),b=0;b<h;b++){var a=g.getValue(b)||0,k=e.getValue(b)||0,r=f.getValue(b)||0,p=d.getValue(b)||0;c.setValue(b,a+","+k+","+r+","+p)}c.setSliceCount(h)}}};VVVV.Nodes.RGBJoin.prototype=new VVVV.Core.Node;
...
this.getMaxInputSliceCount(),b=0;b<h;b++){var a=g.getValue(b),k=e.getValue(b),r=f.getValue(b),p=d.getValue(b),a=VVVV.PinTypes.Color.hsvToRgb(a,k,r);c.setValue(b,a[0]+","+a[1]+","+a[2]+","+p)}c.setSliceCount(h)}};VVVV.Nodes.HSVJoin.prototype=new VVVV.Core.Node;
...
l[a]=z[0];l[a].volume=0;z=function(c){return function(){}}(a);l[a].onprogress=z;l[a].oncanplay=z;l[a].oncanplaythrough=z}filename!=l[a].currentSrc&&($(l[a]).find("source").first().attr("src",filename),l[a].load(),0.5<f.getValue(a)?l[a].play():l[a].pause(),l[a].loaded=!0,r.setValue(a,l[a]),p.setValue(a,l[a]))}if(f.pinIsChanged())for(a=0;a<b;a++)0.5<f.getValue(a)?l[a].play():l[a].pause();if(q.pinIsChanged())for(a=0;a<b;a++)l[a%l.length].loaded&&0.5<=q.getValue(a)&&(l[a%l.length].currentTime=parseFloat(n.getValue(a)),
0.5<f.getValue(a)&&l[a].play());for(a=0;a<b;a++){if(!l[a].paused){r.setValue(a,l[a]);p.setValue(a,l[a]);m.getValue(a)!=l[a].duration&&m.setValue(a,l[a].duration);t.setValue(a,l[a].currentTime);var z=parseFloat(h.getValue(a)),F=parseFloat(c.getValue(a));l[a].currentTime<F&&(l[a].currentTime=F);if(l[a].currentTime>=l[a].duration||0<=z&&l[a].currentTime>=z)0.5<=d.getValue(a)?l[a].currentTime=F:l[a].pause()}if(l[a].videoWidth!=s.getValue(a)||l[a].videoHeight!=v.getValue(a))s.setValue(a,l[a].videoWidth),
v.setValue(a,l[a].videoHeight);u.getValue(a)!=g[l[a].networkState]&&u.setValue(a,g[l[a].networkState]);w.getValue(a)!=e[l[a].readyState]&&w.setValue(a,e[l[a].readyState])}r.setSliceCount(b);p.setSliceCount(b)}};VVVV.Nodes.FileStreamCanvas.prototype=new VVVV.Core.Node;
...
VVVV.PinTypes.Value),c=this.addOutputPin("Status",[""],VVVV.PinTypes.String),h=this.addOutputPin("Body",[""],VVVV.PinTypes.String),q=this.addOutputPin("Fail",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Success",[0],VVVV.PinTypes.Value),k,r,p,m=!1;this.evaluate=function(){this.getMaxInputSliceCount();var a=g.pinIsChanged()||e.pinIsChanged()||f.pinIsChanged()||d.pinIsChanged()&&1==d.getValue(0);1==n.getValue(0)&&n.setValue(0,0);1==q.getValue(0)&&q.setValue(0,0);m&&(h.setValue(0,k),c.setValue(0,void 0),
n.setValue(0,r),q.setValue(0,p),m=!1);a&&(void 0==g.getValue(0)?(h.setValue(0,""),c.setValue(0,"")):$.ajax({url:g.getValue(0),type:"get",success:function(c,d,h){k=c;r=1;m=!0},error:function(c,d){k="";p=1;m=!0}}))}};VVVV.Nodes.HTTPGet.prototype=new VVVV.Core.Node;VVVV.PinTypes.Node={typeName:"Node"};
...
[0],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,k=0,r,p,m,t=0;t<a||0<e.getValue(0)&&(0<k||0!=b%e.values.length);t++)r=g.getValue(t),0==k&&(center=p=m=r,width=0),p=Math.min(r,p),m=Math.max(r,m),k++,0<e.getValue(0)&&k>=e.getValue(b)&&(c.setValue(b,p),h.setValue(b,m),f.setValue(b,p+(m-p)/2),d.setValue(b,Math.abs(m-p)),b++,k=0);-1==e.getValue(0)&&(c.setValue(b,p),h.setValue(b,m),f.setValue(b,p+(m-p)/2),d.setValue(b,Math.abs(m-p)));f.setSliceCount(b+(0<k));
...
[0],VVVV.PinTypes.Value),c=this.addInputPin("Spread Count",[1],VVVV.PinTypes.Value),h=this.addOutputPin("Output",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=parseInt(c.getValue(0)),b=parseFloat(e.getValue(0)),k=parseFloat(d.getValue(0)),r=parseFloat(g.getValue(0)),p=f.getValue(0);""==p&&(p="Centered");var m=b/a;"Block"==p&&(m=b/(a-1));var t=m/2;if("Block"==p||"LeftJustified"==p)t=0;"RightJustified"==p&&(t=m);for(var s,v=0;v<a;v++)s=v*m+t,"Block"!=p&&0!=b&&(s=(s+k*b)%b),s=r-b/2+s,h.setValue(v,
...
c=this.addOutputPin("Hits",[0],VVVV.PinTypes.Value),h=this.addOutputPin("Input Index",[0],VVVV.PinTypes.Value),q=this.addOutputPin("Filter Index",[0],VVVV.PinTypes.Value),n=this.addOutputPin("Found At Position",[0],VVVV.PinTypes.Value);this.evaluate=function(){var a=this.getMaxInputSliceCount();c.setSliceCount(a);var b=f.getValue(0);d.getValue(0);for(var p=0,m=0;m<a;m++){var t=g.getValue(m),s=1;if("MatchesAny"==b||"ContainsAny"==b)s=e.getSliceCount();for(var v=0,u=0;u<s;u++){var w=e.getValue(u+m),
l=[];if("Matches"==b||"MatchesAny"==b)w==t&&l.push(-1);else for(var w=RegExp(w,"gi"),A;A=w.exec(t);)l.push(A.index);for(w=0;w<l.length;w++)h.setValue(p,m%g.getSliceCount()),q.setValue(p,(m+u)%e.getSliceCount()),n.setValue(p,l[w]+1),p++,v++}c.setValue(m,v)}h.setSliceCount(p);q.setSliceCount(p);n.setSliceCount(p)}};VVVV.Nodes.SiftString.prototype=new VVVV.Core.Node;
...
UnderScore:"_",Minus:"-"},k={Single:"'",Double:'"'};this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0,m=0;m<a;m++){var t=g.getValue(m),s=n[e.getValue(m)],v=f.getValue(m),u=k[d.getValue(m)],w=c.getValue(m);""==s&&(s=v);""==s&&(s=void 0);t=t.split(s);s=t.length;u=RegExp("[^"+u+"]*","g");for(v=0;v<s;v++)1==w?h.setValue(b,t[v]):h.setValue(b,t[v].match(u).join("")),q.setValue(b,m),b++}h.setSliceCount(b);q.setSliceCount(b)}};VVVV.Nodes.SeparateString.prototype=new VVVV.Core.Node;
...
this.evaluate=function(){var a=parseFloat(this.inputPins.FOV.getValue(0)),b=parseFloat(this.inputPins["Near Plane"].getValue(0)),f=parseFloat(this.inputPins["Far Plane"].getValue(0)),d=mat4.create();mat4.identity(d);mat4.perspective(360*a,1,b,f,d);this.inputPins["Transform In"].isConnected()&&(a=this.inputPins["Transform In"].getValue(i),mat4.multiply(a,d,d));this.outputPins["Transform Out"].setValue(0,d)}};VVVV.Nodes.Perspective.prototype=new VVVV.Core.Node;
...
g.getValue(b)),d.setValue(a,b),a++;f.setSliceCount(a);d.setSliceCount(a)}};VVVV.Nodes.SelectValue.prototype=new VVVV.Core.Node;
...
[1],VVVV.PinTypes.Value),h=this.addInputPin("Mapping",["Float"],VVVV.PinTypes.Enum);h.enumOptions=["Float","Wrap","Clamp"];var q=this.addOutputPin("Output",[0.5],VVVV.PinTypes.Value);this.evaluate=function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var r=parseFloat(g.getValue(b)),p=parseFloat(e.getValue(b)),m=parseFloat(f.getValue(b)),t=parseFloat(d.getValue(b)),s=parseFloat(c.getValue(b)),v=h.getValue(b);if(m==p)q.setValue(b,t);else{r-=p;m-=p;switch(v){case "Clamp":r=Math.max(0,Math.min(m,
r));break;case "Wrap":r=(m+r%m)%m}q.setValue(b,r*((s-t)/m)+t)}}q.setSliceCount(a)}};VVVV.Nodes.MapValue.prototype=new VVVV.Core.Node;
VVVV.Nodes.MinValue=function(a,b){this.constructor(a,"Min (Value)",b);this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};this.auto_evaluate=!1;var g=this.addOutputPin("Output",[0],VVVV.PinTypes.Value),e=this.addInvisiblePin("Input Count",[0],VVVV.PinTypes.Value),f=[];this.initialize=function(){for(var d=Math.max(2,e.getValue(0)),c=f.length;c<d;c++)f[c]=this.addInputPin("Input "+(c+1),[0],VVVV.PinTypes.Value);f.length=d};this.evaluate=function(){e.pinIsChanged()&&
this.initialize();for(var d=this.getMaxInputSliceCount(),c=0;c<d;c++){for(var a=void 0,b=0;b<f.length;b++)if(void 0==a||f[b].getValue(c)<a)a=f[b].getValue(c);g.setValue(c,a)}g.setSliceCount(d)}};VVVV.Nodes.MinValue.prototype=new VVVV.Core.Node;
...
function(){for(var a=this.getMaxInputSliceCount(),b=0;b<a;b++){var n=g.getValue(b),k=e.getValue(b),r=f.getValue(b),p=d.getValue(b),m=c.values[b];void 0==m&&c.setValue(b,0);if(0.5<=n){m=c.values[b];do n=Math.random()*r,0.5<=k&&(n=Math.round(n));while(0.5<=p&&n==m);c.setValue(b,n)}}c.setSliceCount(a)}};VVVV.Nodes.RandomValue.prototype=new VVVV.Core.Node;
...
[],VVVV.PinTypes.WebGlResource),h=null;this.evaluate=function(){var a=this.renderContexts[0];if(a&&0.5<=d.getValue(0)){for(var b=[],k=[],r=[],p=0;p<this.getMaxInputSliceCount();p++)b[p]=parseFloat(g.getValue(p)),k[p]=parseFloat(f.getValue(p)),r[p]=parseFloat(e.getValue(p));h=new VVVV.Types.VertexBuffer(a,b);h.setSubBuffer("TEXCOORD0",2,k);h.setSubBuffer("NORMAL",3,r);h.create();c.setValue(0,h)}}};VVVV.Nodes.VertexBufferJoin.prototype=new VVVV.Core.Node;
VVVV.Nodes.MeshJoin=function(a,b){this.constructor(a,"Mesh (EX9.Geometry Join)",b);this.auto_nil=!1;this.meta={authors:["Matthias Zauner"],original_authors:["VVVV Group"],credits:[],compatibility_issues:[]};var g=this.addInputPin("Vertex Buffer",[],this),e=this.addInputPin("Indices",[0],VVVV.PinTypes.Value),f=this.addInputPin("Apply",[1],VVVV.PinTypes.Value),d=this.addOutputPin("Mesh",[],VVVV.PinTypes.WebGlResource),c=null;this.evaluate=function(){var a=this.renderContexts[0];a&&0.5<=f.getValue(0)&&
(g.isConnected()?(c=new VVVV.Types.Mesh(a,g.getValue(0),e.values),d.setValue(0,c)):(d.setValue(0,void 0),delete c))}};VVVV.Nodes.MeshJoin.prototype=new VVVV.Core.Node;
...
b=parseInt(e.getValue(0)),n=[],k=[],r=[],p=0,m=0;m<b;m++)for(var t=0;t<a;t++)n.push(parseFloat(t)/(a-1)-0.5),n.push(0.5-parseFloat(m)/(b-1)),n.push(0),p++,k.push(0),k.push(0),k.push(1),r.push(parseFloat(t)/(a-1)),r.push(parseFloat(m)/(b-1));n=new VVVV.Types.VertexBuffer(c,n);n.setSubBuffer("TEXCOORD0",2,r);n.setSubBuffer("NORMAL",3,k);n.create();k=[];for(m=0;m<b-1;m++)for(t=0;t<a-1;t++)r=t+a*m,k.push(r),k.push(r+1),k.push(r+a+1),k.push(r+a+1),k.push(r+a),k.push(r);d=new VVVV.Types.Mesh(c,n,k);f.setValue(0,
...
this.renderContexts[0];if(a){for(var b=parseInt(e.getValue(0)),n=parseInt(f.getValue(0)),k=parseFloat(g.getValue(0)),r=[],p=[],m=[],t=0;t<n+1;t++)for(var s=Math.cos(-parseFloat(t)/n*Math.PI),v=0;v<b;v++){var u=Math.cos(2*(parseFloat(v)/b)*Math.PI)*Math.cos(Math.asin(s)),w=Math.sin(2*(parseFloat(v)/b)*Math.PI)*Math.cos(Math.asin(s));r.push(u*k);r.push(s*k);r.push(w*k);p.push(u);p.push(s);p.push(w);m.push(parseFloat(v)/b);m.push(parseFloat(t)/n)}k=new VVVV.Types.VertexBuffer(a,r);k.setSubBuffer("TEXCOORD0",
2,m);k.setSubBuffer("NORMAL",3,p);k.create();p=[];for(t=0;t<n;t++)for(v=0;v<b;v++)m=b*t,r=v+m,p.push(r),p.push((r+1)%b+m),p.push((r+1)%b+b+m),p.push((r+1)%b+b+m),p.push(r+b),p.push(r);c=new VVVV.Types.Mesh(a,k,p);d.setValue(0,c)}}}};VVVV.Nodes.Sphere.prototype=new VVVV.Core.Node;
...
[15],VVVV.PinTypes.Value),q=this.addInputPin("Resolution Y",[1],VVVV.PinTypes.Value),n=this.addOutputPin("Mesh",[],VVVV.PinTypes.WebGlResource),k=null;this.evaluate=function(){if(this.renderContexts){var a=this.renderContexts[0];if(a){var b=parseInt(h.getValue(0)),m=parseInt(q.getValue(0)),t=parseFloat(g.getValue(0)),s=parseFloat(e.getValue(0)),v=parseFloat(f.getValue(0)),u=parseFloat(d.getValue(0)),w=[],l=[],A=[];w.push(0);w.push(v/2);w.push(0);l.push(0);l.push(1);l.push(0);A.push(0);A.push(0);w.push(0);
w.push(-v/2);w.push(0);l.push(0);l.push(-1);l.push(0);A.push(0);A.push(0);for(var y=0;y<m+1;y++)for(var z=parseFloat(y)/m,F=(z-0.5)*-v,C=0;C<b+1;C++){var H=Math.cos(2*(parseFloat(C)/b)*Math.PI*u-Math.PI*u-Math.PI/2),J=Math.sin(2*(parseFloat(C)/b)*Math.PI*u-Math.PI*u-Math.PI/2),I=z*s+(1-z)*t;w.push(H*I);w.push(F);w.push(J*I);l.push(H);l.push(0);l.push(J);A.push(parseFloat(C)/b);A.push(parseFloat(y)/m)}t=new VVVV.Types.VertexBuffer(a,w);t.setSubBuffer("TEXCOORD0",2,A);t.setSubBuffer("NORMAL",3,l);t.create();
l=[];if(0.5<c.getValue(0))for(z=0;2>z;z++)for(C=0;C<b;C++)l.push(z),l.push(2+C+z+z*m*(b+1)),l.push(2+C+(1-z)+z*m*(b+1));for(y=0;y<m;y++)for(C=0;C<b;C++)z=C+(b+1)*y+2,l.push(z),l.push(z+1),l.push(z+b+2),l.push(z+b+2),l.push(z+b+1),l.push(z);k=new VVVV.Types.Mesh(a,t,l);n.setValue(0,k)}}}};VVVV.Nodes.Cylinder.prototype=new VVVV.Core.Node;
...
"SRC_COLOR";d[a].destBlendMode="ONE";break;case "ColorAsAlphaBlend":d[a].srcBlendMode="SRC_COLOR",d[a].destBlendMode="ONE_MINUS_SRC_COLOR"}f.setValue(a,d[a])}f.setSliceCount(c)}};VVVV.Nodes.BlendWebGL.prototype=new VVVV.Core.Node;
...
(b=0);var q=c.length;if(this.contextChanged||n)q=0;b<q&&c.splice(b,q-b);for(var s=q;s<b;s++)c[s]=new VVVV.Types.Layer,c[s].mesh=e.getValue(0),c[s].shader=h,_(h.uniformSpecs).each(function(a){c[s].uniformNames.push(a.varname);c[s].uniforms[a.varname]={uniformSpec:a,value:void 0}});if(e.pinIsChanged())for(s=0;s<b;s++)c[s].mesh=e.getValue(0);for(s=0;s<b;s++)c[s].shader=h;for(a=0;a<k.length;a++){var v=k[a].pinname.replace(/ /g,"_");if(k[a].pinIsChanged()||q<b)for(s=0;s<b;s++)if("vec"==h.uniformSpecs[v].type){var u=
...
VVVV.Nodes.RendererWebGL=function(a,b){function g(){$(u).detach("mousemove");$(u).detach("mousedown");$(u).detach("mouseup");VVVV.MousePositions[u.id]={x:0,y:0,wheel:0,lb:0,mb:0,rb:0};$(u).mousemove(function(a){var c=2*(a.pageX-$(this).offset().left)/$(this).width()-1;a=-(2*(a.pageY-$(this).offset().top)/$(this).height()-1);VVVV.MousePositions._all.x=c;VVVV.MousePositions._all.y=a;VVVV.MousePositions[u.id].x=c;VVVV.MousePositions[u.id].y=a});$(u).bind("mousewheel",function(a){a=a.originalEvent.wheelDelta/
...
q.markPinAsChanged())}}};this.destroy=function(){$(u).remove()};var z=!1;this.evaluate=function(){l=this.ctxt;if(this.invisiblePins["Descriptive Name"].pinIsChanged()||this.contextChanged)w&&$(w.canvas).hasClass("vvvv-js-generated-renderer")&&$(w.canvas).remove(),this.getContexts(),this.inputPins.Layers.isConnected()&&this.inputPins.Layers.links[0].fromPin.connectionChanged();z||(k.setValue(0,m),r.setValue(0,t),z=!0);if(void 0!=l){if(d.pinIsChanged()&&(!this.renderContexts||!this.renderContexts[0])){var a=
parseInt(d.getValue(0));0<a&&(m=a,$(w.canvas).attr("width",m),k.setValue(0,m))}!c.pinIsChanged()||this.renderContexts&&this.renderContexts[0]||(a=parseInt(c.getValue(0)),0<a&&(t=a,$(w.canvas).attr("height",t),r.setValue(0,t)));this.renderContexts&&this.renderContexts[0]&&l==this.renderContexts[0]?l.bindFramebuffer(l.FRAMEBUFFER,A):l.bindFramebuffer(l.FRAMEBUFFER,null);a=_(f.getValue(0).split(",")).map(function(a){return parseFloat(a)});l.clearColor(a[0],a[1],a[2],a[3]);l.clear(l.COLOR_BUFFER_BIT|
l.DEPTH_BUFFER_BIT);"NONE"==n.getValue(0)?l.disable(l.DEPTH_TEST):l.enable(l.DEPTH_TEST);q.pinIsChanged()&&(q.isConnected()?(s=mat4.create(),mat4.set(q.getValue(0),s)):(s=mat4.create(),mat4.ortho(-1,1,-1,1,-100,100,s)),mat4.scale(s,[1,1,-1]),this.renderContexts&&this.renderContexts[0]&&mat4.scale(s,[1,-1,1]));h.pinIsChanged()&&(v=h.getValue(0));if(this.contextChanged)this.contextChanged=!1;else{l.viewport(0,0,m,t);var b=a=null,e=null;if(this.inputPins.Layers.isConnected()){for(var g=this.inputPins.Layers.values,
u=0;u<g.length;u++)if(layer=g[u],void 0!=layer.shader){a!=layer.shader.shaderProgram&&(l.useProgram(layer.shader.shaderProgram),l.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.PROJECTION].position,!1,s),l.uniformMatrix4fv(layer.shader.uniformSpecs[layer.shader.uniformSemanticMap.VIEW].position,!1,v));var E=layer.renderState;E||(E=defaultWebGlRenderState);E!=b&&E.apply(l);if(layer.mesh!=e||layer.shader.shaderProgram!=a)l.bindBuffer(l.ARRAY_BUFFER,layer.mesh.vertexBuffer.vbo),
...
for(var b=layer.uniformNames.length,G=e=0;G<b;G++){var B=layer.uniforms[layer.uniformNames[G]];if(void 0!=B.value&&!(0<u&&layer.shader.shaderProgram==a&&g[u-1].uniforms[layer.uniformNames[G]]&&B.value==g[u-1].uniforms[layer.uniformNames[G]].value)){start=(new Date).getTime();switch(B.uniformSpec.type){case "mat":l["uniformMatrix"+B.uniformSpec.dimension+"fv"](B.uniformSpec.position,!1,B.value);break;case "vec":l["uniform"+B.uniformSpec.dimension+"fv"](B.uniformSpec.position,B.value);break;case "int":l["uniform"+
...
VVVV.Editors.BrowserEditor.PatchWindow=function(a,b,g){function e(){c.selectAll(".vvvv-node.selected").attr("class",function(a){return a.isIOBox?"vvvv-node vvvv-iobox":"vvvv-node"});p=[]}this.state=UIState.Idle;var f=0,d=0,c,h,q,n,k,r=void 0,p=[],m=a,t=0,s=VVVV.Root+"/patch.html",v=!1,u=0,w=0,l=0,A=0;a.isSubpatch&&(s="patch.html");this.window=g?window:window.open(s,a.nodename,"location=no, width="+a.windowWidth+", height="+a.windowHeight+", toolbar=no");var y=this;window.setTimeout(function(){for(var k=
0;k<m.nodeList.length;k++)t=Math.max(t,m.nodeList[k].id);g?root=d3.select($(g).get(0)):(y.window.document.title=a.nodename,root=d3.select(y.window.document.body));if(!g){$("body",y.window.document).on("copy",function(a){for(var b=$("<PATCH>"),c=[],d=_(p).map(function(a){return a.id}),f=0;f<p.length;f++)b.append(p[f].serialize()),_(p[f].inputPins).each(function(a){for(var b=0;b<a.links.length;b++)0<=d.indexOf(a.links[b].fromPin.node.id)&&c.push(a.links[b])});for(f=0;f<c.length;f++)b.append(c[f].serialize());
...
b);a.preventDefault();return!1});var n=0,q=0;$("body",y.window.document).mousemove(function(a){n=a.pageX;q=a.pageY});$("body",y.window.document).on("paste",function(a){a=a.originalEvent.clipboardData.getData("text/plain");if(!a.match(/^<!DOCTYPE PATCH/))return!1;$patch=$(a);var d=void 0,f=void 0;$patch.find("node bounds").each(function(){var a=parseInt($(this).attr("left")),b=parseInt($(this).attr("top"));if(!d||a<d)d=a;if(!f||b<f)f=b});var g={};$patch.find("node").each(function(){t++;g[$(this).attr("id")]=
t;$(this).attr("createme","pronto");$(this).attr("id",t);var a=parseInt($(this).find("bounds").attr("left"))-d+15*n,b=parseInt($(this).find("bounds").attr("top"))-f+15*q;$(this).find("bounds").attr("left",a);$(this).find("bounds").attr("top",b)});$patch.find("link").each(function(){$(this).attr("createme","pronto");var a=$(this).attr("srcnodeid"),b=$(this).attr("dstnodeid");$(this).attr("srcnodeid",g[a]);$(this).attr("dstnodeid",g[b])});a=$patch.wrapAll("<d></d>").parent().html();b.update(m,a);e();
var h=_(g).map(function(a){return a});c.selectAll(".vvvv-node").each(function(a){0<=h.indexOf(parseInt(a.id))&&(p.push(a),d3.select(this).attr("class","vvvv-node selected"))})})}c=root.append("svg:svg").attr("class","chart").attr("width",Math.max(m.windowWidth,m.boundingBox.width)).attr("height",Math.max(m.windowHeight,m.boundingBox.height));k=c.append("svg:rect").attr("class","background").attr("width",Math.max(m.windowWidth,m.boundingBox.width)).attr("height",Math.max(m.windowHeight,m.boundingBox.height));
g||(c.on("mousemove",function(){if(y.state==UIState.Connecting)c.select(".vvvv-link.current-link").attr("x2",function(a){return d3.event.pageX+(d3.select(this).attr("x1")<d3.event.pageX?-1:1)}).attr("y2",function(a){return d3.event.pageY+(d3.select(this).attr("y1")<d3.event.pageY?-1:1)});else if(y.state==UIState.Moving){var a=d3.event.pageX-f,b=d3.event.pageY-d;c.selectAll(".vvvv-node.selected").attr("transform",function(c){return"translate("+(c.x+a)+","+(c.y+b)+")"});for(var g=0;g<p.length;g++){var h=
p[g];c.selectAll(".vvvv-link line").filter(function(a){return a.fromPin.node.id==h.id}).attr("x1",function(b){return b.fromPin.x+b.fromPin.node.x+a}).attr("y1",function(a){return a.fromPin.y+a.fromPin.node.y+b});c.selectAll(".vvvv-link line").filter(function(a){return a.toPin.node.id==h.id}).attr("x2",function(b){return b.toPin.x+b.toPin.node.x+a}).attr("y2",function(a){return a.toPin.y+a.toPin.node.y+b})}}else y.state==UIState.AreaSelecting&&(l=d3.event.pageX,A=d3.event.pageY,c.select(".selection-area").attr("transform",
"translate("+Math.min(u,l)+", "+Math.min(w,A)+")").attr("width",Math.abs(l-u)).attr("height",Math.abs(A-w)),e(),c.selectAll(".vvvv-node").each(function(a){for(var b=[a.x,a.x+a.getWidth()],c=[a.y,a.y+a.getHeight()],d=!1,f=0;2>f;f++)for(var e=0;2>e;e++)b[f]>=Math.min(u,l)&&(b[f]<=Math.max(u,l)&&c[e]>=Math.min(w,A)&&c[e]<=Math.max(w,A))&&(d=!0);d&&(d3.select(this).attr("class","vvvv-node selected"),p.push(a))}))}).on("contextmenu",function(){y.state==UIState.Connecting&&(c.select(".vvvv-link.current-link").remove(),
y.state=UIState.Idle);d3.event.stopPropagation();d3.event.preventDefault();return!1}).on("mouseup",function(){if(y.state==UIState.Moving){y.state=UIState.Idle;for(var a=d3.event.pageX-f,e=d3.event.pageY-d,g="<PATCH>",h=0;h<p.length;h++)var k=p[h],g=g+("<NODE componentmode='Node' id='"+k.id+"'><BOUNDS type='Node' left='"+15*(a+k.x)+"' top='"+15*(e+k.y)+"' width='"+k.width+"' height='"+k.height+"'/></NODE>");b.update(m,g+"</PATCH>")}y.state==UIState.AreaSelecting&&(c.select(".selection-area").remove(),
...
15*d+"' top='"+15*f+"' width='100' height='100'/>"}else t++,g="<PATCH>"+("<NODE componentmode='Hidden' id='"+t+"' nodename='IOBox (String)' systemname='IOBox (String)'>"),g+="<BOUNDS type='Node' left='"+15*d+"' top='"+15*f+"' width='100' height='100'/>",g+="<PIN pinname='Input String' values='|"+a+"|'/>";b.update(m,g+"</NODE>");e.remove()}$("#node_selection",y.window.document).remove();var d=d3.event.pageX,f=d3.event.pageY,e=$('<div id="node_selection"><input type="text" id="node_filter"/></div>'),
...
function(a){v=a.shiftKey},$(y.window.document).keydown(k),$(y.window.document).keyup(k),$(y.window.document).keydown(function(a){if(46==a.which&&0<p.length){var d="<PATCH>";for(a=0;a<p.length;a++){var f=p[a],d=d+("<NODE id='"+p[a].id+"' deleteme='pronto'/>");_(f.inputPins).each(function(a){_(a.links).each(function(a){d+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})});_(f.outputPins).each(function(a){_(a.links).each(function(a){d+=
"<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"})})}d+="</PATCH>";b.update(m,d);p=[]}else if(115!=a.which&&83!=a.which||!a.ctrlKey){if(73==a.which&&a.ctrlKey)return b.inspector?b.inspector.win.focus():b.openInspector("."),a.preventDefault(),!1;if(65==a.which&&a.ctrlKey)return e(),c.selectAll(".vvvv-node").each(function(a){p.push(a);d3.select(this).attr("class","vvvv-node selected")}),
a.preventDefault(),!1}else return b.save(m.nodename,m.toXML()),a.preventDefault(),!1}),$(y.window).resize(function(){m.windowWidth=$(this).width();m.windowHeight=$(this).height();m.boundingBox.width=Math.max(m.windowWidth,m.boundingBox.width);m.boundingBox.height=Math.max(m.windowHeight,m.boundingBox.height);m.afterUpdate()}));y.drawComplete();m.afterUpdate=function(){y.drawComplete();b.inspector&&window.setTimeout(b.inspector.update,100)};m.afterEvaluate=function(){h.filter(function(a){return a.isIOBox&&
VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel}).each(function(a){VVVV.PinTypes[a.IOBoxInputPin().typeName].makeLabel(this,a)})}},1E3);this.close=function(){this.window.close()};this.drawComplete=function(){h&&h.remove();k&&k.remove();g||m.windowWidth==$(y.window).width()&&m.windowHeight==$(y.window).height()||y.window.resizeTo(m.windowWidth,m.windowHeight);c.attr("width",Math.max(m.windowWidth,m.boundingBox.width)).attr("height",Math.max(m.windowHeight,m.boundingBox.height));c.selectAll(".background").attr("width",
Math.max(m.windowWidth,m.boundingBox.width)).attr("height",Math.max(m.windowHeight,m.boundingBox.height));var a=c.append("svg:g").attr("class","link-group");h=c.selectAll("g.vvvv-node").data(m.nodeList).enter().append("svg:g").attr("class",function(a){var b="vvvv-node";a.isIOBox&&(b+=" vvvv-iobox");0<=p.indexOf(a)&&(b+=" selected");return b}).attr("id",function(a){return"vvvv-node-"+a.id}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"});h.append("svg:rect").attr("class","vvvv-node-background").attr("height",
...
"<PATCH>";_(g.links).each(function(a){h+="<LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/>"});h+="<LINK createme='pronto' srcnodeid='"+f.node.id+"' srcpinname='"+f.pinname+"' dstnodeid='"+g.node.id+"' dstpinname='"+g.pinname+"'/>";h+="</PATCH>";b.update(m,h);c.select(".vvvv-link.current-link").remove();c.select(".vvvv-connection-highlight").remove();y.state=UIState.Idle}m.afterUpdate()}d3.event.stopPropagation()}).on("mousedown",
function(){d3.event.stopPropagation()}),c.selectAll("g.vvvv-link").on("contextmenu",function(a){b.update(m,"<PATCH><LINK deleteme='pronto' srcnodeid='"+a.fromPin.node.id+"' srcpinname='"+a.fromPin.pinname+"' dstnodeid='"+a.toPin.node.id+"' dstpinname='"+a.toPin.pinname+"'/></PATCH>");d3.event.preventDefault()}),h.on("mousedown",function(a){$(".resettable",y.window.document).remove();y.state=UIState.Moving;0>p.indexOf(a)&&(v||e(),d3.select(this).attr("class","vvvv-node selected"),p.push(a));f=d3.event.pageX;
...
VVVV.PinTypes[d.typeName].defaultValue());d.setSliceCount(b);d.node.parentPatch.afterUpdate();a.preventDefault();return!1});for(e=e=0;e<f;e++)if(g=$(c.win.document).find("#values .row").eq(e+1),b||!g.children().first().hasClass("pininputbox")){if(VVVV.PinTypes[a.typeName].openInputBox&&a.direction!=PinDirection.Output&&!a.isConnected()&&void 0!=a.getValue(0)){var p=$('<div class="row value"><div style="height:100%">'+a.getValue(e)+"</div></div>");(function(b){p.find("div").click(function(){VVVV.PinTypes[a.typeName].openInputBox(c.win,
$(this),a,b)})})(e)}else p=$('<div class="row value readonlyvalue"><div style="height:100%">'+a.getValue(e)+"</div></div>");0<g.length?g.replaceWith(p):$(c.win.document).find("#values").append(p);"Color"==a.typeName&&p.find("div").css("background-color","rgba("+_(a.getValue(e).split(",")).map(function(a){return parseInt(255*a)}).join(",")+")")}e++;for(g=$(c.win.document).find("#values .row");e<g.length;e++)g.eq(e).remove()}this.win=window.open(a+"/inspektor.html","inspektor","location=no, width=250, height=600, toolbar=no");
