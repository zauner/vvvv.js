  ...
  return function (patch) {
    ...
    this.detect = function () {
      ...
      function findCluster(node, clusterActive, nonPrimitiveLink) {
        ...
        if (node.inCluster)
          return;
        node.inCluster = clusterActive && nonPrimitiveLink && !node.environments;
        ...
        if (node.inCluster === true) {
          ...
          for (pinname in node.inputPins) {
            node.inputPins[pinname].clusterEdge = node.inputPins[pinname].links.length === 0 || !node.inputPins[pinname].links[0].fromPin.node.inCluster;
          }
