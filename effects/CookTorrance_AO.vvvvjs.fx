...
uniform sampler2D tHeight1;
uniform sampler2D tRoughness1;
uniform sampler2D tAO1;
...
void main(void) {
	...
	vec3 HeightMap = texture2D(tHeight1, vs2psTexCd).xyz;
	vec3 RoughnessMap = texture2D(tRoughness1, vs2psTexCd).xyz;
	vec3 AO_Map = texture2D(tAO1, vs2psTexCd).xyz;
	//applyNormals
    ...
    gl_FragColor.rgb *= AO_Map * Ambient_Color.rgb * env_dif.rgb + refract.rgb * refract_amount + color_diff + color_spec;
}